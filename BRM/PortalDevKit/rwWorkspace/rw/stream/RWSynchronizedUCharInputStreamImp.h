#ifndef rw_stream_RWSynchronizedUCharInputStreamImp_h_
#define rw_stream_RWSynchronizedUCharInputStreamImp_h_

/*********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/stream/RWSynchronizedUCharInputStreamImp.h#1 $
 *
...
extern template class RW_STREAM_GLOBAL RWTInputStreamLockImp<RWUCharInputStream, RWFilteredUCharInputStreamImp>;
#  else
template class RW_STREAM_GLOBAL RWTInputStreamLockImp<RWUCharInputStream, RWFilteredUCharInputStreamImp>;
#  endif
#endif
 ...
 *
 * \brief
 * Implementation of a simple lock stream that can be used with a UTF-16
 * character input stream.
 *
 * Class RWSynchronizedUCharInputStreamImp implements a simple lock stream
 * that is used with UTF-16 character input stream. The stream locks its
 ...
 */
class RW_STREAM_GLOBAL RWSynchronizedUCharInputStreamImp : public RWTInputStreamLockImp<RWUCharInputStream, RWFilteredUCharInputStreamImp>
{

public:

    /**
     * Constructs an RWSynchronizedUCharInputStreamImp instance that uses
     * \a sourceStream as its source of UTF-16 characters, and returns a
     * handle to it. Throws no exceptions.
     *
     * \param sourceStream
     * The output stream that will serve as the source of UTF-16 characters.
     */
    static RWUCharInputStream make(const RWUCharInputStream& sourceStream)
    {
        return RWUCharInputStream(new RWSynchronizedUCharInputStreamImp(sourceStream));
    }
...
protected:
     ...
     * the source of UTF-16 characters.
     *
     * \param sourceStream
     * The input stream that will serve as the source of UTF-16 characters.
     */
    RWSynchronizedUCharInputStreamImp(const RWUCharInputStream& sourceStream);

};

#endif // rw_stream_RWSynchronizedUCharInputStreamImp_h_
