...
#=============================================================
# @(#)$Id: pin_cmp_tcframework.pl /cgbubrm_mainbrm.portalbase/6 2018/03/30 02:42:03 mkothari Exp $ 
# 
...
sub configure_tcframework_properties_files
{
    ...
    if( -f $PROP_File ) {
        ...
        seek( PROPFILE, 0, 0 );
	print INFILE "#infranet.connection=pcp://root.$DM{'db_num'}:$MAIN_CM{'pcm_admin_passwd'}\@$CM{'hostname'}:$MAIN_CM{'port'}/service/admin_client 1\n";
        print INFILE "\n";
        ...
        print INFILE "\n";
        print INFILE "infranet.pcp.ssl.enabled = false\n";
        print INFILE "infranet.pcp.ssl.wallet.location = /home/pin/opt/portal/BRM/wallet/client\n";
        print INFILE "infranet.pcp.ssl.wallet.filename = cwallet.sso\n";

        truncate( INFILE, tell( INFILE ) );
        close( INFILE );
	chmod(0640,$PROP_File);
    }
}
