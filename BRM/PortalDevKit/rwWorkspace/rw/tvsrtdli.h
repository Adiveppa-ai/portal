 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvsrtdli.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Maintains a sorted collection of values implemented as a doubly-linked
 * list.
 *
 * This class maintains an always-sorted collection of values, implemented
 * as a doubly-linked list.
 *
 * Class \c T is the type of items in the collection. Class \c A is an
 ...
 * \sa
 * \parblock
 * RWTValSortedVector is an alternative always-sorted collections.
 * RWTValDlist is an unsorted doubly-linked list of values.
 *
...
public:
     ...
     * responsibility to not violate the ordering of the elements within the
     * collection.
     */
    container_type& std()
    {
    ...
    /**
     * Adds the items from \a a to self, preserving order. Returns the
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValSortedDlist::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTValSortedDlist.
 *
 * RWTValSortedDlistIterator provides an iterator interface to the C++
 * Standard Library-based collection represented by RWTValSortedDlist. (An
 * STL iterator interface is provided through RWTValSortedDlist::iterator.)
 *
 * The order of iteration over an RWTValSortedDlist is dependent on the
 * supplied comparator object supplied as applied to the values stored in
 * the container.
 *
 ...
 * returns a value equivalent to \c false. Continued increments returns a
...
public:
    /**
     * Creates an iterator for the sorted dlist \a s. The iterator begins in
     ...
     * points to an undefined value distinct from the reset value, and the
     ...
     * has been reset or just created, returns \c false, otherwise returns
     ...
     * now points to the last item in the list.
     */
    bool operator--();
     ...
     * \c true. Returns \c true if such an element if found, otherwise
     * \c false. Client data may be passed through parameter \a d.
     */
    ...
    T key() const
    {
        return *(const_cast<RWTValSortedDlistIterator*>(this)->iter_);
    }

    /**
     * Removes the stored value referenced by self from the collection.
     * Returns \c true if the value was successfully removed, otherwise
     * \c false.
     */
     ...
     * forward, such that the expression <tt>(t == \a a)</tt> is \c true.
     * Returns \c true if such an element is successfully removed, otherwise
     * \c false.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValSortedDlist::const_iterator to iterate
 * over the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValSortedDlist.
 *
 ...
 *
 * The order of iteration over an RWTValSortedDlist is dependent on the
 * supplied comparator object as applied to the values stored in the
 * container.
 *
 * The current item referenced by this iterator is undefined after
 * construction or after a call to reset(). The iterator becomes valid after
...
public:
     ...
     * iteration, self referenced the last value in the list, self now
     * points to an undefined value distinct from the reset value and the
     * operator returns \c false. Otherwise, the operator returns \c true.
     */
    bool operator++();
     ...
     * value in the list, it now points to the last item in the list.
     */
    bool operator--();
