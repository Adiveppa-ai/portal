 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tools/datetime.h#1 $
 *
 **********************************************************************
 *
...
#define RW_PICOSECONDS_IN_SEC   (1000000000000LL)
#define RW_PICOSECONDS_IN_MSEC  (1000000000)
 ...
 *
 * Class RWDateTime represents a date and time, stored in milliseconds, from
 * January 1, 1901 00:00:00:000 UTC. Milliseconds are stored in a 64-bit
 * integral type, which is represented by the global typedef \c rwint64.
 *
 * RWDateTime supports the ISO 8601 international standard for the
 * formatting of dates and times. For information and formatting syntax, see
 * the chapter on internationalization in the <em>Essential Tools Module
 * User's Guide</em>.
 *
 * RWDateTime is based on the proleptic Gregorian calendar. It applies
 * Gregorian calendar rules for dates in the future as well as those prior
 * to its introduction in 1582.
 *
 * Four public member functions, all starting with \c julian, let you
 * manipulate the date via Modified Julian days (MJD). Note that the
 * Modified Julian day number is not the same as a date according to the
 * Julian calendar. The <em>Essential Tools Module User's Guide</em>
 ...
 * began at 12 noon. MJD is defined as <tt>MJD = JD - 2400000.5</tt>. The
 * half day is subtracted so that the day starts at midnight in conformance
 * with civil time reckoning.
 *
 * \note
 * Two-digit year specifiers assume the century 1900, so you should create
 * programs that use four-digit year specifiers. You can enforce the use of
 * a four-digit year by defining the build macro #RW_CENTURY_REQD in RCB
 ...
 * instances of this class. Deprecation means that these classes and related
 * methods are no longer supported, and may be removed in a future release.
 *
 ...
 * below.
 *
 * <b>The C Standard Library Type <tt>struct tm</tt></b>
 * <table>
 * <tr>
 ...
 * </table>
 *
 * The default constructor for this class creates an instance with an
 * invalid date and time, unlike RWDate, which holds the current date by
 * default. Consequently, it is much quicker to create a large array using
 * RWDateTime than it is using RWDate. The example below illustrates the
 * difference:
 *
 * \code
 ...
 * \endcode
 *
 * RWDateTime objects can hold a variety of sentinel values that do not
 * refer to actual dates. Consequently it is important to know whether an
 * object holds an actual date before performing many operations. The
 * isSentinel() function returns \c true when an object holds a sentinel
 * value.
 *
 * Note that the use of the isValid() function does not determine whether a
 * member function can be successfully applied to an object, since the null
 * sentinel value represents a valid RWDateTime whose value is not yet
 * determined. For more information about RWDateTime sentinels, see the
 * section on RWDateTime Sentinels in the internationalization chapter of
...
public:
    ...
    /**
     * Specifies whether the constructor should set just the date, just the
     * time, or both. Default is to set both.
     */
    enum SetType {
    ...
    /**
     * Specifies the format to be used for converting between RWCString and
     * RWDateTime using the ISO 8601 standard. This standard converts any
     * date and time to a locale-nonspecific numeric format.
     */
    enum Format {
         ...
         */
        RW_DEPRECATE_ENUMERATOR(iso8601_compat, "Use RWDateTime::iso8601 instead") = 0,
         ...
         * as hour 24 of the previous day rather than hour 0 of the given
         * day.
         */
        iso8601_midnight24 = 2,
    ...
    /**
     * Default constructor. Constructs an RWDateTime with undefined date and
     * time. If \a init_state is set to RWDateTime::setCurrentTime, an
     * instance with the current date and time is created. If \a init_state
     * is set to RWDateTime::null, an instance set to null is created.
     */
    ...
    /**
     * Constructs an RWDateTime from the number of milliseconds since 1
     * January 1901 00:00:00:000 UTC. \c rwint64 is at least a 64-bit
     * integral type.
     */
     ...
     * argument, with the milliseconds portion of the RWDateTime as
     * specified by \a msec. The components are understood to be relative to
     * the time zone passed in, which defaults to local time. Note that the
     * numbering of the months and years in a <tt>struct tm</tt> differs
     * from that used in RWDateTime arguments, which start at the number 1.
     * The arguments included in <tt>struct tm</tt> are shown in the class
     * description.
     *
     * The constructor tests as a precondition the <tt>struct tm</tt>
     * pointer which cannot be null. If null, it asserts in debug builds and
     * throws an object of type RWInternalErr in optimized builds.
     *
     * This constructor produces a valid RWDateTime instance for date-time
     * values between -4713-11-24T00:00:00.000 and
     * 11754508-12-13T23:59:59.999 in the specified time zone. Attempts to
     * create an RWDateTime with values outside this range will result in
     * undefined behavior.
     */
    ...
    /**
     * \deprecated
     * As of SourcePro 10. Provided for compatibility with code that relies
     * on the deprecated RWTime type.
     *
    ...
    /**
     * Constructs an RWDateTime using the RWDate \a d and the time specified
     * by \a hours, \a minutes, \a seconds, and \a msec. The time specified
    ...
    /**
     * Constructs an RWDateTime using the RWTimeTuple \a tt in the specified
     * time zone \a zone. The picosecond value of \a tt is rounded down to
     * the nearest millisecond.
     *
     * If \a tt represents a time before RWDateTime::minDateTime, an
     * RWDateTime::pastSentinel value is constructed.
     *
     * If \a tt represents a time after RWDateTime::maxDateTime, an
     * RWDateTime::futureSentinel value is constructed.
     */
    explicit RWDateTime(const RWTimeTuple& tt, const RWZone& zone = RWZone::local());

    /**
     * Constructs an RWDateTime using the RWTimeTupleOffset \a tt. The
     * picosecond value of \a tt is rounded down to the nearest millisecond.
     *
     * If \a tt represents a time before RWDateTime::minDateTime, an
     * RWDateTime::pastSentinel value is constructed.
     *
     ...
     * RWDateTime::futureSentinel value is constructed.
     */
    explicit RWDateTime(const RWTimeTupleOffset& tt);

    /**
     * Constructs an RWDateTime using the specified day, month, year, hour,
     * minute, second, and millisecond, all relative to the time zone
     * specified, which defaults to local time. The optional locale argument
     * \a loc is used to convert the month name.
     *
     * This constructor produces a valid RWDateTime instance for date-time
     * values between -4713-11-24T00:00:00.000 and
     * 11754508-12-13T23:59:59.999 in the specified time zone. Attempts to
     ...
     * undefined behavior.
     *
     * As a precondition, \a month is tested to determine if it is a null
     ...
     * Constructs an RWDateTime using the specified day, month, year, hour,
     * minute, second, and millisecond, all relative to the time zone
     * specified, which defaults to local time.
     *
     * This constructor produces a valid RWDateTime instance for date-time
     * values between -4713-11-24T00:00:00.000 and
     * 11754508-12-13T23:59:59.999 in the specified time zone. Attempts to
    ...
    /**
     * Converts the string \a str to a date and/or time as indicated by
     * \a set_type, all relative to the time zone specified, which defaults
     * to local time. By default, both the date and time are set using
     * RWLocale \a loc. The constructor expects the date and time portions
     * of the RWCString to be separated by a semicolon when both date and
     * time are being set. The locale argument is used to convert the month
     * name.
     *
     * Because RWLocale cannot rigorously check date input, the member
     * function isValid() must be used to test that the results are a valid
     * date.
     *
     ...
     * values 0 to 99 are treated differently based on how the library was
     * built. In all builds that do not define the macro #RW_CENTURY_REQD,
     ...
     * specified by \a f. RWDateTime currently supports the ISO 8601 format
     * which is numeric and locale-neutral; as such the RWLocale argument is
     * ignored for ISO 8601, but is reserved for future use. The constructor
     * uses the RWZone argument unless the string specifies the UTC time
     * zone or an offset from UTC, in which case the RWZone argument is
     * ignored in favor of UTC. For more information, see the section on
     * applying the ISO standard to time zone offsets, in the
     * internationalization chapter of the <em>Essential Tools Module User's
     * Guide</em>.
     *
     * This constructor handles four-digit year values 0000 to 9999. Passing
     * a string with a year value that is not exactly four digits or is
     * outside of this range will result in undefined behavior.
     *
     * \note
     * \parblock
     ...
     * possible to differentiate between some representations of the two
     * formats. For example. the string \c 190415 could represent either or
     * both of the following:
     * - \e date: the 15th day of the 4th month of the 19th year in the
     * default century
     * - \e time: 19:04:15
     *
     * You can avoid such confusion by using more explicit formats. For
     * example, you can completely and safely avoid all confusion if you are
     * prepend "T" (allowed by the standard in time-only indications) for
     * all times or if you always use extended formats.
     *
     * For more information, see the section on applying the ISO standard to
     * time zone offsets, in the internationalization chapter of the
     * <em>Essential Tools Module User's Guide</em>.
     * \endparblock
     *
     * <em>Two Examples:</em>
     ...
     * \endcode
     *
     * For a date or time supplied singly (for example, "2001-12-24" or
     * "23:59:59"), RWDateTime calls the system to retrieve the current date
     * or time.
     */
     ...
     *
     * This constructor handles years 0 to 9999. Passing a string with a
     * year outside this range will result in undefined behavior.
     *
     * Year values 0 to 99 are treated differently based on how the library
     * was built. In all builds that do not define the macro
     * #RW_CENTURY_REQD, years 0 to 99 are treated as 1900 to 1999. Debug
     * builds that define the macro will produce a debug assertion. Release
     * builds that define the macro will produce an invalid RWDateTime
     * instance.
     */
     ...
     * locale argument. The function defaults to using the RWLocale global
     * locale.
     *
     ...
     *
     * The following table specifies the string produced for a given sentinel
     * value.
     *
     * <table>
     * <tr>
     * <th><b>Sentinel value</b></th>
     * <th><b>String produced</b></th>
     * </tr>
     * <tr>
     * <td>RWDateTime::nullSentinel</td>
     * <td>"NULL"</td>
     * </tr>
     * <tr>
     * <td>RWDateTime::futureSentinel</td>
     * <td>"#INVALID#"</td>
     * </tr>
     * <tr>
     * <td>RWDateTime::pastSentinel</td>
     * <td>"#INVALID#"</td>
     * </tr>
     * <tr>
     * <td>RWDateTime::invalidSentinel</td>
     * <td>"#INVALID#"</td>
     * </tr>
     * <tr>
     * <td>RWDateTime::userSentinel()</td>
     * <td>"#>n<#"
     * where \c n is the integer passed to RWDateTime::userSentinel()</td>
     * </tr>
     * </table>
     */
    RWCString
    ...
    /**
     * \copydoc asString(char, const RWLocale&, const RWZone&)const
     *
     * As a precondition, \a format is tested to determine if it is a null
     * pointer. If null, the method asserts in debug mode, and throws
     * RWInternalErr in optimized builds.
     */
    RWCString
     ...
     *
     * Currently supports the ISO 8601 format. This format is numeric and
     * locale-neutral, and as such does not use the RWLocale argument.
     *
     * For more information, see the section on international standards for
     * dates and times in the internationalization chapter of the
     * <em>Essential Tools Module User's Guide</em>.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
    ...
    /**
     * Returns the value of the Julian day number. This value is one day
     * less than the value of RWDateTime::julian() if the time is before
     * noon UTC (GMT).
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000Z and 11754508-12-13T23:59:59.999Z. Attempts
     ...
     *
     * \note
     * This method throws an exception if called on a sentinel date.
     * Otherwise, it returns a valid day of year.
     */
    unsigned
    day(const RWZone& zone = RWZone::local()) const;

    /**
     * Returns the day of the year (1-366) for this date relative to UTC
     * (GMT).
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000Z and 11754508-12-13T23:59:59.999Z. Attempts
    ...
    /**
     * Returns the day of the month (1-31) for this date relative to UTC
     * (GMT).
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000Z and 11754508-12-13T23:59:59.999Z. Attempts
    ...
    /**
     * Decrements the class's milliseconds data member by \a d *
     * millisecsInDay(), then returns self.
     *
     * \note
     * if this operation results in crossing a DST boundary in the
     * instance's time zone, outputting the date will present with a DST
     * offset as well.
     */
    RWDateTime&
    ...
    /**
     * Decrements the class's milliseconds data member by \a h *
     * millisecsInHour(), then returns self.
     *
     * \note
     * If this operation results in crossing a DST boundary in the
     * instance's time zone, outputting the date will present with a DST
    ...
    /**
     * Decrements the class's milliseconds data member by \a m *
     * millisecsInMin(), then returns self.
     *
     * \note
     * If this operation results in crossing a DST boundary in the
     * instance's time zone, outputting the date will present with a DST
    ...
    /**
     * Decrements the class's milliseconds data member by \a s *
     * millisecsInSec(), then returns self.
     *
     * \note
     * If this operation results in crossing a DST boundary in the
     * instance's time zone, outputting the date will present with a DST
    ...
    /**
     * Decrements the class's milliseconds data member by \a msec, then
     * returns self.
     *
     * \note
     * If this operation results in crossing a DST boundary in the
     ...
     * <tt>struct tm</tt> differs from that used elsewhere in RWDateTime. If
     * the date is a sentinel, the function throws an exception of type
     * RWInternalErr.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
     ...
     * outside that range will result in undefined behavior.
     *
     * As a precondition, \a tmbuf is tested to determine if it is a null
     * pointer. If null, the method asserts in debug mode, and throws
     * RWInternalErr in optimized builds.
     */
    void
    ...
    /**
     * Returns with the <tt>struct tm</tt> argument filled out completely,
     * relative to UTC (GMT). Note that the encoding for months and days of
     * the week used in <tt>struct tm</tt> differs from that used elsewhere
     * in RWDateTime. If the date is a sentinel, the function throws an
     * exception of type RWInternalErr.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000Z and 11754508-12-13T23:59:59.999Z. Attempts
     * to invoke this method with values outside that range will result in
     ...
     * \a mon of the year for this RWDateTime. The default for \a zone is
     * local.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
    ...
    /**
     * Returns the day of the year (1-366) corresponding to the first day of
     * the month and year for this RWDateTime.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
     * specified time zone. Attempts to invoke this method with values
    ...
    /**
     * Returns the hour relative to \a zone. The default for \a zone is
     * local.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
    ...
    /**
     * Increments the class's milliseconds data member by \a d *
     * millisecsInDay(), then returns self.
     *
     * \note
     * If this operation results in crossing a DST boundary in the
    ...
    /**
     * Increments the class's milliseconds data member by \a h *
     * millisecsInHour(), then returns self.
     *
     * \note
     * If this operation results in crossing a DST boundary in the
    ...
    /**
     * Increments the class's milliseconds data member by \a m *
     * millisecsInMin(), then returns self.
     *
     * \note
     * If this operation results in crossing a DST boundary in the
    ...
    /**
     * Increments the class's milliseconds data member by \a s *
     * millisecsInSec(), then returns self.
     *
     * \note
     * If this operation results in crossing a DST boundary in the
    ...
    /**
     * Increments the class's milliseconds data member by \a msec, then
     * returns self.
     *
     * \note
     * If this operation results in crossing a DST boundary in the
    ...
    /**
     * Returns \c true if self is during daylight saving time in the time
     * zone given by \a zone, \c false otherwise.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000Z and 11754508-12-13T23:59:59.999Z. Attempts
     * to invoke this method with values outside that range will result in
    ...
    /**
     * Returns the value of the current date and time as a Modified Julian
     * day number. The integer/whole number part of the double indicates the
     * day, and the fractional part indicates the time. For example, the
     * Modified Julian day number for 1/1/1901 is 2415386. Since Modified
     * Julian days start at midnight GMT, 12 a.m. is 2415386.5 and 6 p.m. is
     * 2415386.75 (these values are computed for the Greenwich meridian - a
     * different time zone modifies the result of \c julianDay).
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000Z and 11754508-12-13T23:59:59.999Z. Attempts
     * to invoke this method with values outside that range will result in
    ...
    /**
     * Sets the value of the date and time to the Modified Julian day number
     * \a j. The integer/whole number part of the double indicates the day,
     * and the fractional part indicates the time. For example, the Julian
     * day number for 1/1/1901 is 2415386. Since Julian days start at
     * midnight GMT, 12 a.m. is 2415386.5 and 6 p.m. is 2415386.75 (these
     * values are interpreted differently in RWDateTime objects that have
     * different time zones).
     *
     * This method produces a valid output for values of \a j from 0 to the
     * maximum value that can be stored in an <tt>unsigned</tt>. Attempts to
     * invoke this method with values outside that range will result in
     * undefined behavior.
     */
    void
    julianDay(double j);
    ...
    /**
     * Returns the number of milliseconds since 00:00:00:000 January 1, 1901
     * UTC.
     */
    rwint64
    milliSeconds() const
    ...
    /**
     * Returns the minute, adjusted to \a zone.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
     * specified time zone. Attempts to invoke this method with values
    ...
    /**
     * Returns the month (1-12) for this date. The default for \a zone is
     * local.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
     ...
     * optional locale argument. The function defaults to using the RWLocale
     * global locale.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
    ...
    /**
     * Returns the date of the next day of the week, where \e Monday = 1,
     * ..., \e Sunday = 7. The variable \a dayNum must be between 1 and 7,
     * inclusive. The default for \a zone is local.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
     * specified time zone. Attempts to invoke this method with values
    ...
    /**
     * Returns the date of the next \a dayName (for example, the date of the
     * next Monday). The weekday name is interpreted according to the
     ...
     * outside that range will result in undefined behavior.
     *
     * As a precondition, \a dayName is tested to determine if it is a null
     * pointer. If null, the method asserts in debug mode, and throws
     * RWInternalErr in optimized builds.
     */
    RWDateTime
     ...
     * \e Monday = 1, ..., \e Sunday = 7. The variable \a dayNum must be
     * between 1 and 7, inclusive. The default for \a zone is local.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
     * specified time zone. Attempts to invoke this method with values
    ...
    /**
     * Returns the day of the previous \a dayName; for example, the date of
     * the previous Monday. The weekday name is interpreted according to the
     * optional locale argument. The function defaults to using the RWLocale
     * global locale.
     *
     * This method produces a valid output for date-time values between
     ...
     * imbued on the stream. The member function isValid() must be used to
     * test whether the results are a valid time.
     */
    std::istream&
     ...
     * imbued on the stream. The member function isValid() must be used to
     * test whether the results are a valid date.
     *
     * This method handles date values with years in the range 0 to 9999.
     * Passing a stream that holds a year outside this range will result in
     * undefined behavior. Year values 0 to 99 are treated differently based
     * on how the library was built. In all builds that do not define the
     * macro #RW_CENTURY_REQD, years 0 to 99 are treated as 1900 to 1999.
     * Debug builds that define the macro will produce a debug assertion.
     * Release builds that define the macro will produce an invalid
     * RWDateTime instance.
     */
    std::istream&
    ...
    /**
     * Restores an instance of RWDateTime from a reference to an RWFile into
     * which an instance has been persisted using the method
     * saveOn(RWFile&)const.
     */
    ...
    /**
     * Saves an instance of RWDateTime to the indicated virtual stream. The
     * RWDateTime can be restored using the method restoreFrom(RWvistream&).
     */
    void saveOn(RWvostream&) const;

    /**
     * Returns the second, adjusted to \a zone.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
     * specified time zone. Attempts to invoke this method with values
    ...
    /**
     * Returns the name of the day of the week for this date, according to
     * the optional locale argument. The function defaults to using the
    ...
    /**
     * Returns the number of the day of the week for this date relative to
     * UTC (GMT).
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000Z and 11754508-12-13T23:59:59.999Z. Attempts
    ...
    /**
     * Returns the year of self. The default for \a zone is local.
     *
     * This method produces a valid output for date-time values between
     * -4713-11-24T00:00:00.000 and 11754508-12-13T23:59:59.999 in the
     * specified time zone. Attempts to invoke this method with values
    ...
    /**
     * Returns the start of daylight saving time (DST) for the given year,
     * in the given time zone. Returns an "invalid" time if DST is not
     * observed in that year and time zone.
     */
    static RWDateTime
    beginDST(unsigned year, const RWZone& zone = RWZone::local());

    /**
     * Returns the number of days in a given month and year. Returns 0 if
     * \a month is not between 1 and 12 inclusive.
     */
    static unsigned
     ...
     * \e day, where \e Monday = 1, ..., \e Sunday = 7. Names are
     * interpreted according to the optional locale argument. The function
     * defaults to using the RWLocale global locale.
     *
     * As a precondition, \a day is tested to determine if it is a null
     * pointer. If null, the method asserts in debug mode, and throws
     * RWInternalErr in optimized builds.
     */
    static unsigned
    ...
    /**
     * Returns \c true if \a day (1-31) is within a given month in a given
     * year.
     */
    static bool
    ...
    /**
     * Returns the end of daylight saving time for the given year, in the
     * given time zone. Returns an "invalid time" if DST is not observed in
     * that year and time zone.
     */
    static RWDateTime
    ...
    /**
     * Returns a number between 1 and 12 corresponding to the given month
     * name \a monthName. Returns 0 for no match. Months are interpreted by
     * the optional locale argument. The function defaults to using the
     * RWLocale global locale.
     *
     * As a precondition, \a monthName is tested to determine if it is a
     * null pointer. If null, the method asserts in debug mode, and throws
     * RWInternalErr in optimized builds.
     */
    static unsigned
    ...
    /**
     * Returns the name of month, where \e January = 1, ..., \e December = 12.
     * Months are interpreted by the optional locale argument. The function
     * defaults to using the RWLocale global locale.
     */
    static RWCString
    ...
    /**
     * Returns the name of the day of the week \a dayNum where \e Monday =
     * 1, ..., \e Sunday = 7. Days are interpreted by the optional locale
     * argument. The function defaults to using the RWLocale global locale.
     */
    static RWCString
...
private:
    ...
    /**
     * Outputs the date and time stored in \a dt on \b std::ostream \a s.
     * The output produced behaves as if a call was made to
     * asString(char, const RWLocale&, const RWZone&)const with the 'c'
     * format specifier passed as the first argument, and the locale imbued
     * on the stream \a s passed as the second argument.
     *
     * \note
     * RWDateTime objects serialized via this function and restored using
     * operator>>(std::istream&, RWDateTime&) are not guaranteed to roundtrip,
     * and should not be used to persist date/time information. In order to
     * persist RWDateTime objects such that they can be restored, see
     * operator<<(RWvostream&, const RWDateTime&) and
     * operator>>(RWvistream&, RWDateTime&).
     */
    friend RW_TOOLS_SYMBOLIC std::ostream&
    operator<< (std::ostream& s, const RWDateTime& dt);

    /**
     * Reads \a dt from \b std::istream \a s. The function reads one full
     * line, and the string it contains is converted according to the locale
     * imbued on the stream. If the string cannot be converted to an
     * RWDateTime, \a dt is set to RWDateTime::invalid.
     *
     * \note
     * RWDateTime objects serialized via
     * operator<<(std::ostream&, const RWDateTime&) and restored using this
     * function are not guaranteed to roundtrip, and should not be used to
     * persist date/time information. In order to persist RWDateTime objects
     * such that they can be restored, see
     * operator<<(RWvostream&, const RWDateTime&) and
     * operator>>(RWvistream&, RWDateTime&).
     */
    ...
    /**
     * Returns the date \a s milliseconds in the future from the date \a d.
     */
     ...
     *
     * \exception RWInternalErr
     * Thrown if either \a dt1 or \a dt2 is a sentinel.
     */
