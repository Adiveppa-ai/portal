 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tools/decport.h#1 $
 *
 **********************************************************************
 *
 ...
 * Returns an RWDecimalPortable which is the result of rounding self
 * according to the applied specifications.
 */
...
/**
 * \ingroup tools_miscellaneous_group currency_decimal_numbers_group
 *
 * \brief
 * Represents an arbitrary precision decimal fraction, providing maximum
 * portability.
 *
 ...
 * However, its portability is gained at the expense of speed and power. For
 * example, RWDecimalPortable does not support all of the functionality of
 * the other decimal classes in the Currency Module. In most cases we
 * recommend that you use another of the decimal classes from the Currency
 * Module, but in situations in which the portability of a number is
 * paramount, RWDecimalPortable is useful.
 *
 ...
 * behaving similarly to the C++ Standard Library function \b std::strtod().
 * For example, a string \c "12E3" creates an RWDecimalPortable value of 12000.
 *
 * If an RWDecimalPortable is provided an invalid string, either in the
 * constructor or through assignment, it <em>does not</em> throw an
 * exception. Use isNumber() to determine if the object is valid.
 *
 * \code
 ...
 * Note that:
 *
 * - The validity or non-validity of the provided string is
 * locale-dependent, as discussed below in the description for the
 * constructor that takes a string, RWDecimalPortable(const char*).
 * - In the first example above, the likely value of the RWDecimalPortable
 * would be 555. That is, the string parser parses until it encounters an
 * invalid character, and constructs the number from the characters up to
 * that point.
 *
 * \synopsis
...
public:
    ...
    /**
     * Parses the initial portion of \a input formatted as a decimal number
     * up to the end of the string, or the first non-decimal character,
     * whichever comes first. The parse behavior is locale-dependent (for
     * decimal points, thousands separator, grouping, etc). If no locale is
     * specified, the default locale is used, and parsing behavior will be
     * defined by that locale, creating possibly unexpected results. To
     * ensure predictable and expected behavior, the program should
     * explicitly specify the locale before using this constructor, and
     * input strings should be formatted according the numeric string facets
     * for that locale.
     *
     * For more information on conversions between strings and numbers, see
     * the section on numbers in the internationalization chapter of the
    ...
    /**
     * Returns self as an RWCString. Format is, for example -23.21 or 0.32
     * or 432.
     */
    operator  RWCString() const;

    /**
     * Returns self as an RWCString using the global locale as determined by
     * the static member function RWLocale::global() for numeric formatting.
    ...
    /**
     * Returns self as an RWCString using the provided RWLocale instance for
     * numeric formatting. For example, an English locale could return
     * strings such as -23.21, 0.32, or 432.
     */
    ...
    /**
     * Returns \c false if self represents a \c NaN, a missing value,
     * infinity or a \c NULL; otherwise returns \c true.
     *
     * \note
     * In this instance, the mathematical rather than the RDBMS meaning of
     * NULL is intended. Please see RWDBNullIndicator.
     */
    bool isNumber() const
    {
    ...
    /**
     * Removes leading zeros before self's decimal point and trailing zeros
     * after.
     */
    void trimZeros();
    ...
    /**
     * Returns the number of bytes required to store self using the saveOn()
     * methods.
     */
...
/**
 * \relates RWDecimalPortable
 * Rudimentary addition operator with minimal efficiency. For efficiency,
 * use one of the decimal classes of the SourcePro Analysis product's
 * Currency Module.
 */
RW_TOOLS_SYMBOLIC RWDecimalPortable
 ...
 * efficiency, use one of the decimal classes of the SourcePro Analysis
 * product's Currency Module.
 */
RW_TOOLS_SYMBOLIC RWDecimalPortable
operator*(const RWDecimalPortable& x, const RWDecimalPortable& y);
...
/**
 * \relates RWDecimalPortable
 * Rudimentary negation operator with minimal efficiency. For efficiency,
 * use one of the decimal classes of the SourcePro Analysis product's
 * Currency Module.
 */
...
/**
 * \relates RWDecimalPortable
 * Rudimentary subtraction operator with minimal efficiency. For efficiency,
 * use one of the decimal classes of the SourcePro Analysis product's
 * Currency Module.
 */
inline RWDecimalPortable
 ...
 *
 * \note
 * If the \ref RWDecimalBase::State "state" of either one of the operands is
 * anything other than \ref RWDecimalBase::normal "normal" or
 * \ref RWDecimalBase::infinitystate "infinitystate", the result of the
...
/**
 * \relates RWDecimalPortable
 * Relational operator. Returns \c true if \a x is greater than or equal to
 * \a y.
 *
 * \note
 * If the \ref RWDecimalBase::State "state" of either one of the operands is
...
/**
 * \relates RWDecimalPortable
 * Relational operator. Returns \c true if \a x is less than or equal to
 * \a y.
 *
 * \note
 * If the \ref RWDecimalBase::State "state" of either one of the operands is
