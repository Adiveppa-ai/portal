 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/bintree.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Represents a group of ordered elements, as sorted by a comparison method,
...
public:
    ...
    /**
     * Returns \c true if self is a subset of the collection \a bt. That is,
     * every item in self must compare equal to a unique item in \a bt.
     */
    bool operator<=(const RWBinaryTree& bt) const;

    /**
     * Returns \c true if self and \a bt are equivalent. That is, they must
     * have the same number of items and every item in self must compare
     * equal to a unique item in \a bt.
     */
    bool operator==(const RWBinaryTree& bt) const;

    /**
     * Special function to balance the tree. In a perfectly balanced binary
     * tree with no duplicate elements, the number of nodes from the root to
     * any external (leaf) node differs by at most one node. Since this
     * collection allows duplicate elements, a perfectly balanced tree is
     * not always possible. Preserves the order of duplicate elements.
     */
    void balance();

    /**
     * Returns the number of nodes between the root node and the farthest
     * leaf. A RWBinaryTree with one entry will have a height of 1. Note
     * that the entire tree is traversed to discover this value.
     */
    size_t height() const;

    /**
     * Invokes the function pointer \a ap on each item in the collection, in
     * order, from smallest to largest. The function pointed to by \a ap
    ...
    /**
     * Returns the first item that compares equal to the item pointed to by
     * \a target, or #rwnil if no item was found.
     */
    virtual RWCollectable* find(const RWCollectable* target) const;

    /**
     * Inserts the item \a c into the collection and returns it. Returns
     * #rwnil if the insertion was unsuccessful. The item \a c is inserted
     * according to the value returned by compareTo(). insert() does not
     * automatically balance the RWBinaryTree. Be careful not to call
     * insert() on a long sequence of sorted items without also calling
     * balance() since the result will be very unbalanced (and therefore
     * inefficient).
     */
    ...
    /**
     * Returns the number of items that compare equal to the item pointed to
     * by \a target.
     */
    virtual size_t occurrencesOf(const RWCollectable* target) const;

    /**
     * Removes the first item that compares equal to the object pointed to
     * by \a target and returns it. Returns #rwnil if no item was found.
     */
 ...
 *
 * Iterator for class RWBinaryTree. Traverses the tree from the smallest to
 * largest element, where "smallest" and "largest" are defined by the
 * virtual function RWBinaryTree::compareTo(). Note that this approach is
 * generally less efficient than using the member function
 * RWBinaryTree::apply().
 *
 ...
 * or some other (valid) operation.
 *
 * Once the iterator has advanced beyond the end of the collection it is no
 * longer valid. Continuing to use it leads to undefined results.
 *
 * \synopsis
 * // Smalltalk typedef:
...
public:
    ...
    /**
     * Moves iterator to the next item that compares equal to the object
     * pointed to by \a target and returns it. If no item is found, returns
 ...
 *
 * \brief
 * Iterator for class RWBinaryTree that allows no changes to the values in
 * the binary tree.
 *
 * Provides a \c const iterator for class RWBinaryTree. Traverses the tree
 * from the smallest to the largest element, where "smallest" and "largest"
...
public:
    ...
    /**
     * Advances iterator to the next largest element and returns a pointer
     * to it. Returns #rwnil when the end of the collection is reached.
     */
