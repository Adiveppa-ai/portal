 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tphset.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * RWTPtrHashSet will not accept an item that is equivalent to an item
 * already in the collection. (RWTPtrHashMultiSet may contain multiple items
 * that are equivalent to each other.) Equality is based on the equality
 * object and not on the equality operator.
 *
 * The \c value type must have operator==() defined. This requirement is
 ...
 * \sa
 * \parblock
 * Class RWTPtrHashMultiSet offers the same interface to a pointer-based
 * collection that accepts multiple items that compare equal to each other.
 *
 * Class \ref rw_hashset "rw_hashset<T*,hasher,key_equal,A>" is the C++
 * Standard Library collection that serves as the underlying implementation
 * for RWTPtrHashSet.
 * \endparblock
 *
...
public:
     ...
     * are stored. (If the equality operator is reflexive, the container
     ...
     * compares equal to <tt>*\a a</tt>, returns \c t. Otherwise, returns
     * #rwnil.
     */
    ...
    /**
     * Returns the number of elements \c t that compare equal to
     * <tt>*\a a</tt>
     */
     ...
     * <tt>*\a a</tt>. Returns the number of items removed.
     */
     ...
     * \a s. Self is replaced by the result.
     *
     * \note
     ...
     * fail if the collection already holds an element with an equivalent
     * key.
     */
    ...
    /**
     * Changes the capacity of self by creating a new hashed set with a
     * capacity of \a sz. This function copies every element of self into
     * the new container and finally swaps the internal representation of
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Constructs an empty hashed set. The underlying hash table
     * representation has \a sz buckets, uses \a h for its hashing function,
...
#  endif // !RW_NO_RVALUE_REFERENCES
     ...
     * \a last). The underlying hash table representation has \a sz buckets,
     * uses \a h for its hashing function, and uses \a eq to determine
    ...
    /**
     * Creates an empty hashed set which uses the hash object \a h and has
     * an initial hash table capacity of \a sz.
     *
     * \note
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrHashSet::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTPtrHashSet.
 *
 * RWTPtrHashSetIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTPtrHashSet. (An STL iterator
 * interface is provided through RWTPtrHashSet::iterator.)
 *
 * Iteration over an RWTPtrHashSet is pseudorandom and dependent on the
...
public:
    /**
     * Creates an iterator for the hashed set \a h. The iterator begins in
     * an undefined state and must be advanced before the first element will
     * be accessible.
     */
     ...
     * iteration, self referenced the last association in the multi-map,
     * self will now point to an undefined value and a value equivalent to
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrHashSet::const_iterator to iterate over
 * the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTPtrHashSet.
 *
 * RWTPtrHashSetConstIterator provides a \c const iterator interface to C++
 * Standard Library-based RWTPtrHashSet collections.
 *
 * Iteration over an RWTPtrHashSet is pseudorandom and dependent on the
 * capacity of the underlying hash table and the hash function being used.
 *
...
public:
    ...
    /**
     * Advances self to the next element. If the iterator has just been
     * reset or created, self now references the first element. If, before
     * iteration, self referenced the last association in the multi-map,
     * self now points to an undefined value and a value equivalent to
     * \c false is returned. Otherwise, a value equivalent to \c true is
     * returned.
     */
    bool operator++()    // Advance and test
    {
