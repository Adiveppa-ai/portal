 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/disktree.h#1 $
 *
 **********************************************************************
 *
 ...
 * applyToKeyAndValue(RWdiskTreeApply, void*). This function <em>may
 * not</em> change the first argument, i.e. the key. The last argument can
 * be anything and is passed through from the call to
...
extern "C" {
     ...
     * depending on whether the first argument argument is less than, equal
     * to, or greater than the second argument, respectively. The third
     * argument is the key length.
     *
     * \sa
...
/**
 * \ingroup tools_file_system_group
 * \class RWBTreeOnDisk
 *
 * \brief
 * Represents an ordered collection of associations of keys and values, with
 * keys ordered based on an external function and with duplicate keys not
 * allowed.
 *
 ...
 * an external function. The user can set this function. Duplicate keys are
 * not allowed. Given a key, the corresponding value can be found.
 *
 * This class is specifically designed for managing a B-tree in a disk file.
 * Keys, defined to be arrays of \c chars, and values, defined by the
 * typedef \c RWstoredValue, are stored and retrieved from a B-tree. The
 * values can represent offsets to locations in a file where the objects are
 * stored.
 *
 * The key length is set by the constructor. By default, this value is 16
 ...
 * length. (Remember, storage and comparison will \e not stop with a null
 * value.)
 * - Use a comparison function (such as \b std::memcmp()) that ignores
 * nulls.
 *
 * This class is meant to be used with class RWFileManager, which manages
 * the allocation and deallocation of space in a disk file.
 *
 * When you construct an RWBTreeOnDisk, you give the location of the root
 * node in the constructor as argument \c start. If this value is #RWNIL
 * (the default), the location is retrieved using RWFileManager::start().
 * You can also use the enumeration #createMode to set whether to use an
 * existing tree (creating one if one doesn't exist) or to force the
 * creation of a new tree. The location of the resultant root node can be
 * retrieved using member function baseLocation().
 *
 * More than one B-tree can exist in a disk file. Each must have its own
 * separate root node. This can be done by constructing more than one
 * RWBTreeOnDisk, each with #createMode set to \c create.
 *
 * The \e order of the B-tree can be set in the constructor. Larger values
...
public:
     ...
     * 16-byte key lengths. It is used only when creating a new tree. If
     * opening a tree for update, its type is determined automatically at
     * runtime.
     */
    enum styleMode {
        /**
        ...
        /**
         * Initialize a new tree using V5.X style trees. The key length is
         * fixed at 16 bytes.
         */
        V5Style
    };
    ...
    /**
     * This enumeration is used by the constructor to determine whether to
     * force the creation of a new tree.
     */
    enum createMode {

        /**
         * Look in the location given by the constructor argument \c start
         * for the root node. If valid, use it. Otherwise, allocate a new
         * tree. This is the default.
         */
        autoCreate,

        /**
         * Forces the creation of a new tree. The argument \c start is
         * ignored.
         */
        create
    };
     ...
     *
     * \param f
     * The file in which the B-tree is to be managed. This is the only
     * required parameter.
     *
     * \param cacheBlocks
     ...
     *
     * \param cmode
     * Determines whether to force the creation of a new tree or to attempt
     ...
     *
     * \param ignoreNull
     * Controls whether to allow embedded nulls in keys. If \c false (the
     * default), then keys end with a terminating null. If \c true, then all
     * \a keylen bytes are significant. Ignored when opening an existing
     * tree.
     *
     * \param start
     * Where to find the root node. If set to #RWNIL (the default), uses the
     * value returned by the RWFileManager::start() member function. Ignored
     * when creating a new tree.
     *
     * \param smode
     * Sets the type of B-tree to create, allowing backwards compatibility,
     * as described in the general class description. The default specifies
     * new V6.X style B-trees. Ignored when opening an existing tree.
     *
     * \param halfOrder
     * One half the order of the B-tree (that is, one half the number of
     * entries in a node). Ignored when opening an existing tree.
     *
     * \param minFill
     * The minimum number of entries allowed in a node (must be less than or
     * equal to \a halfOrder). Ignored when opening an existing tree.
     */
     ...
     * largest, calling the user-provided function pointed to by \a ap with
     * the key and value as arguments. This function should have the
     * prototype:
     *
     * \code
     ...
     * \endcode
     *
     * The function \c yourApplyFunction \b cannot change the key. The value
     * \a x can be anything and is passed through from the call to this
     * member function. This member function may throw an RWFileErr
     * exception.
     */
     ...
     * RWFileManager. This is the value you will pass to a constructor as
     * the \c start argument when you want to open one of several trees
     * stored in one managed file.
     */
    ...
    /**
     * Removes all items from the collection. This member function may throw
    ...
    /**
     * Sets the location where this RWBTreeOnDisk keeps your own
     * application-specific information to \a newlocation. Returns the
     * previous value.
     */
     ...
     * \ref contains(const char*) const "contains(key)".
     */
    ...
    /**
     * Returns \c true if \a key is found, otherwise \c false. If
     * successful, the found key is returned as a reference in \a retKey.
     ...
     * Returns \c true if \a key is found, otherwise \c false. If
     * successful, the found key is returned as a reference in \a retKey,
     * and the value is returned as a reference in \a foundVal. This member
     * function may throw an RWFileErr exception.
     */
     ...
     * pointed to by \a key. Returns #RWNIL if no key is found. This member
     * function may throw an RWFileErr exception.
     */
    ...
    /**
     * Returns the height of the RWBTreeOnDisk. This member function may
     ...
     * insertion, \c false otherwise. This member function may throw an
     * RWFileErr exception.
     */
    ...
    /**
     * Returns the length of the keys for this RWBTreeOnDisk. This number is
     * set when the tree is first constructed, and cannot be changed.
     */
    ...
    /**
     * Returns the minimum number of items that may be found in any non-root
     * node in this RWBTreeOnDisk. This number is set when the tree is first
     ...
     * This number is calculated from the length of the keys and the order
     * of the tree, and cannot be changed. We make it available for your
    ...
    /**
     * Return half the maximum number of items that may be stored in any
     * node in this RWBTreeOnDisk. This number is set when the tree is first
     * constructed and cannot be changed. This method should have been
     * renamed \c halfOrder but is still called order() for backward
     * compatibility.
     */
    ...
    /**
     * Removes the key and value pair with the key that matches \a key. This
    ...
    /**
     * Removes the key and value pair with the key that matches \a key. The
     * removed data is returned in the reference arguments \a retKey and
     * \a retVal. The function returns \c true if the remove operation
     * succeeds, or otherwise \c false. This function may throw an RWFileErr
     * exception.
     */
    bool removeKeyAndValue(const char* key, RWCString& retKey, RWstoredValue& retVal);

    /**
     * Removes the key and value pair with key that matches \a key. The
     * removed value is returned in the reference argument \a retVal. The
     * function returns \c true if the remove operation succeeds; or
     * otherwise \c false. This function may throw an RWFileErr exception.
     */
    ...
    /**
     * Attempts to replace the \c RWstoredValue now associated with \a key
     * by the value \a newVal. If successful, the previous value is returned
     * by reference in \a oldVal, and the method returns \c true, or
     * otherwise \c false.
     */
     ...
     * function. The function should return a number less than, equal to, or
     * greater than zero, depending on whether the first argument is less
     * than, equal to or greater than the second argument, respectively. The
     * third argument is the key length. Possible choices (among others) are
     * \c std::strncmp() (the default), or \c std::strnicmp() (for
     * case-independent comparisons). If you set the comparison function to
     * behave differently after data has already been stored, RWBTreeOnDisk
     * will exhibit undefined behavior that is probably incorrect.
     */
