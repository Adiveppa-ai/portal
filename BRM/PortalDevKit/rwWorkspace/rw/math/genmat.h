 ...
 * genmat.h
 *
 * $Id: //spro/rel2016.1/rw/math/genmat.h#1 $
 *
...
#  error macro RW_MATH_MATRIX_PRODUCT is no longer supported.
#endif
...
public:
    /**
     * Equality operator. Returns \c true if the RWGenMatIteratorBase
     * objects point to the same element.
     */
    ...
    /**
     * Inequality operator. Returns \c false if the RWGenMatIteratorBase
     * objects point to the same element.
     */
    bool operator !=(const RWGenMatIteratorBase<T>& x) const
    {
    ...
    /**
     * Less than operator. Determines the relative logical position between
     * two RWGenMatIteratorBase objects, not necessarily the relative memory
     * location of the elements to which they point.
     */
    bool operator <(const RWGenMatIteratorBase<T>& x) const
    {
     ...
     * between two RWGenMatIteratorBase objects, not necessarily the
     * relative memory location of the elements to which they point.
     */
    bool operator >(const RWGenMatIteratorBase<T>& x) const
    {
    ...
    /**
     * Less than or equal to operator. Determines the relative logical
     * position between two RWGenMatIteratorBase objects, not necessarily
     * the relative memory location of the elements to which they point.
     */
    bool operator <=(const RWGenMatIteratorBase<T>& x) const
    {
        return (*this < x) || (*this == x);
     ...
     * position between two RWMathVecIteratorBase objects, not necessarily
     * the relative memory location of the elements to which they point.
     */
    bool operator >=(const RWGenMatIteratorBase<T>& x) const
    {
 ...
 *
 * Class RWGenMatIterator is the random access iterator for the RWGenMat
 * collection class. The RWGenMatIterator class behaves like a pointer to
 * type \c T, and is used to increment through elements of an RWGenMat.
 *
 * \note
...
public:
    ...
    /**
     * Prefix increment operator. The iterator points to the next element in
     * the RWGenMat. No error condition is set if the iterator goes past the
     * bounds of the matrix. Compare to RWGenMat::begin() and
     * RWGenMat::end() to check that the iterator position is valid.
     */
    ...
    /**
     * Prefix decrement operator. The iterator points to the previous
     * element in the RWGenMat. No error condition is set if the iterator
     * goes past the bounds of the matrix. Compare to RWGenMat::begin() and
     * RWGenMat::end() to check that the iterator position is valid.
     */
    RWGenMatIterator<T>& operator --()
    {
    ...
    /**
     * Postfix increment operator. The iterator points to the next element
     * in the RWGenMat. No error condition is set if the iterator goes past
     * the bounds of the matrix. Compare to RWGenMat::begin() and
     * RWGenMat::end() to check that the iterator position is valid.
     */
    ...
    /**
     * Postfix decrement operator. The iterator points to the previous
     * element in the RWGenMat. No error condition is set if the iterator
     * goes past the bounds of the matrix. Compare to RWGenMat::begin() and
     * RWGenMat::end() to check that the iterator position is valid.
     */
    ...
    /**
     * The RWGenMatIterator is moved forward \a d elements in the RWGenMat.
     * No error condition is set if the iterator goes past the bounds of the
     * matrix. Incrementing by \a d and then decrementing by \a d returns
     * the iterator to its original position, even if doing so takes it past
    ...
    /**
     * The RWGenMatIterator is moved backward \a d elements in the RWGenMat.
     * No error condition is set if the iterator goes past the bounds of the
     * matrix. Incrementing by \a d and then decrementing by \a d returns
     * the iterator to its original position, even if doing so takes it past
     * the bounds of the matrix.
    ...
    /**
     * If the return value is positive, the RWGenMatIterator is that many
     * elements past the RWGenMatIterator \a x. If the return value is
     * negative, the RWGenMatIterator is that many elements before the
     * RWGenMatIterator \a x.
     */
    ...
    /**
     * Returns an RWGenMatIterator that is \a n elements past or before
     * self.
     */
    ...
    /**
     * Returns a reference to the element \a n elements after self if \a n
     * is positive, or \a n elements before \a n if \a n is negative.
     */
 ...
 * The random access iterator for the RWGenMat collection class.
 *
 * Class RWGenMatConstIterator is the random access iterator for the
 * RWGenMat collection class. The RWGenMatConstIterator class behaves like a
 * pointer to type T and is used to increment through elements of an
 * RWGenMat. The RWGenMatConstIterator differs from a plain RWGenMatIterator
 * in that the value of a dereferenced RWGenMatConstIterator may not be
 * changed.
 *
 * \note
...
public:
     ...
     * RWGenMatConstIterator instances can be obtained via the
     * RWGenMat::begin() and RWGenMat::end() member functions of RWGenMat.
     */
    ...
    /**
     * Prefix increment operator. The RWGenMatConstIterator points to the
     * next element in the RWGenMat. No error condition is set if the
     * RWGenMatConstIterator goes past the bounds of the matrix. Compare to
     * RWGenMat::begin() and RWGenMat::end() to check that the
    ...
    /**
     * Prefix decrement operator. The RWGenMatConstIterator points to the
     * previous element in the RWGenMat. No error condition is set if the
     * RWGenMatConstIterator goes past the bounds of the matrix. Compare to
     * RWGenMat::begin() and RWGenMat::end() to check that the
     * RWGenMatConstIterator position is valid.
     */
    ...
    /**
     * Postfix increment operator. The RWGenMatConstIterator points to the
     * next element in the RWGenMat. No error condition is set if the
     * RWGenMatConstIterator goes past the bounds of the matrix. Compare to
     * RWGenMat::begin() and RWGenMat::end() to check that the
     * RWGenMatConstIterator position is valid.
    ...
    /**
     * Postfix decrement operator. The RWGenMatConstIterator points to the
     * previous element in the RWGenMat. No error condition is set if the
     * RWGenMatConstIterator goes past the bounds of the matrix. Compare to
     * RWGenMat::begin() and RWGenMat::end() to check that the
     * RWGenMatConstIterator position is valid.
    ...
    /**
     * The RWGenMatConstIterator is moved forward \a d elements in the
     * RWGenMat. No error condition is set if the RWGenMatConstIterator goes
     * past the bounds of the matrix. Incrementing by \a d and then
     * decrementing by \a d returns the RWGenMatConstIterator to its
     * original position, even if doing so takes it past the bounds of the
     * matrix.
     */
    ...
    /**
     * The RWGenMatConstIterator is moved backward \a d elements in the
     * RWGenMat. No error condition is set if the RWGenMatConstIterator goes
     * past the bounds of the matrix. Incrementing by \a d and then
     * decrementing by \a d returns the RWGenMatConstIterator to its
     * original position, even if doing so takes it past the bounds of the
    ...
    /**
     * Returns a RWGenMatConstIterator that is \a n elements past (or
     * before) self.
     */
    ...
    /**
     * Returns a reference to the element \a n elements after self if \a n
     * is positive, or before self if \a n is negative.
     */
...
public:
    ...
    /**
     * Typedef for the promotion type. For more information, see
    ...
    /**
     * Typedef for the function pointer used in the method apply(). For more
     * information, see rw_numeric_traits<T>::mathFunType.
     */
    typedef typename rw_numeric_traits<T>::mathFunType     mathFunType;

    /**
     * Typedef for the function pointer used in the method apply2(). For
     * more information, see rw_numeric_traits<T>::mathFunType2.
     */
...
public:
    ...
    /**
     * Constructs a 0 x 0 (null) matrix, useful for declaring vectors of
     * matrices. This matrix, like any other matrix, can subsequently be
     * reshaped or resized. See member functions reshape() and resize().
     */
    RWGenMat() : RWMatView() {}
    ...
    /**
     * Constructs a matrix with a specified number of rows and columns. The
     * optional storage indicator determines whether the matrix is stored in
     * \c ROW_MAJOR or \c COLUMN_MAJOR order. The #RWUninitialized type is
     * an enumeration type with only one value, #rwUninitialized. The
     * #rwUninitialized argument is used to distinguish the last dimension
    ...
    /**
     * Constructs a matrix with \a m rows and \a n columns. Initialized with
     * random numbers generated by \a r. The optional storage indicator
     * determines whether the matrix is stored in \c ROW_MAJOR or
    ...
    /**
     * Constructs a matrix from the null terminated character string \a s.
     * The format of the character string is the same as that expected by
     * the global operator
     * \ref operator>>(std::istream&, RWGenMat<T>&) "operator>>" described
     * in this entry. The optional storage indicator determines whether the
     * matrix is stored in \c ROW_MAJOR or \c COLUMN_MAJOR order.
     */
    RWGenMat(const char* s, Storage = COLUMN_MAJOR);

    /**
     * Copy constructor. The new matrix and the old matrix both view the
     * same data.
     */
    ...
    /**
     * A matrix with \a m rows and \a n columns is constructed, using the
     * data in the vector \a dat as initial data. A copy of \a dat is made.
     * The vector \a dat must have at least \a n * \a m elements. The
     * optional storage indicator determines whether the matrix is stored in
     * \c ROW_MAJOR or \c COLUMN_MAJOR order.
     */
    RWGenMat(const T* dat, size_t m, size_t n, Storage = COLUMN_MAJOR);

    /**
     * A matrix with \a m rows and \a n columns is constructed, using the
     * data in the vector \a v. The matrix is a new view of the same data as
     * \a v, so no copy of the data is made. The optional storage indicator
     * determines whether the matrix is stored in \c ROW_MAJOR or
     * \c COLUMN_MAJOR order. If the vector does not have length \a m times
     * \a n, an exception of type \c MATX_NUMBERPOINTS is thrown.
     */
     ...
     * \a re and \a im, with the real part of the matrix equal to \a re and
     * the imaginary part equal to \a im. A new copy of the data is made.
     * The optional storage indicator determines whether the matrix is
     * stored in \c ROW_MAJOR or \c COLUMN_MAJOR order.
     */
     ...
     * in the matrix. A function of type RWGenMat<T>::mathFunType takes and
     * returns a \c T. A function of type RWGenMat<T>::mathFunType2 takes a
     * \c T and returns an RWGenMat<T>::norm_type. For a description of this
     * type, see rw_numeric_traits.
     */
    ...
    RWGenMat<norm_type> apply2(mathFunType2 f) const
    {
        ...
        norm_type* dp = (norm_type*)temp.data();
        typename RWGenMat<T>::const_iterator it;
    ...
    /**
     * Returns an iterator that points to the element in the first row and
     * first column of self. The optional storage specifier determines the
     * order in which the iterator traverses the elements of the matrix; the
     * specifier is independent of the storage format of the matrix. A
     * \c COLUMN_MAJOR iterator proceeds down each column while a
     * \c ROW_MAJOR iterator proceeds along rows.
     *
     * \warning Binary difference and comparison operators between a
     * \c ROW_MAJOR iterator and a \c COLUMN_MAJOR iterator have
    ...
    /**
     * Returns an iterator that points to one element past the last element
     * in the matrix. The optional storage specifier determines the order in
     * which the iterator traverses the elements of the matrix; the
     * specifier is independent of the storage format of the matrix. A
     * \c COLUMN_MAJOR iterator proceeds down each column, while a
     * \c ROW_MAJOR iterator proceeds along rows.
     *
    ...
    /**
     * Returns an iterator that points to the element in the last row and
     * last column of self. The optional storage specifier determines the
     * order in which the iterator traverses the elements of the matrix; the
     * specifier is independent of the storage format of the matrix. A
     * \c COLUMN_MAJOR iterator proceeds down each column while a
     * \c ROW_MAJOR iterator proceeds along rows.
     ...
     * element in the matrix. The optional storage specifier determines the
     * order in which the iterator traverses the elements of the matrix; the
     * specifier is independent of the storage format of the matrix. A
     * \c COLUMN_MAJOR iterator proceeds down each column, while a
     * \c ROW_MAJOR iterator proceeds along rows.
    ...
    /**
     * Returns the number of bytes required to store the matrix to an RWFile
     * using member function
     * \ref RWGenMat::saveOn(RWFile&) const "saveOn(RWFile&)".
     */
    ...
    /**
     * Returns the stride to move through the data from one column to the
     * next. Could be computed as <tt>&A(i,j+1)-&A(i,j)</tt>.
     */
    int        colStride() const
    {
    ...
    /**
     * Returns a pointer to the start of a matrix's data. Should be used
     * with care, as this function accesses the matrix's data directly.
     */
    T*    data()
    {
        return (T*)begin_;
    ...
    RWMathVec<T>  dataVec(Storage storage = COLUMN_MAJOR) const   // Returns vector containing copy of matrix data
    {
        ...
        RW_POSTCONDITION(vindex == v.length());
        return v;
    }
    ...
    /**
     * Invoking deepenShallowCopy() for a matrix guarantees that there is
     * only one reference to that object and that its data are in contiguous
     * storage. The optional storage indicator determines whether the matrix
     ...
     * the main diagonal; <tt>\a idiag = n</tt> implies \c n diagonal slices
     * up from center; <tt>\a idiag = -n</tt> implies \c n diagonal slices
     * down.
     */
    ...
    /**
     * Returns a matrix pick. The results can be used as an lvalue. You can
     * think of the "picked" submatrix as specifying an intersection of the
     * rows listed in \a v1 and the columns listed in \a v2. Before using
     * this function, you must include the header file
     * \c rw\\math\\matpick.h.
     */
    ...
    /**
     * Changes the size of the matrix to \a m rows and \a n columns, adding
     * 0s or truncating as necessary. The optional storage indicator
     * determines whether the matrix is stored in \c ROW_MAJOR or
     * \c COLUMN_MAJOR order.
     */
    ...
    /**
     * Changes the size of the matrix to \a m rows and \a n columns. After
     * reshaping, the contents of the matrix are undefined; that is, they
     * can be garbage, and probably will be. The optional storage indicator
     * determines whether the matrix is stored in \c ROW_MAJOR or
     * \c COLUMN_MAJOR order.
     */
    ...
    /**
     * Restores self from an RWFile. The optional storage indicator
     * determines whether the matrix is stored in \c ROW_MAJOR or
     * \c COLUMN_MAJOR order. To use these functions with a user-defined
     * type T, the corresponding operator \c >> must be defined:
     *
     * \code
    ...
    /**
     * Restores self from a virtual stream. The optional storage indicator
     * determines whether the matrix is stored in \c ROW_MAJOR or
     * \c COLUMN_MAJOR order. To use these functions with a user-defined
     * type T, the corresponding operator \c >> must be defined:
     *
    ...
    /**
     * Returns the stride required to move through the data from one row to
     * the next. Could be computed as <tt>&A(i+1,j)-&A(i,j)</tt>.
     */
    int        rowStride() const
    {
    ...
    /**
     * Stores self in a binary format to an RWFile. If T is a user-defined
     * type, the shift \c operator<< must be defined:
     *
     * \code
    ...
    /**
     * Stores self to a virtual stream. If T is a user-defined type, the
     * shift \c operator<< must be defined:
     *
     * \code
    ...
    /**
     * Returns a vector that views a slice of the matrix. The slice begins
     * at element \a i,j and extends for \a n elements. The increment
     * between successive elements in the vector is \a rowstride rows and
     * \a colstride columns. For example:
     *
     * \code
     * A.slice(n-1,0, n, -1,1)
     * \endcode
    ...
    /**
     * Returns a matrix that views a slice of the matrix. The slice begins
     * at element \a i, \a j and contains \a m rows and \a n columns. The
     * increment between successive elements in the slice's row is
     * \a rowstr1 rows and \a colstr1 columns. The increment between
     * successive elements in the slice's column is \a rowstr2 rows and
     * \a colstr2 columns. For example:
     *
     * \code
     * A.slice(n-1,0, n,n, -1,0, 0,1)
     * \endcode
     *
     * returns a view of the \a n x \a n matrix \c A upside down. A more
     * readable way to accomplish this is:
     *
     * \code
     ...
     * subscripted. An object of type RWRange or RWToEnd, the global object
     * RWAll, or a character string may be substituted for an RWSlice.
     */
     ...
     * <tt>u = v</tt> implies \f$u_{ij} = v_{ij}\f$. The RWGenMat objects
     * must \e conform, that is, have the same dimensions.
     */
    ...
    /**
     * Assignment operator with conventional meaning. The expression
     * <tt>u = s</tt> implies \f$u_{ij} = s\f$.
     */
    RWGenMat<T>& operator=(const T& s);

    /**
     * Returns \c true if self and the argument are equivalent (or not
     * equivalent). That is, they must have the same number of rows as well
     * as columns, and each element in self must equal the corresponding
    ...
    /**
     * Assignment by addition operator with conventional meaning. The
     * expression <tt>u += s</tt> implies \f$u_{ij} = u_{ij}+ s\f$.
     */
    RWGenMat<T>& operator+=(const T& s);
    ...
    /**
     * Assignment by addition operator with conventional meaning. The
     * expression <tt>u += v</tt> implies \f$u_{ij} = u_{ij} + v_{ij}\f$.
     * The RWGenMat objects must \e conform, that is, have the same
     * dimensions.
     */
    ...
    RWGenMat<T>& operator--();
    /**
     * Increments each element of self. This overload is invoked if the
     * operator is used as a postfix operator.
     */
    void        operator++(int)
    {
        ++(*this);
    ...
    /**
     * Decrements each element of self. This overload is invoked if the
     * operator is used as a postfix operator.
     */
    void        operator--(int)
    {
...
/*
 * Specialized declaration for constructor for creating a matrix of DComplexes
 * from two matrices of doubles
 */
template <>
RW_MATH_GLOBAL RWGenMat<DComplex>::RWGenMat(const RWGenMat<double>& re, const RWGenMat<double>& im, Storage storage);


/**
 * \relates RWGenMat
 *
 * Unary minus operator, which is applied element-by-element. For instance,
 * for matrices \a v, and \c w, the expression <tt>w = -v</tt> implies
 * \f$w_{ij} = - v_{ij}\f$.
 */
template <class T>
RWGenMat<T> operator-(const RWGenMat<T>& v);
 ...
 * \relates RWGenMat
 *
 * Unary plus operator, which is applied element-by-element. For instance,
 * for matrices \a v, and \c w, the expression <tt>w = +v</tt> implies
 * \f$w_{ij} = + v_{ij}\f$.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Addition operator, which is applied element-by-element. For instance, for
 * matrices \a u, \a v, and \c w, the expression <tt>w = u + v</tt> implies
 * \f$w_{ij} = u_{ij} + v_{ij}\f$. The two RWGenMat objects must \e conform,
 * that is, have the same numbers of rows and columns, or an exception with
 * value \c RWMATH_MNMATCH occurs.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Subtraction operator, which is applied element-by-element. For instance,
 * for matrices \a u, \a v, and \c w, the expression <tt>w = u - v</tt>
 * implies \f$w_{ij} = u_{ij} - v_{ij}\f$. The two RWGenMat objects must
 * \e conform, that is, have the same numbers of rows and columns, or an
 * exception with value \c RWMATH_MNMATCH occurs.
 */
template <class T>
RWGenMat<T> operator-(const RWGenMat<T>& u, const RWGenMat<T>& v);
 ...
 * \relates RWGenMat
 *
 * Multiplication operator, which is applied element-by-element. For
 * instance, for matrices \a u, \a v, and \c w, the expression
 * <tt>w = u * v</tt> implies \f$w_{ij} = u_{ij} * v_{ij}\f$. The two
 * RWGenMat objects must \e conform, that is, have the same numbers of rows
 * and columns, or an exception with value \c RWMATH_MNMATCH occurs.
 */
template <class T>
RWGenMat<T> operator*(const RWGenMat<T>& u, const RWGenMat<T>& v);
 ...
 * \relates RWGenMat
 *
 * Division operator, which is applied element-by-element. For instance, for
 * matrices \a u, \a v, and \c w, the expression <tt>w = u / v</tt> implies
 * \f$w_{ij} = u_{ij} / v_{ij}\f$. The two RWGenMat objects must \e conform,
 * that is, have the same numbers of rows and columns, or an exception with
 * value \c RWMATH_MNMATCH occurs.
 ...
 *
 * Addition operator, which is applied element-by-element. For instance, for
 * matrices \a u, \c w, and scalar \a s, the expression <tt>w = u + s</tt>
 * implies \f$w_{ij} = u_{ij} + s\f$.
 */
template <class T>
 ...
 *
 * Subtraction operator, which is applied element-by-element. For instance,
 * for matrices \a v, \c w, and scalar \a s, the expression
 * <tt>w = s - v</tt> implies \f$w_{ij} =  s - v_{ij}\f$.
 */
template <class T>
 ...
 *
 * Multiplication operator, which is applied element-by-element. For
 * instance, for matrices \a u, \c w, and scalar \a s, the expression
 * <tt>w = u * s</tt> implies \f$w_{ij} = u_{ij} * s\f$.
 */
template <class T>
RWGenMat<T> operator*(const RWGenMat<T>& u, const T& s);
 ...
 *
 * Division operator, which is applied element-by-element. For instance, for
 * matrices \a v, \c w, and scalar \a s, the expression <tt>w = s / v</tt>
 * implies \f$w_{ij} = s / v_{ij}\f$.
 */
template <class T>
RWGenMat<T> operator/(const T& s, const RWGenMat<T>& v);
 ...
 * \relates RWGenMat
 *
 * Outputs a matrix \a m to \b std::ostream \a s. First, the number of rows
 * and columns is output, then the values, separated by spaces, are output
 * row by row, beginning with a left bracket \c [ and terminating with a
 * right bracket \c ].
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Reads a matrix \a v from istream \a s. First, the number of rows and
 * columns is read, then the matrix values, separated by white space,
 * row-by-row. If the sequence of numbers begins with a left bracket \c [ ,
 * the operator reads to a matching right bracket \c ]. If no bracket is
 * present, it reads to end of file. The matrix \a v is stored in
 * \c COLUMN_MAJOR order.
 */
template <class T>
 ...
 *
 * Multiplication operator, which is applied element-by-element. For
 * instance, for matrices \a v, \c w, and scalar \a s, the expression
 * <tt>w = s * v</tt> implies \f$w_{ij} = s * v_{ij}\f$.
 */
template <class T>
RWGenMat<T> operator*(const T& s, const RWGenMat<T>& v);
 ...
 *
 * Subtraction operator, which is applied element-by-element. For instance,
 * for matrices \a u, \c w, and scalar \a s, the expression
 * <tt>w = u - s</tt> implies \f$w_{ij} = u_{ij} - s\f$.
 */
template <class T>
RWGenMat<T> operator-(const RWGenMat<T>& u, const T& s);
 ...
 *
 * \note
 * The absolute value of a complex number is of type \c double. Therefore,
 * if abs() is invoked for class \ref RWGenMat "RWGenMat<DComplex>", a
 * vector of class \ref RWGenMat "RWGenMat<double>" is returned.
 */
 ...
 * See the example in abs(const RWGenMat<double>&)
 */
RWGenMat<signed char> RW_MATH_SYMBOLIC abs(const RWGenMat<signed char>& M);
 ...
 * \relates RWGenMat
 *
 * Return the \c index of the maximum element of the vector. If instead you
 * want the maximum \c value use function maxValue().
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Return the \c index of the minimum element of the vector. If instead you
 * want the minimum \c value use function minValue().
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Returns the inner product of its two arguments. Takes a matrix \a A and a
 * vector \a v as arguments returns:
 *
 * \f[
 * y_i = \sum_{j=0}^{n-1} A_{ij}v_j
 * \f]
 *
 * The function checks to make sure that the number of columns in \a A
 * equals the number of elements in \a v. Otherwise, an exception with value
 * \c MATX_MATVECPROD occurs.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Returns the inner product of its two arguments. Takes two matrices \a A
 * and \a B returns:
 *
 * \f[
 ...
 *
 * The function checks to make sure that the number of columns in \a A
 * equals the number of rows in \a B. Otherwise, an exception with value
 * \c MATX_MATMATPROD occurs.
 */
template <class T>
 ...
 * \f$y_{ik} = x_{ki}\f$. The matrix returned is a new view of the same data
 * as the argument matrix. Use RWGenMat::copy() or
 * RWGenMat::deepenShallowCopy() to make the views distinct.
 */
template <class T>
 ...
 * \f$y_{i} = cos(x_{i})\f$. The <em>x</em><sub>i</sub> are in radians. For
 * complex arguments, the complex cosine is returned.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Takes \a x as an argument and returns hyperbolic cosines \c y such that
 * \f$y_{i} = cosh(x_{i})\f$. For complex arguments, the complex hyperbolic
 * cosine is returned.
 */
template <class T>
 ...
 * \f]
 *
 * If the number of elements in \a x does not match the number of elements
 * in \a y, an exception with value \c RWMATH_MNMATCH will occur.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * The function sin() takes \a x as an argument and returns \c y such that
 * \f$y_{i} = sin(x_{i})\f$. The <em>x</em><sub>i</sub> are in radians. For
 * complex classes, the complex sine is returned.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * The function sinh() takes \a x as an argument and returns \c y such that
 * \f$y_{i} = sinh(x_{i})\f$. For complex classes, the complex hyperbolic
 * sine is returned.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * The square root function. Takes \a x as an argument and returns \c y such
 * that \f$y_{i} = (x_{i})^{1/2}\f$. For complex classes, the principal
 * value of the complex square root is returned.
 */
template <class T>
 ...
 * \f]
 *
 * where <\a x> is the mean of the matrix and \c N is the number of rows
 * times columns. Note that this is the biased variance.
 */
 ...
 * \relates RWGenMat
 *
 * Returns arc cosines \c y such that \f$y_{i} = cos^{-1} (x_{i})\f$. The
 * \c y<sub>i</sub> (in radians) are in the range \f$0 < y_{i} \leq \pi\f$,
 * for elements <em>x</em><sub>i</sub> with absolute values
 * \f$|x_{i}| \leq 1\f$.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Takes \a x as an argument and returns arc sines \c y such that
 * \f$y_{i} = sin^{-1}(x_{i})\f$. The \c y<sub>i</sub> (in radians) are in
 * the range \f$-\pi/2 < y_{i} \leq \pi/2\f$, for elements
 * <em>x</em><sub>i</sub> with absolute values \f$|x_{i}| \leq 1\f$.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Takes \a x and returns \c y of arc tangents (in radians), such that
 * \f$y_{i} = tan^{-1}x_{i}\f$, where \f$-\pi/2 < y_{i} \leq \pi/2\f$.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Takes two arguments and returns quadrant correct arc tangents \c z (in
 * radians), such that \f$z_{i} = tan^{-1} ( x_{i}/y_{i})\f$. For each
 * element \c i, the expression \f$atan2(x_{i}, y_{i})\f$ is mathematically
 * equivalent to:
 *
 * \f[
 ...
 * \f]
 *
 * At least one of the arguments \a x<sub>i</sub> or \a y<sub>i</sub> must
 * be nonzero.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Takes \a x as an argument and returns \c y such that \c y<sub>i</sub>
 * corresponds to the next integer greater than or equal to
 * <em>x</em><sub>i</sub>. For example, if
 ...
 *
 * Takes \a x as an argument and returns \c y such that \c y<sub>i</sub>
 * corresponds to the largest integer less than or equal to
 * <em>x</em><sub>i</sub>. For example, if <tt>x = [ 1.3, 4.4, -3.2]</tt>,
 * then <tt>y = [ 1, 4, -4].</tt>
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Takes \a x as an argument and returns natural logarithms \c y such that
 * \f$y_{i}=log(x_{i})\f$. The arguments <em>x</em><sub>i</sub> must not be 0.
 * For complex arguments, the principal value of the complex natural
 ...
 * \relates RWGenMat
 *
 * Takes \a x as an argument and returns base 10 logarithms \c y such that
 * \f$y_{i}=log_{10}(x_{i})\f$. The arguments <em>x</em><sub>i</sub> must
 * not be 0.
 */
template <class T>
 ...
 * \relates RWGenMat
 *
 * Returns the arguments \c a<sub>i</sub> (in radians) of \a x in the range
 * \f$-\pi < a_{i} \leq \pi\f$, where
 * \f$a_{i} = tan^{-1}{[Im(x_{i})/Re(x_{i})]}\f$. Multiple valued functions
 * of complex arguments, such as sqrt() and log(), are therefore constrained
 * to their principle value.
 */
 ...
 * \relates RWGenMat
 *
 * Takes complex \a x as an argument and returns the complex conjugates
 * \f$x^{*}\f$. For example, if \f$x_{i} = (2.3, 1.4)\f$, then
 * \f$x_{i}^{*} = (2.3, -1.4)\f$.
 */
 ...
 * \relates RWGenMat
 *
 * Takes complex \a x as an argument and returns \c y containing the
 * imaginary parts \f$y_{i} = Im(x_{i})\f$. With most versions of complex,
 * the results can be used as an l-value:
 *
 * \code
 ...
 * \relates RWGenMat
 *
 * Takes a complex argument \a x and returns real \c y containing the real
 * part \f$y_{i}=Re(x_{i})\f$. With most versions of complex, the results
 ...
 * \relates RWGenMat
 *
 * Takes complex \a x as an argument and returns real \c y containing the
 * norm:
 *
 * \f[
 ...
 * \relates RWGenMat
 *
 * Takes a matrix \a A (of \c M rows by \c N columns) and a matrix \a B (of
 * \c M rows by \c P columns) as arguments, and returns a matrix \c C (of
 * \c N rows by \c P columns) that is equal to <em>A</em>TB, that is, the
 ...
 * \f]
 *
 * If the number of rows in \a A does not match the number of rows in \a B,
 * an exception with value \c RWLAPK_MATMATPROD occurs.
 *
 ...
 * Takes a matrix \a A (of \c M rows by \c N columns) and a matrix \a B (of
 * \c M rows by \c P columns) as arguments and returns a matrix \c C (of
 * \c N rows by \c P columns) which is equal to <tt>conj(\a A)T\a B</tt>,
 * that is, the inner product of the conjugate transpose of \a A with \a B.
 *
 * If the number of rows in \a A does not match the number of rows in \a B,
 * an exception with value \c RWLAPK_MATMATPROD occurs.
 *
 ...
 * \relates RWGenMat
 *
 * Converts an \ref RWGenMat "RWGenMat<int>" instance into a corresponding
 * \ref RWGenMat "RWGenMat<signed char>" instance. Note that this is a
 * narrowing operation; high order bits are removed.
 *
 * \sa
 * RWConvertGenMat
 */
RWGenMat<signed char> RW_MATH_SYMBOLIC toChar(const RWGenMat<int>& V);


/**
 * \relates RWGenMat
 *
 * Converts an \ref RWGenMat "RWGenMat<double>" instance into a
 * corresponding \ref RWGenMat "RWGenMat<float>" instance. Note that this is
 * a narrowing operation; high order bits are removed.
 *
 * \sa
 * RWConvertGenMat
 */
 ...
 *
 * Converts an \ref RWGenMat "RWGenMat<double>" instance into a
 * corresponding \ref RWGenMat "RWGenMat<int>" instance. Note that
 * truncation occurs.
 *
 * \sa
 * RWConvertGenMat
 ...
 * \relates RWGenMat
 *
 * Converts an \ref RWGenMat "RWGenMat<float>" instance into a corresponding
 * \ref RWGenMat "RWGenMat<int>" instance. Note that truncation occurs.
 *
 * \sa
 * RWConvertGenMat
 */
 ...
 *
 * Class RWConvertGenMat converts between RWGenMat types with different
 * datatypes. It assumes that there is a conversion operator from From to
 * To.
 *
 * \synopsis
...
struct RWConvertGenMat {
    /**
     * Constructs a new matrix of type RWGenMat that can be used anywhere as
     * a type RWGenMat. Note that a temporary matrix is created every time
     * this constructor is invoked, but the conversion operator below can be
     * used repeatedly without creating additional temporary matrices.
     */
    ...
    /**
     * Automatic conversion operator that enables class RWConvertGenMat to
     * be used as an RWGenMat. While the actual conversion takes place in
     * the constructor, this operator provides a reference to that converted
     * matrix.
     */
 ...
 * \relates RWGenMat
 *
 * Computes the Frobenius norm, which is the square root of the sum of
 * squares of its entries. For a matrix, the formula is:
 *
 * \f[
 ...
 * \relates RWGenMat
 *
 * Returns the value of the element with largest absolute value. Note that
 * this is not a norm in the mathematical sense of the word.
 */
 ...
 * \relates RWGenMat
 *
 * Conventional mathematical linear algebra multiplication operator; that
 * is, the <tt>i,j</tt> element of the result of a matrix product
 * <em>A%B</em> is the dot product of the <tt>i<sup>th</sup></tt> row of
 * \a A and the <tt>j<sup>th</sup></tt> column of \a B.
 */
template <class T>
