 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tpmmap.h#1 $
 *
 **********************************************************************
 *
 ...
 * comparison object, and allowing duplicate keys.
 *
 ...
 * Standard Library header file \c \<functional\> is an example. Note
 * that keys will be dereferenced before being compared.
 *
 * RWTPtrMultiMap may contain multiple keys that compare equal to each
 * other. (RWTPtrMap will not accept a key that compares equal to any key
 * already in the collection.) Equality is based on the comparison object
 * and not on the equality operator. Given a comparison object \c comp, keys
 * \c a and \c b are equal if
 *
 * \code
 * !comp(a,b) && !comp(b,a).
 * \endcode
 ...
 * RWTPtrMultiSet maintains a pointer-based collection of keys without the
 * associated values.
 *
 * Class <b>std::multimap<K*,T*,deref_compare<C,K,A> ></b> is the C++
...
public:
     ...
     *
     * \deprecated
     * As of SourcePro 12.5, use RWTPtrMultiMap::value_compare instead.
     */
     ...
     * <tt>(*j == *\a key)</tt> is \c true, returns the item associated with
     * \c j. Otherwise, returns #rwnil.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrMultiMap::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTPtrMultiMap.
 *
 * RWTPtrMultiMapIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTPtrMultiMap.(An STL iterator
 * interface is provided through RWTPtrMultiMap::iterator.)
 *
 * The order of iteration over an RWTPtrMultiMap is dependent on the
 * comparator object of the container as applied to the key values of the
 * stored associations.
 *
...
public:
    /**
     * Creates an iterator for the multimap \a m. The iterator begins in an
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrMultiMap::const_iterator to iterate over
 * the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTPtrMultiMap.
 *
 * RWTPtrMultiMapConstIterator provides a \c const iterator interface to the
 * C++ Standard Library-based RWTPtrMultiMap collection. A \c const iterator
 * prohibits changes to the collection it iterates through.
 *
 * The order of iteration over an RWTPtrMultiMap is dependent on the
 * comparator object of the container as applied to the key values of the
 * stored associations.
