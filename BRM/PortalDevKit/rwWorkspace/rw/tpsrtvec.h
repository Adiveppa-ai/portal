 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tpsrtvec.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * A pointer-based collection of values sorted according to a comparison
 * object and implemented as a vector.
 *
 * This class maintains an always-sorted pointer-based collection of values,
 * implemented as a vector. Items are ordered according to a comparison
 * object of type \c C. Class \c T is the type pointed to by the items in
 ...
 * \sa
 * \parblock
 * RWTPtrSortedDlist is an alternative always-sorted pointer-based
 * collection. RWTPtrOrderedVector is an unsorted pointer-based vector.
 *
 * Class <b>std::vector<T*,A<T*> ></b> is the C++ Standard Library
...
public:
     ...
     * \a i must be between \c 0 and one less than the number of entries in
     * self, otherwise the results are undefined.
     *
     * \note
    ...
    /**
     * Removes and returns the first element \c t in self such that the the
    ...
    /**
     * Modifies, if necessary, the capacity of the vector to be at least as
     * large as \a n.
     */
    ...
    /**
     * Constructs a vector by copying and sorting all elements of \a vec.
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Constructs a vector with \a n elements, each initialized to \a p.
     */
