 ...
 * mtharray.h
 *
 * $Id: //spro/rel2016.1/rw/math/mtharray.h#1 $
 *
...
protected:
    ...
    {
        RW_PRECONDITION(npts_.length() == x.npts_.length());
...
public:
    /**
     * Equality operator. Returns \c true if the RWMathArrayIteratorBase
    ...
    /**
     * Inequality operator. Returns \c false if the RWMathArrayIteratorBase
     * objects point to the same element.
     */
    bool operator !=(const RWMathArrayIteratorBase<T>& x) const
    {
     ...
     * two RWMathArrayIteratorBase objects, not necessarily the relative
     ...
     * between two RWMathArrayIteratorBase objects, not necessarily the
     ...
     * position between two RWMathArrayIteratorBase objects, not necessarily
 ...
 *
 * \brief
 * The random access iterator for the RWMathArray collection class.
 *
 * Class RWMathArray is the random access iterator for the RWMathArray
 * collection class. The RWMathArrayIterator class behaves like a pointer to
 * type \c T and is used to increment through elements of an RWMathArray.
 *
 * \synopsis
 * template <class T>
...
public:
     ...
     * Initialized iterators can be obtained via RWMathArray::begin() and
     * RWMathArray::end().
     */
     ...
     * RWMathArrayIterator points to the next or previous element in the
     * RWMathArray. No error condition is set if the iterator goes past the
     * bounds of the array. Compare to RWMathArray::begin() and
     * RWMathArray::end() to check that the iterator position is valid.
     */
    ...
    /**
     * Moves the RWMathArrayIterator forward or backward \a d elements in
     * the RWMathArray. No error condition is set if the iterator goes past
     * the bounds of the array. Incrementing by \a d and then decrementing
     * by \a d returns the iterator to its original position, even if doing
    ...
    /**
     * Returns an RWMathArrayIterator that is \a d elements past (or before)
     * self.
     */
    ...
    /**
     * Returns a reference to the element that is \c n elements after self
     * if \c n is positive, or \c n elements before self if \c n is
     * negative.
     */
 ...
 * pointer to type \c T and is used to increment through elements of an
 * RWMathArray. The RWMathArrayConstIterator differs from a plain iterator
 * in that the value in a dereferenced RWMathArrayConstIterator may not be
 * changed.
 *
 * \synopsis
 * template <class T>
...
public:
    ...
    /**
     * Constructs a RWMathArrayConstIterator from another
     * RWMathArrayIterator or RWMathArrayConstIterator. Initialized
     * RWMathArrayIterator and RWMathArrayConstIterator instances can be
     * obtained via the RWMathArray::begin() and RWMathArray::end() member
     * functions of RWMathArray.
     */
    ...
    /**
     * Prefix or postfix increment or decrement operator. The
     * RWMathArrayConstIterator points to the next or previous element in
     * the RWMathArray. No error condition is set if the
     * RWMathArrayConstIterator goes past the bounds of the array. Compare
     * to RWMathArray::begin() and RWMathArray::end() to check that the
     * RWMathArrayConstIterator position is valid.
     */
    ...
    /**
     * The RWMathArrayConstIterator is moved forward or backward \a d
     * elements in the RWMathArray. No error condition is set if the
     * RWMathArrayConstIterator goes past the bounds of the array.
    ...
    /**
     * Returns a RWMathArrayConstIterator that is \a d elements past (or
     * before) self.
     */
...
public:
    ...
    /**
     * Returns an iterator pointing to one element past the last element of
     * self.
     */
    iterator end()
    {
    ...
    /**
     * Constructs a 0-dimensional array, useful for declaring vectors of
     * arrays. Like any other array, this array can subsequently be reshaped
     * or resized (see member functions reshape() and resize()). Note that
     * since this is a 0-dimensional (not 0 length) array, and since by
     * definition a 0-dimensional array is a scalar, the array has one
     * element of data.
     */
    RWMathArray();
     ...
     * arrays with more than four dimensions. The #RWUninitialized type is
     ...
     * numbers generated by \a r. The constructor taking an integer vector
     * is useful for constructing arrays with more than four dimensions.
     */
    ...
    /**
     * Constructs an array with a specified size. Each element in the array
     * is initialized to \a val. The constructor taking an integer vector is
     * useful for constructing arrays with more than four dimensions.
     */
    ...
    /**
     * Constructs an array from the null terminated character string \a s.
    ...
    /**
     * Copy constructor. The new array and the old array both view the same
     * data.
     */
     ...
     * vector \a dat as initial data. A copy of \a dat is made. The vector
     * \a dat must have at least as many elements as the array. The
     * constructor taking an integer vector is useful for constructing
    ...
    /**
     * Constructs an array using the data in the vector \a vec. The array is
     * a new view of the same data as \a vec, so no copy of the data is
     * made. The vector \a vec must have the same number of elements as the
     * array. The constructor taking an integer vector is useful for
     * constructing arrays with more than four dimensions.
     */
    ...
    /**
     * Constructs a 1-dimensional or 2-dimensional array from a vector or
     * matrix. The resulting array is an alternate view of the same data.
     * This constructor is most often used implicitly by the compiler to
     * pass vectors or matrices to subroutines written to operate on arrays
     * of arbitrary dimension. This allows you to write one subroutine
     * suitable for vectors, matrices, or arrays.
     */
    ...
    /**
     * Constructs a complex array from the double precision arrays \a re and
     * \a im, with the real part of the array equal to \a re and the
     * imaginary part equal to \a im. A new copy of the data is made.
     */
...
public:
     ...
     * in the array. A function of type RWMathArray<T>::mathFunType takes
     * and returns a \c T.
     */
    RWMathArray<T>            apply(mathFunType f) const
    {
        ...
        for (ArrayLooper l(npts, step); l; ++l) {
            const T* sp = data() + l.start;
            RW_ASSERT(l.length <= RW_INT_MAX);
            int i = static_cast<int>(l.length);
            int j = l.stride;
            while (i--) {
                *dp++ = (*f)(*sp);
                sp += j;
    ...
    /**
     * Returns the result of applying the passed function to every element
     * in the array. The function of type RWMathArray<T>::mathFunType2 takes
     * a \c T and returns an RWMathArray<T>::norm_type. See class
    ...
    /**
     * Returns the number of bytes required to store the array to an RWFile
    ...
    /**
     * Returns a pointer to the start of an array's data. Should be used
     * with care, as this accesses the array's data directly.
     */
    T*          data()
    {
        return (T*)begin_;
     ...
     * reference to that object and that its data are in contiguous memory.
     */
    ...
    /**
     * Returns an iterator pointing to one element past the first element of
     * self.
     */
    ...
    /**
     * Makes self a view of data in \a v. The view currently associated with
     * the array is lost.
     */
    ...
    /**
     * Changes the size of the array. After reshaping, the contents of the
     * array are undefined; that is, they can be and probably will be
     * garbage.
     */
    ...
    /**
     * Stores self in a binary format to an RWFile. To use these functions
    ...
    /**
     * Stores self to a RWvostream. To use these functions with a
     * user-defined type, the corresponding \c operator<< must be defined:
     *
     * \code
    ...
    /**
     * Returns an array that views a slice of the array. The slice begins at
     * element \a start and is of size \a lgt. The increment between
     * successive elements in the slice's \c j<sup>th</sup> dimension is
     * given by the \c j<sup>th</sup> column of the matrix \a strider. For
     * 3-dimensional and 4-dimensional arrays, most useful slices can be
     * more simply constructed using subscripting.
     */
    ...
    /**
     * Subscripting operator for the array, with bounds checking. Returns a
     * reference to the element indexed by \a i.
     */
     ...
     * \c RWBOUNDS_CHECK before including the header file. Returns a
     * reference to the element indexed by \a i.
     */
    inline    T&            operator()(const RWIntVec& i);
     ...
     * Bounds checking is enabled by defining the preprocessor macro
     * \c RWBOUNDS_CHECK before including the header file. Returns a
     * reference to the element at (\a i <sup>th</sup>, \a j <sup>th</sup>,
     * \a k <sup>th</sup>) position of the array.
     */
     ...
     * Bounds checking is enabled by defining the preprocessor macro
     * \c RWBOUNDS_CHECK before including the header file. Returns the
     * element at (\a i <sup>th</sup>, \a j <sup>th</sup>, \a k
     ...
     * \c RWBOUNDS_CHECK before including the header file. Returns a
     * reference to the element at (\a i <sup>th</sup>, \a j <sup>th</sup>,
     * \a k <sup>th</sup>, \a l <sup>th</sup>) position of the array.
     */
     ...
     * Bounds checking is enabled by defining the preprocessor macro
     * \c RWBOUNDS_CHECK before including the header file. Returns a
     * non-const RWMathVec that contains the elements at (\a i
     * <sup>th</sup>, \a j <sup>th</sup>, \a k <sup>th</sup>) position of
     * the array. The RWMathVec being returned is a new view of the same
     ...
     * Subscripting operator for the array, with optional bounds checking.
     * Bounds checking is enabled by defining the preprocessor macro
     * \c RWBOUNDS_CHECK before including the header file. Returns a const
     * RWMathVec that contains the elements at (\a i <sup>th</sup>, \a j
     * <sup>th</sup>, \a k <sup>th</sup>) position of the array. The
     * RWMathVec being returned is a new view of the same data as the array
     * being subscripted.
     */
     ...
     * <sup>th</sup>) position of the array. The RWMathVec being returned is
     * a new view of the same data as the array being subscripted.
     */
     ...
     * \c RWBOUNDS_CHECK before including the header file. Returns a const
     * RWMathVec that contains the elements at (\a i <sup>th</sup>, \a j
     * <sup>th</sup>, \a k <sup>th</sup>, \a l <sup>th</sup>) position of
     * the array. The RWMathVec being returned is a new view of the same
     * data as the array being subscripted.
     */
     ...
     * Bounds checking is enabled by defining the preprocessor macro
     * \c RWBOUNDS_CHECK before including the header file. Returns a
     * non-const RWGenMat that contains the elements at (\a i <sup>th</sup>,
     * \a j <sup>th</sup>, \a k <sup>th</sup>) position of the array. The
     * RWGenMat being returned is a new view of the same data as the array
     * being subscripted.
     */
     ...
     * Bounds checking is enabled by defining the preprocessor macro
     * \c RWBOUNDS_CHECK before including the header file. Returns a const
     * RWGenMat that contains the elements at (\a i <sup>th</sup>, \a j
     * <sup>th</sup>, \a k <sup>th</sup>) position of the array. The
     * RWGenMat being returned is a new view of the same data as the array
     * being subscripted.
     */
     ...
     * \c RWBOUNDS_CHECK before including the header file. Returns a
     * non-const RWGenMat that contains the elements at (\a i <sup>th</sup>,
     * \a j <sup>th</sup>, \a k <sup>th</sup>, \a l <sup>th</sup>) position
     ...
     * RWGenMat that contains the elements at (\a i <sup>th</sup>, \a j
     * <sup>th</sup>, \a k <sup>th</sup>, \a l <sup>th</sup>) position of
     * the array. The RWGenMat being returned is a new view of the same data
     * as the array being subscripted.
     */
     ...
     * Bounds checking is enabled by defining the preprocessor macro
     * \c RWBOUNDS_CHECK before including the header file. Returns a
     * non-const RWMathArray that contains the elements at (\a i
     * <sup>th</sup>, \a j <sup>th</sup>, \a k <sup>th</sup>) position of
     * the array. The RWMathArray being returned is a new view of the same
     * data as the array being subscripted.
     */
     ...
     * Bounds checking is enabled by defining the preprocessor macro
     * \c RWBOUNDS_CHECK before including the header file. Returns a const
     * RWMathArray that contains the elements at (\a i <sup>th</sup>, \a j
     * <sup>th</sup>, \a k <sup>th</sup>) position of the array. The
     * RWMathArray being returned is a new view of the same data as the
     * array being subscripted.
     */
     ...
     * <tt>u = v</tt> implies \f$u_{i} = v_{i}\f$ The RWMathArray objects
    ...
    /**
     * Assignment by addition operator with conventional meaning. The
     * expression <tt>u += v</tt> implies \f$u_{i} = u_{i} + v_{i}\f$ The
     * RWMathArray objects must \e conform, that is, have the same
     * dimensions.
     */
    RWMathArray<T>&        operator+=(const RWMathArray<T>& v);
    ...
    /**
     * Assignment by division operator with conventional meaning. The
     * expression <tt>u /= v</tt> implies u_{i} = u_{i} / v_{i} The
     * RWMathArray objects must \e conform, that is, have the same
     * dimensions.
     */
    RWMathArray<T>&        operator/=(const RWMathArray<T>& v);
    ...
    /**
     * Increments or decrements each element of self. The functions taking
     * an integer parameter are invoked if the operator is used as a postfix
     * operator.
     */
     ...
     * equivalent). To be equivalent, they must have the same number of rows
     * as well as columns, and each element in self must equal the
...
#  pragma warning (disable:597)
     ...
     * \ref rw_numeric_traits::promote_type "rw_numeric_traits<T>::promote_type".
     */
...
/*
 * Specialized declaration for constructor for creating an array of DComplexes
 * from two arrays of doubles
 */
template <>
RW_MATH_GLOBAL RWMathArray<DComplex>::RWMathArray(const RWMathArray<double>& re, const RWMathArray<double>& im);
 ...
 *
 * Class RWConvertMathArray converts between RWMathArray types with
 * different datatypes.
 *
 * \synopsis
...
struct RWConvertMathArray {
    /**
     * Constructs a new array of type RWMathArray that can be used anywhere
     * as a type RWMathArray. Note that a temporary array is created every
     ...
     * RWConvertMathArray to be used as an RWMathArray. While the actual
 ...
 * \relates RWMathArray
 *
 * Unary minus operator, with conventional meaning, applied
 * element-by-element. For matrix \a u, and \c w, the expression
 * <tt>w = -u</tt> implies \f$w_{i} = -u_{i}\f$.
 */
template <class T>
 ...
 * \relates RWMathArray
 *
 * Unary plus operator, with conventional meaning, applied
 * element-by-element. For matrix \a u, and \c w, the expression
 * <tt>w = +u</tt> implies \f$w_{i} = +u_{i}\f$.
 */
template <class T>
 ...
 * implies \f$w_{i} = u_{i} + v_{i}\f$. The arrays must \e conform, that is,
 * have the same dimensions, or an exception with value \c RWMATH_NMATCH
 * occurs.
 */
template <class T>
 ...
 * \relates RWMathArray
 *
 * Subtraction operator, with conventional meaning, applied
 * element-by-element. For matrices \a u, \a v, and \c w, the expression
 * <tt>w = u - v</tt> implies \f$w_{i} = u_{i} - v_{i}\f$. The arrays must
 * \e conform, that is, have the same dimensions, or an exception with value
 * \c RWMATH_NMATCH occurs.
 */
template <class T>
RWMathArray<T>  operator-(const RWMathArray<T>& u, const RWMathArray<T>& v);
 ...
 * element-by-element. For matrices \a u, \a v, and \c w, the expression
 * <tt>w = u * v</tt> implies \f$w_{i} = u_{i} * v_{i}\f$. Therefore,
 * operator*() implies an element-by-element multiply, \e not the inner
 * product. The arrays must \e conform, that is, have the same dimensions,
 ...
 * <tt>w = u + s</tt> implies \f$w_{i} = u_{i} + s\f$.
 */
template <class T>
 ...
 *
 * Subtraction operator, with conventional meaning, applied
 * element-by-element. For matrices \a v, \c w, and scaler \a s, the
 * expression <tt>w = s - v</tt> implies \f$w_{i} = s - v_{i}\f$.
 */
template <class T>
 ...
 *
 * Multiplication operator, with conventional meaning, applied
 * element-by-element. For matrices \a u, \c w, and scaler \a s, the
 * expression <tt>w = u * s</tt> implies \f$w_{i} = u_{i} * s\f$.
 */
template <class T>
RWMathArray<T>  operator*(const RWMathArray<T>& u, const T& s);
...
/**
 * Outputs an array \a v to \b std::ostream \a s. First, the size of the
 * array is output in the form 3 x 4 x 3, for example, and then the elements
 * of the array are output separated by spaces beginning with a left bracket
 * \c [ and terminating with a right bracket \c ]. New lines are used to try
 * and convey the structure of the array.
 */
template <class T>
std::ostream&  operator<<(std::ostream& s, const RWMathArray<T>& v);

/**
 * Reads an array \a v from \b std::istream \c s. First, the size of the
 * array is read, then array values separated by white space. The sequence
 * of numbers begins with a size specification, such as 3 x 4 x 3, followed
 * by a left bracket \c [, and the operator reads to a matching right
 * bracket \c ].
 */
template <class T>
 ...
 * if abs() is invoked for class \ref RWMathArray "RWMathArray<DComplex>", a
 * vector of class \ref RWMathArray "RWMathArray<double>" is returned.
 */
 ...
 * See the example in abs(const RWMathArray<double>&).
 */
RWMathArray<signed char>   RW_MATH_SYMBOLIC abs(const RWMathArray<signed char>&);

/**
 * \relates RWMathArray
 *
 * Takes complex <em>x</em> as an argument and returns real <em>y</em>
 * containing the norm:
 *
 * \f[
 ...
 * \relates RWMathArray
 *
 * Converts a 0-dimensional array into a scalar. The array must actually
 * have 0 dimensions or a runtime error occurs.
 */
 ...
 * \relates RWMathArray
 *
 * Converts a 1-dimensional array into a corresponding RWMathVec. The array
 * must actually have 1 dimension or a runtime error occurs. The newly
 * constructed vector is a new view of the same data as the array; a copy of
 ...
 * \relates RWMathArray
 *
 * Converts a 2-dimensional array into a corresponding general matrix class.
 * The array must actually have 2 dimensions or a runtime error occurs. The
 * newly constructed matrix is a new view of the same data as the array; a
 * copy of the data is not made.
 */
 ...
 * \relates RWMathArray
 *
 * Converts an \ref RWMathArray "RWMathArray<double>" instance into a
 * corresponding \ref RWMathArray "RWMathArray<float>" instance. Note that
 ...
 *
 * Converts an \ref RWMathArray "RWMathArray<double>" instance into a
 * corresponding \ref RWMathArray "RWMathArray<int>" instance. Note that
 * truncation occurs.
 *
 * \sa
 * RWConvertMathArray
 ...
 * \relates RWMathArray
 *
 * Converts an \ref RWMathArray "RWMathArray<float>" instance into a
 * corresponding \ref RWMathArray "RWMathArray<int>" instance. Note that
 * truncation occurs.
 *
 * \sa
 ...
 * \relates RWMathArray
 *
 * Converts an \ref RWMathArray "RWMathArray<int>" instance into a
 * corresponding \ref RWMathArray "RWMathArray<signed char>" instance. Note
 * that this is a narrowing operation; high order bits are removed.
 *
 * \sa
 * RWConvertMathArray
 */
RWMathArray<signed char> RW_MATH_SYMBOLIC toChar(const RWMathArray<int>& v);
 ...
 * \f$a_{i} = tan^{-1}[Im(x_{i})/Re(x_{i})]\f$. Multiple valued functions of
 ...
 * \relates RWMathArray
 *
 * Takes complex \e x as an argument and returns the complex conjugates
 ...
 * \relates RWMathArray
 *
 * Takes a RWMathArray an argument and returns <em>\<x\>,</em> the mean
 * value, where:
 *
 * \f[
 ...
 * \relates RWMathArray
 *
 * Takes \a x as an argument and returns natural logarithms \e y such that
 * \f$y_{i}=log(x_{i})\f$. The arguments \a x <sub>i</sub> must not be 0.
 ...
 * \relates RWMathArray
 *
 * Takes \a x as an argument and returns base 10 logarithms \e y such that
 * \f$y_{i}=log_{10}(x_{i})\f$. The arguments \a x <sub>i</sub> must not be 0.
 */
template <class T>
