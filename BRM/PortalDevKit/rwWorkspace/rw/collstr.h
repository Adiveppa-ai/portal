 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/collstr.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * Collectable strings. This class is useful when strings are stored and
 * retrieved as RWCollectable instances, or when they are used as keys in
 * the "dictionary" collection classes. Class RWCollectableString inherits
 * from both class RWCString and class RWCollectable.
 *
...
public:
    ...
    /**
     * Move constructor. The created collectable string takes ownership of
     * the data owned by \a str.
     *
     * \conditional
    ...
    /**
     * Converts from the null-terminated character string \a c. The created
     * string \e copies the data pointed to by \a c, up to the first
     * terminating null. <em>This function is incompatible with \a c strings
     * with embedded nulls. This function may be incompatible with \a c MBCS
     * strings.</em>
     */
    RWCollectableString(const char* c) : RWCString(c) { }

    /**
     * Constructs an RWCollectableString from the character string \a a. The
     * created string \e copies the data pointed to by \a a. Exactly \a N
     * bytes are copied, <em>including any embedded nulls</em>. Hence, the
     * buffer pointed to by \a a must be at least \a N bytes long.
     */
    ...
    /**
     * Constructs an RWCollectableString from the RWCString \a s by
     * \e copying data from \a s.
     */
    RWCollectableString(const RWCString& s) : RWCString(s) { }

    /**
     * Converts from a sub-string. The created string \e copies the
     * substring represented by \a s.
     */
     ...
     * \ref RWCString::compareTo "RWCString::compareTo(*(const RWCString*)c, RWCString::exact)".
     * This compares strings lexicographically, with case considered. It
     * would be possible to define, for instance, \c CaseFoldedString which
     * did comparisons ignoring case. We have deliberately left this as an
     * exercise for two reasons: Because it is both easy to do and not
     * universally needed; and because the presence of both
     * RWCollectableString%s and such a \c CaseFoldedString in any kind of
     * sorted collection has the potential for very confusing behavior,
     * since the result of a comparison would depend on the order in which
    ...
    /**
     * Calls RWCString::operator==() (i.e., the equivalence operator) with
     * \a c as the argument and returns the results.
     */
 ...
 *
 * Collectable wide strings paralleling RWCollectableString. This class is
 * useful when wide strings are stored and retrieved as RWCollectables, or
 * when they are used as keys in the "dictionary" collection classes. Class
 * RWCollectableWString inherits from both class RWWString and class
 * RWCollectable.
 *
 * The virtual functions of the base class RWCollectable have been
 * redefined.
 *
 * \note
 * Encodings used by operating systems for \c wchar_t strings may vary. If
 * your application is dependent on the presence of encodings across
 * different operating systems, you need to ensure that it supports the
 * desired formats.
 *
 * \synopsis
 * #include <rw/collstr.h>
...
public:
     ...
     * string copies the data pointed to by \a a. Exactly \a N characters
     * are copied, including any embedded nulls. Hence, the buffer pointed
     * to by \a a must be at least \a N <tt>* sizeof(wchar_t)</tt> bytes or
     * \a N wide characters long.
     */
    RWCollectableWString(const wchar_t* a, size_t N) : RWWString(a, N) { }

    /**
     * Constructs a string containing the character \a c repeated \a N
     * times.
     */
    RWCollectableWString(wchar_t c, size_t N) : RWWString(c, N) { }

    /**
     * Constructs an RWCollectableWString from the RWWString \a s by copying
     * data from \a s.
     */
     ...
     * RWWString::compareTo(*(const RWWString*)c, RWWString::exact). This
     * compares strings lexicographically, with case considered. It would be
     * possible to define, for instance, \c CaseFoldedString which did
     * comparisons ignoring case. We have deliberately left this as an
     * exercise for two reasons: Because it is both easy to do and not
     * universally needed; and because the presence of both
     * RWCollectableWString%s and \c CaseFoldedString in any kind of sorted
     * collection has the potential for very confusing behavior, since the
     * result of a comparison would depend on the order in which the
     * comparison was done.
     */
    virtual int compareTo(const RWCollectable* c) const;
    ...
    /**
     * Calls RWWString::operator==() (i.e., the equivalence operator) with
     * \a c as the argument and returns the results.
     */
    virtual bool isEqual(const RWCollectable* c) const;
