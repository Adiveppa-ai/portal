 ...
 * genfact.h
 *
 * $Id: //spro/rel2016.1/rw/math/genfact.h#1 $
 *
...
/**
 * \ingroup math_lu_lapack_factorization_group
 *
 * \brief
 * A templatized LU factorization class that holds the LU factorization of a
 * general square matrix of type T.
 *
 * Class RWGenFact is a templatized LU factorization class. This class holds
 * the LU factorization of a general square matrix of type T. Once such a
 * factorization is built, it can be used to invert a matrix, calculate its
 * determinant, or solve a set of simultaneous linear equations.
 *
 * Optionally, a condition number can be calculated and then recovered via
 * member function condition(), which returns the reciprocal of the standard
 * definition for the condition number of a matrix.
 *
 * The result is in the range [0,1]. The closer the result is to 0, the
 * closer the matrix is to being singular. See Dongarra et al. (1979) for
...
public:
    ...
    /**
     * This is a convenience typedef. The condition numbers have type
     * dependent on the traits class.
     */
    typedef typename rw_numeric_traits<T>::norm_type  norm_type;

    /**
     * Sets up an empty LU factorization. Member function factor() can then
     * be used to actually calculate the factorization.
     */
    RWGenFact();

    /**
     * Copy constructor. Reference semantics are used; that is, a
     * <em>shallow copy</em> of the argument is made.
     */
    RWGenFact(const RWGenFact<T>& A);

    /**
     * Constructs an LU factorization from the matrix \a m. If the argument
     * \a estimateCondition is \c true, a condition number is also
     * calculated. Setting \a estimateCondition to \c false saves some time
     * (perhaps 20%), but gives you less information about how
     * ill-conditioned the matrix is. If the matrix \a m is not square, an
     * exception with value \c RWLAPK_NOTSQUARE occurs. This constructor
     * also serves as a type conversion from RWGenMat to RWGenFact
     */
    ...
    /**
     * Recalculates the factorization, this time using the matrix \a m. If
     * the argument \a estimateCondition is \c true, a condition number is
     * also calculated.
     */
    ...
    /**
     * Returns \c true if the factorization fails, perhaps due to
     * singularity, otherwise returns \c false.
     */
    bool fail() const;

    /**
     * Checks to see if the matrix is singular. If isSingular() returns a
     * non-zero value, the matrix is singular. Note that the LU
     * factorization is still defined; however, an attempt to use it to
     * compute the matrix inverse fails.
     */
 ...
 * \relates RWGenFact
 *
 * Calculates the solution to \f$ A*x = b \f$, where \a m is an LU
 * factorization constructed from the RWGenMat \c A.
 *
 * If the factorization <em>m</em> is invalid, perhaps because the original
 * matrix <b>A</b> was singular, an exception with value \c LPAK_CANTSOLVE
 * occurs. If the number of elements in <b>x</b> does not match the order of
 * <b>A</b>, an exception with value \c LPAK_VECLENGTH occurs.
 */
template <class T>
 ...
 * \relates RWGenFact
 *
 * Calculates the solution to \f$ A*x = b \f$, where \a b is an LU
 * factorization constructed from the RWGenMat \a A.
 *
 * If the factorization <em>m</em> is invalid, perhaps because the original
 * matrix <b>A</b> was singular, an exception with value \c LPAK_CANTSOLVE
 * occurs. If the number of elements in <b>x</b> does not match the order of
 ...
 * \relates RWGenFact
 *
 * Returns the determinant of the matrix from which an LU factorization \a A
 * is constructed.
 */
template <class T>
 ...
 * was constructed. If the factorization <em>A</em> is invalid, perhaps
 ...
 *
 * Returns the inverse of the matrix from which an existing LU factorization
 * was constructed. If the factorization of <em>A</em> is invalid, perhaps
 * because the original matrix was singular, an exception with value
 * \c LPAK_CANTSOLVE occurs.
 */
template <class T>
 ...
 * \relates RWGenFact
 *
 * Returns the inverse condition number of the matrix whose LU factorization
 * is \a A.
 */
template <class T>
