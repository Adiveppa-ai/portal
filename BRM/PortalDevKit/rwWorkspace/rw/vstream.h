 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/vstream.h#1 $
 *
 **********************************************************************
 *
...
/**
 * \ingroup tools_virtual_streams_group
 *
 * \brief
 * Abstract base class defining an interface similar to \b std::ios, but
 * without a required association with \b std::streambuf.
 *
 * RWvios is an abstract base class that defines an interface similar to the
 * C++ Standard Library streams class \b std::ios. However, unlike
 * \b std::ios, it does not require association with a \b std::streambuf.
 *
 * This disassociation can be useful for classes that cannot use a
 * \b std::streambuf in their implementation, for example, RWXDRistream in
 * which the XDR model does not permit \b std::streambuf functionality.
 *
 * Specializing classes that do use \b std::streambuf in their
 * implementation (such as RWpistream) can usually just return the
 * corresponding \b std::ios function.
 *
 * \synopsis
 * #include <vstream.h>
 * (abstract base class)
...
public:
     ...
     * Normally, this indicates that some storage or retrieval has failed,
     * but that the stream is still in a usable state.
     */
    virtual int fail() = 0;
    ...
    /**
     * Sets the current error state to \a v. If \a v is zero, then this
 ...
 *
 * \brief
 * Abstract base class providing an interface for format-independent
 * retrieval of fundamental types and fundamental-type arrays.
 *
 * Class RWvistream is an abstract base class that provides an interface for
 ...
 * interface for the storage of the fundamental types.
 *
 * Because the interface of RWvistream and RWvostream is independent of
 * formatting, your application need not be concerned with how variables are
 * actually stored or restored -- functionality that is the responsibility
 * of whatever derived class you choose. For instance, you could use an
 * operating-system independent US-ASCII format (classes RWpistream and
 * RWpostream), a binary format (classes RWbistream and RWbostream), or
 * define your own format (such as an interface to a network).
 *
 * \note
 * Because this is an abstract base class, these design goals cannot be
 * enforced; rather, the description here is merely the model of how a class
 * derived from RWvistream and RWvostream should act.
 *
 * See class RWvostream for additional explanations and examples of
 * format-independent stream storage.
 *
 * \synopsis
...
public:
    ...
    /**
     * Gets and returns the next byte from the input stream, returning its
     * value. Returns \c EOF if end of file is encountered.
     */
    virtual int get() = 0;     // Treat as number; EOF upon eof

    /**
     * Gets the next \c char from the input stream, returning its value in
     * \a c.
     */
    ...
    /**
     * Gets the next \c char from the input stream, returning its value in
     * \a c. \a c is treated as a character.
     */
    ...
    /**
     * Gets the next \c wchar_t from the input stream, returning its value
     * in \a wc.
     */
    ...
    /**
     * Gets the next \c wchar_t from the input stream, returning its value
     * in \a wc. \a wc is treated as a character.
     */
    virtual RWvistream& getChar(wchar_t& wc) = 0;

    /**
     * Gets the next \c bool from the input stream, returning its value in
     * \a b.
     */
    virtual RWvistream& get(bool& b) = 0;

    /**
     * Gets the next \c short from the input stream, returning its value in
     * \a i.
     */
    virtual RWvistream& get(short& i) = 0;

    /**
     * Gets the next <tt>unsigned short</tt> from the input stream,
     * returning its value in \a i.
     */
    virtual RWvistream& get(unsigned short& i) = 0;

    /**
     * Gets the next \c int from the input stream, returning its value in
     * \a i.
     */
    ...
    /**
     * Gets the next \c long from the input stream, returning its value in
     * \a i.
     */
     ...
     *
     * \note
     * This operator function is available only if your compiler supports
    ...
    /**
     * Gets the next \c float from the input stream, returning its value in
     * \a f.
     */
    virtual RWvistream& get(float& f) = 0;

    /**
     * Gets the next \c double from the input stream, returning its value in
     * \a d.
     */
    ...
    /**
     * Gets the next <tt>long double</tt> from the input stream, returning
     * its value in \a d.
     *
     * \note
     * This operator function is available only if your compiler supports
     ...
     *
     * \note
     * This method retrieves raw characters and does not perform any
     * conversions on special characters such as <tt>'\\n'</tt>.
     */
     ...
     * beginning at \a v. If the restore operation stops prematurely because
     * there is no more data available on the stream, because an exception
     * is thrown, or for some other reason, stores what has already been
     * retrieved from the stream into \a v, and sets the failbit.
     *
     * \note
     * This method retrieves raw characters and does not perform any
     * conversions on special characters such as <tt>L'\\n'</tt>.
     */
...
#if !defined(RW_NO_LONG_LONG)
    /**
     * Gets a vector of \a n <tt>long long</tt> and stores them in the array
     * beginning at \a v. If the restore operation stops prematurely because
     * there is no more data available on the stream, because an exception
     * is thrown, or for some other reason, stores what has already been
     * retrieved from the stream into \a v, and sets the failbit.
     ...
     * the array beginning at \a v. If the restore operation stops
     * prematurely because there is no more data available on the stream,
     * because an exception is thrown, or for some other reason, stores what
     * has already been retrieved from the stream into \a v, and sets the
     * failbit.
     *
     * \note
     * This operator function is available only if your compiler supports
    ...
    /**
     * Gets a vector of \a n \c double and stores them in the array
     * beginning at \a v. If the restore operation stops prematurely because
     * there is no more data available on the stream, because an exception
     * is thrown, or for some other reason, stores what has already been
     * retrieved from the stream into \a v, and sets the failbit.
    ...
    /**
     * Restores a character string from the input stream that has been
     * stored to the output stream using RWvostream::putString(), then saves
     * it in the array beginning at \a s. The function stops reading at the
     * end of the string or after \a n <tt>- 1</tt> characters, whichever
     * comes first. If \a n <tt>- 1</tt> characters have been read and the
     * \a n<sup>th</sup> character is not the string terminator, then sets
     * the failbit of the stream. In either case, the string is terminated
     * with a null byte.
     */
     ...
     * at \a s. The function stops reading after \a n \c char. The resulting
    ...
    /**
     * Gets the next \c size_t value from the input stream, returning its
     * value in \a sz.
     */
...
public:
     ...
     *
     * \note
     * The version number can be used to determine how objects should be
     * deserialized from the underlying stream. Attempts to deserialize data
     * that was serialized using a different version cannot be expected to
     * work.
     */
     ...
     *
     * \note
     * The default version number is the maximum supported version that this
     * stream can read.
     */
 ...
 *
 * \brief
 * Abstract base class that provides an interface for format-dependent
 * storage of fundamental types and fundamental-type arrays.
 *
 * Class RWvostream is an abstract base class that provides an interface for
 * format-independent storage of fundamental types, and arrays of
 * fundamental types. Its counterpart, RWvistream, provides a complementary
 * interface for the retrieval of variables of the fundamental types.
 *
 * Because the interface of RWvistream and RWvostream is independent of
 * formatting, your application need not be concerned with how variables are
 * actually stored or restored -- functionality that is the responsibility
 ...
 * define your own format (such as an interface to a network).
 *
 * Note that, because this is an abstract base class, these design goals
 * cannot be enforced; rather, the description here is merely the model of
 * how a class derived from RWvistream and RWvostream should act.
 *
 * \note
 * There is no need to separate variables with whitespace. It is the
 * responsibility of the derived class to delineate variables with
 * whitespace, packet breaks, or whatever might be appropriate for the final
 * output sink. The intended model is one where variables are inserted into
 * the output stream, either individually or as homogeneous vectors, to be
 * restored in the same order using RWvistream.
 *
 * <b>About the Storage and Retrieval of Characters</b> Characters can be
 * thought of as either representing some alpha-numeric or control
 * character, or as the literal number. Generally, the overloaded insertion
 * (<<) and extraction (>>) operators seek to store and restore characters
 * while preserving their symbolic meaning. That is, storage of a newline
 * should be restored as a newline, regardless of its representation on the
 * target machine. By contrast, member functions
 * \ref RWvistream::get() "get()" and put() should treat the character as a
 * literal number whose value is to be preserved.
 *
 * \sa
 * RWpostream.
 *
...
public:
    ...
    /**
     * Stores the \c char \a c to the output stream, preserving its meaning.
     * \a c is treated as a character.
     */
    virtual RWvostream& putChar(char c) = 0;

    /**
     * Stores the <tt>signed char</tt> \a c to the output stream, preserving
     * its value.
     */
    ...
    /**
     * Stores the <tt>signed char</tt> \a c to the output stream, preserving
     * its meaning. \a c is treated as a character.
     */
    ...
    /**
     * Stores the <tt>unsigned char</tt> \a c to the output stream,
     * preserving its meaning. \a c is treated as a character.
     */
    ...
    /**
     * Stores the vector of \a n <tt>signed char</tt> starting at \a v to
     * the output stream. The characters should be treated as literal
     * numbers (i.e., not as a character string).
     */
    ...
    /**
     * Stores the vector of \a n <tt>unsigned short</tt> starting at \a v to
     * the output stream.
     */
    ...
    /**
     * Stores the vector of \a n <tt>unsigned int</tt> starting at \a v to
     * the output stream.
     */
...
#if !defined(RW_NO_LONG_LONG)
    /**
     * Stores the vector of \a n <tt>long long</tt> starting at \a v to the
     * output stream.
     *
     * \note
     * This operator function is available only if your compiler supports
...
#if !defined(RW_NO_LONG_DOUBLE)
    /**
     * Stores the vector of \a n <tt>long double</tt> starting at \a v to
     * the output stream.
     *
     * \note
     * This operator function is available only if your compiler supports
    ...
    /**
     * Stores the character string, <em>including embedded nulls</em>,
     * starting at \a s to the output stream.
     */
     ...
     *
     * \note
     * This function delegates to put(const char*, size_t) if not
     * overridden.
     */
...
public:
     ...
     *
     * \note
     * The version number may be used to determine how objects should be
     * serialized to the underlying stream. This can be used to ensure
     * compatibility with existing code or reduce the amount of data put on
 ...
 *
 * \brief
 * Input iterator that reads successive elements from the input stream for
 * which it was constructed.
 *
 * Class RWvistreamIterator is an input iterator that reads (using
 * \c operator>>) successive elements from the input stream for which it was
 * constructed. After it is constructed, and every time operator++() or
 * operator++(int) is used, the iterator reads and stores a value of \c T.
 *
 ...
 * not defined. For any other iterator value, a <tt>const T*</tt> is
 * returned. The behavior of a program that applies operator++() to an
 * end-of-stream iterator is undefined. It is impossible to store things
 * into RWvistream iterators.
 */
template <typename T>
...
public:
     ...
     * extraction operations. Extracts a single value of type \c T from the
     * stream \p s. If the extraction fails, this iterator becomes an
     * end-of-stream iterator, otherwise this iterator becomes a
    ...
    /**
     * Returns a reference to the last value read from the underlying
     * stream.
     */
     ...
     * to self. If the extraction operation fails, this iterator becomes an
    ...
    /**
     * Makes a copy of self, extracts an item from the underlying stream
     * and, returns the copy that was made before the extraction. If the
     * extract operation fails, this iterator becomes an end-of-stream
     * iterator.
     */
     ...
     * non-end-of-stream iterators are equal when they are associated with
     * the same stream.
     */
 ...
 *
 * \brief
 * Writes successive elements onto the output stream from which it was
 * constructed.
 *
 * Class RWvostreamIterator writes (using \c operator<<) successive elements
 * onto the output stream from which it was constructed. It is not possible
 * to get a value out of the output iterator. Its only use is as an output
 * iterator in situations like the following.
 *
 * \code
...
public:
    ...
    /**
     * Inserts \a value onto the underlying stream and Returns a reference
     * to self.
     */
...
/**
 * Gets the next <tt>signed char</tt> from the input stream and stores it in
 * \a c.
 *
 * \relates RWvistream
 */
...
/**
 * Gets the next <tt>unsigned char</tt> from the input stream and stores it
 * in \a c.
 *
 * \relates RWvistream
 */
...
/**
 * Gets the next \c wchar_t from the input stream and stores it in \a wc.
 *
 * \relates RWvistream
 */
...
/**
 * Gets the next \c bool from the input stream and stores it in \a b.
 *
 * \relates RWvistream
 */
...
/**
 * Gets the next \c short from the input stream and stores it in \a i.
 *
 * \relates RWvistream
 */
...
/**
 * Gets the next <tt>unsigned short</tt> from the input stream and stores it
 * in \a i.
 *
 * \relates RWvistream
 */
...
/**
 * Gets the next \c int from the input stream and stores it in \a i.
 *
 * \relates RWvistream
 */
...
/**
 * Gets the next <tt>unsigned int</tt> from the input stream and stores it
 * in \a i.
 *
 * \relates RWvistream
 */
...
/**
 * Gets the next \c long from the input stream and stores it in \a i.
 *
 * \relates RWvistream
 */
...
/**
 * Gets the next <tt>unsigned long</tt> from the input stream and stores it
 * in \a i.
 *
 * \relates RWvistream
 */
...
#if !defined(RW_NO_LONG_LONG)
/**
 * Gets the next <tt>long long</tt> from the input stream and stores it in
 * \a i.
 *
 * \relates RWvistream
 *
...
/**
 * Gets the next <tt>unsigned long long</tt> from the input stream and
 * stores it in \a i.
 *
 * \relates RWvistream
 *
...
/**
 * Gets the next \c float from the input stream and stores it in \a f.
 *
 * \relates RWvistream
 */
...
/**
 * Gets the next \c double from the input stream and stores it in \a d.
 *
 * \relates RWvistream
 */
...
#if !defined(RW_NO_LONG_DOUBLE)
/**
 * Gets the next <tt>long double</tt> from the input stream and stores it in
 * \a d.
 *
 * \relates RWvistream
 *
