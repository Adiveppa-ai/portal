...
--
--
-- sql script to find the list of schedule tables to be partitioned.
--
...
DECLARE
	--
       	-- By default, pick up the tables that belong to class /schedule
       	--
	...
	AS table_name
        ...
        AND (do.name = '/schedule' OR do.name LIKE '/schedule/%')
      	...
      	EXCEPTION
      		WHEN no_data_found THEN
            		INSERT INTO tables_tobe_partitioned_t (table_name, base_name, object_name, partition_exchanged)
                  	VALUES (UPPER(partition_tables_rec.table_name), 'SCHEDULE_T', '/schedule', 0);
...
EXCEPTION
	WHEN OTHERS THEN
               	pin_upg_common.log_info (C_STEP_NAME, NULL, NULL, 'E', 
			'Error finding the schedule table names to be partitioned', sqlerrm);
END;
/
