--
-- @(#)$Id: topup_tables_tobe_partitioned.sql /cgbubrm_7.5.0.portalbase/1 2015/11/27 06:09:26 nishahan Exp $
--
...
--
--
-- sql script to find the list of topup tables to be partitioned.
--
...
DECLARE
	...
	AS table_name
        ...
        AND (do.name = '/topup' OR do.name LIKE '/topup/%')
      	...
      	EXCEPTION
      		WHEN no_data_found THEN
            		INSERT INTO tables_tobe_partitioned_t (table_name, base_name, object_name, partition_exchanged)
                  	VALUES (UPPER(partition_tables_rec.table_name), 'TOPUP_T', '/topup', 0);
...
EXCEPTION
	WHEN OTHERS THEN
               	pin_upg_common.log_info (C_STEP_NAME, NULL, NULL, 'E', 
			'Error finding the topup table names to be partitioned', sqlerrm);
END;
/
