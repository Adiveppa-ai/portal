...
JAVA_HOME_LOCATION="/home/pin/opt/portal/Thirdparty/jdk1.8.0_401"
...
# extract the JDK version on a single int
singleVersion=0
version=`$JRELOC/bin/$AOUT -version 2>&1 | tr '\r' '\n'`
        if [[ ($version = *"version \""*) ]] ; then
        ver=$(echo $version | sed -e 's/.*version "\(.*\)"\(.*\)/\1/; 1q')
                if [[ $ver = "1."* ]] ; then
                        singleVersion=$(echo $ver | sed -e 's/1\.\([0-9]*\)\(.*\)/\1/; 1q')
                else
                        singleVersion=$(echo $ver | sed -e 's/\([0-9]*\)\(.*\)/\1/; 1q')
                fi
        fi
...
Solaris_SPARC64|Intel_Solaris|MAC-OS-IN64|HP_IA64)
	if [ "$singleVersion" -lt "10" -o "$PLATFORM_NAME" = "HP_IA64" ] ; then
