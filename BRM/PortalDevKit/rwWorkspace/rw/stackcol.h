 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/stackcol.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * A restricted interface to class RWSlistCollectables to implement a last
 * in first out (LIFO) stack.
 *
 * Class RWSlistCollectablesStack represents a restricted interface to class
 * RWSlistCollectables to implement a last in first out (LIFO) stack. A
 * stack is a sequential list for which all insertions and deletions are
 * made at one end (the beginning of the list). Hence, the ordering is
 * determined externally by the ordering of the insertions. Duplicates are
 * allowed.
 *
 ...
 * (see class RWCollectable) to find a match between a target and an item in
 * the stack.
 *
...
public:
    ...
    /**
     * Returns \c __RWSLISTCOLLECTABLESSTACK, i.e. a <em>class
     * identifier</em>, that identifies this object's class.
     */
...
#  if !defined(RW_NO_RVALUE_REFERENCES)
    ...
    /**
     * Returns the item at the top of the stack or returns \c nil if the
     * stack is empty.
     */
