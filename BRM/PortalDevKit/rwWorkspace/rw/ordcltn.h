 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/ordcltn.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Represents a group of ordered items, accessible by an index number, but
 * not accessible by an external key
 *
 ...
 *
 * Class RWOrdered is implemented as a vector of pointers, allowing for more
 * efficient traversing of the collection than the linked list classes
 * RWSlistCollectables and RWDlistCollectables, but with slower insertion in
...
public:
    ...
    /**
     * Constructs an RWOrdered with an initial capacity of
     * RWCollection::DEFAULT_CAPACITY.
     */
    RWOrdered();
    ...
    /**
     * Returns \c true if for every item in self, the corresponding item in
     * \a od at the same index \ref RWCollectable::isEqual() "isEqual()".
     * The two collections must also have the same number of members.
     */
    ...
    /**
     * Returns the first item that \ref RWCollectable::isEqual() "isEqual()"
     * to the item pointed to by \a target, or #rwnil if no item was found.
     */
    ...
    /**
     * Removes the first item that \ref RWCollectable::isEqual() "isEqual()"
     * to the item pointed to by \a target and returns it. Returns #rwnil if
     * no item was found.
     */
    ...
    /**
     * Returns the \a i <sup>th</sup> element in the collection. If \a i is
     * out of range, the function throws an exception of type RWBoundsErr.
     * The results of this function can be used as an lvalue.
     */
    RWCollectable*& operator[](size_t i);

    /**
     * Returns the \a i <sup>th</sup> element in the collection. Bounds
     * checking is enabled by defining the preprocessor directive
     * \c RWBOUNDS_CHECK before including the header file \c ordcltn.h. In
     * this case, if \a i is out of range, the function throws an exception
     * of type RWBoundsErr. The results of this function can be used as an
     * lvalue.
     */
    RWCollectable*& operator()(size_t i);
     ...
     * RWSlistCollectablesStack. The item pointed to by \a c is put at the
    ...
    /**
     * This is an alternative implementation of a stack to class
     * RWSlistCollectablesStack. The last item in the collection is removed
     * and returned. If there are no items in the collection, #rwnil is
     * returned.
     */
    RWCollectable* pop();
    ...
    /**
     * Called by resize() to determine the new container capacity. The
     * default implementation resizes the container in fixed increments. By
     * overriding this method in derived classes, users may alter the
     ...
     * returned. If there are no items in the collection, #rwnil is
     * returned.
     */
 ...
 * Traverses the collection from the first to the last item
 *
 * One of two iterators for class RWOrdered. Traverses the collection from
 * the first to the last item. As a \c const iterator, this iterator does
 * not change anything in the underlying data structure.
 *
