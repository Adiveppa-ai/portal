...
Grammar
{
  ...
  balance_packet_list:
      ...
      SOL42_V670_REL_FORINPUT.BALANCE_PACKET
        {
	  Long origImpactType;
	  Long backoutImpactType;
	  Long PIN_IMPACT_TYPE_TAX 		  = 4;
	  Long PIN_IMPACT_TYPE_RERATED_TAX 	  = 64;
	  Long PIN_IMPACT_TYPE_DISCOUNT 	  = 128;
	  Long PIN_IMPACT_TYPE_DISCOUNT_RERATED   = 256;
	  Long PIN_IMPACT_TYPE_INTEGRATE_PRERATED = 258;
	  Long PIN_IMPACT_TYPE_INTEGRATE_RERATED  = 288;
	  ...
	  origImpactType = edrLong( DETAIL.ASS_PIN.BP.PIN_IMPACT_TYPE, blockEntries - 1, packetEntries);
	  backoutImpactType = origImpactType;

	  if ( origImpactType == PIN_IMPACT_TYPE_INTEGRATE_RERATED )
	  {
		backoutImpactType = PIN_IMPACT_TYPE_INTEGRATE_PRERATED;
	  }
	  else if ( origImpactType == PIN_IMPACT_TYPE_DISCOUNT_RERATED )
	  {
		backoutImpactType = PIN_IMPACT_TYPE_DISCOUNT;
	  }
	  else if ( origImpactType == PIN_IMPACT_TYPE_RERATED_TAX )
	  {
		backoutImpactType = PIN_IMPACT_TYPE_TAX;
	  }
	  if ( origImpactType != backoutImpactType )
	  {
	    edrLong( DETAIL.ASS_PIN.BP.PIN_IMPACT_TYPE, blockEntries - 1, packetEntries) = backoutImpactType; 
	  }
        }
    | /* EMPTY */
    ;
