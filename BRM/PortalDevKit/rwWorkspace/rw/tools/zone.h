 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tools/zone.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Struct used by RWDaylightRule to define the beginning and ending of
 * daylight saving time.
 *
 * RWDaylightBoundary is a struct with no constructors that is used by
 * RWDaylightRule to define the beginning and ending of daylight saving
 * time.
 */
struct RW_TOOLS_SYMBOLIC RWDaylightBoundary {
    ...
    /**
     * A day of the week from <tt>(0 - 6)</tt>, where \c 0 = Sunday, or, if
     * the \c week_ field is \c -1, a day of the month from
     * <tt>(1 - 31)</tt>.
     */
    int weekday_;

    /**
     * Minutes after 12:00 AM, from <tt>(0 - 1439)</tt>. For example, \c 120
     * = 2 AM.
     */
    int minute_;
 ...
 *
 * The RWDaylightRule struct defines daylight saving time rules and is
 * passed to RWZoneSimple's constructor. It can define a single rule for all
 * years or the head of a chain of rules going backwards in time.
 *
 * RWDaylightRule has no constructors. The data members of this structure
 * are as follows:
 */
struct RW_TOOLS_SYMBOLIC RWDaylightRule {
    ...
    /**
     * Four-digit representation of the year in which this rule first goes
     * into effect.
     */
    short firstYear_;

    /**
     * A boolean value that can be used to specify a period of years for
     * which Daylight Saving Time is not observed.
     *
    ...
    /**
     * Indicates the time of year, to the minute, when DST begins during
     * this period.
     */
    RWDaylightBoundary begin_;

    /**
     * Indicates the time of year, to the minute, when standard time resumes
     * during this period.
     */
 ...
 *
 * RWZone is an abstract base class that defines an interface for time zone
 * issues, such as whether or not Daylight Saving Time (DST) is in use, the
 * names and offsets from Greenwich Mean Time (GMT) (also known as
 * Coordinated Universal Time, or UTC), for both standard and Daylight
 * Saving Time, and the start and stop dates for DST, if used. Note that
 * because it is an abstract base class, there is no way to actually enforce
 * these goals -- the description here is merely the model of how a class
 * derived from RWZone should act.
 *
 * Most programs interact with RWZone only by passing an RWZone reference to
 * SourcePro member functions that expect one.
 *
 * DST rules are volatile, often reflecting geographical and political
 * changes. In some cases, the hard-coded, table-driven struct
 * RWDaylightRule does not accurately reflect the locale installed on your
 * machine. RWZone::os() creates a new RWZone containing the daylight rule
 * discovered from the underlying operating system. This way, the operating
 * system itself defines the DST rules.
 *
 * \synopsis
 * #include <time.h>
 * #include <rw/zone.h>
...
public:
    ...
    /**
     * Returns the number of seconds west of GMT for standard time in this
     * zone. The number is negative for zones east of Greenwich, England.
     */
    ...
    /**
     * Returns the name of the standard time zone represented, such as
     * \c "PST". Note that the current date and time have no effect on the
     * return value.
     */
    virtual RWCString timeZoneName() const = 0;

    /**
     * Returns the name of the DST zone represented, such as \c "PDT". Note
     * that the current date and time have no effect on the return value.
     */
    virtual RWCString altZoneName() const = 0;

    /**
     * Populates \a tmbuf for this zone based on \a msec milliseconds since
     * January 1, 1901 00:00:00:000 UTC. If not \a NULL, \a tzOffset and
     * \a tzName are populated with the time zone offset and time zone name
     ...
     * 1901 00:00:00:000 UTC in this zone given the date and time in
     * \a tmbuf.
     */
    ...
    /**
     * Returns \c true if the time and date represented in the
     * <tt>struct tm</tt> argument is in the range of DST for this zone. The
     * elements of \a tmbuf must all be self-consistent; in particular, the
     * \c tm_wday member must agree with the \c tm_year, \c tm_mon, and
     * \c tm_day members.
     */
     ...
     * \a tmbuf. If DST is not observed, the members of \a tmbuf are all set
     * to a negative value. Note that in the southern hemisphere, DST ends
     * at an earlier date than it begins.
     */
     ...
     * this is an instance of RWZoneSimple created with offsets and zone
     * names from the operating system, with U.S. rules for DST, if
     * observed. This is used as a default argument value to member
     * functions of other classes where an RWZone argument is required.
     */
    static const RWZone& local();

    /**
     * Returns a reference to an RWZone representing standard local time,
     * with no DST corrections. By default, this is an instance of
    ...
    /**
     * On the first call, determines DST rules for the years 1970 - 2037,
     * from the underlying operating system. On subsequent calls, a cached
     * copy is used. You can set the DST rules for these years by with:
     *
     * \code
     ...
     * underlying operating system. DST rules for the years 1970 - 2037 are
     * captured in the resulting RWZone instance.
     */
    ...
    /**
     * Names the standard time zones. Its values are intended to be passed
    ...
    /**
     * Used by the static member function dstRule() and by constructors for
     * classes derived from RWZone, including RWZoneSimple.
     */
    enum DstRule {
    ...
    /**
     * Returns one of the built-in DST rules according to \a rule. This
     * function is provided for convenience in constructing RWZoneSimple
     * instances for time zones in which common DST rules are obeyed. RWZone
     * has predefined names for these rules, defined under the enumerations
     * section for this class. If <tt>DstRule NoDST</tt> is provided, then
     * \c 0 is returned. The result of calling dstRule() is normally passed
     * to the RWZoneSimple constructor.
     */
 ...
 *
 * \brief
 * A basic, concrete time zone implementation of the abstract class RWZone,
 * based on the Daylight Saving Time (DST) rules defined in \c struct
 * RWDaylightRule.
 *
 * RWZoneSimple is an implementation of the abstract interface defined by
 * class RWZone. It implements a simple DST rule sufficient to represent all
 * historical U.S. conventions and many European and Asian conventions. It
 * is table-driven and depends on parameters given by the \c struct
 * RWDaylightRule.
 *
 * In many cases, you may want more explicit control of the DST rule for the
 * intended RWZoneSimple. If so, you can build a DST rule with arbitrary
 * begin and end times (see the RWDaylightRule below), and provide it as a
 * parameter to RWZoneSimple.
 *
 * Direct use of RWDaylightRule affords the most general interface to
 * RWZoneSimple. However, a much simpler programmatic interface is offered,
 ...
 * \endcode
 *
 * To install Hawaiian time (where Daylight Saving Time is not observed),
 * use:
 *
 * \code
 ...
 *
 * For example, under Britain's Summer Time rules, alternate timekeeping
 * begins the morning after the third Saturday in April, unless that is
 * Easter (in which case it begins the week before) or unless the Council
 * decides on some other time for that year. In some years Summer Time has
 * been two hours ahead, or has extended through winter without a break.
 * British Summer Time clearly deserves an RWZone class all its own.
 * \endexample
 */
...
public:
    ...
    /**
     * Constructs an RWZoneSimple instance in which DST is computed
     * according to the provided \a rule. If \a rule is zero, DST is not
     * observed.
     *
     * \param rule
     * The DST rule
     *
     * \param stdoff
     * The offset from GMT (in seconds, positive if west of 0 degrees
     * longitude)
     *
     * \param stdname
     * The name of Standard time
     *
     * \param altoff
     * The offset (typically equal to \a tzoff <tt>- 600</tt>) to define
     * when DST is in effect
     *
     * \param altname
     * Alternative name for the time zone when DST is in effect, for
     * example, MDT for Mountain Daylight Time.
     */
    ...
    /**
     * Constructs an RWZoneSimple instance in which DST is not observed.
     * Argument \a tzoff is the offset from GMT (in seconds, positive if
     * west of 0 degrees longitude) and \a tzname is the name of the zone.
     */
     ...
     * specified by the \c StdZone argument. DST is computed according to
     * the \a rule argument, if non-zero; otherwise, DST is not observed.
     */
     ...
     * instances. This is the simplest interface to RWZoneSimple. The
     * argument \a zone is the time zone for which an RWZoneSimple is to be
     * constructed. The second argument defines the DST rule to follow.
     * North America's DST rules are the default.
     */
