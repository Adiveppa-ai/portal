 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tpset.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * A pointer-based collection of values ordered according to a comparison
 * object.
 *
 * This class maintains a pointer-based collection of values, which are
 * ordered according to a comparison object of type \c C. Class \c T is the
 * type pointed to by the items in the collection. Class \c A is an
 * allocator of objects of class \c T. For more information about using
 * custom allocators, please see the <em>Essential Tools Module User's
 * Guide</em>.
 *
 ...
 * \endcode
 *
 * which returns \c true if \c x should precede \c y within the collection.
 * The structure \b std::less<T> from the C++ Standard Library header file
 * \c \<functional\> is an example. Note that items in the collection will
 * be dereferenced before being compared.
 *
 * RWTPtrSet will not accept an item that compares equal to an item already
 * in the collection. (RWTPtrMultiSet may contain multiple items that
 * compare equal to each other.) Equality is based on the comparison object
 * and not on the \c == operator. Given a comparison object \c comp, items
 * \c a and \c b are equal if <tt>!comp(a,b) && !comp(b,a)</tt>.
 *
 ...
 * \sa
 * \parblock
 * Class RWTPtrMultiSet offers the same interface to a pointer-based
 * collection that accepts multiple items that compare equal to each other.
 * RWTPtrMap is a pointer-based collection of key-value pairs.
 *
 * Class <b>std::set<T*,key_compare,A ></b> is the C++ Standard collection
 * that serves as the underlying implementation for RWTPtrSet.
 * \endparblock
 *
...
public:
     ...
     * equal with <tt>*\a a</tt>, otherwise returns \c false.
     */
    ...
    /**
     * Returns the number of elements \c t in self that compare equal with
     * <tt>*\a a</tt>.
     */
     ...
     * equal with <tt>*\a a</tt>. Returns #rwnil if there is no such
     * element.
     */
    ...
    /**
     * Removes all elements \c t in self that compares equal with \c *a.
    ...
    /**
     * Sets self to the union of self and \a s. Elements from each set are
     * dereferenced before being compared.
     *
     * \note
     * The uppercase "U" in \c Union avoids conflict with the C++ reserved
     * word.
     */
    ...
    /**
     * Sets self to the intersection of self and \a s. Elements from each
     ...
     * <tt>(self - s)</tt>. Elements from each set are dereferenced before
     * being compared.
     */
    ...
    /**
     * Sets self to the symmetric difference of self and \a s. Elements from
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrSet::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTPtrSet.
 *
 * RWTPtrSetIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTPtrSet. (An STL iterator
 * interface is provided through RWTPtrSet::iterator.)
 *
 * The order of iteration over an RWTPtrSet is dependent on the comparator
 * object supplied as applied to the values stored in the container.
 *
...
public:
    /**
     * Creates an iterator for the set \a s. The iterator begins in an
     ...
     * iteration, self referenced the last association in the set, self will
     * now reference an undefined value, and a value equivalent to \c false
     * will be returned. Otherwise, a value equivalent to \c true is
     * returned.
     */
    ...
    T* key() const
    {
        return *(const_cast<RWTPtrSetIterator*>(this)->iter_);
    }

    /**
     * Resets the iterator so that after being advanced, it will point to
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrSet::const_iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTPtrSet.
 *
 * RWTPtrSetConstIterator provides a \c const iterator interface to C++
 * Standard Library-based RWTPtrSet collections. A \c const iterator does
 * not modify any information in the underlying data structure.
 *
 * The order of iteration over an RWTPtrSet is dependent on the comparator
 * object supplied as applied to the values stored in the container.
 *
...
public:
     ...
     * iteration, self referenced the last association in the set, self now
     * references an undefined value and a value equivalent to \c false is
