...
/**********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/timer.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Measures two types of time, system and user time, both of which can be
 * reported as cumulative if desired.
 *
 * This class can measure two types of time: system time and user time, both
 * of which can be reported as cumulative, if desired. The timer has two
 ...
 *
 * RWTimer uses different APIs on Windows and UNIX; on Windows it uses
 * \c GetProcessTime, whereas on UNIX it uses the \c getrusage system call.
 *
...
struct RW_TOOLS_SYMBOLIC RWTimer {
    ...
    /**
     * Puts the timer in the "stopped" state. Time does not accumulate while
     * in this state.
     */
    void stop();
    ...
    /**
     * Returns the amount of time, in microseconds, measured by the object.
     * The parameter is an integer bitmask indicating the type of time
     * requested.
     */
    rwint64 time(int = userTime | systemTime) const;

    /**
     * Returns the amount of user and system time, in seconds, that has
     * accumulated while the timer was in the running state.
     */
    double elapsedTime() const;

    /**
     * Returns the number of seconds that have elapsed while the timer was
 ...
 *
 * This class measures elapsed wall clock time. The timer has two states:
...
struct RW_TOOLS_SYMBOLIC RWClockTimer {
    ...
    /**
     * Returns the amount of time in seconds that has accumulated while the
