 ...
 * mthvecop.cc
 *
 * $Id: //spro/rel2016.1/rw/math/mthvecop.cc#1 $
 *
...
RWMathVec<T> operator -(const RWMathVec<T>& s)
{
    ...
    T* sp = (T*)s.data();  // cast away constness to prevent Borland warning
    T* dp = temp.data();
    int j = s.stride();
    while (i--) {
        *dp++ = T(-(*sp));
...
operator *(const RWMathVec<T>& s, const T& scalar)
{
    ...
    RWBlas<T>::scal(static_cast<long>(i), scalar, temp.data(), temp.stride());
    return temp;
}
