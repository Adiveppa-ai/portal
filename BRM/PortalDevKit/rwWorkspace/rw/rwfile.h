 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/rwfile.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * Class RWFile represents an abstraction of a filesystem regular file. The
 * file is manipulated in binary mode by default. This class' interface is
 * based on class \e PFile of the Interviews Class Library (1987, Stanford
 * University). The member function names begin with upper case letters in
 * order to maintain compatibility with class PFile.
 *
 * If a function expects an RWFile to exist, your code should always check
 * isValid() prior to calling that function. Failure to do so may cause a
 ...
 * \endcode
 *
 * Note that RWFile cannot anticipate or define the behavior of a program
 * that calls any member function other than isValid() on an invalid RWFile
 * object.
 *
 * \synopsis
 * #include <rw/rwfile.h>
...
public:
    ...
    /**
     * Constructs an RWFile to be used with the file name \a filename and
     * with mode \a mode. The mode string conforms to POSIX \c fopen modes
     * (see
     * http://www.opengroup.org/onlinepubs/009695399/functions/fopen.html).
     *
     * If \a mode is zero (the default), then the constructor attempts to
     * open an existing file with the given \a filename for update (\c fopen
     * mode \c "rb+"). If this is not possible, then it attempts to create a
     * new file with the given \a filename (\c fopen mode \c "wb+"). The
     * resulting object should be checked for validity using the function
     * isValid().
     *
     * The last parameter \a large_file is used to determine if the file
     * should be opened in large file mode. On systems that are large file
     * enabled, this parameter is ignored, and the file is always opened in
     * large file mode. The default is \c false for compatibility with older
     * versions of SourcePro, meaning that the file is not opened in large
     * file mode. For newer versions and applications, we recommend that you
     * pass a value of \c true for the \a large_file parameter, and that all
     * file positioning calls be done with the large file safe functions
     * SeekToBegin(), SeekToEnd(), SeekTo64(), and CurOffset64().
     *
     * \note
     * Because this class is intended to encapsulate binary operations, it
     * is important that it open or create files in binary mode. If a file
    ...
    /**
     * Returns a non-null \c FILE pointer for a valid file. Provided for
     * users who need to perform implementation-dependent operations on
     * \c FILE. Any changes in the state of the \c FILE object may render
     * the behavior of the corresponding RWFile object undefined.
     */
    FILE* GetStream() const;

    /**
     * Repositions the file pointer to \a offset bytes from the start of the
     * file. Can be used with both small and very large (more than 2GB)
     * files. Returns \c true if the operation is successful. Returns \c -1
     * on error or if the requested seek offset is outside the range
     * supported by the underlying file.
     */
    ...
    /**
     * Returns the current position, in bytes from the start of the file, as
     * a 32-bit integer. Returns \c -1 on error or if the file position is
     * beyond the representable range of #RWoffset. May be used with both
     * small and very large (more than 2GB) files.
     */
    RWoffset   CurOffset() const;

    /**
     * Returns the current offset, in bytes from the start of the file, as a
     * 64-bit integer. May be used with both small and very large (more than
     * 2GB) files.
     */
    RWoffset64 CurOffset64() const;

    /**
     * Repositions the file pointer to the end of the file. Returns \c true
    ...
    /**
     * Erases the contents but does not close the file. Returns \c true if
    ...
    /**
     * Resets error state so that neither Eof() nor Error() returns \c true.
     */
    void ClearErr();
     ...
     * Error() you should call isValid() to determine if the file was
     * opened/created successfully. Failure to do so could result in runtime
     * errors.
     */
    bool Error() const;

    /**
     * Returns \c true if the file contains no data, otherwise \c false, or
     * on error.
     */
    bool IsEmpty() const;
    ...
    /**
     * Returns \c true if the file was successfully opened, there are no I/O
     * errors, and the \c EOF has not been reached.
     */
    bool good() const
    {
    ...
    /**
     * Returns \c true if a file with name \a filename exists and may be
     * accessed according to the \a mode specified. The \a mode may be
     * <tt>OR</tt>ed together from one or more of:
     *
     * <table>
     * <tr>
     * <th>Mode</th>
     ...
     * <tr>
     * <td>\c F_OK</td>
     * <td>Exists (Implied by any of the others)</td>
     * </tr>
     * <tr>
     * <td>\c X_OK</td>
     * <td>Executable or searchable</td>
     * </tr>
     * <tr>
     * <td>\c W_OK</td>
     * <td>Writable</td>
     ...
     * \b access() function, then mode \c X_OK always returns \c false.
     */
    ...
    /**
     * Reads the indicated built-in type. Returns \c true if the read is
     * successful. If you are already at the end of a file, this is
     * interpreted as successfully reading a null string, and returns
     * \c true.
     */
    bool Read(bool& i);
     ...
     *
     * \note
     * This function is available only if your compiler supports the
     ...
     * elements of the array \a i until either \a n characters have been
     * extracted, or if <tt>(delim != EOF)</tt>, the character specified by
     * \a delim has been extracted.
     *
     * If the delimiter is reached before \a n characters are extracted, the
     * delimiter terminates the array \a i; if the delimiter is \c EOF and
     * is reached before \a n characters are extracted, the method null
     * terminates the array \a i.
     *
     * The parameter \a n is the maximum number of characters to read, and
     * must be less than or equal to the size of the array pointed to by
     * \a i. The \a delim parameter specifies a delimiter to use as a
     * termination point for extraction. The value of \a delim must be a
     * valid character converted to \c int, or \c EOF.
     *
     * The optional parameter \a count is a pointer to the actual number of
     * characters read by the function.
     *
     * This function returns \c true if \a n bytes were read from the file,
     * or the specified delimiter was reached.
     *
     * \note
     * The caller is responsible for declaring and allocating the necessary
     * storage for \a i and \a count (if used), before calling this
     * function.
     */
    bool Read(char* i, size_t n, int delim, size_t* count = 0);

    /**
     * Reads \a n instances of the indicated built-in type into a block
     * pointed to by \a i. Returns \c true if the read is successful.
     *
     * The optional parameter \a count is a pointer to the actual number of
     * characters read by the function.
     *
     ...
     *
     * \note
     * If the caller passes \c 0 for \a count, the program will compile
     * without any warnings and call Read(char*, size_t, int, size_t*).
     */
    ...
    /**
     * \deprecated
     * As of SourcePro 11. Because of the risk of buffer overflow, we
     * strongly recommend against using this function. Use
     * Read(char*, size_t, size_t*) or Read(char*, size_t, int, size_t*)
     * instead.
     *
     * Reads a character string, <em>including the terminating null
     * character</em>, into a block pointed to by \a string. Returns \c true
     * if the read is successful.
     *
     * \note
     * The caller is responsible for declaring \a string and for allocating
     * the necessary storage before calling this function.
     */
    ...
    /**
     * Writes the appropriate built-in type. Returns \c true if the write is
     * successful.
     */
    bool Write(bool i);
    ...
    /**
     * Writes a character string, <em>including the terminating null
     * character</em>, from a block pointed to by \a string. Returns \c true
     * if the write is successful. Beware of non-terminated strings when
     * using this function.
     */
    bool Write(const char* string);

    /**
     * Writes \a count instances of the indicated built-in type from a block
     * pointed to by \a i. Returns \c true if the write is successful.
     */
 ...
 *
 * \note
 * This operator is available only if your compiler supports the
 * <tt>long long</tt> type.
 */
...
/**
 * Restores a builtin-type \a a from the RWFile \a f, replacing the previous
 * contents of \a a.
 */
inline RWFile& operator>>(RWFile& f, char& a)
{
