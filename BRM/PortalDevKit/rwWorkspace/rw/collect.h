 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/collect.h#1 $
 *
 **********************************************************************
 *
 ...
 * \ingroup tools_collectables_group
 *
 * The macro RW_DECLARE_COLLECTABLE_CLASS should be used in the declaration
 ...
 * \ingroup tools_collectables_group
 *
 * The macro RW_DEFINE_COLLECTABLE_CLASS_BY_ID should only be used when your
 * class needs to have the same collectable ID or for compatibility with
 * older versions of Tools.h++.
 *
 * If you use this macro, you will be defining an RWClassID that is forever
 ...
 * definition of any class that derives from RWCollectable that doesn't use
 * RW_DEFINE_COLLECTABLE_CLASS_BY_ID.
 *
 * If you use this macro, your class may have a different RWClassID with
 * each run, but it will be forever associated with the given RWStringID.
 *
 * \example
 * \code
 * #include <rw/collect.h>
 ...
 *
 * \brief
 * Contains virtual functions for identifying, hashing, comparing, storing
 * and retrieving collectable objects
 *
 ...
 * this base class typically redefine one or more of them.
 *
 * \synopsis
...
public:
    ...
    /**
     * Returns a new, default-constructed object of the same type as self.
     * The caller is responsible for deleting the object.
     */
    virtual RWCollectable* newSpecies() const;

    /**
     * Returns a new, copy-constructed object of the same type as self. The
     * caller is responsible for deleting the object.
     */
    virtual RWCollectable* copy() const;

    /**
     * Returns a class identification number (with a typedef of
     * <tt>unsigned short</tt>). The default definition returns
     * \c __RWCOLLECTABLE. Identification numbers greater than or equal to
     * 0x8000 (hex) are reserved for Rogue Wave objects. User-defined
     * classes should define isA() to return a number between 0 and 0x7FFF.
     */
    virtual RWClassID isA() const;
     ...
     * call, \a pCl may point to the partly restored object.
     */
    ...
    /**
     * Restores an object inheriting from RWCollectable into \a obj from a
     * virtual stream, replacing the previous contents of \a obj.
     * Recursively calls the virtual function restoreGuts(), taking
     * duplicate objects into account. For more information, see the
     * discussion on Persistence in the <em>Essential Tools Module User's
     * Guide</em>.
     *
     * Various exceptions that could be thrown are RWInternalErr (if the
    ...
    /**
     * Restores an object inheriting from RWCollectable into \a obj from an
     * RWFile, replacing the previous contents of \a obj. Recursively calls
     * the virtual function restoreGuts(), taking duplicate objects into
...
public:
    ...
    // SP-22271 work around a compiler bug for debug builds on Windows
#if defined(_DEBUG) && defined(_WIN32) && defined(__INTEL_COMPILER) && __INTEL_COMPILER == 1600
    RWCollectable(const RWCollectable& c)
    {}
#endif

    /**
     * All functions that inherit class RWCollectable have virtual
     * destructors. This allows them to be deleted by such member functions
     * as removeAndDestroy() without knowing their type.
     */
    virtual ~RWCollectable();

    /**
     * Returns the number of bytes used by the virtual function
     * \ref saveGuts(RWFile&) const "saveGuts(RWFile&)" to store an object.
     * Typically, this involves adding up the space required to store all
     * primitives, plus the results of calling recursiveStoreSize() for all
     * objects inheriting from RWCollectable. See the <em>Essential Tools
     * Module User's Guide</em> for details.
     */
    virtual RWspace binaryStoreSize() const;

    /**
     * The function compareTo() is necessary to sort the items in a
     * collection. If \c p1 and \c p2 are pointers to RWCollectable objects,
     * the statement:
     *
     * \code
     * p1->compareTo(p2);
     ...
     * left to the user. The default definition provided by the base class
     ...
     * \endcode
     *
     * It is important that the hash value be the same for all objects that
     * return \c true to isEqual().
     */
    virtual unsigned hash() const;

    /**
     * Returns \c true if the collectable object "matches" the object at
     * address \a t. The default definition is:
     *
     * \code
     * return this == t;
     * \endcode
    ...
    /**
     * Returns the number of bytes required to store an #rwnil pointer in an
     * RWFile.
     */
    ...
    /**
     * Returns the number of bytes required to store the object using the
     * global operator:
     *
     * \code
     ...
     * although it is not. See the discussion on RWStringID in the
     * <em>Essential Tools Module User's Guide</em> for more information on
     * how to make a non-virtual function act like a virtual function.
     */
    ...
    /**
     * Returns the result of looking up the #RWClassID associated with
     * \a name in the global RWFactory.
     */
    static
    ...
    /**
     * Returns \c true if \a id is the #RWClassID that is associated with an
