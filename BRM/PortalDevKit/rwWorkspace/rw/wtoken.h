 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/wtoken.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Breaks up a string into separate tokens, delimited by arbitrary
 * whitespace. Can be used as an alternative to the C++ Standard Library
 * function \b std::wcstok().
 *
 * Class RWWTokenizer is designed to break a string up into separate tokens,
 * delimited by arbitrary whitespace. It can be thought of as an iterator
 * for strings and as an alternative to the C++ Standard Library function
 * \b std::wcstok() which has the unfortunate side effect of changing the
...
public:
     ...
     * interface, this is the same as the last non-empty token having been
     * returned.
     */
    bool done() const;

    /**
     * Advances to the next token and returns it as a wide substring. The
     * tokens are delimited by any wide character in \a s, or any embedded
     * wide null.
     */
    RWWSubString          operator()(const wchar_t* s);

    /**
     * Advances to the next token and returns it as a substring. The tokens
     * are delimited by any of the first \a num wide characters in \a s.
     * Buffer \a s may contain embedded nulls, and must contain at least
     * \a num wide characters. Tokens will not be delimited by nulls unless
     * \a s contains nulls.
     */
    ...
    /**
     * Advances to the next token and returns it as a substring. The tokens
     * are delimited by any of the four wide characters in \c L, " \t\n\0"
     * (space, tab, newline and null).
     */
    RWWSubString          operator()();

    /**
     * Returns the next token using a delimiter pattern represented by the
     * regular expression pattern \a regex.
     *
     * This method, unlike the other operator() overloads, allows a single
     * occurrence of a delimiter to span multiple characters.
     *
     * For example, consider the RWWTokenizer instance \c tok. The statement
     * <tt>tok(RWWString("ab"))</tt> treats either \c a or \c b as a
     * delimiter character. On the other hand,
     * <tt>tok(RWTRegex<wchar_t>("ab"))</tt> treats the two-character
     * pattern, \c ab, as a single delimiter.
     *
     * This method consumes consecutive occurrences of delimiters and skips
     * over any empty fields that may be present in the string. To obtain
     * empty fields as well as non-empty fields, use the
     * nextToken(RWTRegex<wchar_t>&) method.
     */
    ...
    /**
     * Returns the next token using a delimiter pattern represented by a
     * regular expression pattern.
     *
     * Unlike the other nextToken() overloads, this method allows a single
     * occurrence of a delimiter to span multiple characters.
     *
     * For example, <tt>nextToken(RWWString("ab"))</tt> treats either \c a
     * or \c b as a delimiter character. Conversely,
