 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/bitvec.h#1 $
 *
 **********************************************************************
 *
...
/**
 * \ingroup tools_traditional_collection_group
 *
 * \brief
 * Represents a bit vector whose length can be changed at runtime.
 *
 * Class RWBitVec is a bit vector whose length can be changed at runtime.
 * Because this requires an extra level of indirection, this makes it
 * slightly less efficient than RWTBitVec, whose lengths are fixed at
 * compile time.
 *
 * \synopsis
 * #include <rw/bitvec.h>
...
public:
    ...
    /**
     * Constructs a vector with \a N bits. The initial value of the bits is
     * undefined.
     */
    RWBitVec(size_t N);
    ...
    /**
     * Constructs a vector with \a N bits, initialized to the data in the
     * array of bytes pointed to by \a bp. This array must be at least long
     * enough to contain \a N bits. The identifier \c RWByte is a typedef
     * for an <tt>unsigned char</tt>.
     */
    RWBitVec(const RWByte* bp, size_t N);

    /**
     * Copy constructor. Uses value semantics. The constructed vector is a
     * copy of \a v.
     */
    ...
    /**
     * Move constructor. The constructed vector takes ownership of the data
     * owned by \a v.
     *
     * \conditional
    ...
    /**
     * Logical assignment. Sets each element of self to the logical \c AND
     * of self and the corresponding bit in \a v. Self and \a v must have
     * the same number of elements (i.e., be conformal) or an exception of
     * type RWInternalErr is thrown.
     */
    RWBitVec&             operator&=(const RWBitVec& v)
    {
    ...
    /**
     * Logical assignment. Sets each element of self to the logical \c XOR
     * of self and the corresponding bit in \a v. Self and \a v must have
     * the same number of elements (i.e., be conformal) or an exception of
     * type RWInternalErr is thrown.
     */
    ...
    /**
     * Logical assignment. Sets each element of self to the logical \c OR of
     * self and the corresponding bit in \a v. Self and \a v must have the
     * same number of elements (i.e., be conformal) or an exception of type
     * RWInternalErr is thrown.
     */
    RWBitVec&             operator|=(const RWBitVec& v)
    {
    ...
    // Indexing operators
    /**
     * Returns a reference to bit \a i of self. A helper class, RWBitRef, is
     * used. The result can be used as an lvalue. The index \a i must be
     * between \c 0 and the length of the vector less one. Bounds checking
     * is performed. If the index is out of range, then an exception of type
     * RWBoundsErr is thrown.
     */
     ...
     * used. The result can be used as an lvalue. The index \a i must be
     * between \c 0 and the length of the vector less one. Bounds checking
     * is performed only if the preprocessor macro \c RWBOUNDS_CHECK has
     * been defined before including the header file \c <rw/bitvec.h>. If
     * so, and if the index is out of range, then an exception of type
     * RWBoundsErr is thrown.
     */
    RWBitRef              operator()(size_t i);

    /**
     * Returns the boolean value of bit \a i. The result cannot be used as
     * an lvalue. The index \a i must be between \c 0 and the length of the
     * vector less one. Bounds checking is performed. If the index is out of
     * range, then an exception of type RWBoundsErr is thrown.
     */
     ...
     * Returns the boolean value of bit \a i. The result cannot be used as
     * an lvalue. The index \a i must be between \c 0 and the length of the
     * vector less one. Bounds checking is performed only if the
     * preprocessor macro \c RWBOUNDS_CHECK has been defined before
     * including the header file \c <rw/bitvec.h>. If so, and if the index
     * is out of range, then an exception of type RWBoundsErr is thrown.
     */
    bool             operator()(size_t i) const;

    // Other operators ---
    /**
     * Returns \c true if self and \a v have the same length and if each bit
     * of self is set to the same value as the corresponding bit in \a v.
    ...
    /**
     * Returns \c false if self and \a v have the same length and if each
     * bit of self is set to the same value as the corresponding bit in
     * \a v. Otherwise, returns \c true.
     */
    bool             operator!=(const RWBitVec& v) const
    {
        return !isEqual(v);
    ...
    /**
     * Returns a vector that is the logical \c AND of the vectors \a v1 and
     * \a v2. The two vectors must have the same length or an exception of
     * type RWInternalErr is thrown.
     */
    friend RW_TOOLS_SYMBOLIC RWBitVec      operator&(const RWBitVec& v1, const RWBitVec& v2);
    /**
     * Returns a vector that is the logical \c XOR of the vectors \a v1 and
     * \a v2. The two vectors must have the same length or an exception of
     * type RWInternalErr is thrown.
     */
    friend RW_TOOLS_SYMBOLIC RWBitVec      operator^(const RWBitVec& v1, const RWBitVec& v2);
    /**
     * Returns a vector that is the logical \c OR of the vectors \a v1 and
     * \a v2. The two vectors must have the same length or an exception of
     * type RWInternalErr is thrown.
     */
    friend RW_TOOLS_SYMBOLIC RWBitVec      operator|(const RWBitVec& v1, const RWBitVec& v2);
    ...
    // Member functions
    /**
     * Clears (i.e., sets to \c false) the bit with index \a i. The index
     * \a i must be between \c 0 and the length of the vector less one. No
     * bounds checking is performed. The following are equivalent, although
     * clearBit(size_t) is slightly smaller and faster than using
     * operator()(size_t):
     *
     * \code
     * a(i) = false;
    ...
    /**
     * Returns the index of the first \c false bit in self. Returns #RW_NPOS
     * if there is no \c false bit.
     */
    size_t                firstFalse() const;

    /**
     * Returns the index of the first \c true bit in self. Returns #RW_NPOS
     * if there is no \c true bit.
     */
     ...
     * dynamically resized as necessary. The vector should be in the same
     * format printed by member function printOn(std::ostream&) const.
     */
    std::istream&              scanFrom(std::istream& s);

    /**
     * Sets (i.e., sets to \c true) the bit with index \a i. The index \a i
     * must be between \c 0 and \c size-1. No bounds checking is performed.
     * The following are equivalent, although setBit(size_t) is slightly
     * smaller and faster than using operator()(size_t):
     *
     * \code
     * a(i) = true;
     * a.setBit(i);
    ...
    /**
     * Tests the bit with index \a i. The index \a i must be between \c 0
     * and \c size-1. No bounds checking is performed. The following are
     * equivalent, although testBit(size_t) is slightly smaller and faster
 ...
 * \relates RWBitVec
 *
 * Restores an RWBitVec into \a vec from a virtual stream, replacing the
 * previous contents of \a vec.
 */
 ...
 * \relates RWBitVec
 *
 * Restores an RWBitVec into \a vec from an RWFile, replacing the previous
 * contents of \a vec.
 */
