 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tphmmap.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * A hash-based associative container of pointer types, which allows
 * duplicate keys.
 *
 * This class maintains a pointer-based collection of associations of type
 * <b>std::pair<K* const, T*></b>. These pairs are stored according to a
 * hash object of type \c H. Class \c T is the type pointed to by the items
 * in the collection. Class \c A is an allocator of objects of class \c T.
 ...
 * function that takes two arguments convertible to type \c K and returns a
 * value of type \c bool.
 *
 * \note
 * Any two keys that are equivalent <em>must</em> hash to the same value.
 *
 * This class allows multiple keys that are equivalent to each other. Use
 * the class RWTPtrHashMap if you need a collection that does not accept
 * duplicate keys. Equality is based on the comparison object and not on
 * operator==().
 *
 * The value type must have operator==() defined. This requirement is
 * imposed by the C++ Standard Library.
 *
 * \synopsis
 ...
 * \sa
 * \parblock
 * Class RWTPtrHashMap offers the same interface to a pointer-based
 * collection that does not accept multiple keys that compare equal to each
 * other.
 *
 * \ref rw_hashmultimap "rw_hashmultimap<K*,T*,hasher,key_equal,A>" is the
 * C++ Standard Library-style collection that serves as the underlying
...
public:
     ...
     * As of SourcePro 12.5, use #mapped_type& instead.
     *
     * Typedef for a non-\c const reference to the type of data in this
     * container.
     */
    ...
    /**
     * \deprecated
     * As of SourcePro 12.5, use <tt>const K* const&</tt> instead.
     *
    ...
    /**
     * \deprecated
     * As of SourcePro 12.5, use <tt>const T* const&</tt> instead.
     *
     * Typedef for a \c const reference to the type of data in this
     * container.
     */
    ...
    /**
     * \internal
     * A type representing a comparison function that dereferences the key
    ...
    /**
     * Invokes the function pointer \a fn on each association in the
     * collection. Client data may be passed through parameter \a d.
     */
     ...
     * keys and their associated items. Do not use this method if it is
     * possible that the container holds multiple pointers to the same
     * object because the results are unpredictable.
     */
     ...
     * returns <tt>#value_type(#rwnil,#rwnil)</tt>. Client data may be
     ...
     * <tt>*\a key</tt>, returns \c j, otherwise returns #rwnil.
     */
    ...
    /**
     * If there exists a key \c j in self that compares equal to
     * <tt>*\a key</tt>, returns the item associated with \c j, otherwise
     * returns #rwnil.
     */
    ...
    /**
     * If there exists a key \c j in self that compares equal to
     * <tt>*\a key</tt>, assigns the item associated with \c j to \a tr, and
     * returns \c j. Otherwise, returns #rwnil and leaves the value of \a tr
     * unchanged.
     */
    ...
    /**
     * Returns a reference to the minimum key in the collection. Type \c K
     * must have well-defined less-than semantics
    ...
    /**
     * Returns a reference to the maximum key in the collection. Type \c K
     * must have well-defined less-than semantics
     * (<tt>K::operator<(const K&)</tt> or equivalent).
     */
     ...
     * expression <tt>((*\a fn)(a,\a d))</tt> is \c true. Client data may be
     * passed through parameter \a d.
     */
     ...
     * equal to <tt>*\a key</tt>. Returns #rwnil if there is no such
     * association.
     */
    ...
    /**
     * Removes all associations with key \c j in self that compare equal to
     * <tt>*\a key</tt>. Returns the number of associations removed.
     */
    ...
    /**
     * Changes the capacity of self by creating a new hashed multi-map with
     * a capacity of \a sz. The copies \c resize every association in self
     * into the new container and finally swaps the internal representation
     * of the new container with self.
     */
    void
    resize(size_type sz)
    {
     ...
     * C++ Standard Library-style hashed multi-map \a m.
     */
    ...
    /**
     * Constructs a map by copying elements from the range [\a first,
     * \a last).
     *
     * \c InputIterator is an input iterator type that points to elements
     * that are convertible to #value_type objects.
    ...
    /**
     * Creates an empty hashed multi-map that uses the hash object \a h and
 ...
 * \relates RWTPtrHashMultiMap
 *
 * Streams the collection \a coll to the RWFile \a file, or a reference to
 ...
 * \relates RWTPtrHashMultiMap
 *
 * Restores the contents of the collection \a coll by streaming from the
 * RWFile \a file.
 */
template <class K, class T, class H, class EQ , class A>
 ...
 * heap, the application is responsible for deleting it.
 */
template <class K, class T, class H, class EQ , class A>
 ...
 * \relates RWTPtrHashMultiMap
 *
 * Looks at the next object from the streamed RWFile \a file and either
 * creates a new collection off the heap and sets \a p to point to it, or
 * sets \a p to point to a previously read instance. If a collection is
 * created off the heap, the application is responsible for deleting it.
 */
template <class K, class T, class H, class EQ , class A>
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrHashMultiMap::iterator to iterate over
 * the elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTPtrHashMultiMap.
 *
 * RWTPtrHashMultiMapIterator provides an iterator interface to the C++
 * Standard Library-based collection represented by RWTPtrHashMultiMap. (An
 * STL iterator interface is provided through
 * RWTPtrHashMultiMap::iterator.)
 *
 * Iteration over an RWTPtrHashMultiMap is pseudo-random and dependent on
 * the capacity of the underlying hash table and the hash function being
 * used. The only usable relationship between consecutive elements is that
 * elements that are defined to be equivalent by the equivalence object,
 * \c EQ, will remain adjacent.
 *
 ...
 *
 * The value type must have \c operator==() defined. This requirement is
 * imposed by the C++ Standard Library.
 *
 * \synopsis
 * #include<rw/tphmmap.h>
...
public:
     ...
     * just created, self references the first element. If used when self
     * references the last association in the multimap, self references an
     * undefined value and the method returns a value equivalent to
     * \c false. Otherwise, the method returns a value equivalent to
     * \c true.
     */
     ...
     * iterator and returns its key. If the iterator has advanced past the
     * last item in the multimap, returns #rwnil.
     */
    K* operator()()
    {
    ...
    /**
     * Returns the key portion of the association currently referenced by
     * self. The return value is undefined if self is not referencing a
     * value within the multimap.
     */
    K* key() const
    {
        return (*iter_).first;
    ...
    /**
     * Resets the iterator so that after being advanced it references the
    ...
    /**
     * Resets the iterator so that after being advanced it references the
     * first element of the collection \a m.
     */
    ...
    /**
     * Returns the value portion of the association referenced by self. The
     * return value is undefined if self is not referencing an association
     * within the multimap.
     */
    T* value() const
    {
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrHashMultiMap::const_iterator to iterate
 * over the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTPtrHashMultiMap.
 *
 * RWTPtrHashMultiMapIterator provides a \c const iterator interface to the
 * C++ Standard Library-based container RWTPtrHashMultiMap. This iterator
 * prohibits changes to the collection that is being iterated through.
 *
 * Iteration over an RWTPtrHashMultiMap is pseudo-random and dependent on
 * the capacity of the underlying hash table and the hash function being
 * used. The only usable relationship between consecutive elements is that
...
public:
    /**
     * Creates a \c const iterator for the hashed multi-map \a h. The
    ...
    /**
     * Advanced self to the next element. If the iterator has been reset or
     * just created, self references the first element. If used when self
     * references the last association in the multi-map, self references an
     * undefined value and the method returns a value equivalent to
     * \c false. Otherwise, the method returns a value equivalent to
     * \c true.
     */
     ...
     * Advances self to the next element, dereferences the resulting
     * iterator and returns its key. If the iterator has advanced past the
     * last item in the container, returns #rwnil.
     */
    const K* operator()()
    {
        return ++(*this) ? key() : rwnil;
    ...
    /**
     * Returns the key portion of the association currently referenced by
     * self. The return value is undefined if self is not referencing an
     * association within the multimap.
     */
    const K* key() const
    {
        return (*iter_).first;
