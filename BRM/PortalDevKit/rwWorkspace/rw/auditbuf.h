 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/auditbuf.h#1 $
 *
 **********************************************************************
 *
 ...
 * parameter is the address of the counter to be manipulated by
 * RWAuditFunction.
 */
 ...
 * RWAuditStreamBuffer instance counts all the bytes that pass through the
 * stream. If constructed with a function pointer, RWAuditStreamBuffer calls
 * that function with each byte that passes through the stream. The counting
 * capacity provides for streams the equivalent of the method
 * RWCollectable::recursiveStoreSize(), which is only available for RWFile.
 *
 * \synopsis
...
public:
    /**
     * Constructs a new RWAuditStreamBuffer that may be used only to examine
     * and count every byte that passes into an \b std::ostream that has the
     * RWAuditStreamBuffer instance as its \b std::streambuf. It does not
     * forward the bytes to any stream, nor accept bytes from a stream.
     */
    RWAuditStreamBuffer();
     ...
     * and count every byte that passes into an \b std::ostream that has the
     * RWAuditStreamBuffer instance as its \b std::streambuf. It does not
     * forward the bytes to any stream, nor accept bytes from a stream. The
     * second parameter to the constructor allows you to supply storage for
     ...
     * typical use would be to count or examine the bytes being input from a
     * file through a stream derived from RWvistream. The third parameter to
     * the constructor allows you to supply storage for the byte count. It
     * is optional.
     */
    ...
    /**
     * Constructs a new RWAuditStreamBuffer that passes bytes to and from
     * the \b std::iostream on which it is constructed to and from the
     * \b std::istream that has the RWAuditStreamBuffer instance as its
     * \b std::streambuf. A typical use would be to count or examine the
     * bytes being transferred to and from a file used to store and retrieve
     * changing data. The third parameter to the constructor allows you to
     ...
     * file through a stream derived from RWvostream. The third parameter to
     * the constructor allows you to supply storage for the byte count. It
     * is optional.
     */
