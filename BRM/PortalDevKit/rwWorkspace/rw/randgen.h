 ...
 *
 * $Id: //spro/rel2016.1/rw/randgen.h#1 $
 *
 ...
 * <tt>[0, 1]</tt>.
 *
 * Class RWRandGenerator generates random numbers uniformly distributed over
 * the interval <tt>[0, 1]</tt>. RWRandGenerator objects can be independent
 * or share state with each other. The Essential Math Module contains a
 * global default state that is shared by all generators created with the
 * default constructor. Using the default generator state does not guarantee
 * repeatable sequences, even if you restart() it using a specific seed.
 * This is because the global state may be shared among several applications
 * in a multithreaded environment. There is no protection against multiple
 * threads of control using the global default state at the same time. To
 * achieve repeatable sequences, you must construct a generator with a
 * specific seed value and use the generator for random number generation.
 *
 * \synopsis
