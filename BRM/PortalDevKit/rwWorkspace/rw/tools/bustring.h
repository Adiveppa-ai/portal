 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tools/bustring.h#1 $
 *
 **********************************************************************
 *
...
/**
 * \ingroup tools_string_processing_group
 * \class RWBasicUString
 *
 * \brief
 * Represents and manages an array of UTF-16 values.
 *
 * RWBasicUString is used to manage an array of UTF-16 values in the form of
 * an array of #RWUChar16 values. An instance of RWBasicUString may be
 * constructed to use one of three memory management models:
 *
 * - The initial contents are owned and managed by the client, and the
 * instance assumes no responsibility for deallocating the array. This model
 * is primarily used to treat a static or other long storage-duration array
 * as an RWBasicUString instance. The client array may be treated as
 * read-only storage, or as a writable buffer.
 * - The array is allocated by the client, but the RWBasicUString instance
 * is given the responsibility for deallocating the array.
 * - The source array is copied by the RWBasicUString instance. The client
 * retains ownership of the source array. The instance owns and manages its
 * own internal copy.
 *
 * When deallocation responsibility is passed from the client to an
 * RWBasicUString, the client must supply a function object, or functor,
 * that can be used to deallocate the storage. This mechanism allows the
 * client to choose to use <tt>operator delete[]</tt>, \b std::free(), or
 * custom memory-management mechanisms. The use of an externally supplied
 * deallocation method can also be used to satisfy the heap management
 * requirements of Microsoft Windows dynamically linked libraries, which in
 * some situations may create their own heap in addition to that of the
 * calling process.
 *
 * Given sufficient capacity, RWBasicUString adds a null terminator to any
 * non-static array passed to it. An added null is not considered part of
 * the contents, and is not included in the count returned by length().
 *
 * \synopsis
...
public:
    ...
    /**
     * Declares an alias for the type used to represent non-negative values
     * of the distance or difference type. Used for sizes and indices.
     */
    typedef size_t size_type;

    /**
     * Declares an alias for the type used to represent the distance between
     * two RWBasicUString iterators.
     */
    typedef ptrdiff_t difference_type;

    /*
     * Declares an alias for the type of values stored within an RWBasicUString
     * instance.
     */
    ...
    /**
     * Declares an alias for a \c const reference to the RWBasicUString value
     * type.
     */
    typedef const RWUChar16& const_reference;

    /**
     * A type that provides a random-access iterator over the elements in
     * the container.
     */
    ...
    /**
     * A type that provides a random-access, reverse-order iterator over the
     * elements in the container.
     *
     * This iterator type is \c const to prohibit modification of the
     * underlying RWBasicUString data.
     */
    ...
    /**
     * An enumeration whose values are used to indicate whether the source
     * array provided by a client should be copied or simply referenced.
     */
    enum Duration {

        /**
         * Indicates that a source string or array may have a duration less
         * than that of the RWBasicUString and must therefore be copied,
         */
        Transient,
    ...
    /**
     * Uses the C++ <tt>operator delete[]</tt> expression to deallocate an
     * array that was allocated using <tt>operator new</tt>.
     *
     * The method must be defined as an inline method to ensure that the
     * code generated to implement the delete resides in the client library
     * or executable, and not within the Essential Tools library.
     *
     * The code must be compiled this way to avoid the problems that arise
     * when an application contains some libraries that use debug versions
     * of heap management routines while others do not.
     */
    inline static void USE_DELETE(RWUChar16* p);

    /**
     * Calls the C++ Standard Library function \b std::free() method to
     * deallocate an array that was allocated by a call to \b std::malloc().
     *
     * The method must be defined as an inline method to ensure that the
     * version of \b std::free() called by this method is the same version
     * that is used within the client code, and that it manipulates the same
     * heap as that of \b std::malloc().
     *
     * The code must be compiled this way to avoid the problems that arise
     * when an application contains some libraries that use debug versions
     * of heap management routines while others do not.
    ...
    /**
     * When passed to the constructor of StaticDeallocator, results in a
     * StaticDeallocator instance that does nothing.
     *
     * Use this method when constructing an RWBasicUString instance that has
     * been given access to a client-owned and managed buffer.
     */
    inline static void USE_NONE(RWUChar16* p);

    /**
     * Retrieves a deallocator for use in constructing an RWBasicUString
     * instance that has been given access to a client-owned and managed
     * buffer.
     *
     * \code
     ...
     * \endcode
     *
     * The Deallocator object is owned by this class and must not be
     * deleted.
     */
     ...
     *
     * The new instance either copies the data stored in \a source, or
     * refers to the same storage as \a source, depending on the storage
     * strategy used to construct \a source.
     */
    RWBasicUString(const RWBasicUString& source);

    /**
     * Constructs an RWBasicUString instance that copies or references the
     * contents of \a source, a null-terminated sequence of #RWUChar16
     * values. The new RWBasicUString instance assumes no responsibility for
     * deallocating the storage associated with \a source.
     *
     * If \a duration is #Transient, this method copies the contents of
     * \a source into an internally allocated and managed array.
     *
     * If \a duration is #Persistent, the client retains responsibility for
     * the storage used for \a source. This mode may be used when \a source
     * resides in static or otherwise durable storage. The storage
     * associated with \a source must not be deallocated while the
     * RWBasicUString instance still references it. The original \a source
     * array cannot be modified by any of the non-\c const methods provided
     * by this class. Any attempt to do so forces RWBasicUString to create a
     * copy of \a source.
     */
    ...
    /**
     * Constructs an RWBasicUString instance that copies or references the
     * contents of \a source, an array of #RWUChar16 values that contains
     * \a sourceLength elements and may contain embedded nulls. The new
     * RWBasicUString instance assumes no responsibility for deallocating
     ...
     * with a minimum capacity of \a initialCapacity. If the original
     * allocation does not possess the capacity required by an append,
     * insert, or replace operation, a new buffer allocation is made to
     * accommodate the change in length.
     *
     * If the value passed for \a sourceLength is #RW_NPOS, the source is
     * assumed to be a null-terminated array and the entire array is copied.
     * Otherwise, \a sourceLength characters are copied.
     */
    RWBasicUString(const RWUChar16* source, size_t sourceLength, size_t initialCapacity);

    /**
     * Constructs an RWBasicUString instance that assumes ownership of
     * \a clientBuffer, a dynamically-allocated, null-terminated sequence of
     * #RWUChar16 values. The terminating null may appear at any position
     * within the storage allocated for the Unicode string.
     *
     * The \a deallocator parameter is used to supply a Deallocator object
     * that can be used to deallocate the storage referenced by
     * \a clientBuffer.
     *
     ...
     * buffer. External synchronization is required if multiple threads will
     * have access to the buffer through one or more RWBasicUString
     * instances.
     */
    ...
    /**
     * Constructs an RWBasicUString instance that assumes ownership of
     * \a clientBuffer, a dynamically-allocated array of #RWUChar16 values
     * that contains \a contentLength elements and may contain embedded
     * nulls.
     *
     * The storage required to hold \a contentLength elements may be less
     * than the storage that was allocated for \a clientBuffer.
     *
     * The \a deallocator parameter is used to supply a Deallocator object
     * that can be used to deallocate the storage referenced by
     * \a clientBuffer.
    ...
    /**
     * Constructs an RWBasicUString instance that manipulates
     * \a clientBuffer, a writable, client-supplied array that initially
     * contains \a contentLength elements and whose total usable size is
     * given by \a bufferCapacity.
     *
     * The \a deallocator parameter is used to supply a Deallocator object
     * that can be used to deallocate the storage referenced by
     * \a clientBuffer.
     ...
     * required by an append, insert, or replace operation, the buffer is
     * copied into an internally allocated buffer.
     *
     * Copy-construction or assignment produces an RWBasicUString that
     * refers to the same client-supplied buffer.
     *
     ...
     * converting the null-terminated UCS-4 or UTF-32 encoded \a source into
     * its equivalent UTF-16 representation.
     */
    RWBasicUString(const RWUChar32* source);

    /**
     * Constructs an RWBasicUString from the code units produced by
     * converting the UCS-4 or UTF-32 encoded \a source into its equivalent
     * UTF-16 representation.
     *
     * The number of elements in \a source is specified using
     * \a sourceLength.
     *
    ...
    /**
     * Constructs RWBasicUString containing a zero-length string and a
     * capacity of \a initialCapacity.
     *
    ...
    /**
     * Constructs an RWBasicUString that contains \a repeat copies of the
     * \a codeUnit.
     */
    ...
    /**
     * Constructs an RWBasicUString that contains \a repeat copies of the
     * code unit or surrogate pair of code units produced by converting the
     * UTF-32 \a codePoint into its equivalent UTF-16 representation.
     */
    RWBasicUString(RWUChar32 codePoint, size_t repeat = 1);

    /**
     * Constructs an RWBasicUString instance that contains the code units
     * produced by converting the UTF-8 encoded Unicode text in
    ...
    /**
     * Destroys self. Relinquishes ownership and frees any resources used by
     * self.
     */
    ~RWBasicUString();
     ...
     * Thrown if the sum of \a sourceOffset and \a sourceLength is greater
     * than \ref length() "source.length()".
     */
    inline RWBasicUString& append(const RWBasicUString& source, size_t sourceOffset, size_t sourceLength);

    /**
     * Appends the contents of the null-terminated array \a source to the
     * contents of self.
     */
    inline RWBasicUString& append(const RWUChar16* source);

    /**
     * Appends the contents of the array \a source to the contents of self.
     * The size of the source array is specified using \a sourceLength. The
     * source array may contain embedded nulls.
     */
    inline RWBasicUString& append(const RWUChar16* source, size_t sourceLength);

    /**
     * Converts the code points in the null-terminated array \a source into
     * UTF-16 code units and appends those code units to the contents of
     * self.
     */
    ...
    /**
     * Appends \a repeat copies of \a codeUnit to the contents of self. The
     * code unit may be zero or null.
     */
    inline RWBasicUString& append(RWUChar16 codeUnit, size_t repeat = 1);

    /**
     * Converts the UTF-32 \a codePoint into its equivalent UTF-16
     * representation and appends \a repeat copies of the resultant code
     * unit or surrogate pair of code units to the contents of self. The
     * code point may be zero or null.
     */
     ...
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
     ...
     *
     * \exception RWBoundsErr
     * Thrown if \a offset plus \a length is greater than or equal to
     * length().
     */
    inline size_t boundsCheckLength(size_t offset, size_t length) const;

    /**
     * Returns the maximum number of UTF-16 code units that may be stored in
     * self without allocating new storage.
     */
    inline size_t capacity() const;

    /**
     * Allocates and copies the existing buffer contents as required to
     * provide the capacity specified by \a newCapacity. The new capacity
     * must be greater than or equal to the current value of length();
     * otherwise no reallocation or copy occurs. Returns the resultant
     * capacity of self, which in some cases may be larger than the capacity
     * requested.
     */
    size_t capacity(size_t newCapacity);

    /**
     * Returns the number of Unicode code points encoded by the code units
     * contained in self, including embedded nulls. This method recalculates
     * the result each time it is called. Use codeUnitLength() to determine
     * the number of UTF-16 code units contained in self.
     */
    inline size_t codePointLength() const;

    /**
     * Returns the number of Unicode code points encoded by the code units
     * in the null-terminated sequence \a array. The terminating null is not
    ...
    /**
     * Returns the number of Unicode code points encoded by the code units
     * in the sequence \a array. The size of \a array is specified using
     * \a length.
     */
     ...
     * repeatedly without incurring a significant performance penalty. This
     * method returns the same value as length(). Use codePointLength() to
     * determine the number of Unicode code points contained in self.
     */
     ...
     * null-terminated UTF-32 code point sequence contained in \a array. The
     * terminating null is not included in the count.
     */
    inline static size_t codeUnitLength(const RWUChar32* array);

    /**
     * Returns the number of UTF-16 code units required to encode the UTF-32
     * code point sequence contained in \a array. The size of \a array is
     * specified using \a length.
     */
    inline static size_t codeUnitLength(const RWUChar32* array, size_t length);

    /**
     * Returns a value that describes the lexical ordering between self and
     * \a rhs. The return value should be interpreted as follows:
     *
     ...
     *
     * If the two strings have different lengths, the shorter string appears
     * before the longer string if both strings are identical over the
    ...
    /**
     * Returns a value that describes the lexical ordering between self and
     * the specified range in \a rhs. The return value should be interpreted
     * as follows:
     *
     * - Self appears before \a rhs range if the return value is less than
     * zero.
     * - Self is identical to \a rhs range if the return value is zero.
     * - Self appears after \a rhs range if the return value is greater than
     * zero.
     *
     * If the two strings have different lengths, the shorter string appears
     * before the longer string if both strings are identical over the
     ...
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a rhsOffset and \a rhsLength is greater than
     * \ref length() "\a rhs.length()".
     */
    inline int compareCodePoints(const RWBasicUString& rhs, size_t rhsOffset, size_t rhsLength) const;

    /**
     * Returns a value that describes the lexical ordering between self and
     * the null-terminated array \a rhs. The return value should be
    ...
    /**
     * Returns a value that describes the lexical ordering between self and
     * the array \a rhs whose length is given by \a rhsLength. The return
     ...
     * length of the shorter string.
     *
     * This method compares code unit values, not code point values. This
     * may not produce the desired result if either string contains
     * surrogate pairs or code unit values above the surrogate region. Use
     * compareCodePoints() if code point ordering is required.
     */
    ...
    /**
     * Returns a value that describes the lexical ordering between self and
     * \a rhs. Equivalent to compareCodeUnits(const RWBasicUString&) const.
     */
    inline int compareTo(const RWBasicUString& rhs) const;

    /**
     * Returns a value that describes the lexical ordering between self and
     * the specified range in \a rhs. Equivalent to
    ...
    /**
     * Returns a value that describes the lexical ordering between self and
     * the null-terminated array \a rhs. Equivalent to
    ...
    /**
     * Returns a value that describes the lexical ordering between self and
     * the array \a rhs whose length is give by \a rhsLength. Equivalent to
     * compareCodeUnits(const RWUChar16*,size_t) const.
     */
     ...
     * Returns a value that describes the lexical ordering between self and
     * the array \a rhs whose length is give by \a rhsLength. Equivalent to
     * compareCodePoints(const RWUChar32*,size_t) const.
     */
    inline int compareTo(const RWUChar32* rhs, size_t rhsLength) const;

    /**
     * Returns the code point value encoded by the specified pair of code
     * units. The code units must describe a valid surrogate pair. Any other
     * values produce an undefined and erroneous result.
     */
    inline static RWUChar32 computeCodePointValue(RWUChar16 highSurrogate, RWUChar16 lowSurrogate);

    /**
     * Returns \c true if self contains \a pattern, otherwise returns
     * \c false. A zero-length pattern returns \c true.
     */
     ...
     * \a patternOffset and extends for \a patternLength code units. A
     * zero-length pattern returns \c true.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a patternOffset and \a patternLength is greater
     * than \ref length() "pattern.length()".
     */
    inline bool contains(const RWBasicUString& pattern, size_t patternOffset, size_t patternLength) const;

    /**
     * Returns \c true if self contains \a pattern, otherwise returns
     * \c false. The \a pattern array must be null-terminated. A zero-length
    ...
    /**
     * Returns \c true if self contains \a pattern, otherwise returns
     * \c false. The size of the \a pattern array is specified using
     * \a patternLength. A zero-length pattern returns \c true.
     */
    ...
    /**
     * Returns \c true if self contains \a codeUnit, otherwise returns
     * \c false.
     */
    inline bool contains(RWUChar16 codeUnit) const;

    /**
     * Returns \c true if self contains \a codePoint, otherwise returns
     * \c false.
     */
     ...
     * otherwise returns \c false. The search begins at index \a offset
     * within self. A zero-length pattern returns \c true.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
     ...
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length(), or the sum
     * of \a patternOffset and \a patternLength is greater than
     * \ref length() "pattern.length()".
     */
     ...
     * Returns \c true if the specified range in self contains \a pattern,
     * otherwise returns \c false. The search begins at index \a offset
     * within self. The \a pattern array must be null-terminated. A
     * zero-length pattern returns \c true.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     ...
     * Returns \c true if the specified range in self contains \a pattern,
     * otherwise returns \c false. The search begins at index \a offset
     * within self. The size of the \a pattern array is specified using
     * \a patternLength. A zero-length pattern returns \c true.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
    ...
    /**
     * Returns \c true if the specified range in self contains \a pattern,
     * otherwise returns \c false. The search range starts at index
     * \a offset within self, and extends for \a length code units. A
     * zero-length pattern returns \c true.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
     */
    ...
    /**
     * Returns \c true if the specified range in self contains the specified
     * range in \a pattern, otherwise returns \c false. The search range
     * starts at index \a offset within self and extends for \a length code
     * units. The pattern range starts at index \a patternOffset and extends
     * for \a patternLength code units. A zero-length pattern returns
     * \c true.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length(), or the sum of \a patternOffset and \a patternLength is
     * greater than \ref length() "pattern.length()".
     */
     ...
     * Returns \c true if the specified range in self contains \a pattern,
     * otherwise returns \c false. The search range starts at index
     * \a offset within self and extends for \a length code units. The
     * \a pattern array must be null-terminated. A zero-length pattern
     * returns \c true.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     ...
     * of the \a pattern array is specified using \a patternLength. A
     * zero-length pattern returns \c true.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
    ...
    /**
     * Returns a pointer to the contents of self. The storage referenced by
     * this pointer is owned by RWBasicUString and may not be deleted or
     * modified. The pointer becomes invalid if self is modified or
     * destroyed. The contents may not be null-terminated if the client
     * provided the underlying buffer and there was insufficient space for a
     * terminating null. In all other cases, the contents will be null
     * terminated. The contents may contain embedded nulls, in which case
     * the length of the code-unit sequence is determined using length().
     */
    inline const RWUChar16* data() const;

    /**
     * Returns \c true if self has a length of zero, otherwise returns
     * \c false. This method requires constant time, while
     * <tt>length() == 0</tt> may require linear time to execute. This
     * method is equivalent to isNull().
     */
    inline bool empty() const;

    /**
     * Returns the index of the first occurrence of any code unit in
     * \a codeUnitSet, or #RW_NPOS if none of the code units in the set are
     * found.
     */
     ...
     * Returns the index of the first occurrence of any code unit in
     * \a codeUnitSet, or #RW_NPOS if none of the code units in the set are
     * found. The \a codeUnitSet array must be null-terminated.
     */
     ...
     * Returns the index of the first occurrence of any code unit in
     * \a codeUnitSet, or #RW_NPOS if none of the code units in the set are
     * found. The size of the \a codeUnitSet array is specified using
     * \a numCodeUnits.
     */
     ...
     * \a codePointSet, or #RW_NPOS if none of the code points in the set
     * are found. The \a codePointSet array must be null-terminated.
     */
     ...
     * Returns the index of the first occurrence of any code point in
     * \a codePointSet, or #RW_NPOS if none of the code points in the set
     * are found. The size of the \a codePointSet is specified using
     * \a numCodePoints.
     */
    inline size_t first(const RWUChar32* codePointSet, size_t numCodePoints) const;

    /**
     * Returns the index of the first occurrence of \a codeUnit, or #RW_NPOS
     * if that code unit is not found. This method produces the same result
     * as index(RWUChar16) const.
     */
    inline size_t first(RWUChar16 codeUnit) const;

    /**
     * Returns the index of the first occurrence of \a codePoint, or
     * #RW_NPOS if the code point is not found. This method produces the
     * same result as index(RWUChar32) const.
     */
     ...
     * Returns the index of the first occurrence of any code unit in
     * \a codeUnitSet, or #RW_NPOS if none of the code units in the set are
     * found. The search range starts at index \a offset within self.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
    ...
    /**
     * Returns the index of the first occurrence of any code unit in
     * \a codeUnitSet within the specified range of self, or #RW_NPOS is
     * none of the code units in the set are found. The search range starts
     * at index \a offset within self. The \a codeUnitSet array must be
     * null-terminated.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     ...
     * Returns the index of the first occurrence of any code unit in
     * \a codeUnitSet, or #RW_NPOS if none of the code units in the set are
     * found. The search range starts at index \a offset within self. The
     * size of the \a codeUnitSet array is specified using \a numCodeUnits.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
    ...
    /**
     * Returns the index of the first occurrence of any code point in
     * \a codePointSet within the specified range in self, or #RW_NPOS if
     * none of the code points in the set are found. The search begins at
     * index position \a offset within self. The \a codePointSet array must
     * be null-terminated.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     ...
     * Returns the index of the first occurrence of any code point in
     * \a codePointSet, or #RW_NPOS if none of the code points in the set
     * are found. The search begins at index position \a offset within self.
     * The size of the \a codePointSet array is specified using
     * \a numCodePoints.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
    ...
    /**
     * Returns the index of the first occurrence of \a codeUnit, or #RW_NPOS
     * if the code unit is not found. The search range starts at index
     * \a offset within self.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
    ...
    /**
     * Returns the index of the first occurrence of \a codePoint within the
     * specified range in self, or #RW_NPOS if the code point is not found.
     * The search begins at index position \a offset within self.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
    ...
    /**
     * Returns the index of the first occurrence of any code unit in
     * \a codeUnitSet within the specified range of self, or #RW_NPOS if
     * none of the code units in the set are found. The search range starts
     * at index \a offset within self and extends for \a length code units.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
     ...
     * none of the code units in the set are found. The search range starts
     * at index \a offset within self and extends for \a length code units.
     * The \a codeUnitSet array must be null-terminated.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
     ...
     * none of the code units in the set are found. The search range starts
     * at index \a offset within self, and extends for \a length code units.
     * The size of the \a codeUnitSet array is specified using
     * \a numCodeUnits.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
    ...
    /**
     * Returns the index of the first occurrence of any code point in
     * \a codePointSet within the specified range of self, or #RW_NPOS if
     * the none of the code points in the set are found. The search range
     * starts at index \a offset within self and extends for \a length code
     * units. The \a codePointSet array must be null-terminated.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
     ...
     * the none of the code points in the set are found. The search range
     * starts at index \a offset within self and extends for \a length code
     * units. The size of the \a codePointSet array is specified using
     * \a numCodePoints.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
    ...
    /**
     * Returns the index of the first occurrence of \a codeUnit within the
     * specified range of self, or #RW_NPOS if the code unit is not found.
     * The search range starts at index \a offset within self and extends
     * for \a length code units.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
    ...
    /**
     * Returns the index of the first occurrence of \a codePoint within the
     * specified range of self, or #RW_NPOS if the code point is not found.
     * The search range starts at index \a offset within self and extends
     * for \a length code units.
     *
     * \exception RWBoundsErr
    ...
    /**
     * Returns the high, UTF-16 surrogate (\c 0xD800..\c 0xDBFF) for a code
     * point in the range \c 0x10000..\c 0x10FFFF.
     */
    inline static RWUChar16 highSurrogate(RWUChar32 codePoint);

    /**
     * Returns \c true if the contents and length of self are identical to
     * \a array, otherwise returns \c false. This method is more efficient
    ...
    /**
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. Returns an index value of zero if the
     * pattern length is zero.
     */
    inline size_t index(const RWBasicUString& pattern) const;

    /**
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The pattern range starts at index
     * \a patternOffset and extends for \a patternLength code units. Returns
     * an index value of zero if the pattern length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a patternOffset and \a patternLength is greater
     * than \a pattern.#length().
     */
    inline size_t index(const RWBasicUString& pattern, size_t patternOffset, size_t patternLength) const;

    /**
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The pattern must be null-terminated.
    ...
    /**
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The size of the pattern is specified
    ...
    /**
     * Returns the index of the first occurrence of \a codeUnit, or #RW_NPOS
     * if the code unit is not found. This function produces the same result
     * as first(RWUChar16) const.
     */
    inline size_t index(RWUChar16 codeUnit) const;

    /**
     * Returns the index of the first occurrence of \a codePoint, or
     * #RW_NPOS if the code unit is not found. This function produces the
     * same result as first(RWUChar32) const.
     */
    inline size_t index(RWUChar32 codePoint) const;

    /**
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The search begins at index position
     * \a offset within self. Returns an index value of zero if the pattern
     * length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
    ...
    /**
     * Returns the index of the first occurrence of the specified range in
     * \a pattern, or #RW_NPOS if the pattern is not found. The search
     * begins at index position \a offset within self. The pattern range
     * starts at index \a patternOffset and extends for \a patternLength
     * code units. Returns an index value of zero if the pattern length is
     * zero.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length(), or if the
     * sum of \a patternOffset and \a patternLength is greater than
     * \a pattern.#length().
     */
     ...
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The search begins at index position
     * \a offset within self. The pattern must be null-terminated. Returns
     * an index value of zero if the pattern length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     ...
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The search begins at index position
     * \a offset within self. The size of the pattern is specified using
     * \a patternLength. Returns an index value of zero if the pattern
     * length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
    ...
    /**
     * Returns the index of the first occurrence of \a codeUnit, or #RW_NPOS
     * if the code unit is not found. The search begins at index position
     * \a offset within self. This function produces the same result as
     * first(size_t,RWUChar16) const.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
     ...
     * position \a offset within self. This function produces the same
     * result as first(size_t,RWUChar32) const.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
    ...
    /**
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The search begins at index \a offset
     * within self and extends for \a length code units. Returns an index
     ...
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The search begins at index \a offset
     * within self and extends for \a length code units. The pattern range
     * starts at index \a patternOffset, and extends for \a patternLength
     * code units. Returns an index value of zero if the pattern length is
     * zero.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than length()
     * or if the sum of \a patternOffset and \a patternLength is greater
     * than \ref length() "pattern.length()".
     */
     ...
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The search begins at index \a offset
     * within self and extends for \a length code units. The pattern must be
     * null-terminated. Returns an index value of zero if the pattern length
     * is zero.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     ...
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The search begins at index \a offset
     * within self and extends for \a length code units. The size of the
     * pattern is specified using \a patternLength. Returns an index value
     * of zero if the pattern length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
    ...
    /**
     * Returns the index of the first occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The pattern must be null-terminated. The
     * search begins at index \a offset within self and extends for
     * \a length code units. Returns an index value of zero if the pattern
     * length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
    ...
    /**
     * Returns the index of the first occurrence of \a codeUnit, or #RW_NPOS
     * if the code unit is not found. The search begins at index \a offset
     * within self and extends for \a length code units. This function
     * produces the same result as first(size_t,RWUChar16) const.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
     ...
     * Returns the index of the first occurrence of \a codePoint, or
     * #RW_NPOS if the code unit is not found. The search begins at index
     * \a offset within self and extends for \a length code units. This
     * function produces the same result as first(size_t,RWUChar32) const.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
    ...
    /**
     * Inserts the contents of the specified range in \a source before the
     * code unit at index \a offset within the contents of self.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length() or if the
     * sum of \a sourceOffset and \a sourceLength is greater than
     * \ref length() "\a source.length()".
     */
    ...
    /**
     * Inserts the contents of the array \a source before the code unit at
     * index \a offset within the contents of self. The size of the source
    ...
    /**
     * Converts the code points in the null-terminated array \a source into
     * UTF-16 code units and inserts those code units before the code unit
     * at index \a offset within the contents of self.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
    ...
    /**
     * Inserts \a repeat copies of \a codeUnit before the code unit at index
     * \a offset within the contents of self. The code unit may be zero or
     * null.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
    ...
    /**
     * Converts the UTF-32 \a codePoint into its equivalent UTF-16
     * representation and inserts \a repeat copies of the resultant code
     * unit or surrogate pair of code units before the code unit at index
     * \a offset within the contents of self. The code point may be zero or
     * null.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
    ...
    /**
     * Returns \c true if \a codeUnit encodes the high surrogate of a
     ...
     *
     * - single surrogate code points (\c U+d800..\c U+dfff, 2048 code
     * points)
     * - the last two code points on each plane (\c U+__fffe and
     * \c U+__ffff, 34 code points)
     ...
     * \a codeUnitSet, or #RW_NPOS is none of the code units in the set are
     * found.
     */
    ...
    /**
     * Returns the index of the last occurrence of \a codeUnit, or #RW_NPOS
     * if that code unit is not found.
     */
    inline size_t last(RWUChar16 codeUnit) const;

    /**
     * Returns the index of the last occurrence of \a codePoint, or #RW_NPOS
     * if that code point is not found.
     */
    inline size_t last(RWUChar32 codePoint) const;

    /**
     * Returns the index of the last occurrence of any code unit in
     * \a codeUnitSet within the specified range in self, or #RW_NPOS is
     * none of the code units in the set are found. The search begins at
     * index position \a offset within self.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
    ...
    /**
     * Returns the index of the last occurrence of any code unit in
     * \a codeUnitSet within the specified range in self, or #RW_NPOS if
     * none of the code units in the set are found. The search begins at
     * index position \a offset within self. The \a codeUnitSet array must
     * be null-terminated.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     ...
     * \a codeUnitSet within the specified range in self, or #RW_NPOS if
     * none of the code units in the set are found. The search begins at
     * index position \a offset within self. The size of the \a codeUnitSet
     ...
     * \a codePointSet within the specified range in self, or #RW_NPOS if
     * none of the code points in the set are found. The search begins at
     * index position \a offset within self. The size of the \a codePointSet
    ...
    /**
     * Returns the index of the last occurrence of \a codeUnit, within the
     * specified range in self, or #RW_NPOS if that code unit is not found.
     * The search begins at index position \a offset within self.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
    ...
    /**
     * Returns the index of the last occurrence of \a codePoint within the
     * specified range in self, or #RW_NPOS if that code point is not found.
     * The search begins at index position \a offset within self.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     ...
     * Returns the index of the last occurrence of any code point in
     * \a codePointSet within the specified range of self, or #RW_NPOS if
     * none of the code points in the set are found. The search range starts
     * at index \a offset within self and extends for \a length code units.
     * The \a codePointSet array must be null-terminated.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
     ...
     * none of the code points in the set are found. The search range starts
     * at index \a offset within self, and extends for \a length code units.
     * The size of the \a codePointSet is specified using \a numCodePoints.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
     ...
     * embedded nulls. This method returns the same value as
     * codeUnitLength(). Use codePointLength() to determine the number of
     * Unicode code points contained in self.
     */
    inline size_t length() const;

    /**
     * Returns the maximum number of code units that may be stored in an
     * RWBasicUString instance.
     */
    inline size_t max_size() const;

    /**
     * Replaces the contents of self with the contents of \a source and
     * returns a reference to self.
     */
    RWBasicUString& operator=(const RWBasicUString& source);

    /**
     * Replaces the contents of self with the null-terminated contents of
     * \a source and returns a reference to self.
     */
    RWBasicUString& operator=(const RWUChar16* source);

    /**
     * Replaces the contents of self with the code unit sequence required to
     * represent the null-terminated, UTF-32 encoded contents of \a source
     * in the UTF-16 encoding form and returns a reference to self.
     */
    ...
    /**
     * Replaces the contents of self with the code unit sequence required to
     * represent \a codePoint in the UTF-16 encoding form and returns a
     * reference to self.
     */
    RWBasicUString& operator=(RWUChar32 codePoint);

    /**
     * Appends the contents of \a source to the contents of self and returns
     * a reference to self.
     */
    ...
    /**
     * Appends the contents of the null-terminated array \a source to the
     * contents of self and returns a reference to self.
     */
    ...
    /**
     * Appends \a codeUnit to the contents of self and returns a reference
     * to self.
     */
    inline RWBasicUString& operator+=(RWUChar16 codeUnit);

    /**
     * Appends \a codePoint to the contents of self and returns a reference
     * to self.
     */
    ...
    /**
     * Returns a reference to the code unit at the position specified by
     * \a offset. This non-const variant can be used as an l-value in an
    ...
    /**
     * Returns a pointer to the contents of self. This array is owned by
     * RWBasicUString and may not be deleted or modified. The pointer
     * becomes invalid if self is modified or destroyed.
     *
     * The contents may not be null-terminated if the client provided the
     * underlying buffer and there was insufficient space for a terminating
     * null. In all other cases, the contents will be null terminated.
     *
    ...
    /**
     * Prepends the contents of the null-terminated array \a source to the
     * contents of self.
     */
    inline RWBasicUString& prepend(const RWUChar16* source);

    /**
     * Prepends the contents of the array \a source to the contents of self.
     * The size of the source array is specified using \a sourceLength. The
     * source array may contain embedded nulls.
     */
    ...
    /**
     * Converts the code points in the null-terminated array \a source into
     * UTF-16 code units and prepends those code units to the contents of
     * self.
     */
    ...
    /**
     * Converts the code points in the array \a source into UTF-16 code
     * units and prepends those code units to the contents of self. The size
     * of the source array is specified using \a sourceLength.
     */
    inline RWBasicUString& prepend(const RWUChar32* source, size_t sourceLength);

    /**
     * Prepends \a repeat copies of \a codeUnit to the contents of self. The
     * code unit may be zero or null.
     */
    ...
    /**
     * Converts the UTF-32 \a codePoint into its equivalent UTF-16
     * representation and prepends \a repeat copies of the resultant code
     * unit or surrogate pair of code units to the contents of self. The
     * code point may be zero or null.
     */
    ...
    /**
     * Removes the specified range of code units from the contents of self,
     * and collapses the contents as necessary to produce a contiguous
     * result.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     ...
     * of \a source. The sum of \a offset and \a length must be less than or
     * equal to length().
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     ...
     * \a offset and extends for \a length code units. The range in
     * \a source starts at index \a sourceOffset and extends for
     * \a sourceLength code units.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than length()
     * or the sum of \a sourceOffset and \a sourceLength is greater than
     * \ref length() "\a source.length()".
     */
    ...
    /**
     * Replaces the specified range of code units in self with the contents
     * of the null-terminated array \a source. The range in self starts at
     * index \a offset and extends for \a length code units.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
     ...
     * and extends for \a length code units. The size of the source array is
     * specified using \a sourceLength. The source array may contain
     * embedded nulls.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
    ...
    /**
     * Converts the code points in the null-terminated array \a source into
     * UTF-16 code units and replaces the specified range of code units in
     * self with those code units. The range in self starts at index
    ...
    /**
     * Converts the code points in the array \a source into UTF-16 code
     * units and replaces the specified range of code units in self with
     * those code units. The range in self starts at index \a offset and
     * extends for \a length code units. The size of the source array is
     * specified using \a sourceLength. The source array may contain
     * embedded nulls.
     *
     * \exception RWBoundsErr
     ...
     * and extends for \a length code units. The code unit may be zero or
     * null.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
    ...
    /**
     * Converts the UTF-32 \a codePoint into its equivalent UTF-16
     * representation and replaces the specified range of code units in self
     * with \a repeat copies of the resultant code unit or surrogate pair of
     * code units. The range in self starts at index \a offset and extends
     * for \a length code units. The code point may be zero or null.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
    ...
    /**
     * Pads or truncates the contents of self as required to produce a
     * length() of \a newCodeUnitLength. If the new length is greater than
     * the old length, \a codeUnit is used to pad the contents.
     */
    inline void resize(size_t newCodeUnitLength, RWUChar16 codeUnit);

    /**
     * Pads or truncates the contents of self as required to produce a
     * length() of \a newCodePointLength. If the new length is greater than
     * the old length, \a codePoint is used to pad the contents.
     */
    void resize(size_t newCodePointLength, RWUChar32 codePoint);

    /**
     * Replaces the contents of self with the code unit sequence read from
     * \a vis.
     */
    ...
    /**
     * Returns the index of the last occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. Returns an index value equal to length()
     ...
     * Returns the index of the last occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The pattern must be null-terminated.
     * Returns an index value equal to length() if the pattern length is
     * zero.
     */
     ...
     * Returns the index of the last occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The size of the pattern is specified
     * using \a patternLength. Returns an index value equal to length() if
    ...
    /**
     * Returns the index of the last occurrence of \a codeUnit, or #RW_NPOS
     * if the code unit is not found. This method is equivalent to
     * last(RWUChar16) const.
     */
    inline size_t rindex(RWUChar16 codeUnit) const;

    /**
     * Returns the index of the last occurrence of \a codePoint, or #RW_NPOS
     * if the code unit was not found. This method is equivalent to
     * last(RWUChar32) const.
     */
     ...
     * Returns the index of the last occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The search begins at index \a offset
     * within self and extends through the end of self. Returns an index
     * value equal to length() if the pattern length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
    ...
    /**
     * Returns the index of the last occurrence of the specified range in
     * \a pattern, or #RW_NPOS if the pattern is not found. The search
     * begins at index \a offset within self and extends through the end of
     * self.
     *
     ...
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length(), or the sum
     * of \a patternOffset and \a patternLength is greater than length().
     */
     ...
     * Returns the index of the last occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The search begins at index \a offset
     * within self and extends through the end of self. The pattern must be
     * null-terminated. Returns an index value equal to length() if the
     * pattern length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     ...
     * Returns the index of the last occurrence of \a pattern, or #RW_NPOS
     * if the pattern is not found. The search begins at index \a offset
     * within self and extends through the end of self. The size of the
     * pattern is specified using \a patternLength. Returns an index value
     * equal to length() if the pattern length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
     ...
     * Returns the index of the last occurrence of \a codeUnit, or #RW_NPOS
     * if the code unit is not found. The search begins at index \a offset
     * within self and extends through the end of self. This function
     ...
     * if the code unit is not found. The search begins at index \a offset
     * within self and extends through the end of self. This function
     * produces the same result as last(size_t,RWUChar32) const.
     *
     * \exception RWBoundsErr
     * Thrown if \a offset is greater than or equal to length().
     */
     ...
     * if the pattern is not found. The search begins at index \a offset
     * within self and extends for \a length code units. Returns an index
     * value equal to the sum of \a offset and \a length if the pattern
     * length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     ...
     * Returns the index of the last occurrence of the specified range in
     * \a pattern, or #RW_NPOS if the pattern is not found. The search
     * begins at index \a offset within self and extends for \a length code
     * units.
     *
     * The pattern range begins at index \a patternOffset within \a pattern
     * and extends for \a patternLength code units. Returns an index value
     * equal to the sum of \a offset and \a length if the pattern length is
     * zero.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length(), or the sum of \a patternOffset and \a patternLength is
     * greater than \a pattern.#length().
     */
     ...
     * if the pattern is not found. The search begins at index \a offset
     * within self and extends for \a length code units. The pattern must be
     * null-terminated. Returns an index value equal to the sum of \a offset
     * and \a length if the pattern length is zero.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
     ...
     * Returns the index of the last occurrence of \a codeUnit, or #RW_NPOS
     * if the code unit is not found. The search begins at index \a offset
     * within self and extends for \a length code units. This method is
     * equivalent to last(size_t,RWUChar16) const.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
     ...
     * if the code unit is not found. The search begins at index \a offset
     * within self and extends for \a length code units. This method is
     * equivalent to last(size_t,RWUChar32) const.
     *
     * \exception RWBoundsErr
     * Thrown if the sum of \a offset and \a length is greater than
     * length().
    ...
    /**
     * Writes the binary contents of self to \a file as an array of 16-bit
     * values.
     */
    ...
    /**
     * Returns the offset of the code unit in self that immediately follows
     * a prefix consisting of a repeated set of code units that correspond
     * to \a codePoint. If no such prefix exists, a value of 0 is returned.
     * If the contents of self consists entirely of a repeated sequence of
     * \a codePoint, a value equal to length() is returned. The skip code
     * point defaults to \c U+0020, the US-ASCII space character.
     */
    size_t skipLeading(RWUChar32 codePoint = static_cast<RWUChar32>(0x0020)) const;

    /**
     * Returns the offset of the code unit in self that immediately precedes
     * a suffix consisting of a repeated set of code units that correspond
     * to \a codePoint. If no such suffix exists, a value length() is
     * returned. If the contents of self consists entirely of a repeated
     * sequence of \a codePoint, a value of #RW_NPOS is returned. The skip
     * code point defaults to \c U+0020, the US-ASCII space character.
     */
    size_t skipTrailing(RWUChar32 codePoint = static_cast<RWUChar32>(0x0020)) const;

    /**
     * Returns the size of the code unit sequence contained in self. This
     * method is an alias for length().
     */
    inline size_t size() const;

    /**
     * Returns an #iterator that refers to the position of the first code
     * unit in self.
     */
    inline iterator begin();

    /**
     * Returns an #iterator that refers to the position after the last code
     * unit in self.
     */
    inline iterator end();
    ...
    /**
     * Returns a #const_iterator that refers to the position of the first
     * code unit in self.
     */
    inline const_iterator cbegin() const;

    /**
     * Returns a #const_iterator that refers to the position after the last
     * code unit in self.
     */
    inline const_iterator cend() const;

    /**
     * Returns a #reverse_iterator that refers to the position of the last
     * code unit in self.
     */
    inline reverse_iterator rbegin();

    /**
     * Returns a #reverse_iterator that refers to the position before the
    ...
    /**
     * Swaps the contents of self with that of \a ustr. This method is
     * provided to satisfy the C++ Standard Library requirements for a
     * container.
     */
     ...
     *
     * \exception RWConversionErr
     * Thrown if an illegal or truncated surrogate pair is encountered
    ...
    /**
     * Returns an RWWString containing a UTF-16 or UTF-32 representation of
     * the contents of self, depending on the size of \c wchar_t. If
     * \c sizeof(wchar_t) is \c 2, the RWWString is encoded in UTF-16. If
     * \c sizeof(wchar_t) is \c 4, the RWWString is encoded in UTF-32.
     *
     * This method produces Unicode-encoded wide-character strings. This
     * encoding may not match that of the execution environment. The
     * "native" encoding for \c wchar_t strings may be determined by the
     * current ANSI/POSIX LC_CTYPE locale and the execution environment.
     * Many environments use Unicode (ISO-10646) as the native encoding for
     * \c wchar_t. For those that do, the choice of encoding is typically
     * made based on the size of \c wchar_t.
     *
     * \exception RWConversionErr
     * Thrown if an illegal or truncated surrogate pair is encountered
     * during the conversion to UTF-32. Such errors are ignored if the
     * result is encoded as UTF-16.
     */
     ...
     *
     * This method may be used anywhere an individual code point value is
     * passed to a method:
     *
     * \code
     ...
     *
     * \exception RWConversionErr
     * Thrown if \a codePoint is not a valid Unicode character.
     */
...
private:
     ...
     * \internal
     *
     * Returns a value that describes the lexical code point ordering
     * between the code unit sequence beginning at \a thisSpanStart in self
     * at running for \a thisSpanLength code units, and the code unit
     * sequence identified by \a that and running for \a thatSpanLength code
     * units. If \a thatSpanLength is RW_NPOS, the string \a that is assumed
     * to be null-terminated and the entire string is used for the
     * comparison.
     *
     ...
     * - self appears after \a that if the return value is greater than
     * zero.
     *
     * If the two arrays have different lengths, the shorter array appears
     * before the longer array if both arrays are identical over the length
     ...
     * units. If \a thatSpanLength is RW_NPOS, the string \a that is assumed
     * to be null-terminated and the entire string is used for the
     * comparison. The comparison occurs only after \a that is converted to
     * its equivalent UTF-16 array.
     *
     * The return value should be interpreted as follows:
     * - self appears before \a that if the return value is less than zero.
     * - self is identical to \a that if the return value is zero.
     ...
     *
     * This method compares code unit values, not code point values. This
     * may not produce the desired result if either array contains surrogate
     * pairs or code unit values above the surrogate region Use
     * doCodePointCompare() if code point ordering is required.
     */
     ...
     * pattern defined by \a pattern and \a patternLength, within the
     * substring in self defined by \a offset and \a length. The comparison
     * occurs only after \a pattern is converted to its equivalent UTF-16
     * array. Returns #RW_NPOS if the search pattern was not found within
     * the specified substring of self.
     *
     * If \a patternLength is #RW_NPOS this method determines the length of
     * \a pattern by looking for a terminating null.
     */
     ...
     * pattern defined by \a pattern and \a patternLength, within the
     * substring in self defined by \a offset and \a length. Returns
     * #RW_NPOS if the search pattern was not found within the specified
     * substring of self.
     *
     * If \a patternLength is #RW_NPOS this method determines the length of
     * \a pattern by looking for a terminating null.
     ...
     * \internal
     *
     * Returns the index position of the first occurrence of any code point
     * in \a codePointSet within the substring in self defined by \a offset
     * and \a length. Returns #RW_NPOS if none of the code points were found
     * within the specified substring of self.
     *
     ...
     * \internal
     *
     * Returns the index position of the first occurrence of any code unit
     * in \a codeUnitSet within the substring in self defined by \a offset
     * and \a length. Returns #RW_NPOS if none of the code units were found
     * within the specified substring of self.
     *
     ...
     * \internal
     *
     * Creates space within self for use in replace operations. The code
     * units in the range specified by \a targetOffset and \a targetLength
     * are replaced, and that range is expanded or contracted as necessary
     * to provide an empty region of \a spaceRequired elements in size.
     */
     ...
     * pattern defined by \a pattern and \a patternLength, within the
     * substring in self defined by \a offset and \a length. The comparison
     * occurs only after \a that is converted to its equivalent UTF-16
     * array. Returns #RW_NPOS if the search pattern was not found within
     * the specified substring of self.
     *
     * If \a patternLength is #RW_NPOS this method determines the length of
     ...
     * \internal
     *
     * Returns the relative position of the last occurrence of any code
     * point in \a codePointSet within the substring in self defined by
     * \a offset and \a length. Returns #RW_NPOS if none of the code points
     * were found within the specified substring of self.
     *
     * If \a numCodePoints is #RW_NPOS this method determines the length of
     * \a codePointSet by looking for a terminating null.
     */
     ...
     * \internal
     *
     * Returns the relative position of the last occurrence of any code unit
     * in \a codeUnitSet within the substring in self defined by \a offset
     * and \a length. Returns #RW_NPOS if none of the code units were found
     * within the specified substring of self.
     *
     ...
     * \internal
     *
     * Replaces the range of code units within self starting at
     * \a targetOffset and extending \a targetLength units with an array of
     * code units located at \a sourceBegin and extending \a sourceLength
     * units. If \a sourceLength is #RW_NPOS this method determines the
     * length of source by looking for a terminating null.
     */
     ...
     * \a targetOffset and extending \a targetLength units with an array of
     * code units located at \a sourceBegin and extending \a sourceLength
     * units. The replacement occurs only after the array \a sourceBegin is
     * converted to its equivalent UTF-16 array. If \a sourceLength is
     * #RW_NPOS this method determines the length of source by looking for a
     * terminating null.
     */
    RWBasicUString& doReplace(size_t targetOffset,
                              size_t targetLength,
     ...
     * \internal
     *
     * Replaces the range of code units within self defined by
     * \a targetOffset and \a targetLength with the value \a codeUnit
     * repeated \a repeat times.
     */
    RWBasicUString& doReplace(size_t targetOffset,
                              size_t targetLength,
                              RWUChar16 codeUnit,
     ...
     *
     * Replaces the range of code units within self defined by
     * \a targetOffset and \a targetLength with the code units produced by
     * converting the UCS-4 or UTF-32 code point \a codePoint into a UTF-16
     * representation and repeating that result \a repeat times.
     */
    RWBasicUString& doReplace(size_t targetOffset,
                              size_t targetLength,
                              RWUChar32 codePoint,
     ...
     * The bytes are retrieved from \a source and the resulting code point
     * is stored in \a codePoint. The number of bytes retrieved is added to
     ...
     *
     * \exception RWConversionErr
     * Thrown to report an illegal, irregular, or truncated UTF-8 sequence.
     */
     ...
     * \internal
     *
     * Initializes self from the UTF-8 encoded string \a source whose length
     ...
     * \internal
     *
     * Returns \c true if the content associated with self must be copied
     * before it can be modified, otherwise returns \c false.
     */
     ...
     * contents of self. The caller is responsible for allocating and
     * freeing the buffer.
     *
     * \exception RWConversionErr
     * if an illegal or truncated surrogate pair is encountered during the
     * conversion.
     */
     ...
     * \internal
     *
     * Checks whether \a offset is greater than or equal to \a length. If
     * so, throws an RWBoundsErr exception.
     */
     ...
     * \internal
     *
     * \name Reference-Counted Buffer Management Methods
     *
     * These private methods are used to manage ownership and duration of
     * #RWUChar16 arrays. The life span of an array is governed by a
     * reference count. An array is deleted when it is no longer referenced
     * by any RWBasicUString instances. The Counter class provides the
     * necessary reference counting and deallocation functions.
     */
     ...
     * \internal
     *
     * Decrement the reference count of, and unlink counter_ and increment
     * the reference count of, and link \a counter.
     */
     ...
     * This member always references the array data regardless of the
     * storage strategy. This member may reference:
     ...
     * referenced by self, or zero if array_ references the local array or
     ...
     * heap allocation.
     *
     * This value must be divisible by 4 and 8 to ensure that the overall
     * size of an RWBasicUString is evenly divisible by the word size of the
     * underlying architecture. An RWBasicUString requires 32 bytes on a
     * 32-bit machine and 40 bytes on a 64-bit machine.
     */
     ...
     * \internal
     *
     * Local array used for short strings. Copies of short strings are
     * stored within an instance to avoid costly heap allocations.
     */
     ...
     * \internal
     *
     * The length of the code unit sequence referenced by array_. This
     * member is not used when the sequence is reference-counted.
     */
    size_t length_;
     ...
     * any terminating null, or zero if the client array is static. This
     * member is not used when the current contents are stored in the local
     * array or in referenced-counted storage.
     */
    size_t capacity_;
...
/**
 * \brief
 * Base class for subclasses that release client-allocated storage.
 *
 * Deallocator acts as an abstract base class for subclasses that release
 * client-allocated storage.
 *
 * Deallocator objects are required because RWBasicUString cannot assume
 * that local invocation of methods such as <tt>operator delete[]</tt> or
 * \b std::free() manipulate the same heap as that of the client. Such a
 * case might arise if, for instance, an application contains some libraries
 * that use debug versions of heap management routines while others do not.
 *
 * A Deallocator subclass must override both doDeallocate() methods to avoid
 * the ambiguities produced should the subclass declaration "hide" one of
 * the superclass declarations of these functions.
 */
...
public:
    ...
    /**
     * Forwards a deallocation request to a subclass-defined non-\c const
     * deallocation method.
     */
...
protected:
    ...
    /**
     * A virtual function that must be implemented in each subclass. A
     * subclass implementation should take whatever actions are required to
     * deallocate the storage identified by \a p. The default implementation
     * of this function simply forwards to the \c const version.
     */
    virtual void doDeallocate(RWUChar16* p);

    /**
     * A pure virtual function that must be implemented in each subclass. A
     * subclass implementation should take whatever actions are required to
     * deallocate the storage identified by \a p.
     */
 ...
 *
 * StaticDeallocator forwards deallocation requests to a simple,
 * client-defined, static callback method of the form:
 *
 * \code
...
public:
    ...
    /**
     * Constructs and initializes an instance with a pointer to a
     * client-supplied deallocation function. The \a destroyAfterDeallocate
     * is used to control the life-span of a StaticDeallocator instance. A
     * value of \c true causes the StaticDeallocator to delete itself
 ...
 *
 * This class:
 * - encapsulates the array of RWUChar16 instances that store the UTF-16
 * data
...
public:
    ...
    /**
     * Creates a new Counter instance referencing an external RWUChar16
     * array \a source of length \a size and capacity \a capacity. Uses
     * \a deallocator to delete the array pointed to by \a source once the
...
protected:
    ...
    /**
     * Constructs a Counter instance that assumes ownership of \a source, a
     * buffer of #RWUChar16 values that begins with a null-terminated string
     * of \a length elements and a capacity of \a capacity.
     *
     * The \a deallocator parameter is used to supply the Counter instance
     * with a Deallocator object that can be used to deallocate the storage
     * referenced by \a source.
     */
    Counter(RWUChar16* source, size_t length, size_t capacity, RWBasicUString::Deallocator* deallocator);

    /**
     * Destroys this instance and deallocates the storage used for the
     * #RWUChar16 array.
     */
    ~Counter();
...
public:
    ...
    /**
     * Returns the current reference count. This method should only be used
     * to test for sole-ownership, i.e., a reference count of 1.
     */
    ...
    /**
     * Decrements the reference count and deletes this instance if the
     * result is zero.
     */
     ...
     * otherwise returns \c false.
     *
     * This method requires constant time, while <tt>length() == 0</tt> may
     * require linear time.
     */
    inline bool empty() const;
    ...
    /**
     * Returns the total number of UTF-16 code units that may be stored in
     * self. This is the value specified during construction.
     */
 ...
 * returns \c false. If the two arrays are identical over the length of the
 * shortest array, then the shorter array is considered less than the longer
 * array. The \a lhs array must be null-terminated. This method cannot be
 * used to compare arrays containing embedded nulls. Use
 * RWBasicUString::compareCodeUnits() in that case.
 */
inline bool
 ...
 * otherwise returns \c false. If the two arrays are identical over the
 * length of the shortest array, then the shorter array is considered less
 * than the longer array. The \a lhs array must be null-terminated. This
 * method cannot be used to compare arrays containing embedded nulls. Use
 * RWBasicUString::compareCodeUnits() in that case.
 */
inline bool
operator<=(const RWUChar16* lhs, const RWBasicUString& rhs)
 ...
 * \relates RWBasicUString
 *
 * Returns \c true if \a lhs has the same length and is lexically identical
 * to \a rhs, otherwise returns \c false. The \a lhs array must be
 * null-terminated. This method cannot be used to compare arrays containing
 * embedded nulls. Use RWBasicUString::compareCodeUnits() in that case.
 */
inline bool
operator==(const RWUChar16* lhs, const RWBasicUString& rhs)
{
 ...
 * differs from \a rhs, otherwise returns \c false. The \a lhs array must be
 * null-terminated. This method cannot be used to compare arrays containing
 * embedded nulls. Use RWBasicUString::compareCodeUnits() in that case. This
 * operator represents the logical negation of
 ...
 * otherwise returns \c false. If the two arrays are identical over the
 * length of the shortest array, then the longer array is considered greater
 * than the shorter array. The \a lhs array must be null-terminated. This
 * method cannot be used to compare arrays containing embedded nulls. Use
 * RWBasicUString::compareCodeUnits() in that case.
 */
inline bool
 ...
 * returns \c false. If the two arrays are identical over the length of the
 * shortest array, then the longer array is considered greater than the
 * shorter array. The \a lhs array must be null-terminated. This method
 * cannot be used to compare arrays containing embedded nulls. Use
 * RWBasicUString::compareCodeUnits() in that case.
 */
inline bool
operator>(const RWUChar16* lhs, const RWBasicUString& rhs)
 ...
 * returns \c false. If the two arrays are identical over the length of the
 * shortest array, then the shorter array is considered less than the longer
 * array. The \a rhs array must be null-terminated. This method cannot be
 * used to compare arrays containing embedded nulls. Use
 * RWBasicUString::compareCodeUnits() in that case.
 */
inline bool
 ...
 * otherwise returns \c false. If the two arrays are identical over the
 * length of the shortest array, then the shorter array is considered less
 * than the longer array. The \a rhs array must be null-terminated. This
 * method cannot be used to compare arrays containing embedded nulls. Use
 * RWBasicUString::compareCodeUnits() in that case.
 */
inline bool
 ...
 *
 * Returns \c true if \a lhs has the same length and is lexically identical
 * to \a rhs, otherwise returns \c false. The \a rhs array must be
 * null-terminated. This method cannot be used to compare arrays containing
 * embedded nulls. Use RWBasicUString::compareCodeUnits() in that case.
 */
inline bool
 ...
 *
 * Returns \c true if \a lhs does not have the same length as, or lexically
 * differs from \a rhs, otherwise returns \c false. The \a rhs array must be
 * null-terminated. This method cannot be used to compare arrays containing
 * embedded nulls. Use RWBasicUString::compareCodeUnits() in that case. This
 * operator represents the logical negation of
 ...
 * otherwise returns \c false. If the two arrays are identical over the
 * length of the shortest array, then the longer array is considered greater
 * than the shorter array. The \a rhs array must be null-terminated. This
 * method cannot be used to compare arrays containing embedded nulls. Use
 * RWBasicUString::compareCodeUnits() in that case.
 */
inline bool
 ...
 * returns \c false. If the two arrays are identical over the length of the
 * shortest array, then the longer array is considered greater than the
 * shorter array. The \a rhs array must be null-terminated. This method
 * cannot be used to compare arrays containing embedded nulls. Use
 * RWBasicUString::compareCodeUnits() in that case.
 */
inline bool
 ...
 *
 * Returns \c true if \a lhs has the same length and is lexically identical
 * to \a rhs, otherwise returns \c false.
 */
inline bool
operator==(const RWBasicUString& lhs, const RWBasicUString& rhs)
{
 ...
 * operator==(const RWBasicUString&,const RWBasicUString&).
 */
inline bool
operator!=(const RWBasicUString& lhs, const RWBasicUString& rhs)
{
 ...
 * array. The \a lhs array must be null-terminated. This method cannot be
 * used to compare arrays containing embedded nulls. Use
 * RWBasicUString::compareCodePoints() in that case.
 */
inline bool
 ...
 * to \a rhs, otherwise returns \c false. The \a lhs array must be
 * null-terminated. This method cannot be used to compare arrays containing
 * embedded nulls. Use RWBasicUString::compareCodePoints() in that case.
 */
inline bool
operator==(const RWUChar32* lhs, const RWBasicUString& rhs)
{
 ...
 * null-terminated. This method cannot be used to compare arrays containing
 * embedded nulls. Use RWBasicUString::compareCodePoints() in that case.
 * This operator represents the logical negation of
 ...
 * than the shorter array. The \a rhs array must be null-terminated. This
 * method cannot be used to compare arrays containing embedded nulls. Use
 * RWBasicUString::compareCodePoints() in that case. This operator
...
/**
 * \brief
 * Inserts the contents of a Unicode string into an output stream, padding
 * it with a designated fill character.
 *
 * Defines an iostream manipulator that can be used to insert the contents
 * of a Unicode string into an output stream, padding the Unicode string
 * with a specified fill character until the entire \c width() of the output
 * stream has been filled with code points.
 *
 * If the length of the Unicode string is greater than the \c width() of the
 * output stream, or if the \c width() of the output stream is zero, the
 * entire contents of the Unicode string is inserted into the stream and no
 * padding occurs.
 *
 * A Pad instance is only valid as long as the source string remains
 * unchanged. Do not create persistent instances of this class. It should
 * only be instantiated as a temporary in an insertion expression.
 *
 * \code
 * RWBasicUString ustr = ...;
...
public:
    ...
    /**
     * Constructs a manipulator instance. The default fill character,
     * specified in the second parameter, is the Unicode space character.
     */
    ...
    /**
     * The code point used to pad the string. The default code point is the
 ...
 *
 * Writes the sequence of bytes that are produced when the contents of the
 * RWBasicUString used to construct \a pad are converted into a UTF-8
 * representation.
 *
 * If \c os.width() is greater than the number of code points contained in
 * the source string, the output is padded using the code point specified as
 * a parameter to \a pad. If \c os.width() is less than the number of code
 * points contained the source string, the entire contents of the string are
 * inserted into the output stream.
 *
 * Any padding is inserted after the string if the \c ios_base::left format
 * flag is set, or before if \c ios_base::right is set or neither flag is
 * set.
 *
 * \exception RWConversionErr
 * Thrown to report conversion errors.
 *
 * \exception std::ios_base::failure
 * Thrown to report any errors detected while performing stream operations.
 *
 * \sa
 * RWBasicUString::Pad
 */
 ...
 *
 * - if the stream \c width() is greater than zero, until width() code
 ...
 *
 * Unlike standard extractors, this extractor must consume and discard the
 * trailing whitespace delimiter. This behavior is due to the nature of the
 * UTF-8 encoding scheme, which requires multiple bytes to encode some
 * whitespace characters.
 *
 * For the same reason, this extractor cannot support the \c noskipws
 * condition where the \c ios_base::skipws format flag has been cleared
 * because each extraction would consume one whitespace character. A
 * whitespace manipulator is not provided either, since such a manipulator
 * would consume the first non-whitespace character following a sequence of
 * whitespace characters.
 *
 * \exception RWConversionErr
 * Thrown to report conversion errors.
 ...
 * Thrown to report any errors detected while performing stream operations.
 *
 * If the virtual stream or file experience an error while extracting the
 * string, the operator returns the original string contents unmodified.
 * Check the virtual stream or file to determine if an error occurred on
 * extraction.
 */
//@{
 ...
 * \relates RWBasicUString
 *
 * Converts the contents of \a ustr into a UTF-8 sequence and writes that
 * sequence to the output stream. The output sequence may contain embedded
 * nulls.
 *
 * If the \c width() of the output stream is greater than the number of code
 * points contained in the source string, the output is padded using a
 * single space fill character, \c [U+0020]. If the \c width() is less than
 * the number of code points contained in the source string, the entire
 * contents of the string is inserted into the output stream. Any padding is
 * inserted after the string if the \c ios_base::left format flag is set, or
 * before if \c ios_base::right is set or neither flag is set.
 *
 * \exception RWConversionErr
 * Thrown to report conversion errors.
 *
