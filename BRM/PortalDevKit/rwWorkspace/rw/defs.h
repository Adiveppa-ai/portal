 ...
 *
 * #RWUninitialized is an enumeration type with one value, #rwUninitialized.
 * It is used in various places in SourcePro products to indicate an
 * uninitialized value.
 */
...
#else
// Synchronizes access by all objects holding the same mutex
#  define RW_MT_GUARD(mutex)             RW_UNUSED(mutex)
#  define RW_MT_LOCK_GUARD(type, mutex)  RW_UNUSED(mutex)
#endif
 ...
 *
 * A global function used by the select() method in RWCollection classes to
 * test an RWCollectable object for a user defined property. A
 * testCollectable function has prototype:
 *
 * \code
 ...
 *
 * A global function used by the select() method in RWCollection classes to
 * test a pair of RWCollectable objects for a user defined property. A
 * testCollectablePair function has prototype:
 *
 * \code
 ...
 * \relates RWCollection
 *
 * A global function used by the apply() method in RWCollection classes to
 * apply a user defined action to an RWCollectable object. A
 * applyCollectable function has prototype:
 *
 * \code
...
// Intel C++ 16 for Windows improperly optimizes memcpy_s in a way that can
// result in application crashes. We avoid using secure functions for release
// builds on this platform.
#if !defined(RW_DEBUG) && defined(_MSC_VER) && (__INTEL_COMPILER == 1600)
#  undef RW_SECURE_CHECK
#endif
