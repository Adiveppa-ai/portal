 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tphasht.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * A pointer-based collection of values stored according to a hash object,
 * which allows duplicate values.
 *
 * This class maintains a pointer-based collection of values, which are
 * stored according to a hash object of type \c H. Class \c T is the type
 * pointed to by the items in the collection. Class \c A is an allocator of
 * objects of class \c T. For more information about using custom
 * allocators, please see the <em>Essential Tools Module User's Guide</em>.
 *
 ...
 *
 * Equivalent elements within the collection are grouped together based on
 * an equality object of type \c EQ. \c EQ must ensure this grouping via a
 * function that takes two arguments convertible to type \c T and returns a
 * value of type \c bool.
 *
 * \note
 * Any two elements that are equivalent \e must hash to the same value.
 *
 * This class allows multiple items that are equivalent to each other. Use
 * RWTPtrHashSet if you need a class that does not allow equivalent values.
 *
 ...
 * \sa
 * \parblock
 * Class RWTPtrHashSet offers the same interface to a pointer-based
 * collection that does not allow multiple items that compare equal to each
 * other.
 *
 * Class \ref rw_hashmultiset "rw_hashmultiset<T*,hasher,key_equal,A>" is
...
public:
     ...
     * <tt>operator delete</tt> to destroy the elements pointed to by those
     * items. Do not use this method if multiple pointers to the same
     * element are stored.
     */
    ...
    /**
     * Returns \c true if there exists an element \c t in self that compares
     * equal to <tt>*\a a</tt>, otherwise returns \c false.
     */
    ...
    /**
     * Returns the number of elements \c t in self that compare equal to
     * <tt>*\a a</tt>.
     */
     ...
     * elements removed. Client data may be passed through parameter \a d.
     */
    size_type removeAll(bool(*fn)(const T*, void*), void* d);

    /**
     * Removes all elements \c t in self that compare equal to
     * <tt>*\a a</tt>. Returns the number of elements removed.
     */
    ...
    /**
     * Destructively performs a set-theoretic union operation on self and
     * \a rhs. Self is replaced by the result. Note the uppercase "U" in
     * \c Union to avoid conflict with the C++ reserved word.
     */
    void
    ...
    /**
     * Destructively performs a set-theoretic intersection of self and \a s,
     ...
     * on self and \a rhs. Self is replaced by the result. A symmetric
     * difference can be informally defined as
     * \f$(A \cup B) / (A \cap B)\f$.
     */
    void
    ...
    /**
     * Changes the capacity of self by creating a new hashed multi-set with
     * a capacity of \a sz. Copies every element of self into the new
     ...
     * the \c T and \c EQ objects. This method does not copy or destroy any
     * of the items swapped; rather, it swaps the underlying hash tables.
     */
    void
...
#  endif // !RW_NO_RVALUE_REFERENCES
     ...
     *
     * \note
     * If the value specified for \a sz is zero, the default bucket count is
     * used.
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Constructs a set by copying elements from the range [\a first,
     * \a last). The hash table representation used by this multi-set has
     * \a sz buckets, uses \a h as a hashing function, and \a eq to test for
     * equality between stored elements.
     *
     * \c InputIterator is an input iterator type that points to elements
     * that are convertible to #value_type objects.
     *
    ...
    /**
     * Creates an empty hashed multi-set that uses the hash object \a h and
     * has an initial hash table capacity of \a sz.
     *
     * \note
     * If the value specified for \a sz is zero, the default capacity is
     * used.
     */
 ...
 * is a subset of the other. Elements are dereferenced before being
 * compared.
 */
 ...
 * \relates RWTPtrHashMultiSet
 *
 * Looks at the next element on the input stream \a strm and either creates
 * a new collection off the heap and sets \a p to point to it, or sets \a p
 * to point to a previously read instance. If a collection is created off
 * the heap, the application is responsible for deleting it.
 */
template <class T, class H, class EQ , class A>
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrHashMultiSet::iterator to iterate over
 * the elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTPtrHashMultiSet.
 *
 * RWTPtrHashMultiSetIterator provides an iterator interface to the C++
 * Standard Library-based collection represented by RWTPtrHashMultiSet. (An
 * STL iterator interface is provided through
 * RWTPtrHashMultiSet::iterator.)
 *
 * Iteration over an RWTPtrHashMultiSet is pseudo-random and dependent on
 ...
 * all elements that are defined to be equivalent by the equivalence object,
 * \c EQ, will remain adjacent.
 *
 * The current item referenced by this iterator is undefined after
 * construction or after a call to reset() operation. The iterator becomes
...
public:
     ...
     * references the last association in the multiset, self references an
     ...
     * undefined if self is not referencing an element within the multiset.
     */
    T* key() const
    {
        return *iter_;
    ...
    /**
     * Resets the iterator so that after being advanced it references the
     * first element of the collection \a t.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrHashMultiSet::const_iterator to iterate
 * over the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTPtrHashMultiSet.
 *
 * RWTPtrHashMultiSetConstIterator provides a \c const iterator interface to
 * the C++ Standard Library-based collection represented by
 * RWTPtrHashMultiSet. This iterator prohibits changes to the collection
...
public:
     ...
     * just created, self references the first element. If used when self
     * references the last association in the multi-set, self references an
     * undefined value and the method returns a value equivalent to
     * \c false. Otherwise, the method returns a value equivalent to
     * \c true.
     */
    ...
    /**
     * Returns the value currently referenced by self. The return value is
     * undefined if self is not referencing an association within the
     * multiset.
     */
    const T* key() const
    {
