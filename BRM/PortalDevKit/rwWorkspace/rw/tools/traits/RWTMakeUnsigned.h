 ...
 *
 * $Id: //spro/rel2016.1/rw/tools/traits/RWTMakeUnsigned.h#1 $
 *
 ***************************************************************************/
 ...
 *
 * \brief
 * Specifies an \c unsigned type of equivalent rank
 *
 * Specifies a typedef \c type that is an \c unsigned type of equivalent
 * rank to \c T. \c T must be an integral type other than \c bool.
 *
 * \note
 * This class differs from std::make_unsigned in that it does not support
 * enum types.
 *
 * \example
 * \code
 ...
 */
template <typename T>
struct RWTMakeUnsigned : public RWTMakeUnsignedImp<T> {
#if defined(DOXYGEN)
    /**
     * Specifies a typedef to an \c unsigned_type with the same rank as
     * \c T.
     */
