 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/slistcol.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Represents a group of ordered elements without keyed access, where
 ...
 * stored by RWSlistCollectables must inherit from abstract base class
 * RWCollectable.
 *
 * You must use the virtual function RWCollectable::isEqual() to find a
 * match between a target and an item in the collection.
 *
 * Class RWSlistCollectables is implemented as a singly-linked list, which
 * allows for efficient insertion and removal, but efficient movement in
 * only one direction. This class corresponds to the Smalltalk class
 * \b \e LinkedList.
 *
 * \synopsis
 * // Smalltalk typedef:
...
public:
    ...
    /**
     * Returns \c __RWSLISTCOLLECTABLES, i.e. a <em>class identifier</em>,
...
#  if !defined(RW_NO_RVALUE_REFERENCES)
    /**
     * Move constructor. The constructed RWSlistCollectables takes ownership
    ...
    /**
     * Returns \c true if self and \a s have the same number of members and
     * if, for every item in self, the corresponding item at the same index
     * in \a s isEqual() to it.
     */
    ...
    /**
     * The index \a i must be between \c 0 and the number of items in the
     * collection less one, or an exception of type RWBoundsErr is thrown.
     *
     * \note
    ...
    /**
     * Returns the first item that matches \a target, or #rwnil if no item
     * is found.
     */
     ...
     * \a c. If there is no such item, returns #RW_NPOS.
     */
    ...
    /**
     * Returns \c true if the list contains an item that <em>is identical
 ...
 * the first to last item.
 *
 * Iterator for class RWSlistCollectables. Traverses the linked-list from
 * the first to last item.
 *
 * The "current item" is undefined immediately after construction. You must
 * define it by using operator()() or some other (valid) operation.
 *
 * Once the iterator has advanced beyond the end of the collection, it is no
 * longer valid; continuing to use it will result in undefined behavior.
 *
 * \synopsis
 * // Smalltalk typedef.
...
public:
    ...
    /**
     * Returns \c true if the iterator is at the end of the list, otherwise
     * returns \c false;
     */
    bool atLast() const
    {
    ...
    /**
     * Moves iterator to the next item that
     * \ref RWCollectable::isEqual() "isEqual()" to the item pointed to by
     * \a target and returns it. If no item is found, returns #rwnil and the
    ...
    /**
     * Resets the iterator. Afterwards, the position of the iterator will be
     * undefined.
     */
    virtual void reset()
    {
    ...
    /**
     * Moves iterator to the next item which <em>is identical to</em> the
     * item pointed to by \a e (that is, that has address \a e) and returns
     * it. If no item is found, returns #rwnil and the position of the
    ...
    /**
     * Insert item \a a after the current cursor position and return the
     * item. The cursor's position will be unchanged.
     */
     ...
     * Afterwards, the iterator will be positioned at the previous item in
     * the list. This function is not very efficient in a singly-linked
     * list.
     */
    ...
    /**
     * Moves iterator to the next item in the list that
     * \ref RWCollectable::isEqual() "isEqual()" to the item pointed to by
     * \a target, removes it from the list and returns it. Afterwards, the
     * iterator will be positioned at the previous item in the list. If no
     * item is found, returns #rwnil and the position of the iterator will
     * be undefined.
     */
     ...
     * removes it from the list and returns it. Afterwards, the iterator
     * will be positioned at the previous item in the list. If no item is
     * found, returns #rwnil and the position of the iterator will be
     * undefined.
     */
 ...
 *
 * \brief
 * Const iterator for class RWSlistCollectables that traverses the
 * linked-list from the first to last item.
 *
 * One of two iterators for class RWSlistCollectables. This version is a
 * \c const iterator, that cannot effect any changes to the data structure
 * it is iterating through. This iterator traverses the linked-list from the
 * first to last item.
 *
 * The "current item" is undefined immediately after construction -- you
 * must define it by using operator()() or some other (valid) operation.
 *
 * Once the iterator has advanced beyond the end of the collection it is no
 * longer valid -- continuing to use it will bring undefined results.
 *
 * \synopsis
 * #include <rw/slistcol.h>
 * RWSlistCollectables sc;
...
public:
    ...
    /**
     * Constructs a \c const iterator from an iterator. Immediately after
    ...
    /**
     * Resets the iterator, after which the position of the iterator is
     * undefined.
     */
    virtual void reset()
    {
