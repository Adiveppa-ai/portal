 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tidlist.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * An intrusive list requires that all members of the list inherit from a
 * common base class, in this case RWIsvDlink. The advantage of such a list
 * is that memory and space requirements are minimized. The disadvantage is
 * that the inheritance hierarchy is inflexible, possibly complicating its
 * use with an existing class. Class RWTValDlist is offered as an
 ...
 *
 * \note
 * When you insert an item into an intrusive list, the <em>actual item</em>
 * (not a copy) is inserted. Because each item carries only one link field,
 * the same item cannot be inserted into more than one list, nor can it be
 * inserted into the same list more than once. This class is not
 * CopyConstructible or CopyAssignable but is MoveConstructible and
 * MoveAssignable.
 *
 * \synopsis
...
#  if !defined(RW_NO_RVALUE_REFERENCES)
    /**
     * Move constructor. The constructed RWTIsvDlist takes ownership of the
     * data owned by \a lst.
     *
     * \conditional
...
#  endif
    ...
    /**
     * Returns \c true if there are no items in the list, \c false
     * otherwise.
     */
    bool          isEmpty() const
    {
     ...
     * collection. Client data may be passed through as parameter \a d.
     */
     ...
     * Thrown if the index \a i is not between zero and the number of items
     * in the collection less one.
     */
    ...
    /**
     * Removes <em>and calls delete</em> for each item in the list. Note
     * that this assumes that each item was allocated off the heap.
     */
    void          clearAndDestroy();

    /**
     * Returns \c true if the list contains an item for which the
     * user-defined function pointed to by \a testFun returns \c true.
     *
     * For each item in the list, this function is called with the item as
     * the first argument. Client data may be passed through as parameter
     * \a d.
     */
    ...
    /**
     * Returns \c true if the list contains an item with the address \a a.
     */
     ...
     * function pointed to by \a testFun returns \c true. If there is no
     * such item, then returns #rwnil.
     *
     * For each item in the list, this function is called with the item as
     * the first argument. Client data may be passed through as parameter
     * \a d.
    ...
    /**
     * Returns <em>and removes</em> the first item in the list, or #rwnil if
     * the list is empty.
     */
    TL*           get()
    {
     ...
     * user-defined function pointed to by \a testFun returns \c true. If
     * there is no such item, then returns #RW_NPOS.
     *
     * For each item in the list, this function is called with the item as
     * the first argument. Client data may be passed through as parameter
     * \a d.
    ...
    /**
     * Appends the item pointed to by \a a to the end of the list. This item
    ...
    /**
     * Inserts the item pointed to by \a a at the index position \a i. The
     * item cannot be inserted into more than one list, nor can it be
    ...
    /**
     * Returns (but does not remove) the last item in the list, or #rwnil if
     * the list is empty.
     */
    TL*           last() const
    {
    ...
    /**
     * Returns the number of times which the item pointed to by \a a occurs
     * in the list. Because items cannot be inserted into a list more than
    ...
    /**
     * Prepends the item pointed to by \a a to the beginning of the list.
     */
    void          prepend(TL* a)
    {
     ...
     * function pointed to by \a testFun returns \c true, or #rwnil if there
     * is no such item.
     *
     * For each item in the list, this function is called with the item as
     * the first argument. Client data may be passed through as parameter
    ...
    /**
     * Removes and returns the first item in the list, or #rwnil if there
    ...
    /**
     * Removes and returns the last item in the list, or #rwnil if there are
    ...
    /**
     * Removes and returns the item with address \a a, or #rwnil if there is
     * no such item.
     */
 ...
 *
 * \brief
 * Iterator for class RWTIsvDlist
 *
 * RWTIsvDlistIterator provides an iterator interface for class RWTIsvDlist,
 * allowing sequential access to all the elements of a doubly-linked
 * parameterized intrusive list. Elements are accessed in order, in either
 * direction.
 *
 ...
 * longer valid; its continued use will result in undefined behavior.
 *
 * \synopsis
 * #include <rw/tidlist.h>
 * RWTIsvDlist<T> list;
...
public:
    ...
    // Operators:
    /**
     * Advances the iterator one position, returning a pointer to the new
     * link, or #rwnil if the end of the list has been reached.
     */
    TL*   operator++()
    {
    ...
    /**
     * Reverses the iterator one position, returning a pointer to the new
     * link, or #rwnil if the beginning of the list has been reached.
     */
    TL*   operator--()
    {
        return (TL*)RWIsvDlistIterator::operator--();
    ...
    /**
     * Advances the iterator \a n positions, returning a pointer to the new
     * link, or #rwnil if the end of the list has been reached.
     */
    ...
    /**
     * Reverses the iterator \a n positions, returning a pointer to the new
     * link, or #rwnil if the beginning of the list has been reached.
     */
    TL*   operator-=(size_t n)
    {
     ...
     * pointed to by \a testFun returns \c true and returns it, or #rwnil if
    ...
    /**
     * Returns the link at the iterator's current position. Returns #rwnil
     ...
     * iterator will be positioned at the element immediately before the
     * removed link.
     */
    TL*           remove()
    {
    ...
    /**
     * Advances the iterator to the first link for which the tester function
     * pointed to by \a testFun returns \c true, removes and returns it.
     * Returns \c false if unsuccessful. If successful, the iterator will be
