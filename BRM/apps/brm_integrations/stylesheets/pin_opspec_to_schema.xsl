...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <xsd:schema xmlns:xsd="http://www.w3.org/2001/XMLSchema" 
      ...
      <xsl:comment>Copyright (c) 2007, 2018, Oracle and/or its affiliates. All rights reserved.
        ...
        <xsl:if test="(name() = 'Array' or name()= 'Substruct') ">
          <xsl:choose>
            <xsl:when test="@ClassRef">
              <xsl:variable name="poid_type_for_file_name">
                 <xsl:call-template name="get-schema-file">
                    <xsl:with-param name="input_string" select="@ClassRef"/>
                 </xsl:call-template>
              </xsl:variable>
  ...
  <xsl:template name="sort">
    ...
    <xsl:for-each select="*">
      ...
      <xsl:choose>
        ...
        <xsl:when test="(name() = 'Int') or (name() = 'String') or (name() = 'Decimal') or (name() = 'Timestamp')">
...
$poid_place_holder,')((/)*([a-zA-Z_]*))*))(\s)+[0-9]+(\s)+[0-9]*')"/>
                ...
                <xsl:when test="../@mode='completeWithRev'">
                  <xsl:value-of select="concat('([0-9]*\.)+[0-9]*(\s)+(((/)*|(/)(', 
$poid_place_holder,')((/)*([a-zA-Z_]*))*))(\s)+[0-9]+(\s)+[0-9]+')"/>
                </xsl:when>
                <xsl:when test="../@mode='dbonly'">
                  <xsl:value-of select="'([0-9]*\.)+[0-9]*(\s)+(((/)*|(/)([a-zA-Z_]*)((/)*([a-zA-Z_]*))*))(\s)+[\-]*[0-9]+(\s)+[0-9]*'"/>
                </xsl:when>
                <xsl:when test="../@mode='typeonly'">
                  <xsl:value-of select="concat('([0-9]*\.)+[0-9]*(\s)+(((/)*|(/)(',
$poid_place_holder,')((/)*([a-zA-Z_]*))*))(\s)+[\-]*[0-9]+(\s)+[0-9]*')"/>
                </xsl:when>
              </xsl:choose>
  ...
  <xsl:template name="process_int_string_decimal">
    ...
    <xsl:element name="{$element_name}">
      ...
      <xsl:if test="name()='Int' or name()='Decimal' or name()='Timestamp'">
      <xsl:attribute name="type">
        <xsl:value-of select="$data_type"/>
      </xsl:attribute>
      </xsl:if>
      ...
      <xsl:choose>
        <xsl:when test="./descendant::op:Fields">
          <xsd:complexType name="{$type}">
            <xsl:choose>
             <xsl:when test="@ClassRef">
               <xsl:variable name="objectbaseClass">
                <xsl:choose>
                        <xsl:when test="starts-with(@ClassRef,'/')">
                                <xsl:call-template name="string-replace-all">
                                        <xsl:with-param name="input_string" select="substring-after(@ClassRef,'/')" />
              ...
              <xsl:variable name="data_type" select="concat($objectbaseClass,'ExtensionType')"/>
              <xsd:complexContent>
                 <xsd:extension base="{$data_type}">
                    <xsd:choice minOccurs="0" maxOccurs="unbounded">
                      <xsl:for-each select="op:Fields">
            ...
            <xsl:choose>  
                <xsl:when test="@ClassRef">
                    <xsl:variable name="objectbaseClass">
                        <xsl:choose>
                            <xsl:when test="starts-with(@ClassRef,'/')">
                                <!--xsl:value-of select="substring-after(@abcd,'/')" /-->
                                <xsl:call-template name="string-replace-all">
                                        <xsl:with-param name="input_string" select="substring-after(@ClassRef,'/')" />
                                        <xsl:with-param name="from" select="'/'" />
                                        <xsl:with-param name="to" select="'_'" />
 ...
 <xsl:template name="string-replace-all">
    ...
    <xsl:param name="to" />
    <xsl:choose>
      <xsl:when test="contains($input_string, $from)">
        <xsl:value-of select="substring-before($input_string,$from)" />
        <xsl:value-of select="$to" />
        <xsl:call-template name="string-replace-all">
          <xsl:with-param name="input_string"
           select="substring-after($input_string,$from)" />
  ...
  <xsl:template name="get-schema-file">
    <xsl:param name="input_string" />
    <xsl:choose>
      <xsl:when test="starts-with($input_string,'/')">
        <xsl:variable name="poid_type_pattern" select="substring-after($input_string,'/')" />
          <xsl:variable name="place_holder_poid_type" >
            <xsl:call-template name="substring-before-if-present">
              <xsl:with-param name="input_string" select="$poid_type_pattern"/>
              <xsl:with-param name="pattern" select="'/'"/>
            </xsl:call-template>
          </xsl:variable>
          <xsl:value-of select="concat($place_holder_poid_type,'.xsd')"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:variable name="place_holder_poid_type" >
            <xsl:call-template name="substring-before-if-present">
  ...
  <xsl:template name="substring-before-if-present">
    <xsl:param name="input_string" />
    <xsl:param name="pattern" />
    <xsl:choose>
      <xsl:when test="contains($input_string, $pattern)" >
        <xsl:value-of select="substring-before($input_string, $pattern)" />
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="$input_string" />
      </xsl:otherwise>
