#ifndef __rw_itc_RWTPCValBufferBase_h__
#define __rw_itc_RWTPCValBufferBase_h__
/**************************************************************************
 *
 * $Id: //spro/rel2016.1/rw/itc/RWTPCValBufferBase.h#1 $
 *
...
#  define RWPCValBufferBase RWTPCValBufferBase
#endif
 ...
 * semantics for exchanging values between cooperating threads.
 *
 * RWTPCValBufferBase is the base class for the family of classes that
 * provides buffered producer-consumer synchronization semantics for
 * exchanging of values between cooperating threads.
 *
 ...
 */
template <class Type>
class RWTPCValBufferBase : public RWPCBufferBase
{
public:
...
public:
     ...
     * the function immediately returns by throwing an RWTHRClosedException.
     *
     ...
     *
     * This function takes two parameters. The \a result parameter is a
     ...
     * where the availability and ordering of values is determined by the
     * implementation supplied in derived classes. The returned value is of
     ...
     * is open and empty, this function blocks the calling thread until a
     * value becomes available, or until the buffer is closed, or until the
     ...
     * If the buffer is closed while the calling thread is blocked and
     * waiting within this function for room to write, the calling thread is
     * unblocked and the function return by throwing an
     * RWTHRClosedException.
     *
...
protected:
    ...
    RWTPCValBufferBase(size_t maxCapacity);

    RWTPCValBufferBase(size_t maxCapacity, bool isOpen);
...
template <class Type>
inline
RWTPCValBufferBase<Type>::RWTPCValBufferBase(size_t maxCapacity)
...
template <class Type>
inline
RWTPCValBufferBase<Type>::RWTPCValBufferBase(size_t maxCapacity, bool b)
    : RWPCBufferBase(maxCapacity, b)
{
}

#if defined(RW_NO_IMPLICIT_INCLUSION)
#  include <rw/itc/RWTPCValBufferBase.cc>
#endif

#endif // __rw_itc_RWTPCValBufferBase_h__
