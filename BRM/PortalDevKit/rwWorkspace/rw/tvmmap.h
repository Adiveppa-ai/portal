 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvmmap.h#1 $
 *
 **********************************************************************
 *
 ...
 * partner within the collection. The structure \c std::less<T> from the C++
 * Standard Library header file \c \<functional\> is an example.
 *
 * RWTValMultiMap may contain multiple keys that compare equal to each
 * other. (RWTValMap will not accept a key that compares equal to any key
 * already in the collection.) Equality is based on the comparison object
 * and not on the operator==. Given a comparison object \c comp, keys \c a
 ...
 * \sa
 * \parblock
 * Class RWTValMap offers the same interface to a collection that will not
 * accept multiple keys that compare equal to each other. RWTValMultiSet
 * maintains a collection of keys without the associated values.
 *
 * Class <b>std::multimap<K,T,C,A,std::pair<K const&,T> ></b> is the C++
...
public:
    ...
    /**
     * Removes the first association with key \c j in self where \c j
     ...
     * swaps the underlying hash * tables.
     */
    void
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValMultiMap::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTValMultiMap.
 *
 * RWTValMultiMapIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTValMultiMap. (An STL iterator
 * interface is provided through RWTValMultiMap::iterator.)
 *
...
public:
    /**
     * Creates an iterator for the multi-map \a m. The iterator begins in an
     ...
     * self now references an undefined value and \c false is returned.
     ...
     * \c false if the iterator has advanced past the last element in the
     * collection.
     */
    bool operator()()
    {
    ...
    K key() const
    {
        return (*(const_cast<RWTValMultiMapIterator*>(this)->iter_)).first;
    }
    ...
    T value() const
    {
        return (*(const_cast<RWTValMultiMapIterator*>(this)->iter_)).second;
    }
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValMultiMap::const_iterator to iterate over
 * the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValMultiMap.
 *
 * RWTValMultiMapConstIterator provides a \c const iterator for
 * RWTValMultiMap. A \c const iterator prohibits any changes to the
...
public:
    /**
     * Creates a \c const iterator for the multi-map \a m. The iterator
    ...
    /**
     * Returns the value portion of the association referenced by \c self.
     */
    const T& value() const
    {
        return (*iter_).second;
