 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tpmap.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * A pointer-based collection of associations ordered according to a
 * comparison object.
 *
 * This class maintains a pointer-based collection of associations of type
 * <b>std::pair<K* const, T*></b>. The first part of the association is a
 * key of type \c K*, the second is its associated item of type \c T*. Class
 * \c A is an allocator of objects of class \c T. For more information about
 * using custom allocators, please see the <em>Essential Tools Module User's
 * Guide</em>.
 *
 ...
 * \endcode
 *
 * which returns \c true if \c x and its partner should precede \c y and its
 * partner within the collection. The structure \b std::less<T> from the C++
 * Standard Library header file \c \<functional\> is an example. Note that
 * keys will be dereferenced before being compared.
 *
 * RWTPtrMap will not accept a key that compares equal to any key already in
 * the collection. (RWTPtrMultiMap may contain multiple keys that compare
 * equal to each other.) Equality is based on the comparison object and not
 * on the equality operator. Given a comparison object \c comp, keys \c a
 * and \c b are equal if <tt>!comp(a,b) && !comp(b,a)</tt>.
 *
 ...
 * \sa
 * \parblock
 * Class RWTPtrMultiMap offers the same interface to a pointer-based
 * collection that accepts multiple keys that compare equal to each other.
 * RWTPtrSet maintains a pointer-based collection of keys without the
 * associated items.
 *
 * Class <b>std::map<K*,T*,deref_compare<C,K,A>></b> is the C++ Standard
 * Library collection that serves as the underlying implementation for this
 * class.
 * \endparblock
 *
...
public:
    ...
    /**
     * A type representing a comparison function for #key_type that
    ...
    /**
     * A type representing a comparison function for #value_type that
     * dereferences the arguments before comparing.
     */
     ...
     *
     * \deprecated
     * As of SourcePro 12.5, use RWTPtrMap::value_compare instead.
     */
     ...
     * pointers to the same object are stored. (This could happen even if
     * keys all compare different, since items are not considered during
     * comparison.)
     */
    ...
    /**
     * If there exists a key \c j in self that compares equal to
     * <tt>*\a key</tt>, assigns the item associated with \c j to \c tr, and
    ...
    /**
     * Returns a pointer to the minimum key in the collection. Order is
     * determined according to the comparison object described by the
     * template parameter \c C.
     */
    ...
    /**
     * Returns a pointer to the maximum key in the collection. Order is
     * determined according to the comparison object described by the
     * template parameter \c C.
     */
    ...
    /**
     * Removes all associations with key \c j in self that compares equal to
     ...
     * the key is not in the dictionary, then it will be added with an
     * associated uninitialized pointer of type #mapped_type. Because of
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrMap::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTPtrMap.
 *
 * RWTPtrMapIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTPtrMap. (An STL iterator
 * interface is provided through RWTPtrMap::iterator.)
 *
...
public:
    /**
     * Creates an iterator for the map \a rwm. The iterator begins in an
     ...
     * self. Undefined if self is not referencing a value within the map.
     */
    K* key() const
    {
        return (*iter_).first;
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrMap::const_iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTPtrMap.
 *
 * RWTPtrMapIterator provides a \c const iterator interface to the C++
 * Standard Library-based RWTPtrMap collection. As a \c const iterator, it
 * prohibits any change to the collection upon which it iterates.
 *
 * The order of iteration over an RWTPtrMap is dependent on the comparator
 * object supplied as applied to the key values of the stored associations.
 *
 * The current item referenced by this iterator is undefined after
 * construction or after a call to reset(). The iterator becomes valid after
 * being advanced with either a preincrement or operator()().
 *
 * For both operator++() and operator()(), iterating past the last element
...
public:
     ...
     * iteration, self referenced the last association in the multimap, self
     * now points to an undefined value, and a value equivalent to \c false
     * is returned. Otherwise, a value equivalent to \c true is returned.
     */
    bool operator++()    // Advance and test
    {
        RW_ASSERT(!(head_ == true && tail_ == true));
