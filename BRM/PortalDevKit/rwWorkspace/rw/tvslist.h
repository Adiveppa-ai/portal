 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvslist.h#1 $
 *
 **********************************************************************
 *
 ...
 * singly-linked list. The \c value type must have \c operator== and
 * \c operator< defined. This requirement is imposed by the C++ Standard
 * Library.
 *
 ...
 * \sa
 * \parblock
 * Classes RWTValDeque, RWTValDlist, and RWTValOrderedVector also provide a
 * Rogue Wave interface to C++ Standard Library sequence collections.
 *
 * The Rogue Wave supplied, Standard Library-compliant class rw_slist is the
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Returns the position of the first item \c t in self such that (\c t
     * == \a a), or returns #RW_NPOS if no such item exists.
     */
     ...
     *
     * \exception RWBoundsErr
     * Thrown if this position is not between \c 0 and the number of entries
     * in the collection.
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES
     ...
     *
     * \exception RWBoundsErr
     * Thrown if the index \a i is not between \c 0 and one less than the
     ...
     * Returns the number of items replaced. Client data may be passed
     * through parameter \a d.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValSlist::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTValSlist.
 *
 * RWTValSlistIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTValSlist. (An STL iterator
 * interface is provided through RWTValSlist::iterator.)
 *
 * The order of iteration over an RWTValSlist is dependent on the order of
...
public:
     ...
     * references an undefined value distinct from the reset value, and the
     ...
     * \c true if an element was found and removed, otherwise returns
     * \c false.
     */
     ...
     * Client data may be passed through parameter \a d. Returns \c true if
     * an element was found and removed, otherwise returns \c false.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValSlist::const_iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValSlist.
 *
 * RWTValSlistConstIterator provides a \c const iterator interface for
 * RWTValSlist. A \c const iterator prohibits any changes to the collection
...
public:
    /**
     * Creates a \c const iterator for the singly linked list \a s. The
