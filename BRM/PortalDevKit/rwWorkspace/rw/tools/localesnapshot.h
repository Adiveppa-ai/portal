 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tools/localesnapshot.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Creates an snapshot of a locale by querying the program's environment to
 * determine the formatting conventions in effect at the moment of
 * instantiation
 *
 * The class RWLocaleSnapshot implements the RWLocale interface using C
 * Standard Library facilities. To use it, the program creates an
 * RWLocaleSnapshot instance. The constructor of the instance queries the
 * program's environment (using C Standard Library functions such as
 * \b localeconv(), \b strftime(), and, if available, vendor-specific
 * library functions) to determine the formatting conventions in effect
 * at the moment of instantiation. When done, the locale can then be
 * switched and another instance of RWLocaleSnapshot created.
 *
 * By creating multiple instances of RWLocaleSnapshot, your program can have
 * more than one locale active at the same time, something that is difficult
 * to do with the C Standard Library facilities.
 *
 * \note
 * RWLocaleSnapshot does not encapsulate character set, collation, or
 * message information.
 *
 * Class RWLocaleSnapshot contains a set of public data members initialized
 * by its constructor with information extracted from its execution
 * environment.
 *
 * For a description of the static members of the RWLocale interface that
 * operate on objects of type RWLocaleSnapshot, see the RWLocale class
 * description.
 *
 * \synopsis
 * #include <locale.h>
 ...
 * \example
 * \note
 * To ensure that the asString() methods in this example correctly format
 * the output, you must first install on your system any locale you request
 * before invoking RWLocaleSnapshot().
 *
 * \code
...
private:
    ...
    // time formats
    RWTScopedPointer<char[]> wday_;     // weekday strings
    RWTScopedPointer<char[]> wday_abb_; // weekday strings - abbreviated

    RWTScopedPointer<char[]> mon_;      // month strings
    RWTScopedPointer<char[]> mon_abb_;  // month strings - abbreviated

    RWTScopedPointer<char[]> am_pm_;    // am/pm strings

    RWTScopedPointer<char[]> zone_;     // zone string

    RWTScopedPointer<char[]> d_fmt_;    // date format
    RWTScopedPointer<char[]> t_fmt_;    // time format
    RWTScopedPointer<char[]> dt_fmt_;   // date time format

    RWTScopedPointer<char[]> c_fmt_;    // %c format

    int mo_digits_;

    // monetary formats
    const char* positive_;
    const char* negative_;

    // ctype
    RWTScopedPointer<char[]> lowercase_;
...
public:
    ...
    // Data - lconv members
    /**
     * Defined identically as the correspondingly-named member of the C++
     * Standard Library type \b std::lconv.
     */
     ...
     * - If the value is equal to \c CHAR_MAX, no further grouping is to be
     * performed.
     *
     * \example
     * \code
    ...
    // Constructor
    /**
     * Constructs an RWLocale object by extracting formats from the current
     * locale environment.
     */
    RWLocaleSnapshot();
     ...
     * locale environment. It uses the C++ Standard Library function
     * \b std::setlocale() to set the named locale, and then restores the
     * previous global locale after formats have been extracted. If
     * \a localeName is 0, it uses the current locale. The most useful
     * locale name is the empty string, "", which is a synonym for the
     ...
     *
     * \note
     * These virtual functions follow the interface described under class
     * RWLocale. They generally work by converting values to and from
     * strings using the rules specified by the \b std::lconv values (see
     * http://en.cppreference.com/w/cpp/locale/lconv) encapsulated in self.
     */
...
private:
    ...
    /**
     * \internal
     * Initialize the RWLocaleSnapshot based on the C locale \a locname.
     */
    void populate(const char* locname);

    // extracts all time, monetary and ctype formats
    int extractFormats();

    // not CopyConstructible
    RWLocaleSnapshot(const RWLocaleSnapshot&);

    // not Assignable
    void operator= (const RWLocaleSnapshot&);
};
