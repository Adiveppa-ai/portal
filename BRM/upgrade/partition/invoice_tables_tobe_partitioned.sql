--
-- @(#)$Id: invoice_tables_tobe_partitioned.sql /cgbubrm_7.5.0.portalbase/1 2015/11/27 06:09:21 nishahan Exp $
--
...
--
--
-- sql script to find the list of invoice tables to be partitioned.
--
...
DECLARE
	--
       	-- By default, pick up the tables that belong to the class 
       	--
	...
	AS table_name
        ...
        AND (do.name = '/invoice' OR do.name LIKE '/invoice/%')
      	...
      	EXCEPTION
      		WHEN no_data_found THEN
            		INSERT INTO tables_tobe_partitioned_t (table_name, base_name, object_name, partition_exchanged)
                  	VALUES (UPPER(partition_tables_rec.table_name), 'INVOICE_T', '/invoice', 0);
...
EXCEPTION
	WHEN OTHERS THEN
               	pin_upg_common.log_info (C_STEP_NAME, NULL, NULL, 'E', 
			'Error finding the invoice table names to be partitioned', sqlerrm);
END;
/
