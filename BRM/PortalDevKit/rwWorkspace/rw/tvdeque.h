 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvdeque.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Collection of values implemented as a double-ended queue, or \e deque.
 *
 * This class maintains a collection of values implemented as a double-ended
 * queue, or \e deque. Order is determined externally and elements are
 * accessible by index. Use this class when insertions and deletions usually
 * occur at either the beginning or the end of the collection. Class \c T is
 * the type of items in the collection. Class \c A is an allocator of
 ...
 * \sa
 * \parblock
 * Classes RWTValSlist, RWTValDlist, RWTValSortedDlist, and
 * RWTValOrderedVector also provide a Rogue Wave interface to C++ Standard
 * Library sequence collections. The list classes should be considered for
 * frequent insertions (or removals) in the interior of the collection.
 * Vectors may be more efficient if most insertions and removals occur at
...
public:
     ...
     * <tt>((*\a fn)(t,\a d))</tt> is \c true with \a newVal. Returns the
     * number of items replaced. Client data may be passed through parameter
     * \a d.
     */
...
#if !defined(RW_NO_RVALUE_REFERENCES)
    /**
     * Move constructor. The constructed deque takes ownership of the data
     * owned by \a deq.
     *
     * \conditional
     * This method is only available on platforms with rvalue reference
