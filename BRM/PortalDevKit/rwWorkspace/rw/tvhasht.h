 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvhasht.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Maintains a collection of values stored according to a hash object, and
 * with multiple equivalent values allowed.
 *
 ...
 * \sa
 * \parblock
 * Class RWTValHashSet offers the same interface to a collection that does
 * \b not accept multiple items that compare equal to each other.
 *
 * Class \ref rw_hashmultiset "rw_hashmultiset<T,H,EQ,A>" is the C++
 * Standard Library collection class that serves as the underlying
 * implementation for RWTValHashMultiSet.
 * \endparblock
 *
...
public:
    ...
    /**
     * Clears the collection by removing all items from self. Each item has
     ...
     * equal to \a a, otherwise returns \c false.
     */
    ...
    /**
     * Removes the first element \c t in self that compares equal to \a a
     * and returns \c true. If there is no such element, returns \c false.
     */
    ...
    /**
     * Removes all elements \c t in self that compare equal to \a a. Returns
     ...
     * \a rhs. Self is replaced by the result. Note the uppercase \c 'U' in
    ...
    /**
     * Returns \c true if self is a subset of \a s, otherwise returns
     * \c false.
     */
    bool
...
#  endif // !RW_NO_RVALUE_REFERENCES
     ...
     * a capacity of \a sz. resize() copies every element of self into the
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Constructs an empty set. The underlying hash table representation has
     * \a sz buckets, uses \a h as its hashing function, and uses \a eq to
...
#  endif // !RW_NO_RVALUE_REFERENCES
     ...
     * uses \a h as its hashing function, and uses \a eq to determine
 ...
 * \relates RWTValHashMultiSet
 *
 * Saves the collection \a coll to the RWFile \a file, or a reference to the
 * file if the collection has already been saved.
 */
template <class T, class H, class EQ, class A>
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValHashMultiSet::iterator to iterate over
 * the elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTValHashMultiSet.
 *
 * RWTValHashMultiSetIterator provides an iterator interface to the C++
 * Standard Library-based collection represented by RWTValHashMultiSet. (An
 * STL iterator interface is provided through RWTValHashMultiSet::iterator.)
 *
 ...
 * elements defined to be equivalent by the equivalence object, \c EQ, will
 * be adjacent.
 *
...
public:
    /**
     * Creates an iterator for the hashed multiset \a h. The iterator begins
     ...
     * iteration, self referenced the last value in the multiset, self now
     * references an undefined value and the operator returns \c false,
     ...
     * has advanced past the last element in the container, otherwise
     * returns \c true.
     */
    bool operator()()
    {
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValHashMultiSet::const_iterator to iterate
 * over the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValHashMultiSet.
 *
 * RWTValHashMultiSetConstIterator provides a \c const iterator interface to
 * the C++ Standard Library-based collection represented by
 * RWTValHashMultiSet. A \c const iterator prohibits any changes to the
...
public:
    /**
     * Creates a \c const iterator for the hashed multiset \a h. The
     ...
     * just created, self now references the first element. If, before
     * iteration, self referenced the last value in the multiset, self now
     * references an undefined value and returns \c false, otherwise,
     * returns \c true.
     */
    bool operator++()    // Advance and test
    {
    ...
    /**
     * Advances self to the next element. Returns \c false if the iterator
     * has advanced past the last item in the container, otherwise returns
     * \c true.
     */
    bool operator()()
    {
