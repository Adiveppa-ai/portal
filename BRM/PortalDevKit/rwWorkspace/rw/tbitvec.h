 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tbitvec.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * RWTBitVec is a parameterized bit vector of fixed length \c Size. Unlike
 * class RWBitVec, its length cannot be changed at runtime. The advantage of
 * RWTBitVec is its smaller size, and one less level of indirection,
 * resulting in a slight speed advantage.
 *
 * Bits are numbered from \c 0 through \c Size-1, inclusive.
 *
 * The copy constructor and assignment operator use <em>copy</em> semantics.
 *
 * \synopsis
...
public:
    ...
    /**
     * Logical assignments. Sets each bit of self to the logical \c AND of
     * self and the corresponding bit in \a v.
     */
    RWTBitVec<N>&         operator&=(const RWTBitVec<N>& v)
    {
    ...
    /**
     * Logical assignments. Sets each bit of self to the logical \c XOR of
     * self and the corresponding bit in \a v.
     */
    RWTBitVec<N>&         operator^=(const RWTBitVec<N>& v)
    {
    ...
    /**
     * Logical assignments. Sets each bit of self to the logical \c OR of
     * self and the corresponding bit in \a v.
     */
    RWTBitVec<N>&         operator|=(const RWTBitVec<N>& v)
    {
     ...
     * reference can be used as an lvalue.
     *
     * \exception RWBoundsErr
     * thrown if index \a i is not between \c 0 and \c Size-1, inclusive.
     */
    ...
    /**
     * Returns the value of the \a i <sup>th</sup> bit of self. Bounds
     * checking is performed.
     *
     * \exception RWBoundsErr
     * is thrown if index \a i is not between \c 0 and \c Size-1, inclusive.
     */
    ...
    /**
     * Returns \c true if each bit of self is set to the same value as the
     * corresponding bit in \a v. Otherwise, returns \c false.
     */
    ...
    /**
     * Returns \c true if any bit of self is not set to the same value as
     * the corresponding bit in \a v. Otherwise, returns \c false.
     */
    bool             operator!=(const RWTBitVec<N>& v) const
    {
        return !(operator==(v));
     ...
     * performed. The following two lines are equivalent, although using
     ...
     * The following two lines are equivalent, although using setBit(size_t)
     ...
     * equivalent, although using
     * \ref testBit(size_t) const "testBit(size_t)" is slightly smaller and
