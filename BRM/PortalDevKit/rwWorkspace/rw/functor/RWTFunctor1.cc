/**************************************************************************
 *
 * $Id: //spro/rel2016.1/rw/functor/RWTFunctor1.cc#1 $
 *
...
RWTFunctor1<S1>::RWTFunctor1(RWStaticCtor)
    : RWTFunctor<void(S1)>(RW_STATIC_CTOR)
{
}

template <class S1>
RWTFunctor1<S1>::RWTFunctor1(RWTFunctor1Imp<S1>* functorImp)
    : RWTFunctor<void(S1)>((RWTFunctorImpBase1<void, S1>*)functorImp)
{
}

template <class S1>
RWTFunctor1<S1>::RWTFunctor1(const RWTFunctor1<S1>& second)
    : RWTFunctor<void(S1)>(second)
{
}
...
{
    if (&second != this) {
        RWTFunctor<void(S1)>::operator=(second);
    }
    return *this;
}
