 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/rwbag.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Corresponds to the Smalltalk class Bag, representing a group of unordered
 * elements not accessible by an external key.
 *
 * Class RWBag corresponds to the Smalltalk class Bag. It represents a group
 * of unordered elements, not accessible by an external key. Duplicates are
 * allowed.
 *
 * An object stored by RWBag must inherit abstract base class RWCollectable,
 * with suitable definition for virtual functions RWCollectable::hash() and
 * RWCollectable::isEqual(). The function hash() is used to find objects
 * with the same hash value, then isEqual() is used to confirm the match.
 *
 * Class RWBag is implemented by using an internal hashed dictionary
 * (RWHashDictionary) which keeps track of the number of occurrences of an
 * item. If an item is added to the collection that compares equal
 * (isEqual()) to an existing item in the collection, then the count is
 * incremented. Note that this means that only the first instance of a value
 * is actually inserted: subsequent instances cause the occurrence count to
 * be incremented. This behavior parallels the Smalltalk implementation of
 * Bag.
 *
 * Member function apply() and the iterator are called repeatedly according
 * to the count for an item.
 *
 * See class RWHashTable if you want duplicates to be stored, rather than
 * merely counted.
 *
 * \synopsis
...
public:
    ...
    /**
     * Constructs an empty bag with \a n buckets.
     */
    RWBag(size_t n);
    ...
    /**
     * Move constructor. The constructed RWBag takes ownership of the data
     * owned by \a b.
     *
     * \conditional
    ...
    /**
     * Returns \c true if self and bag \a b have the same number of total
     * entries and if for every key in self there is a corresponding key in
     * \a b which isEqual() and which has the same number of entries.
     */
    bool operator== (const RWBag& b) const;

    /**
     * Invokes the function pointer \a ap on each item in the collection in
     * a generally unpredictable order. If an item has been inserted more
     * than once (i.e., more than one item isEqual()), then apply() will be
     * called that many times. The function pointed to by \a ap should take
     * no action that could change the hash value or the meaning of
     * isEqual() for the items.
     */
    ...
    /**
     * The first item that was inserted into the \c Bag and which equals
     * \a target is returned, or #rwnil if no item is found. Hashing is used
     * to narrow the search.
     */
    virtual RWCollectable* find(const RWCollectable* target) const;

    /**
     * Inserts the item \a c into the collection and returns it, or if an
     * item was already in the collection that isEqual() to \a c, returns
     * the old item and increments its count.
     */
    ...
    /**
     * Returns the number of items that <em>are equal to</em> the item
     * pointed to by \a target.
     */
    virtual size_t occurrencesOf(const RWCollectable* target) const;

    /**
     * Removes and returns the item that isEqual() to the item pointed to by
     * \a target. Returns #rwnil if no item was found.
     */
    virtual RWCollectable* remove(const RWCollectable* target);

    /**
     * Removes the item that isEqual() to the item pointed to by \a target.
     * Destroys the item as well if it is the last occurrence in the
     * collection.
     */
    ...
    /**
     * Inserts the item \a c into the collection with count \a n and returns
     * it, or if an item was already in the collection that isEqual() to
     * \a c, then returns the old item and increments its count by \a n.
     */
    ...
    /**
     * Returns the current number of buckets allocated to the internal hash
     * table.
     */
 ...
 * not accessed in any particular order. If an item was inserted \c N times
 * into the collection, then it will be visited \c N consecutive times.
 *
 ...
 * longer valid, so continuing to use it has undefined results.
 *
 * \synopsis
 * #include <rw/rwbag.h>
 * RWBag b;
...
public:
    ...
    /**
     * Moves the iterator to the next item that isEqual() to the object
     * pointed to by \a target, and returns it. Hashing is used to find the
     * target. If no item is found, returns #rwnil and the position of the
 ...
 * \brief
 * Iterator for class RWBag, which allows sequential access to all the
 * elements of RWBag but allows no changes.
 *
 * Provides an iterator for class RWBag that allows sequential access to all
 * the elements of RWBag and that does not change any member of RWBag. Note
 * that because an RWBag is unordered, elements are not accessed in any
 * particular order. If an item was inserted \c N times into the collection,
 * then it is visited \c N consecutive times.
 *
 * Like all Rogue Wave iterators, the "current item" is undefined
 * immediately after construction -- you must define it by using
 * #operator()() or some other (valid) operation.
...
public:
    ...
    /**
     * Moves the iterator to the next item that isEqual() to the object
     * pointed to by \a target, and returns it. Uses hashing to find the
     * target. If no item is found, returns #rwnil and the position of the
     * iterator is undefined.
     */
