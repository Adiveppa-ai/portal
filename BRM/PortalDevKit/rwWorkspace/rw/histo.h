 ...
 *
 * $Id: //spro/rel2016.1/rw/histo.h#1 $
 *
 ...
 *
 * The class Histogram constructs and maintains a histogram of input data.
 * The count is stored as a public \ref RWMathVec "RWMathVec<int>" base
 * class. It can be used anywhere an \ref RWMathVec "RWMathVec<int>" can be
 * used.
 *
 * \synopsis
 * #include <rw/histo.h>
...
public:
    ...
    /**
     * Constructs a histogram from the data in \a v, with \a nbins bins of
     * data. The bins are of equal size and are scaled with the maximum and
     * minimum data in \a v. The count in the histogram is initialized with
     * the contents of \a v.
     */
    ...
    /**
     * Constructs a histogram with no initial data, with \a nbins bins. The
     * bins are of equal size and are scaled with \a max and \a min.
     */
    Histogram(size_t nbins, double min, double max);

    /**
     * Copy constructor. The new histogram does \e not reference the old
     * histogram's data.
     */
    Histogram(const Histogram&);

    /**
     * Returns the total accumulated number of data values that are greater
    ...
    /**
     * Returns the total accumulated number of data values that are less
     ...
     * that are smaller than the smallest bin, and the number of data values
