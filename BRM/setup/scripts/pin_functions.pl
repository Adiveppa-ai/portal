...
#=======================================================================
# @(#)$Id: pin_functions.pl /cgbubrm_mainbrm.portalbase/9 2018/12/11 02:19:21 njuturu Exp $
...
sub DropTables {
  ...
  if ( $USE_SPECIAL_DD_FILE !~ /^YES$/i ) {
    &ExecuteSQL_Statement_From_File( "$PIN_HOME/sys/dm_$DB{'vendor'}/data/drop_snapshots.source",
				   TRUE, TRUE, %DB );
    if (VerifyPresenceOfTable ("dd_objects_fields_t",%{ $DM{"db"} }) == -1) {
...
#=============================================================
#
#  This function takes backup of the Infranet.properties
#
#  Arguments:
#    $InfranetFile- Infranet.properties file to take backup
#
#=============================================================
sub Configure_Infranet {
  local( $InfranetFile ) = shift( @_ );
  if (defined($psv) ) {
    $InfranetFileBak = $InfranetFile.'_'.$psv.'.bak';
    if((-e "$InfranetFile") && ( ! -e "$InfranetFileBak")) {
        copy($InfranetFile, "$InfranetFileBak");
    }
  }
  else {
    if(-e "$InfranetFile") {
      copy($InfranetFile, "$InfranetFile.bak");
    }
  }
}
...
sub Configure_PinCnf {
  ...
  #This is a special case for upgrade to take backup
  if (defined($psv) ) {
    $PinConfFileBak = $PinConfFile.'_'.$psv.'.bak';
    if((-e "$PinConfFile") && ( ! -e "$PinConfFileBak")) {
       copy($PinConfFile, "$PinConfFileBak");
    }
  }
  #This is for full build case to take backup
  else {
    if(-e "$PinConfFile") {
...
#=============================================================
# This function looks for the next comment section in a file
#
# Arguments:
#    $SearchFrom - Line to search from
#    @File Array - Array containing the file
...
#=============================================================
# This function looks for a keyword in a file.
#
# Arguments:
#    $KeyWord - Keyword to look for
#    @File Array - Array containing the file
#
# $Returns: The array index at which the comment for the entry
# 	    starts (or -1 if not found)
#=============================================================
sub LocateEntryComment {
  local( $Keyword ) = shift( @_ );
  local( @File ) = @_;
  local( $i ) = 0;
...
#=============================================================
#
#  This function Deletes pin.conf entry(multiple lines) based 
#  on a given keyword and also removes corresponding comment.
#
#  Arguments:
#    $PinConfFile- pin.cnf file to modify
...
sub DeletePinConfEntryMulLine {
   ...
   $Start = &LocateStringInFile( $KeyWord, @FileReadIn );
   if ( $Start == -1 ) {
     ## Entry not exists , so nothing to delete,
   } else {
     $End = &LocateNextSectionComment( $Start, @FileReadIn );

        @Temp1 = ( @FileReadIn[ 0 .. ($Start-2)]);
        @Temp2 = ( @FileReadIn[ ($End-1) .. (@FileReadIn - 1) ]);

        @Temp = ( @FileReadIn[ 0 .. ($Start-2)],
...
#=============================================================
#
#  This function replaces a pin.conf entry description comment
#  based on a keywor located 1 line under the #====== comment.
#
#  Arguments:
#    $PinConfFile- pin.cnf file to modify
#    $KeyWord - Keyword to look for for replacing the entry
...
#  Returns: true
#=============================================================
sub ModifyPinConfEntryDescription {
  local( $PinConfFile ) = shift( @_ );
  local( $KeyWord ) = shift( @_ );
  local( $Values ) = shift( @_ );
  ...
  open( PINCONFFILE, "+< $PinConfFile" );
  @FileReadIn = <PINCONFFILE>;
  $Start = &LocateEntryComment( $KeyWord, @FileReadIn );
  $End = &LocateEntry( $KeyWord, @FileReadIn );
  if($Start != -1 && $End != -1 ) {
     @Temp = ( @FileReadIn[ 0 .. ($Start-2)],
                 $Values,
...
#=============================================================
#
#  This function Deletes pin.conf entry based on a given keyword
#  and also removes corresponding comment.
#
#  Arguments:
#    $PinConfFile- pin.cnf file to modify
#    $KeyWord - Keyword to look for for replacing the entry
...
sub DeletePinConfEntry {
   ...
   if ( $Start == -1 ) {
     ...
     $End = &LocateNextSection( $Start, @FileReadIn );
        ...
        @Temp1 = ( @FileReadIn[ 0 .. ($Start-2)]);
        @Temp2 = ( @FileReadIn[ ($End+2) .. (@FileReadIn - 1) ]);

        @Temp = ( @FileReadIn[ 0 .. ($Start-2)],
                  @FileReadIn[ ($End+2) .. (@FileReadIn - 1) ], "\n" );
        @FileReadIn = @Temp;
   }
   seek( PINCONFFILE, 0, 0 );
   print PINCONFFILE @FileReadIn;
...
#=============================================================
#
#  This function replace the given string in the specified file
#
#=============================================================
sub replaceStringInFile {
    local( $oldEntry ) = shift( @_ );
    local( $newEntry ) = shift( @_ );
    local( $fileName ) = shift( @_ );
    $oldEntry =~ s/\//\\\//g;
    $newEntry =~ s/\//\\\//g;
    `perl -p -i -e 's/$oldEntry/$newEntry/g' $fileName`;
}

#=============================================================
#
#  This function replace the given string in all the
#  files located in the specified directory
#
#=============================================================
sub replaceStringInDirectory {
    local( $oldEntry ) = shift( @_ );
    local( $newEntry ) = shift( @_ );
    local( $directoryName ) = shift( @_ );
    my @list = `find $directoryName -type f -exec egrep -l '$oldEntry' {} \\;`;
        foreach $list (@list) {
            chomp($list);
            my $bac = "$list".$psv.".bak";
            my $bac1 = "$psv".".bak";
            if( -e "$list" && !-e "$bac" && $list !~ /$bac1/) {
                system(" perl -p -i.$bac1 -e 's/$oldEntry/$newEntry/g;' $list");
            }
    }
}
...
#=============================================================
#
#  This function adds an entry in the Given file
#
#  Arguments:
#    $File - File to modify
#    $NewParameter    - New parameter/Entry needs to be added
#
#  Returns: true
#=============================================================
sub AddEntryInFile {
   local( $InfanetConfFile ) = shift( @_ );
   local( $NewParameter ) = shift( @_ );

   local( @FileReadIn );
   local( @Temp );

   #  Open the file for in place editing
   open( INFRACONFFILE, "+< $InfanetConfFile" );
   @FileReadIn = <INFRACONFFILE>;

        @Temp = ( @FileReadIn[ 0 .. (@FileReadIn - 1)],
                  $NewParameter."\n" );
        @FileReadIn = @Temp;

   seek( INFRACONFFILE, 0, 0 );
   print INFRACONFFILE @FileReadIn;
   print INFRACONFFILE "\n";
   truncate( INFRACONFFILE, tell( INFRACONFFILE ) );
   close( INFRACONFFILE );
   return TRUE;
}
...
#=============================================================
#
#  This function adds an entry in the Infranet.properties
#
#  Arguments:
#    $InfanetConfFile - Infranet.properties file to modify
#    $Section         - Under which section new parameter should add
#    $NewParameter    - New parameter/Entry needs to be added
#
#  Returns: true
#=============================================================
sub AddInfranetConfEntry {
   local( $InfanetConfFile ) = shift( @_ );
   ...
   #  Open the file for in place editing
   $key = substr($NewParameter, 0, index($NewParameter, '=')-1);
   open( INFRACONFFILE, "+< $InfanetConfFile" );
   @FileReadIn = <INFRACONFFILE>;
   $SectionFound = &LocateStringInFile( $Section, @FileReadIn );
   $EntryFound = &LocateStringInFile( $key, @FileReadIn );

   if ( $SectionFound != -1 && $EntryFound == -1) {
        @Temp = ( @FileReadIn[ 0 .. ($SectionFound+1)],
                  $NewParameter."\n",
                  @FileReadIn[ ($SectionFound+2) .. (@FileReadIn - 1) ], "\n" );
        @FileReadIn = @Temp;
   }
...
sub AddPinConfEntries {
   ...
   foreach $entry (sort keys( %Values ) ) {
     if ( $entry =~ /_description$/i ) {
       ...
       if ( $Tmp > -1 ) {
         #This if block will be executed for upgrade case
         if (defined($psv) ) {
           my $tmpString = $Values{$entry};
           my $tmpValue = substr ( $tmpString, rindex ( $tmpString, " " ) + 1 );
           $tmpString =~ s/$tmpValue//;
	   `perl -ni -e 'print unless (/!^#/ or /$tmpString/)' $PinConfFile`;
...
#=============================================================
#
#  This function appends comments to the existing
#  comments section in pin.conf
#
#  Arguments:
#    $PinConfFile - pin.conf file to modify
#    $KeyWord - Keyword to look for the enry to modify the
#               comment by concatenating the new entry.
#    %Values - Array of entries to use
#
#  Returns: true
#=============================================================
sub AddCommentPinConfEntry {
   local( $PinConfFile ) = shift( @_ );
   local( $KeyWord ) = shift( @_ );
   local( %Value ) =  @_ ;
   local( $FullComment );
  ...
  foreach $entry (sort keys( %Value ) ) {
     if ( $entry =~ /_description$/i ) {
        $FullComment = $Value{$entry};
     }
     if ( $entry =~ /_comment_check$/i ) {
        $CommentCheck = $Value{$entry};
     }
   }
   ...
   open( PINCONFFILE, "+< $PinConfFile" );
   @FileReadIn = <PINCONFFILE>;
   $CommentExists = `grep "$CommentCheck" $PinConfFile | wc -l`;
   $Start = &LocateEntry( $KeyWord, @FileReadIn );
   $Comment = @FileReadIn[($Start-2)];
   ...
   if ( $Start == -1 ) {
      push( @FileReadIn, "\n# $FullComment\n");
        ...
        {
            @Temp = ( @FileReadIn[ 0 .. ($Start-2)],
                      @FileReadIn[ ($Start-1) .. (@FileReadIn - 1) ], "\n" );
        } else {
            @Temp = ( @FileReadIn[ 0 .. ($Start-2)],
                      $FullComment."\n",
                      @FileReadIn[ ($Start-1) .. (@FileReadIn - 1) ], "\n" );
...
sub StartUnixDemon {
  ...
  if($ENV{DOCKER} eq "1" ) {
      &ExecuteShell( TRUE, "start_$DemonName" );
  }
  else {
...
##########################
# Check for testnap connection
##########################
sub chk_test {
        chdir "$PIN_HOME/sys/test";
        my $m = `echo q | testnap 2<&1 `;
        ($m =~ /ERROR:/) ? return 0 : return 1;
}
