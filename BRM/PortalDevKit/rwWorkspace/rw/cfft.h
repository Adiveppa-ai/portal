 ...
 *
 * \brief
 * Performs double precision complex fast Fourier transforms (FFTs) of an
 * \ref RWMathVec "RWMathVec<DComplex>" vector.
 *
 * Class DComplexFFTServer performs double precision complex fast Fourier
 * transforms (FFTs) of an \ref RWMathVec "RWMathVec<DComplex>" vector. The
 * transform of a complex vector is a complex vector; therefore, the server
 * returns a complex vector for either a forward or backward (inverse)
 * transform.
 *
 * At any given moment, the server is configured to transform a vector of a
 * specified length. This length can be set three ways:
 *
 * - At construction time.
 * - With the function setOrder().
 * - By the server from a supplied vector length.
 *
 * The last method means that if the vector length changes, the server
 * automatically reconfigures to the new length. Since this is an expensive
 * calculation, it is most efficient to call a server repeatedly for series
 * of the same length. Different FFT servers may be constructed to handle
 * different lengths.
 *
 * There are no restrictions on the number of points; that is, the number
 ...
 * \f]
 *
 * where \e A and \e X are complex. Note that the sum is not normalized; a
 * call to fourier() followed by a call to ifourier() results in the
 * original series multiplied by \e N.
 *
 * \synopsis
 * #include <rw/cfft.h>
...
public:
    ...
    /**
     * Constructs a complex FFT server designed to transform a vector of
     * length \a order.
     */
    ...
    /**
     * Returns the forward FFT, as defined above, of a complex vector \a v.
     * The expression:
     *
     * \code
     * DComplexFFTServer  f;
     ...
     * \endcode
     *
     * outputs the Fourier transform of \a v as a complex vector with \c N
     * points. If the server is not currently configured to the length of
     * \a v, it reconfigures.
     */
    ...
    /**
     * Returns the inverse FFT, as defined above, of a complex vector \a v.
     * The expression:
     *
     * \code
     * DComplexFFTServer  f;
