 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tools/stdwstring.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \deprecated
 * As of SourcePro 4, use \ref RWTRegex "RWTRegex<wchar_t>" instead.
 *
 * \brief
 * Deprecated. This class is a typedef for
 * \ref RWTRegularExpression "RWTRegularExpression<wchar_t>".
 */
 ...
 *
 * \brief
 * Alternate implementation of RWWSubString when \c RW_COPY_ON_WRITE_STRING
 * is not defined.
 *
 * RWStandardWSubString should be directly used only with instances of class
 * RWStandardWString. RWWSubString should be used with instances of class
 * RWWString.
 *
 * \sa
 * RWWSubString for additional documentation.
 */
 ...
 *
 * \brief
 * Alternate implementation of RWWConstSubString when
 * \c RW_COPY_ON_WRITE_STRING is not defined.
 *
 * RWStandardWConstSubString should be directly used only with instances of
 * class RWStandardWString. RWWConstSubString should be used with instances
 * of class RWWString.
 *
 * \sa
 * RWWConstSubString for additional documentation.
 */
 ...
 *
 * \brief
 * Alternate implementation of RWWString when \c RW_COPY_ON_WRITE_STRING is
 * not defined.
 *
 * Unless the specific performance characteristics of RWStandardWString are
 * required, applications should use RWWString instead. See the
 * <em>Essential Tools Module User's Guide</em> for additional information.
 *
 * \sa
 * RWWString for additional documentation.
 */
 ...
 *
 * \brief
 * Allows some subsection of an RWWString to be addressed by defining a
 * <em>starting position</em> and an \e extent.
 *
 ...
 * RWWSubString objects are constructed by various functions of the
 * RWWString class and then destroyed immediately.
 *
 * A zero length substring is one with a defined starting position and an
 * extent of zero. It can be thought of as starting just before the
 * indicated character, but not including it. It can be used as an lvalue. A
 * null substring is also legal and is frequently used to indicate that a
 * requested substring, perhaps through a search, does not exist. A null
 * substring can be detected with member function isNull(). However, it
 ...
 * \endcode
 *
 * Program output (assuming your platform displays wide characters as
 * US-ASCII if they are in the US-ASCII character set):
 *
 * \code
...
public:
     ...
     * \c str is resized appropriately. If self is the null substring, then
     ...
     * \endcode
     *
     * Copies <tt>a</tt>'s data into the substring <tt>b(2,3)</tt>. The
     * number of elements need not match; if they differ, \c b is resized
     * appropriately. If self is the null substring, then the statement has
     * no effect. Returns a reference to self.
     */
     ...
     * \endcode
     *
     * Copies 5 characters of <tt>a</tt>'s data into the substring
     * <tt>b(2,3)</tt>. The number of elements need not match; if they
     * differ, \c b is resized appropriately. If self is the null substring,
    ...
    /**
     * Returns the \a i <sup>th</sup> character of the substring. The index
     * \a i must be between zero and the length of the substring less one.
     * Bounds checking is enabled by defining the preprocessor macro
     * \c RWBOUNDS_CHECK before including \c <rw/wstring.h>.
     *
     * \exception RWBoundsErr
     * if RWBOUNDS_CHECK is defined and the index is out of range.
     */
     ...
     * Returns the \a i <sup>th</sup> character of the substring. The index
     * \a i must be between zero and the length of the substring less one.
     * \a i is converted to a \c size_t and bounds checking is performed.
     *
     * \exception RWBoundsErr
     ...
     *
     * \deprecated
     * As of SourcePro 12, use startData() instead.
     */
    ...
    /**
     * Changes all upper-case letters in self to lower-case. Uses the C++
     * Standard Library function \b std::towlower().
     */
    void toLower();

    /**
     * Changes all lower-case letters in self to upper-case. Uses the C++
     * Standard Library function \b std::towupper().
     */
    void toUpper();
 ...
 * <em>starting position</em> and an \e extent.
 *
 * The class RWWConstSubString allows some subsection of an RWWString to be
 * addressed by defining a <em>starting position</em> and an \e extent. For
 * example, the 7th through the 11th elements, inclusive, would have a
 * starting position of 7 and an extent of 5. The specification of a
 * starting position and extent can also be done on your behalf by such
 * functions as RWWString::strip(). There are no public constructors other
 * than two that allow you to create an RWWConstSubString from an existing
 * RWWSubString or RWWConstSubString. In general, RWWConstSubString objects
 * are constructed by various functions of the RWWString class and then
 * destroyed immediately.
 *
 * A <em>zero length</em> substring is one with a defined starting position
 * and an extent of zero. It can be thought of as starting just before the
 * indicated character, but not including it. It can be used as an lvalue. A
 * null substring is also legal and is frequently used to indicate that a
 * requested substring, perhaps through a search, does not exist. A null
 * substring can be detected with member function isNull(). However, it
...
public:
     ...
     *
     * \note
     * The resulting substring instance should not be used if the RWWString
     * associated with \a str is modified after the substring is
     * instantiated.
     */
 ...
 *
 * \brief
 * Offers powerful and convenient facilities for manipulating wide character
 * strings.
 *
 ...
 *
 * \note
 * RWWString is designed for use with wide character strings. To manipulate
 * multibyte character sequences, use RWCString.
 *
 * This string class manipulates wide characters of the fundamental type
 * \c wchar_t. These characters are generally two or four bytes, and can be
 * used to encode richer code sets than the classic \c char type. Because
 * \c wchar_t characters are all the same size, indexing is fast.
 *
 * Conversion to and from multibyte and US-ASCII forms are provided by the
 * RWWString constructors, and by the RWWString member functions isAscii(),
 ...
 *
 * Parameters of type <tt>const wchar_t*</tt> must not be passed a value of
 * zero. This is detected in the debug version of the library.
 *
 * A separate RWWSubString class supports substring extraction and
 * modification operations. RWWConstSubString supports substring extractions
 * on \c const RWWString instances.
 *
 * \synopsis
 * #include <rw/wstring.h>
 * RWWString a;
...
public:
    ...
    enum caseCompare {
         ...
         * only differs from #ignoreCase in comparison operations, where
         * changing case can affect the order of the strings.
         */
        ignoreCaseStrict
    };
     ...
     *
     * \deprecated
     * As of SourcePro 12, provided for compatibility with code that relies
     * on the enumeration RWStandardWString::scopeType.
     *
     * Specifies whether regular expression functions replace the first #one
     * substring matched by the regular expression or replaces #all
    ...
    /**
     * Specifies that a function should assume US-ASCII content when
     * converting from a <tt>const char*</tt> string.
     */
    enum ascii_ {
         ...
         *
         * \deprecated
         * As of SourcePro 12, use #ascii instead.
         */
    ...
    /**
     * Specifies that a function should assume multibyte content when
     * converting from a <tt>const char*</tt> string.
     */
    enum multiByte_ {
     ...
     *
     * \deprecated
     * As of SourcePro 12, use SubString instead.
     *
     ...
     *
     * \deprecated
     * As of SourcePro 12, use ConstSubString instead.
     *
     * Retained for backward compatibility with SourcePro 11.1
     */
    ...
    /**
     * Move constructor. The created string takes ownership of the data
     * owned by \a str.
     *
     * \conditional
     ...
     * \e capacity (that is, the size it can grow to without resizing) is
     * given by the parameter \a cap. We recommend creating an RWSize_T
     * value from a numerical constant to pass into this constructor. While
     * RWSize_T knows how to convert \c size_t values to itself, conforming
     * compilers choose the conversion to \c char instead.
     */
    ...
    /**
     * Constructs a string containing the character \a c repeated \a count
     * times.
     */
     ...
     *
     * \note
     * This function is incompatible with \a str strings with embedded
     * nulls.
     */
     ...
     * characters are copied, including any embedded nulls. Hence, the
     * buffer pointed to by \a str must be at least \a len
    ...
    /**
     * Constructs a string containing the character \a c repeated \a count
     * times. The character \a c is widened to a \c wchar_t.
     */
     ...
     * The character \a c is widened to a \c wchar_t.
     */
     ...
     * string contained in \a str. The conversion is done using the C++
     * Standard Library function \b std::mbstowcs().
     *
     * This constructor can be used as follows:
     *
     * \code
    ...
    /**
     * Constructs a wide character string from the US-ASCII character string
     * contained in \a str, copying exactly \a len characters including any
     * embedded nulls. Hence, the buffer pointed to by \a str must be long
     * enough to hold \a len characters.
     */
    ...
    /**
     * Constructs a wide character string from the multibyte character
     * string contained in \a str, copying exactly \a len characters
     * including any embedded nulls. Hence, the buffer pointed to by \a str
     * must be long enough to hold \a len characters.
     */
     ...
     *
     * \deprecated
     * As of SourcePro 12, use RWWString(const RWWString&) instead.
     *
    ...
    /**
     * Converts from substring. The created wide string copies the substring
     * represented by \a str.
     */
     ...
     *
     * \deprecated
     * As of SourcePro 12, use RWWString(const RWCString&, RWWString::ascii)
     * instead.
     *
    ...
    ~RWStandardWString()
    {
        ...
        data_.replace(0, length(), length(), static_cast<wchar_t>('0'));
#endif
    }
    ...
    /**
     * Assignment operator. Copies the character \a c into self. Returns a
     * reference to self.
     */
    ...
    /**
     * Assignment operator. The string copies the data from \a str. Returns
     * a reference to self.
     */
    ...
    /**
     * Appends the null-terminated character string pointed to by \a str to
    ...
    /**
     * Returns the \a i <sup>th</sup> character in self. The index \a i must
     * be between 0 and the length of the string less one. Bounds checking
     * is performed if the preprocessor macro \c RWBOUNDS_CHECK has been
     * defined before including \c <rw/wstring.h>.
     *
     * \exception RWBoundsErr
     * if RWBOUNDS_CHECK is defined and the index is out of range.
     */
    ...
    /**
     * Returns the \a i <sup>th</sup> character in self. The index \a i must
     * be between 0 and the length of the string less one. \a i is converted
    ...
    /**
     * Substring operator. Returns a substring of self with length \a len,
     * starting at index \a start. The sum of \a start plus \a len must be
     * less than or equal to the string length.
     *
     * \exception RWBoundsErr
     * if the library was built using the \c RW_DEBUG flag, and \a start and
     * \a len are out of range.
     */
     ...
     * occurrence of the null-terminated string \a pat in self. If \a pat is
     * not found, a null substring instance is returned.
     *
     * \note
     * This function is incompatible with \a pat strings with embedded
     * nulls.
     */
     ...
     * \a len characters are copied, <em>including any embedded nulls</em>.
     * Hence, the buffer pointed to by \a str must be at least \a len
     * characters long. Returns a reference to self.
     */
    inline RWStandardWString& append(const wchar_t* str, size_t len);

    /**
     * Appends a copy of the string \a str to self. Returns a reference to
     * self.
     */
    ...
    /**
     * Assigns the null-terminated wide character string pointed to by
     * \a str to self, replacing existing content. Returns a reference to
     * self.
     *
     * \note
     * This function is incompatible with \a str strings with embedded
    ...
    /**
     * Assigns a copy of the wide character string \a str to self, replacing
     * existing content. Exactly \a len bytes are copied, <em>including any
     * embedded nulls</em>. Hence, the buffer pointed to by \a str must be
     * at least \a len characters long. Returns a reference to self.
     */
    ...
    /**
     * A non-binding request to alter the capacity of self to \c cap. The
     * capacity is never reduced below the current number of characters.
     ...
     * according to the result of calling the C++ Standard Library function
     * \b std::wcscoll() on self and the argument \a str. This method
    ...
    /**
     * Lexicographically compares self to \a str. Returns the following
     * values based on the results of the comparison:
     *
     * <table>
     * <tr>
     * <td align="right">&lt;0</td>
     * <td>self precedes \a str.</td>
     * </tr>
     * <tr>
     * <td align="right">0</td>
     * <td>self is equal to \a str.</td>
     * </tr>
     * <tr>
     * <td align="right">&gt;0</td>
     * <td>self follows \a str.</td>
     * </tr>
     * </table>
     *
     * If \a cmp is either RWCString::ignoreCase or
     * RWCString::ignoreCaseStrict, the strings are normalized before they
     * are compared.
     */
     ...
     *
     * \deprecated
     * As of SourcePro 12, use operator=(const RWStandardWString&) and
    ...
    /**
     * Provides access to the RWWString instances's data as a
     * null-terminated string. This datum is owned by the RWWString and may
     * not be deleted or changed. If the RWWString object itself changes or
     * goes out of scope, the pointer value previously returned becomes
     * invalid. While the string is null-terminated, note that its length is
     * still given by the member function length(). That is, it may contain
     * embedded nulls.
     */
    const wchar_t* data() const
    {
     ...
     * self. Returns #RW_NPOS if there is no match.
     */
    ...
    /**
     * Returns the index of the first occurrence in self of any character in
     * \a str. Exactly \a len characters in \a str are checked <em>including
     * any embedded nulls</em> so \a str must point to a buffer containing
    ...
    /**
     * Pattern matching. Starting with index \a start, searches for the
     * first occurrence of \a pat in self and returns the index of the start
     * of the match. Returns #RW_NPOS if there is no such pattern. Case
     * sensitivity is according to \a cmp.
     */
    ...
    /**
     * Inserts a copy of the string \a str into self at index \a pos.
     * Returns a reference to self.
     */
    ...
    /**
     * Inserts the first \a len characters of \a str into self at index
     * \a pos. If \a len is greater than the length of \a str, the entire
     * string is inserted. Returns a reference to self.
     */
    ...
    /**
     * Returns the index of the last occurrence of the character \a c in
     * self. Returns #RW_NPOS if there is no match.
     */
    ...
    /**
     * Returns the index of the last occurrence in self of any character in
     * \a str. Returns #RW_NPOS if there is no match.
     *
     * \note
     * This function is incompatible with \a str strings with embedded
    ...
    /**
     * Prepends \a count copies of character \a c to self. Returns a
     * reference to self.
     */
    RWStandardWString& prepend(wchar_t c, size_t count = 1);

    /**
     * Prepends a copy of the null-terminated character string pointed to by
    ...
    /**
     * Prepends a copy of the string \a str to self. Returns a reference to
     * self.
     */
    ...
    // istream input
    /**
     * Reads characters from the input stream \a strm, replacing the
     * previous contents of self, until \c EOF is reached. The input stream
     * is treated as a sequence of wide characters, each of which is read
     * and stored in the RWWString object. Null characters are treated the
    ...
    /**
     * Reads characters from the input stream \a strm, replacing the
     * previous contents of self, until a newline (or an \c EOF) is
     * encountered. The newline is removed from the input stream but is not
     * stored. The input stream is treated as a sequence of wide characters,
     * each of which is read and stored in the RWWString object. Null
     * characters are treated the same as other characters. If the
     * \a skipWhite argument is \c true, then whitespace is skipped (using
     * the iostream library manipulator \c ws) before saving characters.
     *
     * \note
     * RWWString is designed for use with wide character strings; to
     * manipulate multibyte character sequences, use RWCString.
    ...
    /**
     * Reads characters from the input stream \a strm, replacing the
     * previous contents of self, until an \c EOF or null terminator is
     * encountered. The input stream is treated as a sequence of wide
     * characters, each of which is read and stored in the RWWString object.
     *
     * \note
     * RWWString is designed for use with wide character strings; to
     * manipulate multibyte character sequences, use RWCString.
    ...
    /**
     * Reads characters from the input stream \a strm, replacing the
     * previous contents of self, until an \c EOF or the delimiting
     * character \a delim is encountered. The delimiter is removed from the
     * input stream but is not stored. The input stream is treated as a
     * sequence of wide characters, each of which is read and stored in the
     * RWWString object. Null characters are treated the same as other
     * characters.
     *
     * \note
     * RWWString is designed for use with wide character strings; to
     ...
     * sequence of wide characters, each of which is read and stored in the
     * RWWString object. Null characters are treated the same as other
     * characters. If the \a skipWhite argument is \c true, then leading
     * white space is skipped (using the iostream library manipulator
     * \c ws).
     *
     * \note
     * RWWString is designed for use with wide character strings; to
     ...
     * previous contents of self, until trailing whitespace or an \c EOF is
     * encountered. The trailing whitespace is left on the input stream.
     * Only US-ASCII whitespace characters are recognized, as defined by the
     * C++ Standard Library function \b std::iswspace(). The input stream is
     * treated as a sequence of wide characters, each of which is read and
     * stored in the RWWString object.
     *
     * \note
     * RWWString is designed for use with wide character strings; to
     * manipulate multibyte character sequences, use RWCString.
     ...
     * \internal
     * Reads characters from the input stream \a strm, replacing the
     * previous contents of self, until \c EOF is reached. Null characters
     ...
     * \internal
     * Reads characters from the input stream \a strm, replacing the
     * previous contents of self, until a newline (or \c EOF) is
     * encountered. The newline is removed from the input stream but is not
     * stored. Null characters are treated the same as other characters. If
     ...
     * \internal
     * Reads characters from the input stream \a strm, replacing the
     * previous contents of self, until a null terminator (or \c EOF) is
     * encountered.
     */
     ...
     * \internal
     * Reads characters from the input stream \a strm, replacing the
     * previous contents of self, until the delimiting character \a delim
     * (or \c EOF) is encountered. The delimiter is removed from the input
     ...
     * of self, until trailing white space or \c EOF is encountered. The
     * white space is left on the input stream. Null characters are treated
     * the same as other characters. White space is identified by the C++
     * Standard Library function \b std::iswspace().
     */
    std::wistream& readToken(std::wistream& strm);

    /**
     * Removes the characters to the end of string, starting at the position
     * \a pos, which must be no greater than length(). Returns a reference
     * to self.
     */
    ...
    /**
     * Replaces the substring beginning at \a pos and of length \a len with
     * the null-terminated string \a str. If the sum of \a pos and \a len is
     * greater than the length of self, the substring replaced is from
     ...
     *
     * \exception RWBoundsErr
     * If \a pos is greater than the length of self.
     */
    ...
    /**
     * Replaces the substring beginning at \a pos and of length \a len with
     * the string \a str. If the sum of \a pos and \a len is greater than
     * the length of self, the substring replaced is from \a pos to end of
     * string.
     *
     * Returns a reference to self.
     *
    ...
    /**
     * Replaces the substring beginning at \a pos and of length \a len with
     * \a strLen characters from \a str. If the sum of \a pos and \a len is
     * greater than the length of self, the substring replaced is from
     * \a pos to end of string.
     *
     * Returns a reference to self.
    ...
    /**
     * Replaces the substring beginning at \a pos and of length \a len with
     * the first \a strLen characters of string \a str. If the sum of \a pos
    ...
    /**
     * Changes the length of self to \a len characters, adding blanks (i.e.,
     * <tt>L' '</tt>) or truncating as necessary.
     */
    void resize(size_t len);

    /**
     * \internal
     * Restores an instance of RWWString from \a vis into which an instance
     * had been persisted using saveOn(RWvostream&).
     */
    void restoreFrom(RWvistream& vis);

    /**
     * \internal
     * Restores an instance of RWWString from \a fil into which an instance
     * had been persisted using saveOn(RWFile&).
     */
     ...
     * that occurs before index \a end and returns the index of the start of
     * the match. Returns #RW_NPOS if there is no such pattern. Case
     * sensitivity is according to \a cmp.
     *
     * \note
     * - This member function is unavailable on RWWString if
     ...
     * \a patlen characters from \a pat in self that occurs before index
     * \a end and returns the index of the start of the match. Returns
     * #RW_NPOS if there is no such pattern. Case sensitivity is according
     * to \a cmp.
     *
     * \note
    ...
    /**
     * \internal
     * Saves an instance of RWWString to \a vos. The instance can be
     * restored using the member function restoreFrom(RWvistream&).
     */
    void saveOn(RWvostream& vos) const;

    /**
     * \internal
     * Saves an instance of RWWString to \a fil. The instance can be
     * restored using the member function restoreFrom(RWFile&).
     */
    void saveOn(RWFile& fil) const;

    /**
     * Returns an ASCII representation of self. Any non-US-ASCII characters
     * in self simply have the high bits stripped off. Use isAscii() to
     * determine whether this function is safe to use.
     */
     ...
     * \b std::wcstombs() doesn't handle them. Returns an empty string if
     ...
     * Equivalent to calling:
     * \code
     * RWWString(str, RWWString::multiByte);
     * \endcode
     */
     ...
     *
     * \deprecated
     * As of SourcePro 12, use toMultiByte() instead.
     */
     ...
     * As of SourcePro 12.
     *
     * It has no effect on an RWWString instance and may be removed from a
     * future release.
     */
    RW_DEPRECATE_FUNC("")
...
protected:
     ...
     *
     * \deprecated
     * As of SourcePro 12. This function is no longer used.
     */
    RW_DEPRECATE_FUNC("")
     ...
     * length \a len in self to lower-case, using the C++ Standard Library
     * function \b std::towlower().
     */
     ...
     * length \a len in self to upper-case, using the C++ Standard Library
     * function \b std::towupper().
     */
...
private:
    ...
    /**
     * \internal
     * Converts the multibyte string \a str into a wide character string and
     * returns it. The length of the resulting wide character string is
     * returned via the out parameter \a wideLen.
     */
     ...
     * string and returns it. The length (in bytes) of the resulting
     * multibyte character string is returned via the out parameter
     * \a mbLen.
     */
...
RWStandardWString::readLine(std::istream& is, bool skipWhite)
{
    ...
    return readToDelim(is, static_cast<wchar_t>('\n'));
}

inline std::istream&
RWStandardWString::readString(std::istream& is)
{
    return readToDelim(is, static_cast<wchar_t>('\0'));
}
...
{
    data_.resize(N, static_cast<wchar_t>(' '));
}
...
/**
 * \relates RWWString
 * Returns the result of applying \b std::wcsxfrm() to \a str to allow
 * quicker collation than RWWString::collate().
 *
 * \note
...
/**
 * \relates RWWString
 * Returns a copy of \a str with all upper-case characters replaced with
 * lower-case characters. This function is the equivalent of calling:
 *
 * \code
 * RWWString tmp(str);
 * tmp.toLower();
...
/**
 * \relates RWWString
 * Returns a copy of \a str with all lower-case characters replaced with
 * upper-case characters. This function is the equivalent of calling:
 *
 * \code
 * RWWString tmp(str);
 * tmp.toUpper();
...
/**
 * \relates RWWString
 * Concatenates \a lhs and \a rhs and returns the resulting string.
 */
RWStandardWString
...
/**
 * \relates RWWString
 * Concatenates \a lhs and the null-terminated string \a rhs and returns the
 * resulting string.
 *
 * \note
...
/**
 * \relates RWWString
 * Concatenates the null-terminated string \a lhs and \a rhs and returns the
 * resulting string.
 *
 * \note
 ...
 * \relates RWWString
 *
 * Outputs an RWWString on ostream \a os as a sequence of bytes. This is
 * done literally from the byte content of the characters.
 */
 ...
 * \relates RWWString
 *
 * Calls <tt>\a str.readToken(\a s)</tt>. That is, a token is read from the
 * input stream \a s.
 */
inline std::istream&
 ...
 *
 * Outputs an RWWSubString on ostream \a os as a sequence of Bytes. This is
 * done literally from the byte content of the characters.
 */
inline std::ostream&
 ...
 *
 * Outputs an RWWConstSubString on ostream \a os as a sequence of Bytes. This is
 * done literally from the byte content of the characters.
 */
inline std::ostream&
 ...
 */
inline std::wistream&
operator>>(std::wistream& s, RWStandardWString& str)
{
    return str.readToken(s);
}
 ...
 * \relates RWWString
 *
 * Outputs an RWWString on wostream \a os as a sequence of wide characters.
 */
inline std::wostream&
 ...
 * \relates RWWSubString
 *
 * Outputs an RWWSubString on wostream \a os as a sequence of wide characters.
 */
inline std::wostream&
 ...
 * \relates RWWConstSubString
 *
 * Outputs an RWWConstSubString on wostream \a os as a sequence of wide characters.
 */
inline std::wostream&
 ...
 *
 * If an error occurs while extracting the string, the operator returns the
 * original string contents unmodified. Check the virtual stream to
 ...
 *
 * If an error occurs while extracting the string, the operator returns the
 * original string contents unmodified. Check the file to determine if an
 * error occurred on extraction.
 */
inline RWFile&
...
/**
 * \relates RWWString
 * Returns \c true if \a lhs is lexicographically equal to \a rhs. Otherwise
 * returns \c false. Use member RWWString::collate() or
 * RWWString::strXForm() for locale-sensitive comparisons.
 */
inline bool
operator==(const RWStandardWString& lhs, const wchar_t* rhs)
...
/**
 * \relates RWWString
 * Returns \c true if \a lhs lexicographically precedes \a rhs. Otherwise
 * returns \c false. Use member RWWString::collate() or
 * RWWString::strXForm() for locale-sensitive comparisons.
 */
inline bool
 ...
 *
 * \deprecated
 * As of SourcePro 12, use RWWString::hash(const RWWString&) instead.
 */
...
RWStandardWSubString::RWStandardWSubString(const RWStandardWString& str, size_t pos, size_t n)
    : str_(const_cast<RWStandardWString*>(&str)), begin_(pos), extent_(n)
{
#ifdef RW_DEBUG
