 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/regexp.h#1 $
 *
 **********************************************************************
 *
 ...
 * -#
 * \parblock
 * A backslash (\c \\) followed by any special character matches the literal
 * character itself. i.e., this "escapes" the special character.
 *
 * \note
 * There is one exception to this rule. \c \^\c <em>char</em> is interpreted
 * as a control character: thus \c \^R is control-R. To match the circumflex
 * \c ^ itself, use \c \\x5e in US-ASCII environments.
 ...
 * -# A set of characters enclosed in brackets (\c []) is a one-character RE
 * that matches any of the characters in that set. <em>Example:</em> "[akm]"
 * matches either an "a", "k", or "m". A range of characters can be
 * indicated with a dash. <em>Example:</em> "[a-z]" matches any lower-case
 * letter. However, if the first character of the set is the caret (^), then
 * the RE matches any character except those in the set. It does not match
 * the empty string. <em>Example:</em> [^akm] matches any character except
 * "a", "k", or "m". The caret loses its special meaning if it is not the
 ...
 * occur zero or once in the string -- no more. For example, xy?z matches
 * either xyz or xz.
 ...
 * </table>
 *
 * The most frequent problem with using this class is specifying a backslash
 * character to be parsed. If you are attempting to parse a regular
 * expression that contains backslashes, you must be aware that the C++
 * compiler and the regular expression constructor both assume that any
 * backslashes are intended to escape the following character. Thus, to
 * specify a regular expression that exactly matches \c "a\a", you would
 * have to create the regular expression using four backslashes as follows:
 * the regular expression needs to see \c "a\\a", and for that to happen,
 * the compiler would have to see \c "a\\\\a".
 *
 * \code
 * RWCRegexp reg("a\\\\a");
 *                 ^|^|
 ...
 * the ones marked with \c | are seen by the regular expression parser. At
 * that point, the backslash marked \c 1 is an escape, and the one marked
 * \c 2 is actually put into the regular expression.
 *
 * Similarly, if you really need to escape a character, such as a '\c .' you
 * have to pass two backslashes to the compiler:
 *
 * \code
 * RWCRegexp regDot("\\.")
 *                   ^|
 ...
 * the one marked with \c | is seen by the regular expression constructor as
 * an escape for the following '\c .'.
 *
 * \synopsis
 * #include <rw/regexp.h>
...
public:
    ...
    enum statVal {
        ...
        /**
         * Pattern exceeded maximum length. (To change the amount of space
         * allocated for a pattern, edit \c regexp.cpp to change the value
         * of \c RWCRegexp::maxpat_, then recompile and insert the changed
         * object into the appropriate library.)
         */
        TOOLONG
    };

    /**
     * Constructs a regular expression from the pattern given by \a pat. The
    ...
    /**
     * Copy constructor. Uses value semantics -- self is a copy of \a r.
     */
     ...
     * \a len. Using an invalid regular expression for the search throws an
     * exception of type RWInternalErr.
     *
     * \note
     * This member function is relatively clumsy to use -- class RWCString
