/**************************************************************************
 *
 * $Id: //spro/rel2016.1/rw/itc/RWTPCPtrBufferBase.cc#1 $
 *
...
/*****************************************************************************
        ...
        RWTPCPtrBufferBase<Type> - Templatized base class for buffers with producer-
                consumer synchronization semantics whose entries are stored by-address.
...
template <class Type>
size_t
RWTPCPtrBufferBase<Type>::_entries(void) const
{
...
template <class Type>
Type*
RWTPCPtrBufferBase<Type>::read(void)
{
...
template <class Type>
bool
RWTPCPtrBufferBase<Type>::tryRead(Type*& result)
{
    LockGuard lock(monitor());
    bool retval;
...
template <class Type>
RWWaitStatus
RWTPCPtrBufferBase<Type>::read(Type*& result, unsigned long milliseconds)
{
    RWUserTimer timer;
    ...
    while (!_canRead() && isOpen_ && RW_THR_COMPLETED == waitStatus) {
...
template <class Type>
Type*
RWTPCPtrBufferBase<Type>::peek(void)
{
    LockGuard lock(monitor());
...
template <class Type>
bool
RWTPCPtrBufferBase<Type>::tryPeek(Type*& value)
{
    LockGuard lock(monitor());
    bool result;
    if (_canRead()) {
...
template <class Type>
RWWaitStatus
RWTPCPtrBufferBase<Type>::peek(Type*& result, unsigned long milliseconds)
{
    RWUserTimer  timer;
    unsigned long elapsedMilliseconds;
    RWWaitStatus waitStatus = RW_THR_COMPLETED;
...
template <class Type>
void
RWTPCPtrBufferBase<Type>::write(Type* value)
{
    LockGuard lock(monitor());
...
template <class Type>
bool
RWTPCPtrBufferBase<Type>::tryWrite(Type* value)
{
    LockGuard lock(monitor());
    bool result;
    if (_canWrite() && isOpen_) {
        ...
        if (!hasInvokedFullCallback_ &&
            ...
            // Yes, this thread is the first writer thread to find the buffer
            // full since the list time the buffer was less than full, or
            // since the last callback registration - it will need to invoke
...
template <class Type>
RWWaitStatus
RWTPCPtrBufferBase<Type>::write(Type* value, unsigned long milliseconds)
{
    RWUserTimer  timer;
    unsigned long elapsedMilliseconds;
    RWWaitStatus waitStatus = RW_THR_COMPLETED;
...
template <class Type>
Type*
RWTPCPtrBufferBase<Type>::_read(void)
{
...
template <class Type>
Type*
RWTPCPtrBufferBase<Type>::_peek(void)
{
...
template <class Type>
void
RWTPCPtrBufferBase<Type>::_flush(void)
{
    buffer_.clear();
    // Indicate that the queue has exited the full state
    hasInvokedFullCallback_ = false;
