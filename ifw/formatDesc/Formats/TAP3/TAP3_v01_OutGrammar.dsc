...
//------------------------------------------------------------------------------
// Module Description:
//   Output grammar file for the TAP3_v01 CDR format
//
// Open Points:
//   <open points>
//
...
//
// $RCSfile: TAP3_v01_OutGrammar.dsc,v $
// $Revision: 1.41 $
// $Author: cdiab $
// $Date: 2002/01/22 16:27:22 $
// $Locker:  $
//------------------------------------------------------------------------------
// $Log: TAP3_v01_OutGrammar.dsc,v $
// Revision 1.41  2002/01/22 16:27:22  cdiab
...
//
// Revision 1.40  2002/01/21 14:45:22  cdiab
...
//
// Revision 1.39  2001/11/29 13:31:13  cdiab
...
//
// Revision 1.38  2001/11/21 15:17:17  cdiab
...
//
// Revision 1.34  2001/10/23 18:10:00  cdiab
// PETS #40891: TAP3 parse error : v04 read / v01 v04 write
//
// Revision 1.33  2001/10/19 11:58:51  cdiab
...
//
// Revision 1.32  2001/10/19 11:37:08  cdiab
// PETS #40694: some errors are still in the TAP3 processing
// check that charge!=0 before summing it to the totalCharge.
//
// Revision 1.31  2001/10/18 16:31:28  cdiab
// PETS #40694: some errors are still in the TAP3 processing
...
//
// Revision 1.25  2001/10/10 16:00:10  cdiab
// PETS #40293: correct little formats error in Tap3 implementation
// on input:
...
//
// Revision 1.24  2001/10/09 15:26:44  cdiab
...
//
// Revision 1.23  2001/10/04 16:46:00  cdiab
...
//
// Revision 1.22  2001/10/01 16:27:25  cdiab
...
//
// Revision 1.21  2001/09/26 11:58:00  cdiab
...
//
// Revision 1.20  2001/09/21 12:59:45  cdiab
...
//
// Revision 1.19  2001/09/20 13:09:43  cdiab
...
//
// Revision 1.18  2001/09/20 12:08:14  cdiab
...
//
// Revision 1.17  2001/09/18 13:04:23  cdiab
...
//
// Revision 1.16  2001/09/14 16:18:38  cdiab
// PETS #39333: in TAP3, add ability to select tap or local currency
// if EXCHANGE_RATE is not > 0.0 (ie not set), use 1.0 to still be able to process.
//
// Revision 1.15  2001/09/14 14:16:54  cdiab
// PETS #39333: in TAP3, add ability to select tap or local currency.
...
//
// Revision 1.14  2001/09/05 15:38:48  cdiab
// PETS #38890:
// last commit was done with wrong Pets number
//
// Revision 1.13  2001/09/05 12:24:42  cdiab
...
//
// Revision 1.12  2001/08/03 13:34:16  cdiab
...
//
// Revision 1.11  2001/07/20 12:21:32  cdiab
// addendum to PETS #36769 : we attach an error to the EDR when it is rejected.
//
// Revision 1.10  2001/07/19 15:29:13  cdiab
// PETS #37234 : using wrong sparator for IPv6 detection (correct is :)
//
// Revision 1.9  2001/07/18 14:00:35  cdiab
// PETS #36769 : need possibility to reject an EDR from iScript on output
// Used in TAP3_v01_OutGrammar.dsc to reject edrs with invalid RECORD_TYPE for IFW_TAP3.
//
// Revision 1.8  2001/07/12 10:40:04  cdiab
// -PETS #36928 : rename regestry root to ifw, remove console prints of start/end
// processing time.
//
// Revision 1.7  2001/06/07 08:10:44  cdiab
// - input and output OK (looping generated output on input works)
//
// Revision 1.6  2001/06/06 12:40:03  cdiab
// backup - before test of the generated output.
//
// Revision 1.5  2001/06/05 15:40:12  cdiab
// now uses TapDecimalPlaces to calculate AbsoluteAmount values.
//
// Revision 1.4  2001/06/01 15:33:58  cdiab
// backup - and Charge information is partly build on output.
//
// Revision 1.3  2001/05/31 15:33:25  cdiab
// backup, VAS,MSS,SCU done.
//
// Revision 1.2  2001/05/30 15:09:10  cdiab
// backup, header for MOC, MTC, GPRS are written to output.
//
// Revision 1.1  2001/05/18 17:11:25  cdiab
// First backup in cvs. Only input works.
//
...
iScript
{
  ...
  const Long theTap3ReleaseVersionNumber       = 1;
  ...
  function updateTap3HeaderWithCodeLists
  {
    ...
    if (taxEntries > 0)
    {
      ...
      for (i=1 ; i<=taxEntries ; i=i+1)
      {
        ...
        asnTreeAddString("IFW_TAP3.TaxType.TAX_TYPE", "15" ); //ie composite
  ...
  function Long addChargeInformation(Long startCPidx, Long nbCP)
  {
    ...
    if (record_Type == record_MOC)
    {
      ...
      {
        asnTreeAddString("IFW_TAP3.CallType.CALL_TYPE",		edrString(DETAIL.CONNECT_TYPE));    
      }
      if (edrString(DETAIL.CONNECT_SUB_TYPE) != "")
      {
        asnTreeAddString("IFW_TAP3.CallTypeSubtype.CALL_TYPE_SUBTYPE",edrString(DETAIL.CONNECT_SUB_TYPE));   
      }
    }
...
//    	    (startEXCHANGE_RATE	   == edrDecimal(DETAIL.ASS_CBD.CP.EXCHANGE_RATE, 0, currentCPidx))	and
	    ...
	    (currentCPidx	   <  nbCP ) )
    {
      ...
      if (  ((edrString(DETAIL.ASS_CBD.CP.CHARGED_CURRENCY_TYPE, 0, currentCPidx) == "R")and
             (edrString(DETAIL.ASS_CBD.CP.PRICEMODEL_TYPE,       0, currentCPidx) == "S"))
	  or((edrString(DETAIL.ASS_CBD.CP.CHARGED_CURRENCY_TYPE, 0, currentCPidx) == "R")and
    ...
    if (edrLong(DETAIL.WHOLESALE_CHARGED_TAX_RATE) != 0)
    {
      ...
      asnTreeAddInteger("IFW_TAP3.TaxCode.TAX_CODE", getTaxRateCode(longToStr(edrLong(DETAIL.WHOLESALE_CHARGED_TAX_RATE))));
      asnTreeAddInteger("IFW_TAP3.Charge.CHARGE", 0);
  ...
  function Bool processMOC
  {
    ...
    if (edrLong(DETAIL.QOS_REQUESTED) != 0)
    {
      asnTreeAddInteger("IFW_TAP3.RadioChannelRequested.RADIO_CHANNEL_REQUESTED", edrLong(DETAIL.QOS_REQUESTED) );
    }
    if (edrLong(DETAIL.QOS_USED) != 0)
    {
      asnTreeAddInteger("IFW_TAP3.RadioChannelUsed.RADIO_CHANNEL_USED", edrLong(DETAIL.QOS_USED) );
    }
  ...
  function Bool processGPRS
  {
    ...
    asnTreeAddString("IFW_TAP3.AccessPointName.ACCESS_POINT_NAME", edrString(DETAIL.ASS_GPRS_EXT.APN_ADDRESS, 0));
    ...
    asnTreePushTag("IFW_TAP3.QoSRequested");
    ...
    asnTreePop(); //asnTreePushTag("IFW_TAP3.QoSRequested");  
    asnTreePushTag("IFW_TAP3.QoSUsed");
    ...
    asnTreePop(); //asnTreePushTag("IFW_TAP3.QoSUsed");  
  ...
  function Bool processMSS
  {
    ...
    String basicDualServiceprefix  = strSubstr(edrString(DETAIL.ASS_GSMW_EXT.BASIC_DUAL_SERVICE,0), 0, 1 ); //take the first 1 digit
    String basicDualServiceSuffixe = strSubstr(edrString(DETAIL.ASS_GSMW_EXT.BASIC_DUAL_SERVICE,0), 1, 2 ); //take the last  2 digits
...
//logStdout( "NOTIFICATION WAS DETECTED\n");
	...
	// no FileTypeIndicator in notification for TAP0301 but >=TAP0303 has it!
...
//logStdout( "IFW_TAP3.TRAILER.STD_MAPPING "+ recordNum +"(TotaChargeAmount="+decimalToStr(edrDecimal(TRAILER.TOTAL_WHOLESALE_CHARGED_VALUE))+")\n"  );
	...
	asnTreeAddString("IFW_TAP3.TotalTaxValue.TOTAL_TAX_VALUE", "00");
