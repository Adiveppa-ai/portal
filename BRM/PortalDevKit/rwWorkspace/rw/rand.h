...
public:
    /**
     * Function call operator. Defined by derived classes to return a random
     * number from the appropriate probability distribution.
     */
 ...
 *
 * \brief
 * Abstract base class from which the random number generator classes
 * derive.
 *
 * Class RWTRand is the abstract base class for the random number generator
 * classes. The classes derived from RWTRand generate random numbers from
 * specific probability distributions. Each of these derived classes
 * generates a random number from a uniform distribution on the interval [0,
 * 1], and from this deviate derives a random number generator from the
 * appropriate probability distribution. The template parameter \c Generator
 * is a function object that provides this uniform [0, 1] deviate.
 * Specifically, \c Generator is presumed to be an object that overloads the
 * function call operator to return a uniform random number of type
 * \c double between 0 and 1.
 *
 * The class Generator must have the following semantics, where \c gen is of
 * type Generator:
 *
 * <table>
 * <tr>
...
public:
    /**
     * Uses the default constructor of class Generator to initialize the
     * underlying uniform random number generator.
     */
    RWTRand();

    /**
     * Initializes the underlying uniform random number generator with \a g.
     */
    ...
    /**
     * Returns the underlying uniform random number generator as a \c const
     * reference.
     */
    ...
    /**
     * Returns a reference to the underlying uniform random number
     * generator.
     */
    Generator& generator()
    {
 ...
 *
 * \brief
 * Used to generate random numbers from a uniform distribution in an
 * interval [a, b].
 *
 * Class RWTRandUniform generates random numbers from a uniform distribution
 * in an interval [a, b]. The algorithm used to generate the random numbers
 * begins with random numbers generated from a uniform distribution on the
 * interval [0, 1]. Thus, the speed and statistical properties of the
 * uniform number generator may be changed by varying the Generator template
 * parameter responsible for generating the uniformly distributed numbers.
 *
 * The class Generator must be a function object whose function call
 * operator returns a uniform random double between 0 and 1. The class
 * RWRandGenerator may be used.
 *
 * \synopsis
 * #include <rw/rand.h>
...
public:
    ...
    /**
     * Constructs a generator with underlying uniform [0, 1] generator \a g
     * and range [\a a, \a b]
     */
    ...
    /**
     * Assigns random numbers to \a n elements in \a out. Returns the number
 ...
 * \f]
 *
 * where \f$ \mu\f$ is the mean, and \f$ \sigma ^2\f$ is the variance. The
 * algorithm used to generate the random numbers begins with random numbers
 * generated from a uniform distribution on the interval [0, 1]. Thus, the
 * speed and statistical properties of the normal random numbers may be
 * varied by varying the Generator template parameter, which is responsible
 * for generating these uniformly distributed numbers.
 *
 * The class Generator must be a function object whose function call
 * operator returns a uniform random double between 0 and 1. The class
 * RWRandGenerator supplied with the library may be used.
 *
 * \synopsis
 * #include <rw/rand.h>
...
public:
    ...
    /**
     * Constructs a generator with the \a mean specified by the first
     * parameter and \a variance specified by the second parameter.
     */
    ...
    /**
     * Constructs a generator with underlying uniform [0, 1] generator \a g
     * and the specified \a mean and \a variance.
     */
 ...
 *
 * Class RWTRandExponential generates random numbers from an exponential
 * distribution:
 *
 * \f[
 * g(w) =
 ...
 * \f]
 *
 * The algorithm used to generate the random numbers begins with random
 * numbers generated from a uniform distribution on the interval [0, 1].
 * Thus, the speed and statistical properties of the exponentially
 * distributed random numbers may be changed by varying the Generator
 * template parameter responsible for generating these uniformly distributed
 * numbers.
 *
 * The class Generator must be a function object whose function call
 * operator returns a uniform random double between 0 and 1. The class
 ...
 * on the interval [0, 1]. Thus, the speed and statistical properties of the
 * gamma random number generator may be varied by varying the Generator
 * template parameter, which is responsible for generating the uniformly
 * distributed numbers.
 *
 * The class Generator must be a function object whose function call
 * operator returns a uniform random double between 0 and 1. The class
 ...
 *
 * Class RWTRandPoisson generates random numbers from a Poisson
 * distribution:
 *
 * \f[
 ...
 * \f]
 *
 * where \f$ \mu\f$ is the mean. The algorithm used to generate the random
 * numbers begins with random numbers generated from a uniform distribution
 * on the interval [0, 1]. Thus, the speed and statistical properties of the
 * Poisson random number generator may be varied by varying the Generator
 * template parameter, which is responsible for generating the uniformly
 * distributed numbers.
 *
 * The class Generator must be a function object whose function call
 ...
 *
 * Class RWTRandBinomial generates random numbers from a binomial
 * distribution:
 *
 * \f[
 * \begin{array}{l l l}
 ...
 * \f]
 *
 * where \c n is the number of trials, and \c p is the probability of
 * success. The algorithm used to generate the random numbers begins with
 * random numbers generated from a uniform distribution on the interval [0,
 * 1]. Thus, the speed and statistical properties of the binomial random
 * number generator may be changed by varying the Generator template
 * parameter responsible for generating the uniformly distributed numbers.
 *
 * The class Generator must be a function object whose function call
 * operator returns a uniform random double between 0 and 1. The class
...
public:
    ...
    /**
     * Constructs a generator with underlying uniform [0, 1] generator \a g,
     * number of trials = \a n, and probability of success = \a p.
     */
