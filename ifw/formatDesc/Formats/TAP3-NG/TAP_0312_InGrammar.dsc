...
/////////////////////////////////////////////////////////////////////////
  include "RAP_0105_FatalReturn.dsc";
  ...
  Long toReturnTapDecimalPlaces;
  String toReturn_TapSeqNum;
  String toReturnTapCurrency;
  String theRecipient;
  String theSender;
  ...
  function Bool onRollback
  {
    ...
    rlbkStr = edrRollbackReason(); //Rollback from validation script

    if( strSearchRegExpr ( rlbkStr, "TransferBatch|BatchControlInfo|AccountingInfo|NetworkInfo|MessageDescription|AuditControlInfo|Notification") >= 0)
    { 

    String transID      = longToStr(decimalToLong( tamTransId() ));
    ...
    String toReturn_StreamName     = CURRENT_STREAM_NAME;

    // set the RAP Sender and Recipient field
    String toReturn_Sender        = theRecipient;
    String toReturn_Recipient     = theSender;

    // get the real name of the rollbacked files that need to be post-processed
    if (toReturn_StreamName != "")
    {
      // Create the RAP file only if the stream is defined
      if (strRapOutStreamPath != "")
      {
        //Create the Rap file Name
        ...
        if (strRapSequencer != "")
        {
                       ...
                       sendArray, receiveArray) == true )
          {
            ...
            String msgBuffer="ROLLBACK: stream(" +toReturn_StreamName+
                        "), The RAP file sent back with a RapSequence(" +toReturn_RapSeqNum+")";
        ...
        if (toReturn_RapSeqNum != "")
        {
          ...
          toReturn_TapSeqNum = strPad(toReturn_TapSeqNum, "0", 5, false);
          ...
          writeRapFile( toReturn_RapFileName,
                        toReturn_Sender,
                        toReturn_Recipient,
                        toReturn_RapSeqNum,
                        sysdate(),
                        ...
                        1,      //RapSpecificationVersionNumber,
                        5,      //RapReleaseVersionNumber,
                        " ",    //typeInd
                        toReturn_TapSeqNum,
                        toReturnTapDecimalPlaces,
                        toReturnTapCurrency,
                        0,      //totRetCharge
                        1,      //rapDetWritten
                        0,      //pathItemId
          ...
          String msgBuffer="ROLLBACK: Returned stream("+toReturn_StreamName+
                    ") - RAPfile("+toReturn_RapFileName+
                    ") using sequence("+toReturn_RapSeqNum+")";
  ...
  function onField(String currToken)
  {
    switch (currToken)
    {
      case "TAP_0312.SpecificationVersionNumber.SpecificationVersionNumber":
      {
        if ( currContext == "HEADER.notification.specificationVersionNumber" or
             currContext == "HEADER.batchControlInfo.specificationVersionNumber" )
        {
      ...
      case "TAP_0312.ReleaseVersionNumber.ReleaseVersionNumber":
      {
        if ( currContext == "HEADER.notification.releaseVersionNumber" or
             currContext == "HEADER.batchControlInfo.releaseVersionNumber" )
        {
      ...
      case "TAP_0312.FileSequenceNumber.FileSequenceNumber":
      {
        if ( currContext == "HEADER.notification.fileSequenceNumber" or
             currContext == "HEADER.batchControlInfo.fileSequenceNumber" )
        {
          toReturn_TapSeqNum = longToStr( strToLong( edrStringEx( currContext, indices, nIndices )));
        }
      }
      break;
      
      case "TAP_0312.Sender.Sender":
      {
        if ( currContext == "HEADER.notification.sender" or
             currContext == "HEADER.batchControlInfo.sender" )
        {
          theSender = edrStringEx ( currContext, indices, nIndices );
        }
      }
      break;
      
      case "TAP_0312.Recipient.Recipient":
      {
        if ( currContext == "HEADER.notification.recipient" or
             currContext == "HEADER.batchControlInfo.recipient" )
        {
          theRecipient = edrStringEx ( currContext, indices, nIndices );
        }
      }
      break;
    
      case "TAP_0312.TapDecimalPlaces.TapDecimalPlaces":
      {
        if ( currContext == "HEADER.accountingInfo.tapDecimalPlaces" )
        {
          toReturnTapDecimalPlaces = edrLongEx ( currContext, indices, nIndices );
        }
      }
      break;

      case "TAP_0312.TapCurrency.TapCurrency":
      {
        if ( currContext == "HEADER.accountingInfo.tapCurrency" )
        {
          toReturnTapCurrency = edrStringEx ( currContext, indices, nIndices );
        }
      }
      break;
  ...
  function onBlockEnd(String currToken)
  {
    switch (currToken)
    {
      case "TAP_0312.Notification":
      {
        edrNew(TRAILER, CONTAINER_TRAILER);

        edrLong(TRAILER.NOTIFICATION_END_INDEX) = edrCurrentTokenIndex();
      }
      break;

      case "TAP_0312.BatchControlInfo":
      {
        edrLong(HEADER.BATCH_CTRL_INFO_END_INDEX) = edrCurrentTokenIndex();
      }
      break;

      case "TAP_0312.AccountingInfo":
      {
        edrLong(HEADER.ACCOUNTING_INFO_END_INDEX) = edrCurrentTokenIndex();
      }
      break;

      case "TAP_0312.NetworkInfo":
      {
        edrLong(HEADER.NETWORK_INFO_END_INDEX) = edrCurrentTokenIndex(); 
      }
      break;

      case "TAP_0312.MessageDescriptionInfoList":
      {
       edrLong(HEADER.MESSAGE_DESCRIPTION_END_INDEX) = edrCurrentTokenIndex(); 
      }
      break;

      case "TAP_0312.AuditControlInfo":
      {
        edrLong(TRAILER.AUDIT_CONTROL_INFO_END_INDEX) = edrCurrentTokenIndex(); 
      }
      break;
    }
  }
  ...
  function onBlockStart(String currToken)
  {
    switch (currToken)
    {
      case "TAP_0312.Notification":
      {
        ...
        currContext = "HEADER.notification";
        indices[0] = 0; nIndices = 1;

        edrAddDatablockEx(currContext, indices, 0);

        edrLong(HEADER.NOTIFICATION_START_INDEX) = edrCurrentTokenIndex();
      }
      break;
      ...
      case "TAP_0312.BatchControlInfo":
      {
        ...
        currContext = "HEADER.batchControlInfo";
        indices[0] = 0; nIndices = 1;

        edrAddDatablockEx(currContext, indices, 0);

        edrLong(HEADER.BATCH_CTRL_INFO_START_INDEX) = edrCurrentTokenIndex();
      }
      break;

      case "TAP_0312.AccountingInfo":
      {
        currContext = "HEADER.accountingInfo";
        indices[0] = 0; nIndices = 1;

        edrAddDatablockEx(currContext, indices, 0);

        edrLong(HEADER.ACCOUNTING_INFO_START_INDEX) = edrCurrentTokenIndex();
      }
      break;

      case "TAP_0312.NetworkInfo":
      {
        currContext = "HEADER.networkInfo";
        indices[0] = 0; nIndices = 1;

        edrAddDatablockEx(currContext, indices, 0);

        edrLong(HEADER.NETWORK_INFO_START_INDEX) = edrCurrentTokenIndex(); 
      }
      break;

      case "TAP_0312.MessageDescriptionInfoList":
      {
        currContext = "HEADER.messageDescriptionInfo";
        indices[0] = 0; nIndices = 1;

        edrAddDatablockEx(currContext, indices, 0);

        edrLong(HEADER.MESSAGE_DESCRIPTION_START_INDEX) = edrCurrentTokenIndex(); 
      }
      break;

      case "TAP_0312.MobileOriginatedCall":
      {
      ...
      case "TAP_0312.MobileTerminatedCall":
      {
        edrNew(DETAIL, CONTAINER_DETAIL);
      ...
      case "TAP_0312.SupplServiceEvent":
      {
        edrNew(DETAIL, CONTAINER_DETAIL);
      ...
      case "TAP_0312.ServiceCentreUsage":
      {
        edrNew(DETAIL, CONTAINER_DETAIL);
      ...
      case "TAP_0312.ContentTransaction":
      {
        edrNew(DETAIL, CONTAINER_DETAIL);
      ...
      case "TAP_0312.MessagingEvent":
      {
        edrNew(DETAIL, CONTAINER_DETAIL);

        currContext = "DETAIL.messagingEvent";
        indices[0] = 0; nIndices = 1;
      ...
      case "TAP_0312.MobileSession":
      {
        edrNew(DETAIL, CONTAINER_DETAIL);

        currContext = "DETAIL.mobileSession";
        indices[0] = 0; nIndices = 1;
      ...
      case "TAP_0312.AuditControlInfo":
      {
        ...
        currContext = "TRAILER.auditControlInfo";
        indices[0] = 0; nIndices = 1;

        edrAddDatablockEx(currContext, indices, 0);

        edrLong(TRAILER.AUDIT_CONTROL_INFO_START_INDEX) = edrCurrentTokenIndex();
      }
      break;
    }
  }
...
/////////////////////////////////////////////////////////////////////////
  ...
  AbsoluteAmount:                                                             
      TAP_0312.AbsoluteAmount                                        
      {                                                                          
        __onField("TAP_0312.AbsoluteAmount.AbsoluteAmount");    
  ...
  AccessPointNameNI:                                                             
      TAP_0312.AccessPointNameNI                                        
      {                                                                          
        __onField("TAP_0312.AccessPointNameNI.AccessPointNameNI");    
  ...
  AccessPointNameOI:                                                             
      TAP_0312.AccessPointNameOI                                        
      {                                                                          
        __onField("TAP_0312.AccessPointNameOI.AccessPointNameOI");    
  ...
  AccountingInfo:                                               
      TAP_0312.AccountingInfo                                 
      {                                                                   
        __onBlockStart("TAP_0312.AccountingInfo", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.AccountingInfo");     
  ...
  ActualDeliveryTimeStamp:                                               
      TAP_0312.ActualDeliveryTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.ActualDeliveryTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ActualDeliveryTimeStamp");     
  ...
  AddressStringDigits:                                                             
      TAP_0312.AddressStringDigits                                        
      {                                                                          
        __onField("TAP_0312.AddressStringDigits.AddressStringDigits");    
  ...
  AdvisedCharge:                                                             
      TAP_0312.AdvisedCharge                                        
      {                                                                          
        __onField("TAP_0312.AdvisedCharge.AdvisedCharge");    
  ...
  AdvisedChargeCurrency:                                                             
      TAP_0312.AdvisedChargeCurrency                                        
      {                                                                          
        __onField("TAP_0312.AdvisedChargeCurrency.AdvisedChargeCurrency");    
  ...
  AdvisedChargeInformation:                                               
      TAP_0312.AdvisedChargeInformation                                 
      {                                                                   
        __onBlockStart("TAP_0312.AdvisedChargeInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.AdvisedChargeInformation");     
  ...
  AgeOfLocation:                                                             
      TAP_0312.AgeOfLocation                                        
      {                                                                          
        __onField("TAP_0312.AgeOfLocation.AgeOfLocation");    
  ...
  AsciiString:                                                             
      TAP_0312.AsciiString                                        
      {                                                                          
        __onField("TAP_0312.AsciiString.AsciiString");    
  ...
  AuditControlInfo:                                               
      TAP_0312.AuditControlInfo                                 
      {                                                                   
        __onBlockStart("TAP_0312.AuditControlInfo", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.AuditControlInfo");     
  ...
  BCDString:                                                             
      TAP_0312.BCDString                                        
      {                                                                          
        __onField("TAP_0312.BCDString.BCDString");    
  ...
  BasicService:                                               
      TAP_0312.BasicService                                 
      {                                                                   
        __onBlockStart("TAP_0312.BasicService", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.BasicService");     
  ...
  BasicServiceCode:                                                       
      TAP_0312.BasicServiceCode                                  
      {                                                                    
        __onBlockStart("TAP_0312.BasicServiceCode", false);    
        //logStdout("Parsed Choice BasicServiceCode\n");                 
      }                                                         
      BasicServiceCode_elems                                   
      ASN1.BlockEnd                                                
      {                                                         
        __onBlockEnd("TAP_0312.BasicServiceCode");  
  ...
  BasicServiceCodeList_opt:                                                                
      TAP_0312.BasicServiceCodeList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.BasicServiceCodeList", true);                  
        //logStdout("Parsed List BasicServiceCodeList\n");                                
      }                                                                                 
      BasicServiceCodeList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.BasicServiceCodeList");                          
  ...
  BasicServiceUsed:                                               
      TAP_0312.BasicServiceUsed                                 
      {                                                                   
        __onBlockStart("TAP_0312.BasicServiceUsed", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.BasicServiceUsed");     
  ...
  BasicServiceUsedList_opt:                                                                
      ...
      {                                                                                 
        __onBlockStart("TAP_0312.BasicServiceUsedList", true);                  
        //logStdout("Parsed List BasicServiceUsedList\n");                                
      }                                                                                 
      BasicServiceUsedList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.BasicServiceUsedList");                          
  ...
  BatchControlInfo:                                               
      TAP_0312.BatchControlInfo                                 
      {                                                                   
        __onBlockStart("TAP_0312.BatchControlInfo", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.BatchControlInfo");     
  ...
  BearerServiceCode:                                                             
      TAP_0312.BearerServiceCode                                        
      {                                                                          
        __onField("TAP_0312.BearerServiceCode.BearerServiceCode");    
  ...
  CallEventDetail_elems:            
    ...
    | messagingEvent_CallEventDetail     
    | mobileSession_CallEventDetail     
    ;                                 
  ...
  CallEventDetailList_opt:                                                                
      TAP_0312.CallEventDetailList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.CallEventDetailList", true);                  
        //logStdout("Parsed List CallEventDetailList\n");                                
      }                                                                                 
      CallEventDetailList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.CallEventDetailList");                          
        //logStdout("CallEventDetailList List END.\n");                                  
      }                                                                                 
    |                                                                                   
    ;                                                                                   
  ...
  CallEventDetailList:                                                                    
      CallEventDetailList                                                                 
      CallEventDetail_CallEventDetailList                                                    
    |                                                                                   
    ;                                                                                   
  ...
  CallEventDetailsCount:                                                             
      TAP_0312.CallEventDetailsCount                                        
      {                                                                          
        __onField("TAP_0312.CallEventDetailsCount.CallEventDetailsCount");    
  ...
  CallEventStartTimeStamp:                                               
      TAP_0312.CallEventStartTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.CallEventStartTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.CallEventStartTimeStamp");     
  ...
  CallOriginator:                                               
      TAP_0312.CallOriginator                                 
      {                                                                   
        __onBlockStart("TAP_0312.CallOriginator", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.CallOriginator");     
  ...
  CallReference:                                                             
      TAP_0312.CallReference                                        
      {                                                                          
        __onField("TAP_0312.CallReference.CallReference");    
  ...
  CallTypeGroup:                                               
      TAP_0312.CallTypeGroup                                 
      {                                                                   
        __onBlockStart("TAP_0312.CallTypeGroup", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.CallTypeGroup");     
  ...
  CallTypeLevel1:                                                             
      TAP_0312.CallTypeLevel1                                        
      {                                                                          
        __onField("TAP_0312.CallTypeLevel1.CallTypeLevel1");    
  ...
  CallTypeLevel2:                                                             
      TAP_0312.CallTypeLevel2                                        
      {                                                                          
        __onField("TAP_0312.CallTypeLevel2.CallTypeLevel2");    
  ...
  CallTypeLevel3:                                                             
      TAP_0312.CallTypeLevel3                                        
      {                                                                          
        __onField("TAP_0312.CallTypeLevel3.CallTypeLevel3");    
  ...
  CalledNumber:                                                             
      TAP_0312.CalledNumber                                        
      {                                                                          
        __onField("TAP_0312.CalledNumber.CalledNumber");    
  ...
  CalledPlace:                                                             
      TAP_0312.CalledPlace                                        
      {                                                                          
        __onField("TAP_0312.CalledPlace.CalledPlace");    
  ...
  CalledRegion:                                                             
      TAP_0312.CalledRegion                                        
      {                                                                          
        __onField("TAP_0312.CalledRegion.CalledRegion");    
  ...
  CallingNumber:                                                             
      TAP_0312.CallingNumber                                        
      {                                                                          
        __onField("TAP_0312.CallingNumber.CallingNumber");    
  ...
  CamelDestinationNumber:                                                             
      TAP_0312.CamelDestinationNumber                                        
      {                                                                          
        __onField("TAP_0312.CamelDestinationNumber.CamelDestinationNumber");    
  ...
  CamelInvocationFee:                                                             
      TAP_0312.CamelInvocationFee                                        
      {                                                                          
        __onField("TAP_0312.CamelInvocationFee.CamelInvocationFee");    
  ...
  CamelServiceKey:                                                             
      TAP_0312.CamelServiceKey                                        
      {                                                                          
        __onField("TAP_0312.CamelServiceKey.CamelServiceKey");    
  ...
  CamelServiceLevel:                                                             
      TAP_0312.CamelServiceLevel                                        
      {                                                                          
        __onField("TAP_0312.CamelServiceLevel.CamelServiceLevel");    
  ...
  CamelServiceUsed:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.CamelServiceUsed", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.CamelServiceUsed");     
  ...
  CauseForTerm:                                                             
      TAP_0312.CauseForTerm                                        
      {                                                                          
        __onField("TAP_0312.CauseForTerm.CauseForTerm");    
  ...
  CellId:                                                             
      TAP_0312.CellId                                        
      {                                                                          
        __onField("TAP_0312.CellId.CellId");    
  ...
  Charge:                                                             
      TAP_0312.Charge                                        
      {                                                                          
        __onField("TAP_0312.Charge.Charge");    
  ...
  ChargeDetail:                                               
      TAP_0312.ChargeDetail                                 
      {                                                                   
        __onBlockStart("TAP_0312.ChargeDetail", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ChargeDetail");     
  ...
  ChargeDetailList_opt:                                                                
      TAP_0312.ChargeDetailList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.ChargeDetailList", true);                  
        //logStdout("Parsed List ChargeDetailList\n");                                
      }                                                                                 
      ChargeDetailList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.ChargeDetailList");                          
  ...
  ChargeDetailTimeStamp:                                               
      TAP_0312.ChargeDetailTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.ChargeDetailTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ChargeDetailTimeStamp");     
  ...
  ChargeInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.ChargeInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ChargeInformation");     
  ...
  ChargeInformationList_opt:                                                                
      ...
      {                                                                                 
        __onBlockStart("TAP_0312.ChargeInformationList", true);                  
        //logStdout("Parsed List ChargeInformationList\n");                                
      }                                                                                 
      ChargeInformationList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.ChargeInformationList");                          
  ...
  ChargeRefundIndicator:                                                             
      TAP_0312.ChargeRefundIndicator                                        
      {                                                                          
        __onField("TAP_0312.ChargeRefundIndicator.ChargeRefundIndicator");    
  ...
  ChargeType:                                                             
      TAP_0312.ChargeType                                        
      {                                                                          
        __onField("TAP_0312.ChargeType.ChargeType");    
  ...
  ChargeableSubscriber:                                                       
      TAP_0312.ChargeableSubscriber                                  
      {                                                                    
        __onBlockStart("TAP_0312.ChargeableSubscriber", false);    
        //logStdout("Parsed Choice ChargeableSubscriber\n");                 
      }                                                         
      ChargeableSubscriber_elems                                   
      ASN1.BlockEnd                                                
      {                                                         
        __onBlockEnd("TAP_0312.ChargeableSubscriber");  
  ...
  ChargeableUnits:                                                             
      TAP_0312.ChargeableUnits                                        
      {                                                                          
        __onField("TAP_0312.ChargeableUnits.ChargeableUnits");    
  ...
  ChargedItem:                                                             
      TAP_0312.ChargedItem                                        
      {                                                                          
        __onField("TAP_0312.ChargedItem.ChargedItem");    
  ...
  ChargedParty:                                               
      TAP_0312.ChargedParty                                 
      {                                                                   
        __onBlockStart("TAP_0312.ChargedParty", false);   
        //logStdout("Parsed Sequence ChargedParty\n");              
      }                                                                   
      imsi_ChargedParty     
      msisdn_ChargedParty     
      publicUserId_ChargedParty     
      homeBid_ChargedParty     
      homeLocationDescription_ChargedParty     
      imei_ChargedParty     
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ChargedParty");     
        //logStdout("ChargedParty Sequence END.\n");         
      }                                                            
    ;                                                              
  ...
  ChargedPartyEquipment:                                               
      TAP_0312.ChargedPartyEquipment                                 
      {                                                                   
        __onBlockStart("TAP_0312.ChargedPartyEquipment", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ChargedPartyEquipment");     
  ...
  ChargedPartyHomeIdList_opt:                                                                
      TAP_0312.ChargedPartyHomeIdList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.ChargedPartyHomeIdList", true);                  
        //logStdout("Parsed List ChargedPartyHomeIdList\n");                                
      }                                                                                 
      ChargedPartyHomeIdList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.ChargedPartyHomeIdList");                          
  ...
  ChargedPartyHomeIdentification:                                               
      TAP_0312.ChargedPartyHomeIdentification                                 
      {                                                                   
        __onBlockStart("TAP_0312.ChargedPartyHomeIdentification", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ChargedPartyHomeIdentification");     
  ...
  ChargedPartyIdList_opt:                                                                
      TAP_0312.ChargedPartyIdList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.ChargedPartyIdList", true);                  
        //logStdout("Parsed List ChargedPartyIdList\n");                                
      }                                                                                 
      ChargedPartyIdList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.ChargedPartyIdList");                          
  ...
  ChargedPartyIdType:                                                             
      TAP_0312.ChargedPartyIdType                                        
      {                                                                          
        __onField("TAP_0312.ChargedPartyIdType.ChargedPartyIdType");    
  ...
  ChargedPartyIdentification:                                               
      TAP_0312.ChargedPartyIdentification                                 
      {                                                                   
        __onBlockStart("TAP_0312.ChargedPartyIdentification", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ChargedPartyIdentification");     
  ...
  ChargedPartyIdentifier:                                                             
      TAP_0312.ChargedPartyIdentifier                                        
      {                                                                          
        __onField("TAP_0312.ChargedPartyIdentifier.ChargedPartyIdentifier");    
  ...
  ChargedPartyInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.ChargedPartyInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ChargedPartyInformation");     
  ...
  ChargedPartyLocation:                                               
      TAP_0312.ChargedPartyLocation                                 
      {                                                                   
        __onBlockStart("TAP_0312.ChargedPartyLocation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ChargedPartyLocation");     
  ...
  ChargedPartyLocationList_opt:                                                                
      TAP_0312.ChargedPartyLocationList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.ChargedPartyLocationList", true);                  
        //logStdout("Parsed List ChargedPartyLocationList\n");                                
      }                                                                                 
      ChargedPartyLocationList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.ChargedPartyLocationList");                          
  ...
  ChargedPartyStatus:                                                             
      TAP_0312.ChargedPartyStatus                                        
      {                                                                          
        __onField("TAP_0312.ChargedPartyStatus.ChargedPartyStatus");    
  ...
  ChargedUnits:                                                             
      TAP_0312.ChargedUnits                                        
      {                                                                          
        __onField("TAP_0312.ChargedUnits.ChargedUnits");    
  ...
  ChargingId:                                                             
      TAP_0312.ChargingId                                        
      {                                                                          
        __onField("TAP_0312.ChargingId.ChargingId");    
  ...
  ChargingPoint:                                                             
      TAP_0312.ChargingPoint                                        
      {                                                                          
        __onField("TAP_0312.ChargingPoint.ChargingPoint");    
  ...
  ChargingTimeStamp:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.ChargingTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ChargingTimeStamp");     
  ...
  ClirIndicator:                                                             
      TAP_0312.ClirIndicator                                        
      {                                                                          
        __onField("TAP_0312.ClirIndicator.ClirIndicator");    
  ...
  Commission:                                                             
      TAP_0312.Commission                                        
      {                                                                          
        __onField("TAP_0312.Commission.Commission");    
  ...
  CompletionTimeStamp:                                               
      TAP_0312.CompletionTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.CompletionTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.CompletionTimeStamp");     
  ...
  ContentChargingPoint:                                                             
      TAP_0312.ContentChargingPoint                                        
      {                                                                          
        __onField("TAP_0312.ContentChargingPoint.ContentChargingPoint");    
  ...
  ContentProvider:                                               
      TAP_0312.ContentProvider                                 
      {                                                                   
        __onBlockStart("TAP_0312.ContentProvider", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ContentProvider");     
  ...
  ContentProviderIdList_opt:                                                                
      TAP_0312.ContentProviderIdList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.ContentProviderIdList", true);                  
        //logStdout("Parsed List ContentProviderIdList\n");                                
      }                                                                                 
      ContentProviderIdList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.ContentProviderIdList");                          
  ...
  ContentProviderIdType:                                                             
      TAP_0312.ContentProviderIdType                                        
      {                                                                          
        __onField("TAP_0312.ContentProviderIdType.ContentProviderIdType");    
  ...
  ContentProviderIdentifier:                                                             
      TAP_0312.ContentProviderIdentifier                                        
      {                                                                          
        __onField("TAP_0312.ContentProviderIdentifier.ContentProviderIdentifier");    
  ...
  ContentProviderName:                                                             
      TAP_0312.ContentProviderName                                        
      {                                                                          
        __onField("TAP_0312.ContentProviderName.ContentProviderName");    
  ...
  ContentServiceUsed:                                               
      TAP_0312.ContentServiceUsed                                 
      {                                                                   
        __onBlockStart("TAP_0312.ContentServiceUsed", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ContentServiceUsed");     
  ...
  ContentServiceUsedList_opt:                                                                
      ...
      {                                                                                 
        __onBlockStart("TAP_0312.ContentServiceUsedList", true);                  
        //logStdout("Parsed List ContentServiceUsedList\n");                                
      }                                                                                 
      ContentServiceUsedList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.ContentServiceUsedList");                          
  ...
  ContentTransaction:                                               
      TAP_0312.ContentTransaction                                 
      {                                                                   
        __onBlockStart("TAP_0312.ContentTransaction", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ContentTransaction");     
  ...
  ContentTransactionBasicInfo:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.ContentTransactionBasicInfo", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ContentTransactionBasicInfo");     
  ...
  ContentTransactionCode:                                                             
      TAP_0312.ContentTransactionCode                                        
      {                                                                          
        __onField("TAP_0312.ContentTransactionCode.ContentTransactionCode");    
  ...
  ContentTransactionType:                                                             
      TAP_0312.ContentTransactionType                                        
      {                                                                          
        __onField("TAP_0312.ContentTransactionType.ContentTransactionType");    
  ...
  CseInformation:                                                             
      TAP_0312.CseInformation                                        
      {                                                                          
        __onField("TAP_0312.CseInformation.CseInformation");    
  ...
  Currency:                                                             
      TAP_0312.Currency                                        
      {                                                                          
        __onField("TAP_0312.Currency.Currency");    
  ...
  CurrencyConversion:                                               
      TAP_0312.CurrencyConversion                                 
      {                                                                   
        __onBlockStart("TAP_0312.CurrencyConversion", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.CurrencyConversion");     
  ...
  CurrencyConversionList_opt:                                                                
      TAP_0312.CurrencyConversionList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.CurrencyConversionList", true);                  
        //logStdout("Parsed List CurrencyConversionList\n");                                
      }                                                                                 
      CurrencyConversionList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.CurrencyConversionList");                          
  ...
  CustomerIdType:                                                             
      TAP_0312.CustomerIdType                                        
      {                                                                          
        __onField("TAP_0312.CustomerIdType.CustomerIdType");    
  ...
  CustomerIdentifier:                                                             
      TAP_0312.CustomerIdentifier                                        
      {                                                                          
        __onField("TAP_0312.CustomerIdentifier.CustomerIdentifier");    
  ...
  DataInterChange_elems:            
      transferBatch_DataInterChange     
    | notification_DataInterChange     
    ;                                 
                                      
  transferBatch_DataInterChange:                                                       
      {                                                                                 
        currBlock = "transferBatch";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      TransferBatch                                                           
    ;                                                                                   
  ...
  DataVolume:                                                             
      TAP_0312.DataVolume                                        
      {                                                                          
        __onField("TAP_0312.DataVolume.DataVolume");    
  ...
  DataVolumeIncoming:                                                             
      TAP_0312.DataVolumeIncoming                                        
      {                                                                          
        __onField("TAP_0312.DataVolumeIncoming.DataVolumeIncoming");    
  ...
  DataVolumeOutgoing:                                                             
      TAP_0312.DataVolumeOutgoing                                        
      {                                                                          
        __onField("TAP_0312.DataVolumeOutgoing.DataVolumeOutgoing");    
  ...
  DateTime:                                               
      TAP_0312.DateTime                                 
      {                                                                   
        __onBlockStart("TAP_0312.DateTime", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.DateTime");     
  ...
  DateTimeLong:                                               
      TAP_0312.DateTimeLong                                 
      {                                                                   
        __onBlockStart("TAP_0312.DateTimeLong", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.DateTimeLong");     
  ...
  DefaultCallHandlingIndicator:                                                             
      TAP_0312.DefaultCallHandlingIndicator                                        
      {                                                                          
        __onField("TAP_0312.DefaultCallHandlingIndicator.DefaultCallHandlingIndicator");    
  ...
  DepositTimeStamp:                                               
      TAP_0312.DepositTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.DepositTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.DepositTimeStamp");     
  ...
  Destination:                                               
      TAP_0312.Destination                                 
      {                                                                   
        __onBlockStart("TAP_0312.Destination", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.Destination");     
  ...
  DestinationNetwork:                                                             
      TAP_0312.DestinationNetwork                                        
      {                                                                          
        __onField("TAP_0312.DestinationNetwork.DestinationNetwork");    
  ...
  DialledDigits:                                                             
      TAP_0312.DialledDigits                                        
      {                                                                          
        __onField("TAP_0312.DialledDigits.DialledDigits");    
  ...
  Discount:                                                             
      TAP_0312.Discount                                        
      {                                                                          
        __onField("TAP_0312.Discount.Discount");    
  ...
  DiscountApplied:                                                       
      TAP_0312.DiscountApplied                                  
      {                                                                    
        __onBlockStart("TAP_0312.DiscountApplied", false);    
        //logStdout("Parsed Choice DiscountApplied\n");                 
      }                                                         
      DiscountApplied_elems                                   
      ASN1.BlockEnd                                                
      {                                                         
        __onBlockEnd("TAP_0312.DiscountApplied");  
  ...
  DiscountCode:                                                             
      TAP_0312.DiscountCode                                        
      {                                                                          
        __onField("TAP_0312.DiscountCode.DiscountCode");    
  ...
  DiscountInformation:                                               
      TAP_0312.DiscountInformation                                 
      {                                                                   
        __onBlockStart("TAP_0312.DiscountInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.DiscountInformation");     
  ...
  DiscountRate:                                                             
      TAP_0312.DiscountRate                                        
      {                                                                          
        __onField("TAP_0312.DiscountRate.DiscountRate");    
  ...
  DiscountableAmount:                                                             
      TAP_0312.DiscountableAmount                                        
      {                                                                          
        __onField("TAP_0312.DiscountableAmount.DiscountableAmount");    
  ...
  Discounting:                                               
      TAP_0312.Discounting                                 
      {                                                                   
        __onBlockStart("TAP_0312.Discounting", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.Discounting");     
  ...
  DiscountingList_opt:                                                                
      TAP_0312.DiscountingList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.DiscountingList", true);                  
        //logStdout("Parsed List DiscountingList\n");                                
      }                                                                                 
      DiscountingList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.DiscountingList");                          
  ...
  DistanceChargeBandCode:                                                             
      TAP_0312.DistanceChargeBandCode                                        
      {                                                                          
        __onField("TAP_0312.DistanceChargeBandCode.DistanceChargeBandCode");    
  ...
  EarliestCallTimeStamp:                                               
      TAP_0312.EarliestCallTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.EarliestCallTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.EarliestCallTimeStamp");     
  ...
  ElementId:                                                             
      TAP_0312.ElementId                                        
      {                                                                          
        __onField("TAP_0312.ElementId.ElementId");    
        //logStdout("Parsed Leaf ElementId\n");                         
      }                                                                          
    ;                                                                            
  ...
  ElementType:                                                             
      TAP_0312.ElementType                                        
      {                                                                          
        __onField("TAP_0312.ElementType.ElementType");    
        //logStdout("Parsed Leaf ElementType\n");                         
      }                                                                          
    ;                                                                            
  ...
  EquipmentId:                                                             
      TAP_0312.EquipmentId                                        
      {                                                                          
        __onField("TAP_0312.EquipmentId.EquipmentId");    
  ...
  EquipmentIdType:                                                             
      TAP_0312.EquipmentIdType                                        
      {                                                                          
        __onField("TAP_0312.EquipmentIdType.EquipmentIdType");    
  ...
  EventReference:                                                             
      TAP_0312.EventReference                                        
      {                                                                          
        __onField("TAP_0312.EventReference.EventReference");    
        //logStdout("Parsed Leaf EventReference\n");                         
      }                                                                          
    ;                                                                            
  ...
  ExchangeRate:                                                             
      TAP_0312.ExchangeRate                                        
      {                                                                          
        __onField("TAP_0312.ExchangeRate.ExchangeRate");    
  ...
  ExchangeRateCode:                                                             
      TAP_0312.ExchangeRateCode                                        
      {                                                                          
        __onField("TAP_0312.ExchangeRateCode.ExchangeRateCode");    
  ...
  FileAvailableTimeStamp:                                               
      TAP_0312.FileAvailableTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.FileAvailableTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.FileAvailableTimeStamp");     
  ...
  FileCreationTimeStamp:                                               
      TAP_0312.FileCreationTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.FileCreationTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.FileCreationTimeStamp");     
  ...
  FileSequenceNumber:                                                             
      TAP_0312.FileSequenceNumber                                        
      {                                                                          
        __onField("TAP_0312.FileSequenceNumber.FileSequenceNumber");    
  ...
  FileTypeIndicator:                                                             
      TAP_0312.FileTypeIndicator                                        
      {                                                                          
        __onField("TAP_0312.FileTypeIndicator.FileTypeIndicator");    
  ...
  FixedDiscountValue:                                                             
      TAP_0312.FixedDiscountValue                                        
      {                                                                          
        __onField("TAP_0312.FixedDiscountValue.FixedDiscountValue");    
  ...
  GeographicalLocation:                                               
      TAP_0312.GeographicalLocation                                 
      {                                                                   
        __onBlockStart("TAP_0312.GeographicalLocation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.GeographicalLocation");     
  ...
  GprsBasicCallInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.GprsBasicCallInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.GprsBasicCallInformation");     
  ...
  GprsCall:                                               
      TAP_0312.GprsCall                                 
      {                                                                   
        __onBlockStart("TAP_0312.GprsCall", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.GprsCall");     
  ...
  GprsChargeableSubscriber:                                               
      TAP_0312.GprsChargeableSubscriber                                 
      {                                                                   
        __onBlockStart("TAP_0312.GprsChargeableSubscriber", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.GprsChargeableSubscriber");     
  ...
  GprsDestination:                                               
      TAP_0312.GprsDestination                                 
      {                                                                   
        __onBlockStart("TAP_0312.GprsDestination", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.GprsDestination");     
  ...
  GprsLocationInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.GprsLocationInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.GprsLocationInformation");     
  ...
  GprsNetworkLocation:                                               
      TAP_0312.GprsNetworkLocation                                 
      {                                                                   
        __onBlockStart("TAP_0312.GprsNetworkLocation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.GprsNetworkLocation");     
  ...
  GprsServiceUsed:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.GprsServiceUsed", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.GprsServiceUsed");     
  ...
  GsmChargeableSubscriber:                                               
      TAP_0312.GsmChargeableSubscriber                                 
      {                                                                   
        __onBlockStart("TAP_0312.GsmChargeableSubscriber", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.GsmChargeableSubscriber");     
  ...
  GuaranteedBitRate:                                                             
      TAP_0312.GuaranteedBitRate                                        
      {                                                                          
        __onField("TAP_0312.GuaranteedBitRate.GuaranteedBitRate");    
  ...
  HSCSDIndicator:                                                             
      TAP_0312.HSCSDIndicator                                        
      {                                                                          
        __onField("TAP_0312.HSCSDIndicator.HSCSDIndicator");    
  ...
  HexString:                                                             
      TAP_0312.HexString                                        
      {                                                                          
        __onField("TAP_0312.HexString.HexString");    
  ...
  HomeBid:                                                             
      TAP_0312.HomeBid                                        
      {                                                                          
        __onField("TAP_0312.HomeBid.HomeBid");    
  ...
  HomeIdType:                                                             
      TAP_0312.HomeIdType                                        
      {                                                                          
        __onField("TAP_0312.HomeIdType.HomeIdType");    
  ...
  HomeIdentifier:                                                             
      TAP_0312.HomeIdentifier                                        
      {                                                                          
        __onField("TAP_0312.HomeIdentifier.HomeIdentifier");    
  ...
  HomeLocationDescription:                                                             
      TAP_0312.HomeLocationDescription                                        
      {                                                                          
        __onField("TAP_0312.HomeLocationDescription.HomeLocationDescription");    
  ...
  HomeLocationInformation:                                               
      TAP_0312.HomeLocationInformation                                 
      {                                                                   
        __onBlockStart("TAP_0312.HomeLocationInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.HomeLocationInformation");     
  ...
  HorizontalAccuracyDelivered:                                                             
      TAP_0312.HorizontalAccuracyDelivered                                        
      {                                                                          
        __onField("TAP_0312.HorizontalAccuracyDelivered.HorizontalAccuracyDelivered");    
  ...
  HorizontalAccuracyRequested:                                                             
      TAP_0312.HorizontalAccuracyRequested                                        
      {                                                                          
        __onField("TAP_0312.HorizontalAccuracyRequested.HorizontalAccuracyRequested");    
  ...
  IMSSignallingContext:                                                             
      TAP_0312.IMSSignallingContext                                        
      {                                                                          
        __onField("TAP_0312.IMSSignallingContext.IMSSignallingContext");    
  ...
  ISPList_opt:                                                                
      TAP_0312.ISPList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.ISPList", true);                  
        //logStdout("Parsed List ISPList\n");                                
      }                                                                                 
      ISPList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.ISPList");                          
  ...
  ImeiOrEsn:                                                       
      TAP_0312.ImeiOrEsn                                  
      {                                                                    
        __onBlockStart("TAP_0312.ImeiOrEsn", false);    
        //logStdout("Parsed Choice ImeiOrEsn\n");                 
      }                                                         
      ImeiOrEsn_elems                                   
      ASN1.BlockEnd                                                
      {                                                         
        __onBlockEnd("TAP_0312.ImeiOrEsn");  
  ...
  InternetServiceProvider:                                               
      TAP_0312.InternetServiceProvider                                 
      {                                                                   
        __onBlockStart("TAP_0312.InternetServiceProvider", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.InternetServiceProvider");     
  ...
  InternetServiceProviderIdList_opt:                                                                
      TAP_0312.InternetServiceProviderIdList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.InternetServiceProviderIdList", true);                  
        //logStdout("Parsed List InternetServiceProviderIdList\n");                                
      }                                                                                 
      InternetServiceProviderIdList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.InternetServiceProviderIdList");                          
  ...
  IspIdType:                                                             
      TAP_0312.IspIdType                                        
      {                                                                          
        __onField("TAP_0312.IspIdType.IspIdType");    
  ...
  IspIdentifier:                                                             
      TAP_0312.IspIdentifier                                        
      {                                                                          
        __onField("TAP_0312.IspIdentifier.IspIdentifier");    
  ...
  LCSQosDelivered:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.LCSQosDelivered", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.LCSQosDelivered");     
  ...
  LCSQosRequested:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.LCSQosRequested", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.LCSQosRequested");     
  ...
  LCSRequestTimestamp:                                               
      TAP_0312.LCSRequestTimestamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.LCSRequestTimestamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.LCSRequestTimestamp");     
  ...
  LCSSPIdentification:                                               
      TAP_0312.LCSSPIdentification                                 
      {                                                                   
        __onBlockStart("TAP_0312.LCSSPIdentification", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.LCSSPIdentification");     
  ...
  LCSSPIdentificationList_opt:                                                                
      TAP_0312.LCSSPIdentificationList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.LCSSPIdentificationList", true);                  
        //logStdout("Parsed List LCSSPIdentificationList\n");                                
      }                                                                                 
      LCSSPIdentificationList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.LCSSPIdentificationList");                          
  ...
  LCSSPInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.LCSSPInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.LCSSPInformation");     
  ...
  LCSTransactionStatus:                                                             
      TAP_0312.LCSTransactionStatus                                        
      {                                                                          
        __onField("TAP_0312.LCSTransactionStatus.LCSTransactionStatus");    
  ...
  LatestCallTimeStamp:                                               
      TAP_0312.LatestCallTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.LatestCallTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.LatestCallTimeStamp");     
  ...
  LocalCurrency:                                                             
      TAP_0312.LocalCurrency                                        
      {                                                                          
        __onField("TAP_0312.LocalCurrency.LocalCurrency");    
  ...
  LocalTimeStamp:                                                             
      TAP_0312.LocalTimeStamp                                        
      {                                                                          
        __onField("TAP_0312.LocalTimeStamp.LocalTimeStamp");    
  ...
  LocationArea:                                                             
      TAP_0312.LocationArea                                        
      {                                                                          
        __onField("TAP_0312.LocationArea.LocationArea");    
  ...
  LocationIdType:                                                             
      TAP_0312.LocationIdType                                        
      {                                                                          
        __onField("TAP_0312.LocationIdType.LocationIdType");    
  ...
  LocationIdentifier:                                                             
      TAP_0312.LocationIdentifier                                        
      {                                                                          
        __onField("TAP_0312.LocationIdentifier.LocationIdentifier");    
  ...
  LocationInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.LocationInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.LocationInformation");     
  ...
  LocationService:                                               
      TAP_0312.LocationService                                 
      {                                                                   
        __onBlockStart("TAP_0312.LocationService", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.LocationService");     
  ...
  LocationServiceUsage:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.LocationServiceUsage", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.LocationServiceUsage");     
  ...
  MaximumBitRate:                                                             
      TAP_0312.MaximumBitRate                                        
      {                                                                          
        __onField("TAP_0312.MaximumBitRate.MaximumBitRate");    
  ...
  MessageDescription:                                                             
      TAP_0312.MessageDescription                                        
      {                                                                          
        __onField("TAP_0312.MessageDescription.MessageDescription");    
  ...
  MessageDescriptionCode:                                                             
      TAP_0312.MessageDescriptionCode                                        
      {                                                                          
        __onField("TAP_0312.MessageDescriptionCode.MessageDescriptionCode");    
  ...
  MessageDescriptionInfoList_opt:                                                                
      TAP_0312.MessageDescriptionInfoList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.MessageDescriptionInfoList", true);                  
        //logStdout("Parsed List MessageDescriptionInfoList\n");                                
      }                                                                                 
      MessageDescriptionInfoList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.MessageDescriptionInfoList");                          
  ...
  MessageDescriptionInfoList:                                                                    
      MessageDescriptionInfoList                                                                 
      MessageDescriptionInformation_MessageDescriptionInfoList                                                    
    |                                                                                   
    ;                                                                                   
                                                                                        
  MessageDescriptionInformation_MessageDescriptionInfoList:                                                       
      {                                                                                 
        currBlock = "MessageDescriptionInformation";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      MessageDescriptionInformation                                                           
    ;                                                                                   
                                                                   
  MessageDescriptionInformation_opt:                                           
      MessageDescriptionInformation                                            
    |                                                              
    ;                                                              
                                                                   
  MessageDescriptionInformation:                                               
      TAP_0312.MessageDescriptionInformation                                 
      {                                                                   
        __onBlockStart("TAP_0312.MessageDescriptionInformation", false);   
        //logStdout("Parsed Sequence MessageDescriptionInformation\n");              
      }                                                                   
      messageDescriptionCode_MessageDescriptionInformation     
      messageDescription_MessageDescriptionInformation     
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.MessageDescriptionInformation");     
        //logStdout("MessageDescriptionInformation Sequence END.\n");         
      }                                                            
    ;                                                              
  ...
  MessageStatus:                                                             
      TAP_0312.MessageStatus                                        
      {                                                                          
        __onField("TAP_0312.MessageStatus.MessageStatus");    
  ...
  MessageType:                                                             
      TAP_0312.MessageType                                        
      {                                                                          
        __onField("TAP_0312.MessageType.MessageType");    
  ...
  MessagingEvent:                                               
      TAP_0312.MessagingEvent                                 
      {                                                                   
        __onBlockStart("TAP_0312.MessagingEvent", false);   
        //logStdout("Parsed Sequence MessagingEvent\n");              
      }                                                                   
      messagingEventService_MessagingEvent     
      chargedParty_MessagingEvent     
      rapFileSequenceNumber_MessagingEvent     
      simToolkitIndicator_MessagingEvent     
      geographicalLocation_MessagingEvent     
      eventReference_MessagingEvent     
      recEntityCodeList_MessagingEvent     
      networkElementList_MessagingEvent     
      locationArea_MessagingEvent     
      cellId_MessagingEvent     
      serviceStartTimestamp_MessagingEvent     
      nonChargedParty_MessagingEvent     
      exchangeRateCode_MessagingEvent     
      callTypeGroup_MessagingEvent     
      charge_MessagingEvent     
      taxInformationList_MessagingEvent     
      operatorSpecInformation_MessagingEvent     
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.MessagingEvent");     
        //logStdout("MessagingEvent Sequence END.\n");         
      }                                                            
    ;                                                              
                                                                   
  messagingEventService_MessagingEvent:                                                       
      {                                                                                 
        currBlock = "messagingEventService";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      MessagingEventService_opt                                                       
    ;                                                                                   
                                                                                        
  chargedParty_MessagingEvent:                                                       
      {                                                                                 
        currBlock = "chargedParty";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      ChargedParty_opt                                                       
    ;                                                                                   
  ...
  networkElementList_MessagingEvent:                                                       
      {                                                                                 
        currBlock = "networkElementList";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      NetworkElementList_opt                                                       
    ;                                                                                   
  ...
  serviceStartTimestamp_MessagingEvent:                                                       
      {                                                                                 
        currBlock = "serviceStartTimestamp";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      ServiceStartTimestamp_opt                                                       
    ;                                                                                   
                                                                                        
  nonChargedParty_MessagingEvent:                                                       
      {                                                                                 
        currBlock = "nonChargedParty";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      NonChargedParty_opt                                                       
    ;                                                                                   
  ...
  MessagingEventService:                                                             
      TAP_0312.MessagingEventService                                        
      {                                                                          
        __onField("TAP_0312.MessagingEventService.MessagingEventService");    
        //logStdout("Parsed Leaf MessagingEventService\n");                         
      }                                                                          
    ;                                                                            
  ...
  MinChargeableSubscriber:                                               
      TAP_0312.MinChargeableSubscriber                                 
      {                                                                   
        __onBlockStart("TAP_0312.MinChargeableSubscriber", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.MinChargeableSubscriber");     
  ...
  MoBasicCallInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.MoBasicCallInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.MoBasicCallInformation");     
  ...
  MobileOriginatedCall:                                               
      TAP_0312.MobileOriginatedCall                                 
      {                                                                   
        __onBlockStart("TAP_0312.MobileOriginatedCall", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.MobileOriginatedCall");     
  ...
  MobileSession:                                               
      TAP_0312.MobileSession                                 
      {                                                                   
        __onBlockStart("TAP_0312.MobileSession", false);   
        //logStdout("Parsed Sequence MobileSession\n");              
      }                                                                   
      mobileSessionService_MobileSession     
      chargedParty_MobileSession     
      rapFileSequenceNumber_MobileSession     
      simToolkitIndicator_MobileSession     
      geographicalLocation_MobileSession     
      locationArea_MobileSession     
      cellId_MobileSession     
      eventReference_MobileSession     
      recEntityCodeList_MobileSession     
      serviceStartTimestamp_MobileSession     
      causeForTerm_MobileSession     
      totalCallEventDuration_MobileSession     
      nonChargedParty_MobileSession     
      requestedDestination_MobileSession
      sessionChargeInfoList_MobileSession     
      operatorSpecInformation_MobileSession     
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.MobileSession");     
        //logStdout("MobileSession Sequence END.\n");         
      }                                                            
    ;                                                              
                                                                   
  mobileSessionService_MobileSession:                                                       
      {                                                                                 
        currBlock = "mobileSessionService";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      MobileSessionService_opt                                                       
    ;                                                                                   
  ...
  requestedDestination_MobileSession:
      {
        currBlock = "requestedDestination";
        //logStdout("currContext= " + currContext + "\n");
      }
      RequestedDestination_opt
    ;

  sessionChargeInfoList_MobileSession:                                                       
      {                                                                                 
        currBlock = "sessionChargeInfoList";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      SessionChargeInfoList_opt                                                       
    ;                                                                                   
  ...
  MobileSessionService:                                                             
      TAP_0312.MobileSessionService                                        
      {                                                                          
        __onField("TAP_0312.MobileSessionService.MobileSessionService");    
        //logStdout("Parsed Leaf MobileSessionService\n");                         
      }                                                                          
    ;                                                                            
  ...
  MobileTerminatedCall:                                               
      TAP_0312.MobileTerminatedCall                                 
      {                                                                   
        __onBlockStart("TAP_0312.MobileTerminatedCall", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.MobileTerminatedCall");     
  ...
  Msisdn:                                                             
      TAP_0312.Msisdn                                        
      {                                                                          
        __onField("TAP_0312.Msisdn.Msisdn");    
  ...
  MtBasicCallInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.MtBasicCallInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.MtBasicCallInformation");     
  ...
  Network:                                               
      TAP_0312.Network                                 
      {                                                                   
        __onBlockStart("TAP_0312.Network", false);   
  ...
  NetworkAccessIdentifier:                                                             
      TAP_0312.NetworkAccessIdentifier                                        
      {                                                                          
        __onField("TAP_0312.NetworkAccessIdentifier.NetworkAccessIdentifier");    
  ...
  NetworkElement:                                               
      TAP_0312.NetworkElement                                 
      {                                                                   
        __onBlockStart("TAP_0312.NetworkElement", false);   
        //logStdout("Parsed Sequence NetworkElement\n");              
      }                                                                   
      elementType_NetworkElement     
      elementId_NetworkElement     
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.NetworkElement");     
        //logStdout("NetworkElement Sequence END.\n");         
      }                                                            
    ;                                                              
                                                                   
  elementType_NetworkElement:                                                       
      {                                                                                 
        currBlock = "elementType";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      ElementType_opt                                                       
    ;                                                                                   
  ...
  NetworkElementList_opt:                                                                
      TAP_0312.NetworkElementList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.NetworkElementList", true);                  
        //logStdout("Parsed List NetworkElementList\n");                                
      }                                                                                 
      NetworkElementList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.NetworkElementList");                          
        //logStdout("NetworkElementList List END.\n");                                  
      }                                                                                 
    |                                                                                   
    ;                                                                                   
  ...
  NetworkElementList:                                                                    
      NetworkElementList                                                                 
      NetworkElement_NetworkElementList                                                    
    |                                                                                   
    ;                                                                                   
                                                                                        
  NetworkElement_NetworkElementList:                                                       
      {                                                                                 
        currBlock = "NetworkElement";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      NetworkElement                                                           
    ;                                                                                   
  ...
  NetworkIdType:                                                             
      TAP_0312.NetworkIdType                                        
      {                                                                          
        __onField("TAP_0312.NetworkIdType.NetworkIdType");    
  ...
  NetworkIdentifier:                                                             
      TAP_0312.NetworkIdentifier                                        
      {                                                                          
        __onField("TAP_0312.NetworkIdentifier.NetworkIdentifier");    
  ...
  NetworkInfo:                                               
      TAP_0312.NetworkInfo                                 
      {                                                                   
        __onBlockStart("TAP_0312.NetworkInfo", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.NetworkInfo");     
  ...
  NetworkInitPDPContext:                                                             
      TAP_0312.NetworkInitPDPContext                                        
      {                                                                          
        __onField("TAP_0312.NetworkInitPDPContext.NetworkInitPDPContext");    
  ...
  NetworkList_opt:                                                                
      TAP_0312.NetworkList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.NetworkList", true);                  
        //logStdout("Parsed List NetworkList\n");                                
      }                                                                                 
      NetworkList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.NetworkList");                          
  ...
  NetworkLocation:                                               
      TAP_0312.NetworkLocation                                 
      {                                                                   
        __onBlockStart("TAP_0312.NetworkLocation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.NetworkLocation");     
  ...
  NonChargedNumber:                                                             
      TAP_0312.NonChargedNumber                                        
      {                                                                          
        __onField("TAP_0312.NonChargedNumber.NonChargedNumber");    
  ...
  NonChargedParty:                                               
      TAP_0312.NonChargedParty                                 
      {                                                                   
        __onBlockStart("TAP_0312.NonChargedParty", false);   
        //logStdout("Parsed Sequence NonChargedParty\n");              
      }                                                                   
      nonChargedPartyNumber_NonChargedParty     
      nonChargedPublicUserId_NonChargedParty     
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.NonChargedParty");     
        //logStdout("NonChargedParty Sequence END.\n");         
      }                                                            
    ;                                                              
                                                                   
  nonChargedPartyNumber_NonChargedParty:                                                       
      {                                                                                 
        currBlock = "nonChargedPartyNumber";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      NonChargedPartyNumber_opt                                                       
    ;                                                                                   
                                                                                        
  nonChargedPublicUserId_NonChargedParty:                                                       
      {                                                                                 
        currBlock = "nonChargedPublicUserId";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      NonChargedPublicUserId_opt                                                       
    ;                                                                                   
  ...
  NonChargedPartyNumber:                                                             
      TAP_0312.NonChargedPartyNumber                                        
      {                                                                          
        __onField("TAP_0312.NonChargedPartyNumber.NonChargedPartyNumber");    
        //logStdout("Parsed Leaf NonChargedPartyNumber\n");                         
      }                                                                          
    ;                                                                            
  ...
  NonChargedPublicUserId:                                                             
      TAP_0312.NonChargedPublicUserId                                        
      {                                                                          
        __onField("TAP_0312.NonChargedPublicUserId.NonChargedPublicUserId");    
        //logStdout("Parsed Leaf NonChargedPublicUserId\n");                         
      }                                                                          
    ;                                                                            
  ...
  RequestedDestination:
      TAP_0312.RequestedDestination
      {
        __onBlockStart("TAP_0312.RequestedDestination", false);
        //logStdout("Parsed Sequence RequestedDestination\n");
      }
      requestedNumber_RequestedDestination
      requestedPublicUserId_RequestedDestination
      ASN1.BlockEnd
      {
        __onBlockEnd("TAP_0312.RequestedDestination");
        //logStdout("RequestedDestination Sequence END.\n");
      }
    ;

  requestedNumber_RequestedDestination:
      {
        currBlock = "requestedNumber";
        //logStdout("currContext= " + currContext + "\n");
      }
      RequestedNumber_opt
    ;
  ...
  requestedPublicUserId_RequestedDestination:
      {
        currBlock = "requestedPublicUserId";
        //logStdout("currContext= " + currContext + "\n");
      }
      RequestedPublicUserId_opt
    ;
  ...
  RequestedNumber:
      TAP_0312.RequestedNumber
      {
        __onField("TAP_0312.RequestedNumber.RequestedNumber");
        //logStdout("Parsed Leaf RequestedNumber\n");
      }
    ;
  ...
  RequestedPublicUserId:
      TAP_0312.RequestedPublicUserId
      {
        __onField("TAP_0312.RequestedPublicUserId.RequestedPublicUserId");
        //logStdout("Parsed Leaf RequestedPublicUserId\n");
      }
    ;
  ...
  Notification:                                               
      TAP_0312.Notification                                 
      {                                                                   
        __onBlockStart("TAP_0312.Notification", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.Notification");     
  ...
  NumberOfDecimalPlaces:                                                             
      TAP_0312.NumberOfDecimalPlaces                                        
      {                                                                          
        __onField("TAP_0312.NumberOfDecimalPlaces.NumberOfDecimalPlaces");    
  ...
  NumberString:                                                             
      TAP_0312.NumberString                                        
      {                                                                          
        __onField("TAP_0312.NumberString.NumberString");    
  ...
  ObjectType:                                                             
      TAP_0312.ObjectType                                        
      {                                                                          
        __onField("TAP_0312.ObjectType.ObjectType");    
  ...
  OperatorSpecInfoList_opt:                                                                
      ...
      {                                                                                 
        __onBlockStart("TAP_0312.OperatorSpecInfoList", true);                  
        //logStdout("Parsed List OperatorSpecInfoList\n");                                
      }                                                                                 
      OperatorSpecInfoList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.OperatorSpecInfoList");                          
  ...
  OperatorSpecInformation:                                                             
      ...
      {                                                                          
        __onField("TAP_0312.OperatorSpecInformation.OperatorSpecInformation");    
  ...
  OrderPlacedTimeStamp:                                               
      TAP_0312.OrderPlacedTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.OrderPlacedTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.OrderPlacedTimeStamp");     
  ...
  OriginatingNetwork:                                                             
      TAP_0312.OriginatingNetwork                                        
      {                                                                          
        __onField("TAP_0312.OriginatingNetwork.OriginatingNetwork");    
  ...
  PDPContextStartTimestamp:                                               
      TAP_0312.PDPContextStartTimestamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.PDPContextStartTimestamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.PDPContextStartTimestamp");     
  ...
  PacketDataProtocolAddress:                                                             
      TAP_0312.PacketDataProtocolAddress                                        
      {                                                                          
        __onField("TAP_0312.PacketDataProtocolAddress.PacketDataProtocolAddress");    
  ...
  PaidIndicator:                                                             
      TAP_0312.PaidIndicator                                        
      {                                                                          
        __onField("TAP_0312.PaidIndicator.PaidIndicator");    
  ...
  PartialTypeIndicator:                                                             
      TAP_0312.PartialTypeIndicator                                        
      {                                                                          
        __onField("TAP_0312.PartialTypeIndicator.PartialTypeIndicator");    
  ...
  PaymentMethod:                                                             
      TAP_0312.PaymentMethod                                        
      {                                                                          
        __onField("TAP_0312.PaymentMethod.PaymentMethod");    
  ...
  PdpAddress:                                                             
      TAP_0312.PdpAddress                                        
      {                                                                          
        __onField("TAP_0312.PdpAddress.PdpAddress");    
  ...
  PercentageRate:                                                             
      TAP_0312.PercentageRate                                        
      {                                                                          
        __onField("TAP_0312.PercentageRate.PercentageRate");    
  ...
  PlmnId:                                                             
      TAP_0312.PlmnId                                        
      {                                                                          
        __onField("TAP_0312.PlmnId.PlmnId");    
  ...
  PositioningMethod:                                                             
      TAP_0312.PositioningMethod                                        
      {                                                                          
        __onField("TAP_0312.PositioningMethod.PositioningMethod");    
  ...
  PriorityCode:                                                             
      TAP_0312.PriorityCode                                        
      {                                                                          
        __onField("TAP_0312.PriorityCode.PriorityCode");    
  ...
  PublicUserId:                                                             
      TAP_0312.PublicUserId                                        
      {                                                                          
        __onField("TAP_0312.PublicUserId.PublicUserId");    
        //logStdout("Parsed Leaf PublicUserId\n");                         
      }                                                                          
    ;                                                                            
  ...
  RapFileSequenceNumber:                                                             
      TAP_0312.RapFileSequenceNumber                                        
      {                                                                          
        __onField("TAP_0312.RapFileSequenceNumber.RapFileSequenceNumber");    
  ...
  RecEntityCode:                                                             
      TAP_0312.RecEntityCode                                        
      {                                                                          
        __onField("TAP_0312.RecEntityCode.RecEntityCode");    
  ...
  RecEntityCodeList_opt:                                                                
      TAP_0312.RecEntityCodeList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.RecEntityCodeList", true);                  
        //logStdout("Parsed List RecEntityCodeList\n");                                
      }                                                                                 
      RecEntityCodeList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.RecEntityCodeList");                          
  ...
  RecEntityId:                                                             
      TAP_0312.RecEntityId                                        
      {                                                                          
        __onField("TAP_0312.RecEntityId.RecEntityId");    
  ...
  RecEntityInfoList_opt:                                                                
      TAP_0312.RecEntityInfoList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.RecEntityInfoList", true);                  
        //logStdout("Parsed List RecEntityInfoList\n");                                
      }                                                                                 
      RecEntityInfoList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.RecEntityInfoList");                          
  ...
  RecEntityInformation:                                               
      TAP_0312.RecEntityInformation                                 
      {                                                                   
        __onBlockStart("TAP_0312.RecEntityInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.RecEntityInformation");     
  ...
  RecEntityType:                                                             
      TAP_0312.RecEntityType                                        
      {                                                                          
        __onField("TAP_0312.RecEntityType.RecEntityType");    
  ...
  Recipient:                                                             
      TAP_0312.Recipient                                        
      {                                                                          
        __onField("TAP_0312.Recipient.Recipient");    
  ...
  ReleaseVersionNumber:                                                             
      TAP_0312.ReleaseVersionNumber                                        
      {                                                                          
        __onField("TAP_0312.ReleaseVersionNumber.ReleaseVersionNumber");    
  ...
  RequestedDeliveryTimeStamp:                                               
      TAP_0312.RequestedDeliveryTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.RequestedDeliveryTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.RequestedDeliveryTimeStamp");     
  ...
  ResponseTime:                                                             
      TAP_0312.ResponseTime                                        
      {                                                                          
        __onField("TAP_0312.ResponseTime.ResponseTime");    
  ...
  ResponseTimeCategory:                                                             
      TAP_0312.ResponseTimeCategory                                        
      {                                                                          
        __onField("TAP_0312.ResponseTimeCategory.ResponseTimeCategory");    
  ...
  SMSDestinationNumber:                                                             
      TAP_0312.SMSDestinationNumber                                        
      {                                                                          
        __onField("TAP_0312.SMSDestinationNumber.SMSDestinationNumber");    
  ...
  SMSOriginator:                                                             
      TAP_0312.SMSOriginator                                        
      {                                                                          
        __onField("TAP_0312.SMSOriginator.SMSOriginator");    
  ...
  ScuBasicInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.ScuBasicInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ScuBasicInformation");     
  ...
  ScuChargeType:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.ScuChargeType", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ScuChargeType");     
  ...
  ScuChargeableSubscriber:                                                       
      TAP_0312.ScuChargeableSubscriber                                  
      {                                                                    
        __onBlockStart("TAP_0312.ScuChargeableSubscriber", false);    
        //logStdout("Parsed Choice ScuChargeableSubscriber\n");                 
      }                                                         
      ScuChargeableSubscriber_elems                                   
      ASN1.BlockEnd                                                
      {                                                         
        __onBlockEnd("TAP_0312.ScuChargeableSubscriber");  
  ...
  ScuTimeStamps:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.ScuTimeStamps", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ScuTimeStamps");     
  ...
  Sender:                                                             
      TAP_0312.Sender                                        
      {                                                                          
        __onField("TAP_0312.Sender.Sender");    
  ...
  ServiceCentreUsage:                                               
      TAP_0312.ServiceCentreUsage                                 
      {                                                                   
        __onBlockStart("TAP_0312.ServiceCentreUsage", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ServiceCentreUsage");     
  ...
  ServiceStartTimestamp:                                               
      TAP_0312.ServiceStartTimestamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.ServiceStartTimestamp", false);   
        //logStdout("Parsed Sequence ServiceStartTimestamp\n");              
      }                                                                   
      localTimeStamp_ServiceStartTimestamp     
      utcTimeOffsetCode_ServiceStartTimestamp     
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ServiceStartTimestamp");     
        //logStdout("ServiceStartTimestamp Sequence END.\n");         
      }                                                            
    ;                                                              
  ...
  ServingBid:                                                             
      TAP_0312.ServingBid                                        
      {                                                                          
        __onField("TAP_0312.ServingBid.ServingBid");    
  ...
  ServingLocationDescription:                                                             
      TAP_0312.ServingLocationDescription                                        
      {                                                                          
        __onField("TAP_0312.ServingLocationDescription.ServingLocationDescription");    
  ...
  ServingNetwork:                                                             
      TAP_0312.ServingNetwork                                        
      {                                                                          
        __onField("TAP_0312.ServingNetwork.ServingNetwork");    
  ...
  ServingPartiesInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.ServingPartiesInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ServingPartiesInformation");     
  ...
  SessionChargeInfoList_opt:                                                                
      TAP_0312.SessionChargeInfoList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.SessionChargeInfoList", true);                  
        //logStdout("Parsed List SessionChargeInfoList\n");                                
      }                                                                                 
      SessionChargeInfoList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.SessionChargeInfoList");                          
        //logStdout("SessionChargeInfoList List END.\n");                                  
      }                                                                                 
    |                                                                                   
    ;                                                                                   
  ...
  SessionChargeInfoList:                                                                    
      SessionChargeInfoList                                                                 
      SessionChargeInformation_SessionChargeInfoList                                                    
    |                                                                                   
    ;                                                                                   
                                                                                        
  SessionChargeInformation_SessionChargeInfoList:                                                       
      {                                                                                 
        currBlock = "SessionChargeInformation";                                                   
        //logStdout("currContext= " + currContext + "\n");                           
      }                                                                                 
      SessionChargeInformation                                                           
    ;                                                                                   
                                                                   
  SessionChargeInformation_opt:                                           
      SessionChargeInformation                                            
    |                                                              
    ;                                                              
                                                                   
  SessionChargeInformation:                                               
      TAP_0312.SessionChargeInformation                                 
      {                                                                   
        __onBlockStart("TAP_0312.SessionChargeInformation", false);   
        //logStdout("Parsed Sequence SessionChargeInformation\n");              
      }                                                                   
      chargedItem_SessionChargeInformation     
      exchangeRateCode_SessionChargeInformation     
      callTypeGroup_SessionChargeInformation     
      chargeDetailList_SessionChargeInformation     
      taxInformationList_SessionChargeInformation     
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.SessionChargeInformation");     
        //logStdout("SessionChargeInformation Sequence END.\n");         
      }                                                            
    ;                                                              
  ...
  SimChargeableSubscriber:                                               
      TAP_0312.SimChargeableSubscriber                                 
      {                                                                   
        __onBlockStart("TAP_0312.SimChargeableSubscriber", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.SimChargeableSubscriber");     
  ...
  SimToolkitIndicator:                                                             
      TAP_0312.SimToolkitIndicator                                        
      {                                                                          
        __onField("TAP_0312.SimToolkitIndicator.SimToolkitIndicator");    
  ...
  SpecificationVersionNumber:                                                             
      TAP_0312.SpecificationVersionNumber                                        
      {                                                                          
        __onField("TAP_0312.SpecificationVersionNumber.SpecificationVersionNumber");    
  ...
  SsParameters:                                                             
      TAP_0312.SsParameters                                        
      {                                                                          
        __onField("TAP_0312.SsParameters.SsParameters");    
  ...
  SupplServiceActionCode:                                                             
      TAP_0312.SupplServiceActionCode                                        
      {                                                                          
        __onField("TAP_0312.SupplServiceActionCode.SupplServiceActionCode");    
  ...
  SupplServiceCode:                                                             
      TAP_0312.SupplServiceCode                                        
      {                                                                          
        __onField("TAP_0312.SupplServiceCode.SupplServiceCode");    
  ...
  SupplServiceEvent:                                               
      TAP_0312.SupplServiceEvent                                 
      {                                                                   
        __onBlockStart("TAP_0312.SupplServiceEvent", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.SupplServiceEvent");     
  ...
  SupplServiceUsed:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.SupplServiceUsed", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.SupplServiceUsed");     
  ...
  TapCurrency:                                                             
      TAP_0312.TapCurrency                                        
      {                                                                          
        __onField("TAP_0312.TapCurrency.TapCurrency");    
  ...
  TapDecimalPlaces:                                                             
      TAP_0312.TapDecimalPlaces                                        
      {                                                                          
        __onField("TAP_0312.TapDecimalPlaces.TapDecimalPlaces");    
  ...
  TaxCode:                                                             
      TAP_0312.TaxCode                                        
      {                                                                          
        __onField("TAP_0312.TaxCode.TaxCode");    
  ...
  TaxIndicator:                                                             
      TAP_0312.TaxIndicator                                        
      {                                                                          
        __onField("TAP_0312.TaxIndicator.TaxIndicator");    
  ...
  TaxInformation:                                               
      TAP_0312.TaxInformation                                 
      {                                                                   
        __onBlockStart("TAP_0312.TaxInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TaxInformation");     
  ...
  TaxInformationList_opt:                                                                
      TAP_0312.TaxInformationList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.TaxInformationList", true);                  
        //logStdout("Parsed List TaxInformationList\n");                                
      }                                                                                 
      TaxInformationList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.TaxInformationList");                          
  ...
  TaxRate:                                                             
      TAP_0312.TaxRate                                        
      {                                                                          
        __onField("TAP_0312.TaxRate.TaxRate");    
  ...
  TaxType:                                                             
      TAP_0312.TaxType                                        
      {                                                                          
        __onField("TAP_0312.TaxType.TaxType");    
  ...
  TaxValue:                                                             
      TAP_0312.TaxValue                                        
      {                                                                          
        __onField("TAP_0312.TaxValue.TaxValue");    
  ...
  TaxableAmount:                                                             
      TAP_0312.TaxableAmount                                        
      {                                                                          
        __onField("TAP_0312.TaxableAmount.TaxableAmount");    
  ...
  Taxation:                                               
      TAP_0312.Taxation                                 
      {                                                                   
        __onBlockStart("TAP_0312.Taxation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.Taxation");     
  ...
  TaxationList_opt:                                                                
      TAP_0312.TaxationList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.TaxationList", true);                  
        //logStdout("Parsed List TaxationList\n");                                
      }                                                                                 
      TaxationList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.TaxationList");                          
  ...
  TeleServiceCode:                                                             
      TAP_0312.TeleServiceCode                                        
      {                                                                          
        __onField("TAP_0312.TeleServiceCode.TeleServiceCode");    
  ...
  ThirdPartyInformation:                                               
      TAP_0312.ThirdPartyInformation                                 
      {                                                                   
        __onBlockStart("TAP_0312.ThirdPartyInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.ThirdPartyInformation");     
  ...
  ThirdPartyNumber:                                                             
      TAP_0312.ThirdPartyNumber                                        
      {                                                                          
        __onField("TAP_0312.ThirdPartyNumber.ThirdPartyNumber");    
  ...
  ThreeGcamelDestination:                                                       
      TAP_0312.ThreeGcamelDestination                                  
      {                                                                    
        __onBlockStart("TAP_0312.ThreeGcamelDestination", false);    
        //logStdout("Parsed Choice ThreeGcamelDestination\n");                 
      }                                                         
      ThreeGcamelDestination_elems                                   
      ASN1.BlockEnd                                                
      {                                                         
        __onBlockEnd("TAP_0312.ThreeGcamelDestination");  
  ...
  TotalAdvisedCharge:                                                             
      TAP_0312.TotalAdvisedCharge                                        
      {                                                                          
        __onField("TAP_0312.TotalAdvisedCharge.TotalAdvisedCharge");    
  ...
  TotalAdvisedChargeRefund:                                                             
      TAP_0312.TotalAdvisedChargeRefund                                        
      {                                                                          
        __onField("TAP_0312.TotalAdvisedChargeRefund.TotalAdvisedChargeRefund");    
  ...
  TotalAdvisedChargeValue:                                               
      TAP_0312.TotalAdvisedChargeValue                                 
      {                                                                   
        __onBlockStart("TAP_0312.TotalAdvisedChargeValue", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TotalAdvisedChargeValue");     
  ...
  TotalAdvisedChargeValueList_opt:                                                                
      TAP_0312.TotalAdvisedChargeValueList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.TotalAdvisedChargeValueList", true);                  
        //logStdout("Parsed List TotalAdvisedChargeValueList\n");                                
      }                                                                                 
      TotalAdvisedChargeValueList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.TotalAdvisedChargeValueList");                          
  ...
  TotalCallEventDuration:                                                             
      TAP_0312.TotalCallEventDuration                                        
      {                                                                          
        __onField("TAP_0312.TotalCallEventDuration.TotalCallEventDuration");    
  ...
  TotalCharge:                                                             
      TAP_0312.TotalCharge                                        
      {                                                                          
        __onField("TAP_0312.TotalCharge.TotalCharge");    
  ...
  TotalChargeRefund:                                                             
      TAP_0312.TotalChargeRefund                                        
      {                                                                          
        __onField("TAP_0312.TotalChargeRefund.TotalChargeRefund");    
  ...
  TotalCommission:                                                             
      TAP_0312.TotalCommission                                        
      {                                                                          
        __onField("TAP_0312.TotalCommission.TotalCommission");    
  ...
  TotalCommissionRefund:                                                             
      TAP_0312.TotalCommissionRefund                                        
      {                                                                          
        __onField("TAP_0312.TotalCommissionRefund.TotalCommissionRefund");    
  ...
  TotalDataVolume:                                                             
      TAP_0312.TotalDataVolume                                        
      {                                                                          
        __onField("TAP_0312.TotalDataVolume.TotalDataVolume");    
  ...
  TotalDiscountRefund:                                                             
      TAP_0312.TotalDiscountRefund                                        
      {                                                                          
        __onField("TAP_0312.TotalDiscountRefund.TotalDiscountRefund");    
  ...
  TotalDiscountValue:                                                             
      TAP_0312.TotalDiscountValue                                        
      {                                                                          
        __onField("TAP_0312.TotalDiscountValue.TotalDiscountValue");    
  ...
  TotalTaxRefund:                                                             
      TAP_0312.TotalTaxRefund                                        
      {                                                                          
        __onField("TAP_0312.TotalTaxRefund.TotalTaxRefund");    
  ...
  TotalTaxValue:                                                             
      TAP_0312.TotalTaxValue                                        
      {                                                                          
        __onField("TAP_0312.TotalTaxValue.TotalTaxValue");    
  ...
  TotalTransactionDuration:                                                             
      TAP_0312.TotalTransactionDuration                                        
      {                                                                          
        __onField("TAP_0312.TotalTransactionDuration.TotalTransactionDuration");    
  ...
  TrackedCustomerEquipment:                                               
      TAP_0312.TrackedCustomerEquipment                                 
      {                                                                   
        __onBlockStart("TAP_0312.TrackedCustomerEquipment", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TrackedCustomerEquipment");     
  ...
  TrackedCustomerHomeId:                                               
      TAP_0312.TrackedCustomerHomeId                                 
      {                                                                   
        __onBlockStart("TAP_0312.TrackedCustomerHomeId", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TrackedCustomerHomeId");     
  ...
  TrackedCustomerHomeIdList_opt:                                                                
      TAP_0312.TrackedCustomerHomeIdList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.TrackedCustomerHomeIdList", true);                  
        //logStdout("Parsed List TrackedCustomerHomeIdList\n");                                
      }                                                                                 
      TrackedCustomerHomeIdList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.TrackedCustomerHomeIdList");                          
  ...
  TrackedCustomerIdList_opt:                                                                
      TAP_0312.TrackedCustomerIdList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.TrackedCustomerIdList", true);                  
        //logStdout("Parsed List TrackedCustomerIdList\n");                                
      }                                                                                 
      TrackedCustomerIdList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.TrackedCustomerIdList");                          
  ...
  TrackedCustomerIdentification:                                               
      TAP_0312.TrackedCustomerIdentification                                 
      {                                                                   
        __onBlockStart("TAP_0312.TrackedCustomerIdentification", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TrackedCustomerIdentification");     
  ...
  TrackedCustomerInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.TrackedCustomerInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TrackedCustomerInformation");     
  ...
  TrackedCustomerLocList_opt:                                                                
      TAP_0312.TrackedCustomerLocList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.TrackedCustomerLocList", true);                  
        //logStdout("Parsed List TrackedCustomerLocList\n");                                
      }                                                                                 
      TrackedCustomerLocList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.TrackedCustomerLocList");                          
  ...
  TrackedCustomerLocation:                                               
      TAP_0312.TrackedCustomerLocation                                 
      {                                                                   
        __onBlockStart("TAP_0312.TrackedCustomerLocation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TrackedCustomerLocation");     
  ...
  TrackingCustomerEquipment:                                               
      TAP_0312.TrackingCustomerEquipment                                 
      {                                                                   
        __onBlockStart("TAP_0312.TrackingCustomerEquipment", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TrackingCustomerEquipment");     
  ...
  TrackingCustomerHomeId:                                               
      TAP_0312.TrackingCustomerHomeId                                 
      {                                                                   
        __onBlockStart("TAP_0312.TrackingCustomerHomeId", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TrackingCustomerHomeId");     
  ...
  TrackingCustomerHomeIdList_opt:                                                                
      TAP_0312.TrackingCustomerHomeIdList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.TrackingCustomerHomeIdList", true);                  
        //logStdout("Parsed List TrackingCustomerHomeIdList\n");                                
      }                                                                                 
      TrackingCustomerHomeIdList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.TrackingCustomerHomeIdList");                          
  ...
  TrackingCustomerIdList_opt:                                                                
      TAP_0312.TrackingCustomerIdList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.TrackingCustomerIdList", true);                  
        //logStdout("Parsed List TrackingCustomerIdList\n");                                
      }                                                                                 
      TrackingCustomerIdList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.TrackingCustomerIdList");                          
  ...
  TrackingCustomerIdentification:                                               
      TAP_0312.TrackingCustomerIdentification                                 
      {                                                                   
        __onBlockStart("TAP_0312.TrackingCustomerIdentification", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TrackingCustomerIdentification");     
  ...
  TrackingCustomerInformation:                                               
      ...
      {                                                                   
        __onBlockStart("TAP_0312.TrackingCustomerInformation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TrackingCustomerInformation");     
  ...
  TrackingCustomerLocList_opt:                                                                
      TAP_0312.TrackingCustomerLocList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.TrackingCustomerLocList", true);                  
        //logStdout("Parsed List TrackingCustomerLocList\n");                                
      }                                                                                 
      TrackingCustomerLocList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.TrackingCustomerLocList");                          
  ...
  TrackingCustomerLocation:                                               
      TAP_0312.TrackingCustomerLocation                                 
      {                                                                   
        __onBlockStart("TAP_0312.TrackingCustomerLocation", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TrackingCustomerLocation");     
  ...
  TrackingFrequency:                                                             
      TAP_0312.TrackingFrequency                                        
      {                                                                          
        __onField("TAP_0312.TrackingFrequency.TrackingFrequency");    
  ...
  TrackingPeriod:                                                             
      TAP_0312.TrackingPeriod                                        
      {                                                                          
        __onField("TAP_0312.TrackingPeriod.TrackingPeriod");    
  ...
  TransactionAuthCode:                                                             
      TAP_0312.TransactionAuthCode                                        
      {                                                                          
        __onField("TAP_0312.TransactionAuthCode.TransactionAuthCode");    
  ...
  TransactionDescriptionSupp:                                                             
      TAP_0312.TransactionDescriptionSupp                                        
      {                                                                          
        __onField("TAP_0312.TransactionDescriptionSupp.TransactionDescriptionSupp");    
  ...
  TransactionDetailDescription:                                                             
      TAP_0312.TransactionDetailDescription                                        
      {                                                                          
        __onField("TAP_0312.TransactionDetailDescription.TransactionDetailDescription");    
  ...
  TransactionIdentifier:                                                             
      TAP_0312.TransactionIdentifier                                        
      {                                                                          
        __onField("TAP_0312.TransactionIdentifier.TransactionIdentifier");    
  ...
  TransactionShortDescription:                                                             
      TAP_0312.TransactionShortDescription                                        
      {                                                                          
        __onField("TAP_0312.TransactionShortDescription.TransactionShortDescription");    
  ...
  TransactionStatus:                                                             
      TAP_0312.TransactionStatus                                        
      {                                                                          
        __onField("TAP_0312.TransactionStatus.TransactionStatus");    
  ...
  TransferBatch:                                               
      TAP_0312.TransferBatch                                 
      {                                                                   
        __onBlockStart("TAP_0312.TransferBatch", false);   
        //logStdout("Parsed Sequence TransferBatch\n");              
      }                                                                   
      batchControlInfo_TransferBatch     
      accountingInfo_TransferBatch     
      networkInfo_TransferBatch     
      messageDescriptionInfo_TransferBatch     
      callEventDetails_TransferBatch     
      auditControlInfo_TransferBatch     
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TransferBatch");     
        //logStdout("TransferBatch Sequence END.\n");         
      }                                                            
    ;                                                              
  ...
  TransferCutOffTimeStamp:                                               
      TAP_0312.TransferCutOffTimeStamp                                 
      {                                                                   
        __onBlockStart("TAP_0312.TransferCutOffTimeStamp", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.TransferCutOffTimeStamp");     
  ...
  TransparencyIndicator:                                                             
      TAP_0312.TransparencyIndicator                                        
      {                                                                          
        __onField("TAP_0312.TransparencyIndicator.TransparencyIndicator");    
  ...
  UserProtocolIndicator:                                                             
      TAP_0312.UserProtocolIndicator                                        
      {                                                                          
        __onField("TAP_0312.UserProtocolIndicator.UserProtocolIndicator");    
  ...
  UtcTimeOffset:                                                             
      TAP_0312.UtcTimeOffset                                        
      {                                                                          
        __onField("TAP_0312.UtcTimeOffset.UtcTimeOffset");    
  ...
  UtcTimeOffsetCode:                                                             
      TAP_0312.UtcTimeOffsetCode                                        
      {                                                                          
        __onField("TAP_0312.UtcTimeOffsetCode.UtcTimeOffsetCode");    
  ...
  UtcTimeOffsetInfo:                                               
      TAP_0312.UtcTimeOffsetInfo                                 
      {                                                                   
        __onBlockStart("TAP_0312.UtcTimeOffsetInfo", false);   
      ...
      ASN1.BlockEnd                                                   
      {                                                            
        __onBlockEnd("TAP_0312.UtcTimeOffsetInfo");     
  ...
  UtcTimeOffsetInfoList_opt:                                                                
      TAP_0312.UtcTimeOffsetInfoList                                               
      {                                                                                 
        __onBlockStart("TAP_0312.UtcTimeOffsetInfoList", true);                  
        //logStdout("Parsed List UtcTimeOffsetInfoList\n");                                
      }                                                                                 
      UtcTimeOffsetInfoList                                                                 
      ASN1.BlockEnd                                                                        
      {                                                                                 
        __onBlockEnd("TAP_0312.UtcTimeOffsetInfoList");                          
  ...
  VerticalAccuracyDelivered:                                                             
      TAP_0312.VerticalAccuracyDelivered                                        
      {                                                                          
        __onField("TAP_0312.VerticalAccuracyDelivered.VerticalAccuracyDelivered");    
  ...
  VerticalAccuracyRequested:                                                             
      TAP_0312.VerticalAccuracyRequested                                        
      {                                                                          
        __onField("TAP_0312.VerticalAccuracyRequested.VerticalAccuracyRequested");    
