	  ...
	  //--------------------------------------------------------------------
          ...
          for ( i = 0; i < records; i = i+1 )
          {
            ...
            if (addInvoiceDataBool == true)
            {
              ...
              edrOutputMapToBuffer(tmpBuffer,"V670_EVENT_LOADER.ASSOCIATED_INVOICE_DATA.STD_MAPPING");
		...
		edrOutputMapToBuffer(tmpBuffer,"V670_EVENT_LOADER.BALANCE_IMPACT.STD_MAPPING", i, j);
              }
              ...
              edrOutputMapToBuffer(tmpBuffer,"V670_EVENT_LOADER.INVOICE_DATA_TERMINATOR.STD_MAPPING", i, 0);

              Long invlength = strLength(tmpBuffer);
              //--------------------------------------------------------------------
              // Write the default message in case length execeeds 4k
              //--------------------------------------------------------------------
              if(invlength >= 4000)
              {
                tmpBuffer = "MAX_LENGTH_EXCEEDED" + "\n";
              }

              outputWrite(tmpBuffer);
            }
            else
            {
              outputWrite("\n");
