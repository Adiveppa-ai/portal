 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/rwtime.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Deprecated. Represents a time, stored as the number of seconds since
 * 00:00:00 January 1, 1901 GMT.
 *
 * \deprecated
 * As of SourcePro 10, use RWDateTime instead.
 *
 * Class RWTime represents a time, stored as the number of seconds since
 * 00:00:00 January 1, 1901 GMT. The <em>Essential Tools User's Guide</em>
 * chapter on date and time classes describes how to set the time zone for
 * your compiler. Failure to do so may result in GMT times being wrong.
 *
 * \note
 ...
 *
 * \note
 * Because the default constructor for this class creates an instance
 * holding the current date and time, constructing a large array of RWTime
 * may be slow.
 *
 * \code
 ...
 * Essential Tools Module template collections should consider the
 * following:
 *
 * \code
 ...
 * RWTime, the copy constructor amounts to an assignment of one \c long to
...
public:
    ...
    /**
     * Constructs a time with \a s seconds since 00:00:00 January 1, 1901
     * GMT. If \a s <tt>== 0</tt>, an invalid time is constructed.
     *
     * \note
     * For small \a s, this may be prior to January 1, 1901 in your time
     * zone.
     *
     * The compiler can parse 0 as either an integer or a pointer. Since
     * there is also a constructor that takes a pointer (to
     * <tt>struct tm</tt>, if you want to construct a time from the unsigned
     * \c long value 0, you must be explicit:
     *
     * \code
    ...
    /**
     * Constructs a time for a given \a date, \a hour, \a minute, and
     * \a second, relative to the time zone \a zone, which defaults to local
     * time.
     *
     * \note
     * The maximum RWTime is much sooner than maximum RWDate. (In fact, it
     * is on Feb. 5, 2037 for platforms with 4-byte \c longs.) This is a
     * consequence of the fact that RWTime counts seconds, while RWDate
     * deals only with full days.
     */
    ...
    /**
     * Constructs a time for the given \a date, extracting the time from the
     * string \a str. The string \a str should contain only the time. The
     * time is understood to be relative to the time zone \a zone, which
     * defaults to local time. The specified \a locale is used for
     * formatting information. Use function isValid() to check the results.
     *
     * \note
     ...
     * \c tm_hour, \c tm_min, and \c tm_sec components of the
     * <tt>struct tm</tt> argument. These components are understood to be
    ...
    /**
     * Fills all members of the <tt>struct tm</tt> argument, adjusted to the
     * time zone specified by the RWZone argument. If the time is invalid,
     * the <tt>struct tm</tt> members are all set to -1.
     *
     * \note
     * The encoding of <tt>struct tm</tt> members is different from that
     * used in RWTime and RWDate functions.
     */
    void
    ...
    /**
     * Returns self as a string, formatted by the RWLocale argument, with
     * the time zone adjusted according to the RWZone argument. Formats are
     * as defined by the C++ Standard Library function \b std::strftime().
     * The default format is the date followed by the time:
     * <tt>"%x %X"</tt>. The exact format of the date and time returned is
     * dependent upon the Locale-dependent \c \%x and \c \%X formatting. For
     * more information, see RWLocale.
     */
    RWCString
     ...
     * the time zone adjusted according to the RWZone argument. Formats are
     * as defined by the C++ Standard Library function \b std::strftime().
     * For more information, see RWLocale.
     */
    RWCString
    ...
    /**
     * Compares self to the contents of \a t, returning a value from the
     * table below based on the result of the comparison.
     *
     * <table>
     * <tr>
     * <td>0</td>
    ...
    /**
     * Prefix increment operator. Adds one second to self, then returns the
     * results.
     */
    RWTime
    operator++ ()
    ...
    /**
     * Postfix increment operator. Adds one second to self, returning the
     * initial value.
     */
    RWTime
    operator++ (int)
 ...
 * \relates RWTime
 *
 * Restores an RWTime into \a t from the RWFile \a file, replacing the
 * previous contents of \a t.
 */
inline RWFile&
