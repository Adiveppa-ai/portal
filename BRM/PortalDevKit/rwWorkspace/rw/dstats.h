 ...
 *
 * $Id: //spro/rel2016.1/rw/dstats.h#1 $
 *
 ...
 *
 * \headerfileloc{#include <rw/dstats.h>}
 *
 ...
 * If an event has a probability \a p of occurring and we make \a N tries,
 * binomialPF() returns the probability that the event will occur \a m
 * times:
 *
 * \f[
 ...
 * has the probability distribution <em>a exp(-ax)</em>.
 */
 ...
 * \headerfileloc{#include <rw/dstats.h>}
 *
 * Returns the factorial of \a n, which is written <em>n!</em>. Stores
 * previously calculated factorials internally for speed.
 */
 ...
 * \headerfileloc{#include <rw/dstats.h>}
 *
 * Returns the Gaussian probability function for Gaussian mean \a m and
 ...
 * \f]
 *
 * where <em>\<x\></em> is the mean of the vector and \f$ \sigma \f$ is its
 * standard deviation (that is, the square root of the variance). The -3
 * term is designed to make the kurtosis 0 for a normal distribution.
 */
 ...
 * \headerfileloc{#include <rw/dstats.h>}
 *
 * Returns the Lorentzian probability function, where \a m is the mean of
 * the distribution and \a w is the full width at half maximum of the
 * distribution.
 */
 ...
 * number of unit rate Poisson random events occurring in a given time
 * interval. The Poisson distribution represents an approximation to the
 * binomial distribution for the special case where the average number of
 * events is very much smaller than the possible number. The function
 * poissonPF() returns the probability that a Poisson random event with
 * Poisson mean \a m will occur, given \a n observations:
 *
 * \f[
 ...
 * \headerfileloc{#include <rw/dstats.h>}
 *
 * Returns an estimate of the machine roundoff error in units of \a x,
 * typically 1. This is frequently used to determine whether a number is
 * <em>near 0</em> relative to other numbers of order \a x.
 */
