--
-- @(#)$Id: event_tables_tobe_partitioned.sql /cgbubrm_7.5.0.portalbase/1 2015/11/27 06:09:19 nishahan Exp $
--
...
--
--
-- sql script to find the list of event tables to be partitioned.
--
...
DECLARE
	--
       	-- By default, pick up the tables that belong to class /event%
       	--
	...
	AND (do.name = '/event' OR do.name LIKE '/event/%')
...
BEGIN
      	...
      	EXCEPTION
      		WHEN no_data_found THEN
            		INSERT INTO tables_tobe_partitioned_t (table_name, base_name, object_name, partition_exchanged)
                  	VALUES (UPPER(partition_tables_rec.table_name), 'EVENT_T', '/event', 0);
	...
	--
	-- Now delete the event tables that are obsolete from the list.
	-- There is no need to partition them.
	--

	BEGIN
		DELETE FROM tables_tobe_partitioned_t WHERE table_name = 'EVENT_AGGREGATION_NETFLOW_T';
		DELETE FROM tables_tobe_partitioned_t WHERE table_name = 'EVENT_NETFLOW_CATEGORIZATION_T';
		DELETE FROM tables_tobe_partitioned_t WHERE table_name = 'EVENT_NETFLOW_TOTAL_USAGE_T';
		DELETE FROM tables_tobe_partitioned_t WHERE table_name = 'EVENT_NETFLOW_USAGE_RECORDS_T';
		DELETE FROM tables_tobe_partitioned_t WHERE table_name = 'EVENT_NETFLOW_USAGE_T';
		DELETE FROM tables_tobe_partitioned_t WHERE table_name = 'EVENT_BILLING_ITEMS_T';
		DELETE FROM tables_tobe_partitioned_t WHERE table_name = 'EVENT_ACTIVITY_MCIS_REVENUE_T';
		DELETE FROM tables_tobe_partitioned_t WHERE table_name = 'EVENT_BILLING_ADJ_ACCT_T';

		COMMIT;
        EXCEPTION
                WHEN OTHERS THEN
                        pin_upg_common.log_info (C_STEP_NAME, NULL, NULL, 'E',
                                        'Error deleting from the tables_tobe_partitioned_t table', sqlerrm);
        END;
...
EXCEPTION
	WHEN OTHERS THEN
               	pin_upg_common.log_info (C_STEP_NAME, NULL, NULL, 'E', 
			'Error finding the event table names to be partitioned', sqlerrm);
END;
/
