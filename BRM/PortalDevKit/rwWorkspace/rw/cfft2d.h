 ...
 *
 * $Id: //spro/rel2016.1/rw/cfft2d.h#1 $
 *
 ...
 *
 * \brief
 * Performs two-dimensional (2-D) double precision complex fast Fourier
 * transforms (FFTs) of an \ref RWGenMat "RWGenMat<DComplex>" matrix.
 *
 * Class DComplexFFT2DServer performs two-dimensional (2-D) double precision
 * complex fast Fourier transforms (FFTs) of an
 * \ref RWGenMat "RWGenMat<DComplex>" matrix. The transform of a complex
 * matrix is a complex matrix; therefore, the server returns a complex
 * matrix for either a forward or backward transform.
 *
 * At any given moment, the server is configured to transform a matrix with
 * specified dimensions. These dimensions can be set one of three ways:
 *
 ...
 * automatically reconfigures to the new size. Since this is an expensive
 * calculation, it is most efficient to call the server repeatedly for
 * matrices of the same size. Different FFT servers may be constructed to
 ...
 * \f]
 *
 * As with the 1-D FFT, a call to fourier() followed by a call to ifourier()
 * results in the original matrix multiplied by the total number of
 * elements, that is, the number of rows times the number of columns.
 *
 * \synopsis
 * #include <rw/cfft2d.h>
...
public:
     ...
     * dimensions. The dimensions are initialized when the server is used.
     */
    DComplexFFT2DServer();

    /**
     * Constructs a complex FFT server designed to transform a matrix with
     * \a nrows rows and \a ncols columns.
     */
    DComplexFFT2DServer(size_t nrows, size_t ncols);

    /**
     * Constructs a complex FFT server designed to transform a square matrix
     * with \a n rows and columns.
     */
