 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvrtarry.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * This class represents a virtual array of elements of type \c T of almost
 * any length. Individual elements are brought into physical memory as
 * needed. If an element is updated, it is automatically marked as "dirty"
 * and will be rewritten to the swapping medium.
 *
 * The swap space is provided by an abstract page heap specified by the
 ...
 * before all virtual arrays built from it have been destroyed.
 *
 * The class supports reference counting using a copy-on-write technique,
 * so, for example, returning a virtual array by value is efficient. Be
 * aware, however, that if the copy-on-write machinery must make a copy,
 * large arrays can be time-consuming.
 *
 * For efficiency, more than one element can (and should) be put on a page.
 * The actual number of elements is equal to the page size divided by the
 * element size, rounded downwards. Example: for a page size of 512 bytes,
 * and an element size of 8, then 64 elements would be put on a page.
 *
 * The indexing operator operator[](long) actually returns an object of type
 * RWTVirtualElement. Consider this example:
 *
 * \code
 * double d = vec[j];
 * vec[i] = 22.0;
 ...
 * \ref RWTValVirtualArray "RWTValVirtualArray<double>". The expression
 * \c vec[j] returns an object of type RWTVirtualElement<double>, which
 * contains a reference to the element being addressed. In the first line,
 * this expression is being used to initialize a \c double. The class
 * RWTVirtualElement contains a type conversion operator to convert itself
 * to a \c T, in this case a \c double. The compiler uses this to initialize
 * \c d in the first line. In the second line, the expression \c vec[i] is
 * being used as an lvalue. In this case, the compiler uses the assignment
 * operator for RWTVirtualElement. This assignment operator recognizes that
 * the expression is being used as an lvalue and automatically marks the
 * appropriate page as "dirty," thus guaranteeing that it will be written
 * back out to the swapping medium.
 *
 * Slices, as well as individual elements, can also be addressed. These
 * should be used wherever possible as they are much more efficient because
 * they allow a page to be locked and used multiple times before unlocking.
 *
...
public:
    ...
    /**
     * Constructs a vector of length \a size. The pages for the vector are
     * allocated from the page heap given by \a heap which can be of any
     * type.
     */
    RWTValVirtualArray(long size, RWVirtualPageHeap* heap);

    /**
     * Constructs a vector as a copy of \a v. The resultant vector uses the
     * same heap and has the same length as \a v. The actual copy is not
     ...
     * vector uses the same heap as the vector whose slice is being taken.
     * Its length is given by the length of the slice. The copy is made
     * immediately.
     */
    ...
    /**
     * Sets self to a copy of \a v. The resultant vector will use the same
     * heap and have the same length as \a v. The actual copy will not be
     * made until a write, minimizing the amount of heap allocations and
     * copying that must be done.
     */
     ...
     * uses the same heap as the vector whose slice is being taken. Its
     * length is given by the length of the slice. The copy is made
     * immediately.
     */
    void                  operator=(const RWTVirtualSlice<T>& sl);

    /**
     * Sets all elements in self equal to \a val, and returns a copy of
     * \a val. This operator is actually quite efficient because it can work
     * with many elements on a single page at once.
     */
    ...
    /**
     * Returns a reference to the value at index \a i. The results can be
     * used as an lvalue.
     *
     * \exception RWBoundsErr
     * Thrown if index \a i is not between \c 0 and one less than the number
    ...
    /**
     * Returns a reference to a slice of self. The value \a start is the
     * starting index of the slice, the value \a length its extent. The
     * results can be used as an lvalue.
     */
     ...
     * length is increased, then the value of the new elements is undefined.
     */
