 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/dlistcol.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * Class RWDlistCollectables represents a group of ordered items, not
 ...
 * and removal. An object stored by RWDlistCollectables must inherit the
 * abstract base class RWCollectable.
 *
 * Class RWDlistCollectables is implemented as a doubly-linked list, which
 * allows for efficient insertion and removal, as well as for movement in
 * either direction.
 *
 * \synopsis
...
#  if !defined(RW_NO_RVALUE_REFERENCES)
    /**
     * Move constructor. The constructed RWDlistCollectables takes ownership
     * of the data owned by \a lst.
     *
     * \conditional
    ...
    /**
     * Returns \c true if self and \a d have the same number of items and
     * if, for every item in self, the corresponding item in the same
     * position in \a d \ref RWCollectable::isEqual() "isEqual()" to the
     * item in self.
     */
    bool  operator==(const RWDlistCollectables& d) const;

    /**
     * Inserts the item at the end of the collection and returns it. Returns
    ...
    /**
     * Invokes the function pointer \a ap on each member in the collection,
     ...
     * The value \a i must be between zero and the number of items in the
     * collection less one, or the function throws an exception of type
     * RWBoundsErr. Note that for a linked list, these functions must
     * traverse all the links, making them particularly inefficient.
     */
    ...
    /**
     * Returns the first item that RWCollecatble::isEqual() to the item
     * pointed to by \a target, or #rwnil if no item is found.
     */
     ...
     * \ref RWCollectable::isEqual() "isEqual()" to the item pointed to by
     * \a c, or #RW_NPOS if there is no such index.
     */
    ...
    /**
     * Adds the item to the end of the collection and returns it. Returns
     * #rwnil if the insertion was unsuccessful.
     */
    ...
    /**
     * Adds a new item to the collection at position \a inx. The item
     * previously at position \a i is moved to \a i + 1, and so forth. The
     * index \a inx must be between zero and the number of items in the
     * collection, or the function throws an exception of type RWBoundsErr.
     */
     ...
     * \ref RWCollectable::isEqual() "isEqual()" to the item pointed to by
     * \a target. Returns #rwnil if there is no such item.
     */
    ...
    /**
     * Returns true if the list contains an item that <em>is identical
     * to</em> the item pointed to by \a e (that is, that has the address
     * \a e).
     */
    ...
    /**
     * Returns the first item that <em>is identical to</em> the item pointed
     * to by \a e (that is, that has the address \a e), or #rwnil if none is
     * found.
     */
    ...
    /**
     * Returns the number of items that <em>are identical to</em> the item
     * pointed to by \a e (that is, that have the address \a e).
     */
    ...
    /**
     * Removes and returns the first item that <em>is identical to</em> the
     * item pointed to by \a e (that is, that has the address \a e). Returns
 ...
 *
 * Once the iterator has advanced beyond the end of the collection it is no
 * longer valid -- continuing to use it brings undefined results.
 *
 * \synopsis
 * #include <rw/dlistcol.h>
...
public:
    ...
    /**
     * Constructs an RWDlistCollectablesIterator from an
     * RWDlistCollectables. Immediately after construction, the position of
     ...
     * \ref RWCollectable::isEqual() "isEqual()" to the item pointed to by
     * \a target and returns it. If no item is found, the function returns
     * #rwnil and the position of the iterator is undefined.
     */
    ...
    /**
     * Resets the iterator. Afterwards, the position of the iterator is
     * undefined.
     */
    virtual void reset()
    {
    ...
    /**
     * Moves the iterator to the next item which <em>is identical to</em>
     * the item pointed to by \a e (that is, that has address \a e) and
     * returns it. If no item is found, the function returns #rwnil and the
     ...
     * Afterwards, the iterator is positioned at the previous item in the
     * list.
     */
    ...
    /**
     * Moves the iterator to the next item in the list which isEqual() to
     * the item pointed to by \a target, removes it from the list and
     * returns it. Afterwards, the iterator is positioned at the previous
     * item in the list. If no item is found, the function returns #rwnil
    ...
    /**
     * Moves iterator to the next item in the list which <em>is identical
     * to</em> the item pointed to by \a e (that is, that has address \a e),
     * removes it from the list and returns it. Afterwards, the iterator is
     * positioned at the previous item in the list. If no item is found, the
     * function returns #rwnil and the position of the iterator is
     * undefined.
     */
 ...
 * iterator, this iterator does not contain any functions that change the
 * data structure it is iterating through.
 *
 * This iterator traverses the linked-list from the first (head) to the last
 * (tail) item. Functions are provided for moving in \e either direction.
 *
 * As with all Rogue Wave iterators, the "current item" is undefined
 * immediately after construction -- you must define it by using
 * operator()() or some other (valid) operation.
...
public:
    ...
    /**
     * Constructs an RWDlistCollectablesConstIterator from an
     * RWDlistCollectablesIterator. Immediately after construction, the
     * position of the iterator is undefined.
     */
     ...
     * pointed to by \a target and returns it. If no item is found, the
     * function returns #rwnil and the position of the iterator is
     * undefined.
     */
    ...
    /**
     * Moves the iterator to the next item that <em>is identical to</em> the
     * item pointed to by \a e (that is, that has address \a e) and returns
     * it. If no item is found, the function returns #rwnil and the position
