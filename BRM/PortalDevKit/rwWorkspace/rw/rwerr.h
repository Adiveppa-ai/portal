 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/rwerr.h#1 $
 *
 **********************************************************************
 *
...
/**
 * \ingroup tools_miscellaneous_group
 *
 * \brief
 * Base class for the hierarchy of exceptions in the Essential Tools Module
 *
 * This class is the base class for the hierarchy of exceptions in the
 * Essential Tools Module. (See the section on error reporting and handling
 * in the <em>Essential Tools Module</em> User's Guide.) Although the entire
 * class is documented here, your code usually should not create or directly
 * throw RWxmsg objects. Instead, you should expect to write code that may
 * catch an RWxmsg or one of its derived classes.
 *
 * Essential Tools Module classes that need to report exceptional
 * circumstances call RWTHROW(). RWTHROW() throws the RWxmsg if exceptions
 * are enabled. Otherwise, RWTHROW() calls an appropriate current Essential
 * Tools Module error handler function (see the section on error reporting
 * and handling in the <em>Essential Tools Module</em> User's Guide).
 *
 * RWxmsg provides a basic interface for exceptions, from which an exception
 * string is always available. This exception string is obtained through the
 * why() method, and is represented as a <tt>const char*</tt>, single-byte
 * character sequence.
 *
 * \synopsis
 * #include <rw/rwerr.h>
...
public:
    /**
     * Creates an exception that returns \a msg when its why() method is
     * called.
     */
     ...
     * class instance \a msg. (See the section on messages in the
...
protected:
    ...
    /**
     * Creates an exception that returns \a msg when its why() method is
     * called. If \a doCopy is \c true, copies \a msg into a new buffer on
     * the heap, otherwise simply keeps a pointer to the given location of
     * \a msg.
     */
 ...
 *
 * \brief
 * Exception class that reports errors caused by external sources over which
 * the library has no control.
 *
 * RWExternalErr is derived from RWxmsg, and is used to report errors caused
 * by external sources over which the library has no control. Currently,
 * this class is used to report errors from RWFileManager and RWCollectable
 ...
 *
 * \brief
 * Exception class that reports either no header or an invalid header in an
 * RWeostream or RWeistream.
 *
 ...
 * conversion errors produced by classes such as RWBasicUString. The begin()
 * and end() methods may be used to extract and report the offending code
 * unit sequence within the conversion source. The value returned by the
 * end() method may be used as an offset for restarting the conversion at
 * the first code unit following the bad sequence.
 */
...
public:
    ...
    /**
     * Returns the index of the first code unit in the invalid, irregular,
     * or truncated encoding sequence.
     */
    size_t begin() const;

    /**
     * Returns the index of the last code unit in the invalid, irregular, or
     * truncated encoding sequence.
     */
    size_t end() const;
 ...
 *
 * \brief
 * Exception class that reports errors that occur within the Essential Tools
 * Module
 *
 * RWInternalErr is a type of error that is used to report errors that occur
 * within the Essential Tools Module. For example, RWDateTime uses this
 * class to report operations on invalid date/time objects.
 */
 ...
 * an RWTPtrOrderedVector may report this type of error when an invalid
 * index is provided to the RWTPtrOrderedVector<T>::at(size_type) method.
 */
 ...
 *
 * Class RWRegexErr is an exception class used to report errors from within
 * RWTRegex.
 *
 * RWRegexErr is derived from RWInternalErr, and is therefore part of the
 * RWxmsg class hierarchy. Classes in this hierarchy are used to report
 * errors from within the Essential Tools Module.
 *
 * RWRegexErr customizes RWInternalErr to provide not only an error message
 * (inherited from RWxmsg), but also an error code and an error offset.
 *
 * The error code is an RWTRegex<T>::RWTRegexStatus enumeration constant
 * represented as a \c size_t, and is useful in generating error messages in
 * the wide characters required for many non-Latin languages.
 *
 * When RWTRegex generates an error, it creates an RWRegexErr object to
 * represent the error. A copy of this object is saved in the RWTRegex
 * object, and is available through a call to RWTRegex::getStatus().
 *
 * Then, RWTHROW() is used to report the error. If exceptions are enabled,
 * the function throws the RWRegexErr object as an exception. If not, then a
...
public:
    /**
     * Initializes the error object to contain the specified error message,
     * \a msg. The error code, \a eCode, is stored as well as the offset,
     * \a eOffset, into the pattern of the error.
     */
 ...
 * \relates RWxmsg
 *
 * RWTHROW() throws the exception \a a if exceptions are enabled. Otherwise,
 * RWTHROW() invokes the error handler callback function set by the last
 * call to rwSetErrHandler() or the default callback if one has not been
 * explicitly set.
 */
