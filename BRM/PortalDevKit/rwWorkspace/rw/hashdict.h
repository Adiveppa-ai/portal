 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/hashdict.h#1 $
 *
 **********************************************************************
 *
...
public:
    ...
    /**
     * Constructs an empty hashed dictionary using
     * RWCollection::DEFAULT_CAPACITY hashing buckets.
     */
    RWHashDictionary();
    ...
    /**
     * Move constructor. The constructed RWHashDictionary takes ownership of
     * the data owned by \a hd.
     *
     * \conditional
    ...
    /**
     * Returns \c true if for every key-value pair in self, there is a
     * corresponding key in \a hd that isEqual(). Their corresponding values
     * must also be equal.
     */
    ...
    /**
     * Returns \c true if self and \a hd have the same number of entries and
     * if for every key-value pair in self, there is a corresponding key in
     * \a hd that isEqual(). Their corresponding values must also be equal.
     */
    bool operator==(const RWHashDictionary& hd) const;
    ...
    /**
     * Removes all key-value pairs in the collection, and deletes the key
     * \e and the value.
     */
    virtual void clearAndDestroy();

    /**
     * Returns the key which isEqual() to the object pointed to by
     * \a target, or #rwnil if no key was found.
     */
    virtual RWCollectable* find(const RWCollectable* target) const;

    /**
     * Returns the key which isEqual() to the item pointed to by \a target,
     * or #rwnil if no key was found. The value is put in \a v. You are
     * responsible for defining \a v before calling this function.
     */
     ...
     * pointed to by \a target, or #rwnil if no key was found.
     */
    RWCollectable* findValue(const RWCollectable* target) const;

    /**
     * Returns the value associated with the key which isEqual() to the item
     * pointed to by \a target, or #rwnil if no key was found. Replaces the
     * value with \a newValue (if a key was found).
     */
    RWCollectable* findValue(const RWCollectable* target, RWCollectable* newValue) const;

    /**
     * Adds a copy of the RWCollectableAssociation \a a to the collection
     * and returns its key. If an item in the collection already has a key
     * which isEqual() to \a a, then #rwnil is returned and the item is not
     * inserted.
     *
     * \note
     * If \a a is not an RWCollectableAssociation, this function throws
     * std::bad_cast.
     */
     ...
     * pointed to by \a target. Returns the key, or #rwnil if no match was
     * found.
     */
    virtual RWCollectable* remove(const RWCollectable* target);

    /**
     * Removes \e and deletes the key and value pair where the key isEqual()
     * to the item pointed to by \a target. Note that both the key and the
     * value are deleted. Does nothing if the key is not found.
     */
     ...
     * Removes the key and value pair where the key isEqual() to the item
     * pointed to by \a target. Returns the key, or #rwnil if no match was
     * found. The value part of the removed pair is put in \a v. You are
     * responsible for defining \a v before calling this function.
     */
    ...
    /**
     * Evaluates the function pointed to by \a tst for the key of each item
     * in the RWHashDictionary. It inserts keys and values for which the
     * function returns \c true into a new RWHashDictionary allocated off
     * the heap and returns a pointer to this new collection. Because the
     * new dictionary is allocated <em>off the heap</em>, you are
     * responsible for deleting it when done. This is a \c virtual function
     * which hides the non-virtual function inherited from RWCollection.
     */
    virtual RWCollection* select(RWtestCollectable tst, void* x) const;

    /**
     * Evaluates the function pointed to by \a testfunc for both the key and
     * the value of each item in the RWHashDictionary. It inserts keys and
     * values for which the function returns \c true into a new
     * RWHashDictionary allocated off the heap and returns a pointer to this
     * new collection. Because the new dictionary is allocated <em>off the
     * heap</em>, you are responsible for deleting it when done. This is a
     * \c virtual function which hides the non-virtual function inherited
     * from RWCollection.
     */
 ...
 *
 * Iterator for class RWHashDictionary, allowing sequential access to all
 * the elements of RWHashDictionary. Since RWHashDictionary is unordered,
...
public:
    ...
    /**
     * Constructs an iterator for an RWHashDictionary collection.
     ...
     * to the object pointed to by \a target. Returns the key or #rwnil if
     * no key was found.
     */
     ...
     * Returns #rwnil if the cursor is at the end of the collection. Use
     * member function value() to recover the value.
     */
    ...
    /**
     * Removes the key-value pair at the current iterator position. Returns
     * the key, or #rwnil if there was no key-value pair.
     */
    RWCollectable* remove();

    /**
     * Moves the iterator to the next key-value pair where the key isEqual()
     * to the object pointed to by \a target. Removes the key-value pair,
     * returning the key or #rwnil if there was no match.
     */
 ...
 *
 * \brief
 * Allows sequential access to all the elements of RWHashDictionary and, as
 * a \c const iterator, does not change anything in the underlying data
 * structure
 *
 * One of two iterators for class RWHashDictionary. This iterator allows
 * sequential access to all the elements of RWHashDictionary, and, as a
 * \c const iterator, does not change anything in the underlying data
 * structure. Since RWHashDictionary is unordered, elements are not accessed
