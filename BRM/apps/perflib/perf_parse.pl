...
#------------------------------------------------------------------------------#
# Function     : op_on_opname_stack
#
# Description  : Determine whether this is a recursive opcode call or not ...
#                If this is a bucket, check one level less as the last level
#                is always going to be the same opcode!!
#
# Input        : op         Opcode name
# Input        : bucket_id  Bucket ID
#
# Output       : None
#
# Return       : 1 if it's recursive, 0 if not
#------------------------------------------------------------------------------#
sub op_on_opname_stack {
   my ($op, $bucket_id) = @_;
   my $n = scalar(@G_OPNAME_STACK);
   $n-- if ($bucket_id);
   for (0..($n-1)) {
      if ($op eq $G_OPNAME_STACK[$_]) {
         return 1;
      }
   }
   return 0;
...
# Function     : fmt_number
#
# Description  : Format a number with separator ...
#
# Input        : n     Number to format
#
# Output       : None
#
# Return       : formatted number with commas inserted.
#------------------------------------------------------------------------------#
sub fmt_number {
  my ($n) = @_;
  $n =~ s/(^\s*|\s*$)//g;
  $n = reverse($n);
  $n =~ s/(\d\d\d)(?=\d)(?!\d*\.)/${1}$G_THOUSANDS_SEP/g;
...
sub oppath_on_filter_path {
   ...
   for ($i = 0; $i <= $#{$aref}; $i++) {
      if ($i > $#{$fp_aref}) {
         ...
         return 1;
      } elsif (# Allow READ_OBJ or PCM_OP_READ_OBJ as filter to take into
               # account $p_remove_pcm_op
               $fp_aref->[$i] ne $aref->[$i] &&
               "PCM_OP_$fp_aref->[$i]" ne $aref->[$i] &&
               $fp_aref->[$i] ne "PCM_OP_$aref->[$i]")
      {
...
#                is printed with buckets first (and then, by time)
#
# Input        : a    Sort key A
#                b    Sort key B
#
...
sub aggregate_sort_func {
   if ($printing_full_path) {
      my $a_idx = $G_SORT_REF->{$a}->[AGG_BUCKET_ID_IDX];
      my $b_idx = $G_SORT_REF->{$b}->[AGG_BUCKET_ID_IDX];
      if ($a_idx > 0 && $b_idx > 0) {
         return $a_idx - $b_idx;
      } elsif ($a_idx <= 0 && $b_idx > 0) {
         return -1;
      } elsif ($b_idx <= 0 && $a_idx > 0) {
         return 1;
      }
   }
   my $rv = $G_SORT_REF->{$b}->[AGG_DURATION_IDX] <=> $G_SORT_REF->{$a}->[AGG_DURATION_IDX];
   return $rv if ($rv);
   return $a cmp $b;
}
...
sub print_main_report {
   ...
   if ($printing_full_path) { 
      $xl_recid = 0;
      print_report_level (\%G_FULL_PATH, undef, 0, $elapsed, $opcode_elapsed, "");
   } else {
      $xl_recid = 0;
      print_report_level (\%G_SIMPLE, undef, 0, $elapsed, $opcode_elapsed, "");
   }
}
...
#                phref           Parent hash reference (in full-path)
...
sub print_report_level {
   my ($href, $phref, $depth, $elapsed, $opcode_elapsed, $base_opstr) = @_;
   my $duration;
   my ($pct_tot, $pct_ela);
   ...
   foreach my $aggr_key (sort aggregate_sort_func keys %{$href}) {
      ...
      if ($p_filter_type ne "F" || $count >= 0) {
         ...
         if ($bucket_id) {
            ...
            # bucket. Note that in the case of full-path data, the totals
            # are in the "parent" hash. For simple aggregates, they're all
            # in the main hash.
            #
            my $new_aref;
            ...
            if (defined($phref)) {
               $new_aref = $phref->{$new_aggr_key};
            } else {
               $new_aref = $href->{$new_aggr_key};
            }

            my $bucket_pct = 0.0;
            if ($p_buckets == 1) {  # -b (percentage  of calls)
               if ($new_aref->[AGG_COUNT_IDX] > 0) {
                  $bucket_pct = ($count * 100.0) / $new_aref->[AGG_COUNT_IDX];
               }
            } elsif ($p_buckets == 2) { # -B - percentage of time)
               if ($new_aref->[AGG_DURATION_IDX] > 0.0) {
                  $bucket_pct = ($duration * 100.0) / $new_aref->[AGG_DURATION_IDX];
               }
            }
         ...
         foreach my $opt (split(//, $p_attributes)) {
            ...
            if ($len) {
               ...
               if ($p_thousands_sep) {
                  printf $col_attributes{$opt}{hfmt},
                     fmt_number(sprintf($col_attributes{$opt}{dfmt}, $data));
               } else {
      ...
      if ($sub_ref) {
         print_report_level ($sub_ref, $href, $depth+1, $elapsed, $opcode_elapsed,
                             $dot_opstr);
      }
   }
}
...
sub process_data_row {
   ...
   #
   # Handle recursive opcode calls by renaming the opcode to inclue the level
   # at which it is nested...
   #
   my $nested = 0;
   pop(@G_OPNAME_STACK) if ($opstatus eq "F");
   $nested = 1 if (op_on_opname_stack($opname, $bucket_id));
   push(@G_OPNAME_STACK, $opname) if ($opstatus eq "S");

   $opname = "${opname}_${oplevel}" if ($nested); # Modify recursive calls 

   #
   # Build aggregation key for this operation ...
   #
   ...
   if ($opstatus eq "F") {
      ...
      #
      # Buckets should not count in these totals (so just add them to the
      # bucket level, but don't roll-them up)...
      #
      my $min_oplevel = $bucket_id > 0 ? ($oplevel - 1) : 0;
      for ($i = $oplevel - 1; $i >= $min_oplevel; $i--) {
      ...
      if ($opname ne "VRT_OP_ELAPSED" && $p_fp_depth >= $oplevel) {
         ...
         # call-stack.
         #
         if ($p_cache && $cache_calls > 0 && !$bucket_id) {
...
sub usage {
   print STDERR <<XXX;
usage : $CMD [-h] [-d] [-c] [-v] [-b|-B] [-l] [-p|-P] [-s<depth>] [-S]
...
where : -h            This 'help' message
        ...
        -b|-B         Show opcode "buckets" (if there were any in the trace
                      file). Opcode buckets are the sub-divisions of opcode
                      timings into specific time ranges. if -b is used the
                      percentage of calls in a bucket is shown (otherwise the
                      percentage of time in a bucket is shown).
        ...
        -t            Show numbers with thousands separators (only screen)
...
sub opcode_name {
   ...
   } elsif (exists($map_ref->{$opcode})) {
      $opname = $map_ref->{$opcode};
   }

   $opname = "PCM_OP_CONTEXT_OPEN" if ($opname eq "VRT_OP_CONTEXT_OPEN");
   $opname = "PCM_OP_CONTEXT_CLOSE" if ($opname eq "VRT_OP_CONTEXT_CLOSE");
...
$p_attributes = "otpcear"; # opcode/total/pct/calls/errors/avg/recs
...
$p_thousands_sep = 0;      # Do we need to print numbers with thousands separator?
$G_THOUSANDS_SEP = ",";    # Thousands separator (default)
$G_NUMERIC_WIDTH = 9;      # Basic definition of numeric width

#
# Login filter 'paths' ...
#
@login_filter = qw(PCM_OP_ACT_LOGIN PCM_OP_ACT_LOGOUT PCM_OP_ACT_FIND_VERIFY
                   ACT_LOGIN ACT_LOGOUT ACT_FIND_VERIFY);
...
#
getopts('a:hdcCvlpPf:F:s:Sx:X:m:bBrn:i:Dz:Nt', \%opt) or usage();
if ($opt{h}) { usage() }
if ($opt{d}) { $p_debug = 1 }
...
if ($opt{b}) {
   if ($p_buckets == 2) {
      printf STDERR "ERROR: options 'b' and 'B' are mutually exclusive\n"; 
      exit(1);
   }
   $p_buckets = 1;
}
...
#
# Determine numeric width (9 + decimal places) ...
#
$p_nw = $G_NUMERIC_WIDTH + $p_dp;
if ($opt{t}) {
   $p_thousands_sep = 1;
   $p_nw += ($G_NUMERIC_WIDTH / 3);
}
...
if ($has_opt_a && $p_show_src) {
   ...
   # opcode/source/total/calls/errors/average/records
   $p_attributes = "ostpcear";
}
