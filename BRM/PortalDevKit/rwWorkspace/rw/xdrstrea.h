 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/xdrstrea.h#1 $
 *
 **********************************************************************
 *
 ...
 * transmitted using XDR routines.
 *
 * Class RWXDRistream enables decoding an XDR structure to a machine
 * representation, providing the ability to decode all the standard data
 * types and vectors of those data types.
 *
 * An XDR stream must first be created by calling the appropriate creation
 * routine. XDR streams currently exist for encoding/decoding of data to or
 * from standard iostreams and file streams, TCP/IP connections and Unix
 * files, and memory. These creation routines take arguments that are
 * tailored to the specific properties of the stream. After the XDR stream
 * has been created, it can then be used as the argument to the constructor
 * for a RWXDRistream object.
 *
 * RWXDRistream can be interrogated as to the status of the stream using
 * member functions bad(), clear(), eof(), fail(), good(), and rdstate().
 *
 * \conditional
...
public:
    ...
    /**
     * Initializes an RWXDRistream from the \b std::streambuf \a sb. \a sb
...
#    if !defined(RW_NO_XDR_HYPER_OP)
     ...
     * Thrown if the necessary support for reading a <tt>long long</tt> from
     * an XDR is not available.
     *
     * \note
     ...
     *
     * \exception RWInternalErr
     * Thrown if the necessary support for reading an
...
#  if !defined(RW_NO_LONG_DOUBLE)
    /**
     * \exception RWInternalErr
     * This function is not supported with XDR streams.
     *
     * \note
     * This operator function is available only if your compiler supports
 ...
 * transmitted using XDR routines.
 *
 * Class RWXDRostream enables outputting from a stream and encoding an XDR
 * structure from a machine representation. RWXDRostream provides the
 * ability to encode the standard data types and vectors of those data
 * types.
 *
 * An XDR stream must first be created by calling the appropriate creation
 * routine. XDR streams currently exist for encoding/decoding of data to or
 ...
 *
 * RWXDRostream can be interrogated as to the status of the stream using
 * member functions bad(), clear(), eof(), fail(), good(), and rdstate().
 *
 * \conditional
 * This class is not supported on Windows.
...
public:
    ...
    /**
     * Initializes an RWXDRostream from the \b std::streambuf \a sb. \a sb
     * must already be allocated.
     */
     ...
     *
     * \note
     * In a 64-bit environment, this method fails if the value of \a i is
...
#    if !defined(RW_NO_XDR_HYPER_OP)
     ...
     *
     * \exception RWInternalErr
     * Thrown if the necessary support for storing a <tt>long long</tt> on
     * an XDR is not available.
     *
     * \note
     * This operator function is available only if your compiler supports
     ...
     *
     * \exception RWInternalErr
     * Thrown if the necessary support for storing an
...
#  endif
     ...
     *
     * \note
     * In a 64-bit environment, this method fails if the value of one of the
     * stored elements in \a v is outside the range of a 32-bit signed
     * integer.
     */
     ...
     * \note
     * In a 64-bit environment, this method fails if the value of one of the
     * stored elements in \a v is outside the range of a 32-bit unsigned
     * integer.
     */
