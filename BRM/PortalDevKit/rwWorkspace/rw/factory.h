 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/factory.h#1 $
 *
 **********************************************************************
 *
 ...
 * associated "creator functions." A creator function has the prototype:
 *
 * \code
 * RWCollectable* aCreatorFunction();
 * \endcode
 *
 * This function creates an instance of a particular class. For a given
 * #RWClassID tag, the appropriate function is selected, invoked and the
 * resultant pointer returned. Because any object created this way is
 * created off the heap, you are responsible for deleting it when done.
 *
 * A one-of-a-kind global RWFactory can be accessed using getRWFactory(). It
 * is guaranteed to have creator functions in it for all of the classes
 * referenced by your program. See the section in the <em>Essential Tools
 * Module User's Guide</em> about RWFactory.
 *
 * \synopsis
...
public:
    ...
    /**
     * Adds to RWFactory the global function pointed to by \a uc, which
     * creates an instance of an object with #RWClassID \a id.
     */
    void addFunction(RWuserCreator uc, RWClassID id);

    /**
     * Allocates a new instance of the class with #RWClassID \a id off the
     * heap and returns a pointer to it. Returns #rwnil if \a id does not
     * exist. Because this instance is allocated <em>off the heap</em>, you
     * are responsible for deleting it when done.
     */
     ...
     * #RWClassID \a id. If \a id does not exist in the factory, no action
     * is taken.
     */
    ...
    /**
     * Adds to RWFactory the global function pointed to by \a uc, which
     * creates an instance of an object with #RWClassID \a id and RWStringID
     * \a sid.
     */
    void addFunction(RWuserCreator uc, RWClassID id, RWStringID sid);

    /**
     * Allocates a new instance of the class with RWStringID \a sid off the
     * heap and returns a pointer to it. Returns #rwnil if \a sid does not
     * exist. Because this instance is allocated <em>off the heap</em>, you
     * are responsible for deleting it when done.
     */
    ...
    /**
     * Returns from RWFactory a pointer to the global function associated
     * with RWStringID \a sid. Returns #rwnil if \a sid does not exist.
     */
    RWuserCreator getFunction(RWStringID sid) const;

    /**
     * Removes from RWFactory the global function associated with RWStringID
     * \a sid. If \a sid does not exist in the factory, no action is taken.
     */
    void removeFunction(RWStringID sid);

    /**
     * Looks up the RWStringID associated with \a id and returns it. If
     * there is no such association, returns RWStringID("NoID").
     */
    RWStringID getStringID(RWClassID id) const;

    /**
     * Looks up the #RWClassID associated with \a sid and returns it. If
     * there is no such association, returns \c __RWUNKNOWN.
     */
    ...
    struct RW_TOOLS_SYMBOLIC MetaFunctions {
        MetaFunctions():
            ...
            saveToRWvostream(0),
            restoreFromRWFile(0),
            restoreFromRWvistream(0)
        {}

        MetaFunctions(RWuserCreator createFunc, RWCollectableSaveToRWFile saveToRWFileFunc, RWCollectableSaveToRWvostream saveToRWvostreamFunc, RWCollectableRestoreFromRWFile restoreFromRWFileFunc, RWCollectableRestoreFromRWvistream restoreFromRWvistreamFunc):
            create(createFunc),
            saveToRWFile(saveToRWFileFunc),
            saveToRWvostream(saveToRWvostreamFunc),
            restoreFromRWFile(restoreFromRWFileFunc),
            restoreFromRWvistream(restoreFromRWvistreamFunc)
        {}
...
private:
     ...
     * (\a create), saving the associated object (\a saveFile,
     * \a saveStream) and restoring the associated object (\a restoreFile,
     * \a restoreStream).
     */
 ...
 * \relates RWFactory
 *
 * Returns a pointer to the global RWFactory object instance, creating it if
 * necessary.
 */
