 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/sortvec.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * RWSortedVectorIterator is a typedef for RWOrderedIterator. For
 * information on how to use it, please see RWOrderedIterator.
 */
 ...
 * \relates RWSortedVector
 *
 * RWSortedVectorConstIterator is a typedef for RWOrderedConstIterator. For
 * information on how to use it, please see RWOrderedConstIterator.
 */
 ...
 * maintain the vector in sorted order.
 *
 ...
 *
 * \note
 * The vector is sorted; do not modify elements contained in the vector in
...
public:
    ...
    /**
     * Returns \c __RWSORTEDVECTOR, i.e. a <em>class identifier</em>, that
     * identifies this object's class.
     */
...
public:
    ...
    /**
     * Constructs an empty RWSortedVector that has an initial capacity of
     * RWCollection::DEFAULT_CAPACITY items. The capacity is increased
    ...
    /**
     * Constructs an empty RWSortedVector that has an initial capacity of
     * \a size items. The capacity is increased automatically as needed.
     */
    ...
    /**
     * Move constructor. The constructed RWSortedVector takes ownership of
     * the data owned by \a sv.
     *
     * \conditional
     ...
     * \a sv at the same index isEqual(). The two collections must also have
    ...
    /**
     * Performs a binary search to return the index of the first item that
     * compares equal to the \a target item, or #RW_NPOS if no such item can
     * be found.
     *
     * \note
     * Derived methods RWOrdered::find() and RWOrdered::remove() use this
     * virtual function to perform their search. Hence, these methods called
     * on an instance of RWSortedVector results in a binary search.
     */
    virtual size_t index(const RWCollectable* target) const;

    /**
     * Performs a binary search to insert the item pointed to by \a c after
     * all items that compare less than or equal to it, but before all items
     * that compare greater to it. Returns \a c if the insertion was
     * successful, otherwise returns #rwnil.
     */
     ...
     * out of range, an exception of type RWBoundsErr is thrown. The return
     * value cannot be used as an lvalue.
     */
