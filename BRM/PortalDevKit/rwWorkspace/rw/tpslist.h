 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tpslist.h#1 $
 *
 **********************************************************************
 *
 ...
 * a singly-linked list. Class \c T is the type pointed to by the items in
 ...
 * \sa
 * \parblock
 * Classes RWTPtrDlist, RWTPtrDeque, and RWTPtrOrderedVector also provide a
 * Rogue Wave pointer-based interface to C++ Standard sequence collections.
 *
 * Class \ref rw_slist "rw_slist<T*,A>" is the C++ Standard Library
...
public:
     ...
     * of items replaced. Client data may be passed through parameter \c d.
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Constructs a singly-linked list by copying all elements of \a lst.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrSlist::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTPtrSlist
 *
 * RWTPtrSlistIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTPtrSlist. (An STL iterator
 * interface is provided through RWTPtrSlist::iterator.)
 *
 * The order of iteration over an RWTPtrSlist is dependent upon the order of
 * insertion of items into the container.
 *
 ...
 *
 * If the \c slist is modified, the iterator is no longer valid. This is
...
public:
    /**
     * Creates an iterator for the list \a lst. The iterator begins in an
     ...
     * will now reference an undefined value distinct from the reset value,
     * and a value equivalent to \c false will be returned. Otherwise, a
     * value equivalent to \c true is returned.
     */
    bool operator++();

    /**
     * Behaves as if the operator++() member function had been applied \a n
     * times.
     */
     ...
     * such that the expression <tt>(*t == *\a a)</tt> is \c true. If no
     * such element is found, returns #rwnil. Leaves self referencing the
     ...
     * element pointed to by self. Leaves self referencing the prior item,
     * or in \c reset condition.
     */
     ...
     * list. Leaves self referencing the prior item, or in \c reset
     * condition.
     */
    T* remove();
     ...
     * \c true. Leaves self referencing the prior item, or in \c reset
     * condition.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrSlist::const_iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTPtrSlist.
 *
 * RWTPtrSlistConstIterator provides a \c const iterator interface to C++
 * Standard Library-based RWTPtrSlist collections. A \c const iterator does
 * not change anything in the data structure it iterates over.
 *
 * The order of iteration over an RWTPtrSlist is dependent upon the order of
 * insertion of items into the container.
 *
...
public:
    /**
     * Creates a \c const iterator for the list \a lst. The iterator begins
    ...
    /**
     * Behaves as if the operator++() member function had been applied \c n
     * times.
     */
    bool operator+=(size_t n);
    ...
    /**
     * Returns the first element \c t encountered by iterating self forward,
     * such that the expression (*t&nbsp;==&nbsp;*a) is \c true. If no such
     * element is found, returns #rwnil. Leaves self referencing the found
     * item or "off the end."
     */
