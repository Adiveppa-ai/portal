...
iScript
{
  ...
  const Long RapReleaseVersionNumber        = 5;

  include "RAP_0105_MissingReturn.dsc";
  include "RAP_0105_StopReturn.dsc";

  // constants
  const String extension_GSMW     ="520";
  const String extension_LTE      ="530";
  ...
  const String type_MSG           = "081";
  const String type_MSESS         = "082";
  ...
  const Long OprtMsgIdx       = 6;  
  const Long ValidationRuleID = 7;
  const Long MaxParamCount    = 8;
...
//  Long    theTap3ReleaseVersionNumber;
  ...
  function BEGIN
  {
    ...
    reg_StopRapStream     = reg_TapIOModule + ".Module.StopRapStream";
    String strStopRapStream;
    strStopRapStream     =  regString(reg_StopRapStream);
    strStopRapStream = strToUpper(strStopRapStream);
    bStropRapStream = false;
    if(strStopRapStream != "" and strStopRapStream == "TRUE")
    {
       bStropRapStream = true;
    } 
  ...
  // -------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------
  function updateTapCurrency
  {
    asnTreeGetStoredNode(tapcurrencyInfoIdx);  // ie. asnTreePushTag("RAP_0105.RapBatchControlInfo");
  ...
  function handleMissingError
  {
    ...
    if (current_StreamName != "")
    {
      ...
      if (toReturn_RapSeqNum != "")
      {
        ...
        asnTreeAddString("TAP_0312.Sender.Sender",    toReturn_Sender);
        asnTreeAddString("TAP_0312.Recipient.Recipient", toReturn_Recipient);
        ...
        asnTreeAddInteger("TAP_0312.TapDecimalPlaces.TapDecimalPlaces",3);
    ...
    asnTreePushTag("RAP_0105.ErrorDetail");
      ...
      { 
        asnTreePushTag("RAP_0105.ErrorContext");
          ...
          asnTreeAddInteger("RAP_0105.ItemLevel.ItemLevel", i+1);
      ...
      if (checkNumber >= 0)
      {
        asnTreeAddInteger("RAP_0105.ItemOffset.ItemOffset", checkNumber);
      }

      checkNumber = strToLong(paramList[TAP_ErrCode]);
      asnTreeAddInteger("RAP_0105.ErrorCode.ErrorCode", checkNumber);
    asnTreePop(); // asnTreePushTag("RAP_0105.ErrorDetail");
  }

  // -------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------
  function setOperatorSpecInfoList 
  {
    Long    i;
    Long    nbElem;
    String  tags[];

    if ( strSearchRegExpr(paramList[OprtMsgIdx], "[^ \t]") >= 0 )
    {
      if (operatorSpecPresence == false)
      {
        operatorSpecPresence = true;
        asnTreePushTag("RAP_0105.OperatorSpecList");
      }
      nbElem = strSplit(tags, paramList[OprtMsgIdx], ";");
	  
      if ( nbElem == 0)
      {
        asnTreeAddString("TAP_0312.OperatorSpecInformation.OperatorSpecInformation", paramList[OprtMsgIdx] );
      }
      else
      {
        for (i=0 ; i < nbElem ; i=i+1)
	{
          asnTreeAddString("TAP_0312.OperatorSpecInformation.OperatorSpecInformation", tags[i] );
	}
      }
    }
  }
  ...
  function handleSevereError
  {
    ...
    Long    paramCount=0;
    Bool    foundOne=false;
    operatorSpecPresence = false;

    rapDetailsWritten = rapDetailsWritten + 1;

    asnTreePushTag("RAP_0105.SevereReturn");
    ...
    //Following loop is for operator spec information
    for ( i=0; i < edrNumErrors(); i=i+1 )
    {
      if (edrGetError(i) == "ERR_TAP3_RET")
      {
        ...
        if ( (paramCount == MaxParamCount) and
             (paramList[TAP_Severity] == "severe") )
        {
          setOperatorSpecInfoList();
        }
      }
    }

    if (operatorSpecPresence == true)
    {
      asnTreePop(); //asnTreePushTag("RAP_0105.OperatorSpecList");
    }

    asnTreePop(); // asnTreePushTag("RAP_0105.SevereReturn");
...
Grammar
{
  ...
  receive_stream:
    ...
    trailer
    {
      ...
      // go back to header and store the tapCurrency
      if(bStropRapStream == false)
      {
        updateTapCurrency();
  ...
  header:
    HEADER
    {
      ...
      asnTreePushTag("RAP_0105.RapBatchControlInfo");
        ...
        {
          asnTreeAddInteger("TAP_0312.TapDecimalPlaces.TapDecimalPlaces",
                              edrLong(HEADER.TAP_DECIMAL_PLACES) );
        }
        tapcurrencyInfoIdx = asnTreeStoreNode();
  ...
  oneDetail:
    DETAIL
    {
      ...
      if( tapCurrency == "")
      {
        tapCurrency = edrString(DETAIL.WHOLESALE_CHARGED_AMOUNT_CURRENCY);
      }
