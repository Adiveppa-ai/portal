 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvhset.h#1 $
 *
 **********************************************************************
 *
 ...
 * to a hash object of type \c H. Class \c T is the type of items in the
 * collection. Class \c A is an allocator of objects of class \c T. For more
 ...
 * \sa
 * \parblock
 * Class RWTValHashMultiSet offers the same interface to a collection that
 * does accept multiple items that compare equal to each other.
 *
 * Class \ref rw_hashset "rw_hashset<T,H,EQ,A>" is the C++ Standard Library
...
public:
     ...
     * \a s. Self is replaced by the result. Note the use of the uppercase
     ...
     * returns \c true unless the collection already holds an element with
     * the equivalent key.
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES
     ...
     * capacity of \a sz. resize() copies every element of self into the new
...
#  endif
    ...
    /**
     * Creates an empty hashed set that uses the hash object \a h and has an
     * initial hash table capacity of \a sz.
     *
     * \note
 ...
 * to it, or sets \a p to point to a previously read instance. If a
 * collection is created off the heap, the application is responsible for
 * deleting it.
 */
template <class T, class H, class EQ , class A>
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValHashSet::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTValHashSet.
 *
 * RWTValHashSetIterator provides an iterator interface to the C++ Standard
 * Library collection represented by RWTValHashSet. (An STL iterator
 * interface is provided through RWTValHashSet::iterator.)
 *
 * Iteration over an RWTValHashSet is pseudorandom and dependent on the
...
public:
    /**
     * Creates an iterator for the hashset \a h. The iterator begins in an
     ...
     * iteration, self referenced the last value in the multiset, self will
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValHashSet::const_iterator to iterate over
 * the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValHashSet.
 *
 * RWTValHashSetConstIterator provides a \c const iterator for
 * RWTValHashSet. A \c const iterator prohibits any changes to the
...
public:
    ...
    /**
     * Advances self to the next element. If the iterator has been reset or
     * just created self references the first element. If, before iteration,
     * self referenced the last value in the multiset, self references an
     * undefined value and returns \c false. Otherwise, it returns \c true.
     */
    bool operator++()    // Advance and test
    {
        RW_ASSERT(!(head_ == true && tail_ == true));
