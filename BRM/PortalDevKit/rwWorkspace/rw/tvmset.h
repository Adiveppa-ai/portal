 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvmset.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * RWTValMultiSet may contain multiple items that compare equal to each
 * other. (RWTValSet will not accept an item that compares equal to an item
 ...
 * \sa
 * \parblock
 * Class RWTValSet offers the same interface to a collection that will not
 * accept multiple items that compare equal to each other. RWTValMultiMap
...
public:
     ...
     * the \c C object. This method does not copy or destroy any of the
     * items swapped; rather, it swaps the underlying hash tables.
     */
    void
 ...
 *
 * RWTValMultiSetIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTValMultiSet. (An STL iterator
 * interface is provided through RWTValMultiSet::iterator.)
 *
 * The order of iteration over an RWTValMultiSet is dependent on the
 * supplied comparator object parameter \c C as applied to the values stored
 * in the container.
 *
...
public:
    /**
     * Creates an iterator for the multi-set \a h. The iterator begins in an
     ...
     * iteration, self referenced the last association in the multi-set,
    ...
    T key() const
    {
        return *(const_cast<RWTValMultiSetIterator*>(this)->iter_);
    }
 ...
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValMultiSet.
 *
 * RWTValMultiSetConstIterator provides a \c const iterator for
 * RWTValMultiSet. A \c const iterator prohibits any changes to the
...
public:
    /**
     * Creates a \c const iterator for the multi-set \a h. The iterator
