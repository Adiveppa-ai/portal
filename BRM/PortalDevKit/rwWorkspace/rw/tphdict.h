 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tphdict.h#1 $
 *
 **********************************************************************
 *
 ...
 * RWTPtrHashMultiMap if you need multiple keys to be equivalent to each
 * other. Equality is based on the comparison object, not on
 * \c operator==().
 *
 * The \c value type must have \c operator==() defined. This requirement is
 ...
 * \sa
 * \parblock
 * Class RWTPtrHashMultiMap offers the same interface to a pointer-based
 * collection that accepts multiple keys that compare equal to each other.
 *
 * Class \ref rw_hashmap "rw_hashmap<K*,T*,hasher,key_equal,A>" is the C++
...
public:
    ...
    /**
     * Returns a pointer to the minimum key in the collection. Type \c K
    ...
    /**
     * Returns a pointer to the maximum key in the collection. Type \c K
     * must have well-defined less-than semantics
     * (<tt>K::operator<(const K&)</tt> or equivalent).
     */
    ...
    /**
     * If <tt>find(\a key)</tt> would succeed, erases the pair associated
     ...
     * associations removed. Client data may be passed through parameter
     * \a d.
     */
     ...
     * keys and their associated items. It is important that the equality
     * object associated with this container successfully exclude duplicate
     * entries since using this method when there are duplicates results in
     ...
     * the key is not in the dictionary, is is added with an associated
     * uninitialized pointer of type #mapped_type. Because of this, if there
     * is a possibility that a key will not be in the dictionary, this
    ...
    /**
     * Changes the capacity of self by creating a new hashmap with a
     * capacity of \a sz. Then copies every association in self into the new
     ...
     * associations from \a rhs into self.
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Constructs a pointer-based hashmap by copying all associations from
     * \a m.
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Creates an empty hashmap that uses the hash object \a h and has an
     * initial capacity of \a sz.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrHashMap::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTPtrHashMap.
 *
 * RWTPtrHashMapIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTPtrHashMap. (An STL iterator
 * interface is provided through RWTPtrHashMap::iterator.)
 *
 * Iteration over an RWTPtrHashMap is pseudo-random and dependent on the
...
public:
    /**
     * Creates an iterator for the hashmap \a h. The iterator begins in an
     ...
     * references the last association in the hashmap, self references an
     ...
     * last item in the hashmap, returns #rwnil.
     */
    K* operator()()
    {
    ...
    /**
     * Returns the key portion of the association pointed to by self. The
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrHashMap::const_iterator to iterate over
 * the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTPtrHashMap.
 *
 * RWTPtrHashMapConstIterator provides a \c const iterator interface to the
 * C++ Standard Library-based container RWTPtrHashMap. This iterator
 * prohibits any changes to the underlying collection.
 *
 * Iteration over an RWTPtrHashMap is pseudo-random and dependent on the
 * capacity of the underlying hash table and the hash function being used.
 *
 ...
 *
 * For both operator++() and operator()(), iterating past the last element
 * returns a value equivalent to boolean \c false. Once this state is
 * reached, continued increments return a value equivalent to \c false until
 * reset() is called.
 *
 * The value type must have \c operator==() defined. This requirement is
 * imposed by the C++ Standard Library.
...
public:
     ...
     * in an undefined state and must be advanced before the first
     ...
     * association within the hashmap.
     */
    const K* key() const
    {
        return (*iter_).first;
    ...
    /**
     * Returns the value portion of the association currently referenced by
     * self. The return value is undefined if self is not referencing an
     * association within the hashmap.
     */
    const T* value() const
