 ...
 *
 * The class RWFileStat provides file statistical information in a portable
 * manner. RWFileStat is:
 *
 * - not Assignable
...
struct RW_TOOLS_SYMBOLIC RWFileStat {
    ...
    // Ctors
    /**
     * Constructs the object by using a stat function on the filename given
     * in \a s. The call succeeds or throws an exception of type
     * <tt>(or derived of) RWInternalErr</tt> . The error can be inspected
     * by looking at \c errno. If the filename indicates a symbolic link, it
     * uses a stat function on the target of the symbolic link rather than
     * the symbolic link itself when the parameter \a follow_links is
     * \c true (default). Otherwise, it uses a stat function on the symbolic
     * link.
     */
    ...
    /**
     * Returns an integer which is a bit mask value indicating the type of
     * the file and the permissions for it.
     */
