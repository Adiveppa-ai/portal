 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvhmmap.h#1 $
 *
 **********************************************************************
 *
 ...
 * \c H. Class \c T is the type of items in the collection. Class \c A is an
 ...
 *
 * RWTValHashMultiMap may contain multiple keys that are equivalent to each
 * other. (RWTValHashMap will not accept a key that is equivalent to any key
 * already in the collection.) Equality is based on the comparison object
 * and not on the equality operator.
 *
 ...
 * \sa
 * \parblock
 * Class RWTValHashMap offers the same interface to a collection that will
 * not accept multiple keys that compare equal to each other.
 *
 * Class \ref rw_hashmultimap "rw_hashmultimap<K,T,H,EQ,A>" is the C++
...
public:
     ...
     * accessibility to the C++ Standard Library interface as well as
     ...
     * #key_type must have well-defined less-than semantics
     * (<tt>K::operator<(const &)</tt> or equivalent).
     */
    ...
    /**
     * Returns the number of keys \c j in self that compares equal to
     * \a key.
     *
     * \note
     * This method requires the type #mapped_type to have a default
     ...
     * capacity of \a sz. This method then copies every element of self into
     * the new container and finally swaps the internal representation of
     * the new container with self.
     */
    void
    resize(size_type sz)
    {
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValHashMultiMap::iterator to iterate over
 * the elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTValHashMultiMap.
 *
 * RWTValHashMultiMapIterator provides an iterator interface to the C++
 * Standard Library-based collection represented by RWTValHashMultiMap. (An
 * STL iterator interface is provided through RWTValHashMultiMap::iterator.)
 *
...
public:
    /**
     * Creates an iterator for the hash multimap \a h. The iterator begins
     ...
     * iterator, and returns \c false if the iterator has advanced past the
     * last item in the container; otherwise returns \c true.
     */
    bool operator()()
    {
        return ++(*this);
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValHashMultiMap::const_iterator to iterate
 * over the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValHashMultiMap.
 *
 * RWTValHashMultiMapConstIterator provides a \c const iterator for
 * RWTValHashMultiMap. A \c const iterator prohibits any changes to the
 * collection it is iterating over.
 *
 * Iteration over an RWTValHashMultiMap is pseudorandom and dependent on the
 * capacity of the underlying hash table and the hash function being used.
 * The only usable relationship between consecutive elements is that
...
public:
     ...
     * Advances self to the next element, dereferences the resulting
     * iterator and returns \c false if the iterator has advanced past the
     * last item in the container and \c true otherwise.
     */
    bool operator()()
    {
        return ++(*this);
