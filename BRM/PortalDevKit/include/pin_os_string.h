/*
 *	@(#)%Oracle Version:  pin_os_string.h:RWSmod7.3.1Int:1:2007-Sep-12 04:46:01 %
 *      
...
size_t pin_strlcat(
	...
	const char*             src,
	size_t                  size );
#define pin_strncat( dest, dmax, src, n ) \
	(__pin_strncat( __FILE__, __LINE__, dest, dmax, src, n))
#define pin_strncpy( dest, dmax, src, n ) \
	(__pin_strncpy( __FILE__, __LINE__, dest, dmax, src, n))

#define pin_strnlen( str,size ) \
	(strnlen(str,size))

#define pin_realpath(path, res_path, res_path_max, ebufp) 			\
	((ebufp != (pin_errbuf_t *)0) && ((ebufp)->pin_err == PIN_ERR_NONE)) ? 	\
		(ebufp)->line_no = __LINE__, (ebufp)->filename = PIN_FILE_SOURCE_ID, __pin_realpath(__FILE__, __LINE__, path, res_path, res_path_max, ebufp) : (NULL)

#define pin_qsort(base, nmemb, size, compare_func, ebufp )			\
{										\
	if ((ebufp != (pin_errbuf_t *)0) && ((ebufp)->pin_err == PIN_ERR_NONE)) { 	\
		(ebufp)->line_no = __LINE__;					\
		(ebufp)->filename = PIN_FILE_SOURCE_ID;					\
		__pin_qsort(__FILE__, __LINE__, base, nmemb, size, compare_func, ebufp);\
	}									\
}

#define pin_ctime(timep, buf, location, ebufp) 					\
{										\
	if ((ebufp != (pin_errbuf_t *)0) && ((ebufp)->pin_err == PIN_ERR_NONE)) { 	\
		(ebufp)->line_no = __LINE__;					\
		(ebufp)->filename = PIN_FILE_SOURCE_ID;    				\
		__pin_ctime(__FILE__, __LINE__, timep, buf, location, ebufp);	\
	}									\
}

#define pin_localtime(timep, tm_ptr, location, ebufp) 				\
{										\
	if ((ebufp != (pin_errbuf_t *)0) && ((ebufp)->pin_err == PIN_ERR_NONE)) { 	\
		(ebufp)->line_no = __LINE__;					\
		(ebufp)->filename = PIN_FILE_SOURCE_ID;    				\
		__pin_localtime(__FILE__, __LINE__, timep, tm_ptr, location, ebufp);\
	}									\
}
