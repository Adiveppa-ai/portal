 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/hashtab.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * A simple hash table for objects inheriting from RWCollectable. Uses
 * chaining (as implemented by class RWSlistCollectables) to resolve hash
 * collisions
 *
 ...
 * \ref RWCollectable::hash() "hash()" and
 * \ref RWCollectable::isEqual() "isEqual()".
 *
 * To find an object that matches a key, the key's virtual function
 * \ref RWCollectable::hash() "hash()" is first called to determine in which
 * bucket the object occurs. The bucket is then searched linearly by calling
 * the virtual function \ref RWCollectable::isEqual() "isEqual()" for each
 * candidate, with the key as the argument. The first object to return
 * \c true is the returned object.
 *
 ...
 * greatly exceeds the number of buckets then efficiency sags because each
 * bucket must be searched linearly. The number of buckets can be changed by
 * calling member function resize(). This requires that all objects be
 * rehashed.
 *
...
public:
    ...
    /**
     * Constructs an empty hash table with RWCollection::DEFAULT_CAPACITY
     * buckets.
     */
    RWHashTable();
    ...
    /**
     * Copy constructor. Creates a new hash table as a shallow copy of the
     * table \a t. The new table has the same number of buckets as the old
     * table. Hence, the members need not be and will not be rehashed.
     */
     ...
     * the two tables have the same number of buckets. Hence, the members
    ...
    /**
     * Returns \c true if self is a subset of \a t, that is, every element
     * of self has a counterpart in \a t which
     * \ref RWCollectable::isEqual() "isEqual()".
     */
    bool    operator<= (const RWHashTable& t) const;

    /**
     * Returns \c true if self and \a t have the same number of elements and
     * if for every key in self there is a corresponding key in \a t which
     * \ref RWCollectable::isEqual() "isEqual()".
     */
    bool    operator== (const RWHashTable& t) const;
     ...
     * no particular order, due to the nature of hashing collections. The
     * function pointed to by \a ap should take no action that could change
     * the hash value or equality properties of the objects.
     */
     ...
     * rehashing all the members of the collection. If \a n is zero, resizes
     * to <tt>3*</tt>entries()<tt>/2</tt>.
     */
 ...
 *
 * Iterator for class RWHashTable, which allows sequential access to all the
 * elements of RWHashTable.
 *
 * \note
 * An RWHashTable is unordered, elements are not accessed in any particular
 * order.
 *
...
public:
    ...
    /**
     * Constructs an iterator for an RWHashTable. After construction, the
    ...
    /**
     * Removes the item at the current iterator position from the
     * collection.
     */
     ...
     * Moves the iterator to the next item which
     * \ref RWCollectable::isEqual() "isEqual()" to the item pointed to by
     * \a target, removes it from the collection and returns it. If no item
 ...
 *
 * \brief
 * Allows sequential access to all the elements of RWHashTable and, as a
 * \c const iterator, it does not change anything in the underlying data
 * structure
 *
 * One of two iterators for class RWHashTable. As a \c const iterator, it
 * does not change anything in the underlying data structure.
 *
...
public:
    ...
    /**
     * Constructs a \c const iterator for an RWHashTable. After
     * construction, the position of the iterator is undefined.
     */
    RWHashTableConstIterator(const RWHashTable& h);

    /**
     * Copy constructor. After construction, the position of the iterator is
     * undefined.
     */
