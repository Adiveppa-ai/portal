 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/pstream.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Specializes the abstract base class RWvistream to restore variables
 * stored in a portable US-ASCII format by RWpostream
 *
 * Class RWpistream specializes the abstract base class RWvistream to
 * restore variables stored in a portable US-ASCII format by RWpostream.
 *
 * You can think of RWpistream and RWpostream as an US-ASCII veneer over an
 * associated \b std::streambuf which are responsible for formatting
 ...
 * dependent. Because RWpistream and RWpostream retain no information about
 * the state of their associated \b std::streambuf, their use can be freely
 * exchanged with other users of the \b std::streambuf (such as
...
public:
    ...
    /**
     * Returns a reference to the underlying C++ Standard Library input
     * stream.
     */
    std::istream& std();
    ...
    /**
     * Gets a vector of \c char and stores it in the array beginning at
     * \a v. If the restore operation stops prematurely because there is no
     * more data available on the stream, because an exception is thrown, or
     * for some other reason, get() stores what has already been retrieved
     * from the stream into \a v, and sets the failbit.
     *
     * \note
     ...
     * string, use the function getString().
     */
    ...
    /**
     * Restores a character string from the input stream that was stored to
     * the output stream with RWpostream::putString and stores it in the
     * array beginning at \a s. The function stops reading at the end of the
     * string or after \a n <tt>- 1</tt> characters, whichever comes first.
     * If \a n <tt>- 1</tt> characters have been read and the \a n
     * <sup>th</sup> character is not the string terminator, then the
 ...
 *
 * \brief
 * Specializes the abstract base class RWvostream to store variables in a
 * portable (printable) US-ASCII format
 *
 * Class RWpostream specializes the abstract base class RWvostream to store
 * variables in a portable (printable) US-ASCII format. The results can be
 * restored by using its counterpart RWpistream.
 *
 * You can think of RWpistream and RWpostream as a US-ASCII veneer over an
 * associated \b std::streambuf. They are responsible for formatting
 * variables and escaping characters such that the results can be
 * interchanged between any machines. As such, they are slower than their
 * binary counterparts RWbistream and RWbostream, which are more machine
 * dependent. Because RWpistream and RWpostream retain no information about
 ...
 * \b std::istream or \b std::ifstream).
 *
 * The goal of class RWpostream and RWpistream is to store variables using
 * nothing but printable US-ASCII characters. Hence, nonprintable characters
 * must be converted into an external representation where they can be
 * recognized. Furthermore, other characters may be merely bit values (a bit
 * image, for example), having nothing to do with characters as symbols. For
 * example,
 *
 * \code
 ...
 * \endcode
 *
 * The expression <tt>pstr << c</tt> treats \c c as a symbol for a newline,
 * an unprintable character. The expression <tt>pstr.put(c)</tt> treats \c c
 * as the literal number "10".
 *
 * \note
 * Variables should not be separated with white space. Such white space is
 * interpreted literally and would be read back in as a character string.
 *
...
public:
    ...
    /**
     * Initializes an RWpostream from the \b std::streambuf associated with
     * the \b std::ostream \a str.
     */
    ...
    /**
     * Returns a reference to the underlying C++ Standard Library output
     * stream.
     */
    std::ostream& std();
    ...
    /**
     * Changes the precision used for writing \c float and \c double data.
     * Returns the previously set precision. At construction, the precision
     * is set to #RW_DEFAULT_PRECISION.
     */
    int precision(int p)
    {
    ...
    /**
     * Returns the currently set precision used for writing \c float and
     * \c double data. At construction, the precision is set to
     ...
     * using a portable format. This member only preserves US-ASCII
     * numerical codes, not the corresponding character symbol.
     */
