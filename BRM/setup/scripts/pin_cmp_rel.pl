...
#=============================================================
# @(#)$Id: pin_cmp_rel.pl /cgbubrm_mainbrm.portalbase/3 2018/03/30 02:42:06 mkothari Exp $
# 
...
sub configure_rel_config_files {
  ...
  &Configure_Infranet("$PROP_pathname");
...
#
  ...
  if ( $DoUpdateSuspense eq TRUE )
  {
    ...
    #
    $i = 0;
    my $tempString = 'infranet.rel.default.load_util';
    my $check = `grep $tempString $PROP_path | wc -l`;
    if ($check == 0) {
      seek( PROSUSPEN, 0, 0 );
      push( @Array_PROP, <<END
  ...
  if ( $MAIN_DM{'db'}->{'vendor'} =~ /oracle/i ) {
	...
	for ($i= 0; $i <= $#plb ; $i = $i + 2) {
	  if ( -f $plb[$i] ) {
        ...
        if ( -f "../../apps/rel_monitor/install_rel_monitor.plb" ) {
                print "Loading install_rel_monitor.plb\n";
                &ExecutePLB_file("../../apps/rel_monitor/install_rel_monitor.plb",
                                                  "rel_monitor", %DM_ORACLE );
        }
        if ( -f "../../apps/rel_monitor/rel_monitor_proc.plb" ) {
                print "Loading rel_monitor_proc.plb\n";
                &ExecutePLB_file("../../apps/rel_monitor/rel_monitor_proc.plb",
                                                  "rel_monitor", %DM_ORACLE );
        }
        if ( -f "../../apps/rel_manager/rel_manager.plb" ) {
                print "Loading rel_manager.plb\n";
                &ExecutePLB_file("../../apps/rel_manager/rel_manager.plb",
                                                  "rel_manager", %DM_ORACLE );
        }
...
sub configure_prop_file {  
  ...
  &Configure_Infranet("$PROPFILE/Infranet.properties");
  ...
  while ( <INFILE> )
  {
    ...
    $_ =~ s/^\s*infranet\.rel\.dbport.*/infranet\.rel\.dbport = $MAIN_DM{'db'}->{'port'}/i;
    $_ =~ s/^\s*infranet\.rel\.dbhost.*/infranet\.rel\.dbhost = $MAIN_DM{'db'}->{'Host'}/i;
  ...
  truncate( INFILE, tell( INFILE ) );
  close( INFILE );
  chmod(0640,"$PROPFILE/Infranet.properties");
