 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/model.h#1 $
 *
 **********************************************************************
 *
 ...
 * function changed(void*) is called, the list of dependents is traversed,
 * calling RWModelClient::updateFrom(RWModel*, void*) for each one, with
 * itself as the first argument. Subclasses of RWModelClient should be
...
public:
    ...
    /**
     * Sets up the internal ordered list of dependents when called by the
     * specializing class.
     */
    RWModel();

    /**
     * Copy constructor. The constructed instance gets a copy of the
     * dependents list from \a m.
     */
    RWModel(const RWModel& m);

    /**
     * Assignment operator. Self gets a copy of the dependents list from
     * \a m.
     */
    ...
    /**
     * Move assignment. Self takes ownership of the dependents list owned by
     * \a m.
     *
     * \conditional
     * This method is only available on platforms with rvalue reference
    ...
    /**
     * Adds the object pointed to by \a m to the list of dependents of self.
     */
    void                  addDependent(RWModelClient* m);

    /**
     * Removes the object pointed to by \a m from the list of dependents of
     * self.
     */
     ...
     * RWModelClient::updateFrom(RWModel*, void*) for each one, with self as
     * the first argument and \a d as the second argument.
     */
 ...
 * a Model-View-Controller architecture. Class RWModel, supplies the "Model"
 * leg. See class RWModel for details.
 *
 * \synopsis
 * #include <rw/model.h>
 * (abstract base class)
...
public:
    ...
    /**
     * Deriving classes should supply an appropriate definition for this
     * pure virtual function. The overall semantics of the definition should
     * be to update self from the data presented by the object pointed to by
     * \a p. That is, self is considered a dependent of the object pointed
     * to by \a p. The pointer \a d is available to pass client data.
     *
     * \note
     * Changes to the client list <em>must not</em> happen during a call to
     * updateFrom().
     */
