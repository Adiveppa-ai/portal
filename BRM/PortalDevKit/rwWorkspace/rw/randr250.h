 ...
 ***************************************************************************
 *
 * $Id: //spro/rel2016.1/rw/randr250.h#1 $
 *
 ...
 * <tt>[0, 1]</tt>.
 *
 * Class RWRandGenR250 generates random numbers uniformly distributed over
 ...
 * generators derived from abstract base class RWTRand. RWRandGenR250 is a
 * generalized feedback shift register generator.
 *
 * Unlike class RWRandGenerator, each instance of RWRandGenR250 contains and
 * maintains its own, independent state. This can make it easier to use in
 * some situations. The "internal state" of an instance of RWRandGenR250
 * consists of a seed, an array of unsigned longs known as the "state
 * vector" and an integer index into that state vector. (S. Kirkpatrick and
 * E. Stoll, Journal of Computational Physics, 40, p. 517 (1981).
 *
 * \synopsis
...
public:
    ...
    /**
     * Returns \c true for two distinct instances of RWRandGenR250 if they
     * will return identical sequences of values.
     */
    bool operator==(const RWRandGenBasicR250& u) const;

    /**
     * Returns \c true for two distinct instances of RWRandGenR250 if their
     * internal states differ. \"<tt>(x != x)</tt>\" returns \c false.
     */
    bool operator!=(const RWRandGenBasicR250& u) const;
