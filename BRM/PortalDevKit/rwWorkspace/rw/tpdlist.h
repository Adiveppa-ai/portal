 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tpdlist.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * A pointer-based collection of values, implemented as a doubly-linked
 * list.
 *
 * This class maintains a pointer-based collection of values, implemented as
 * a doubly-linked list. Class \c T is the type pointed to by the items in
 * the collection. Class \c A is an allocator of objects of class \c T. For
 * more information about using custom allocators, please see the
 ...
 * \sa
 * \parblock
 * Classes RWTPtrDeque, RWTPtrSlist, and RWTPtrOrderedVector also provide a
 * Rogue Wave pointer-based interface to C++ Standard Library sequence
 * collections.
 *
 * Class <b>std::list<T*,A></b> is the C++ Standard Library collection that
 ...
 *
 * \example
 * In this example, a pointer-based doubly-linked list of user type \c Dog
 * is exercised.
 *
 * \code
 * //
...
public:
    ...
    /**
     * \deprecated
     * As of SourcePro 12.5, use <tt>const T*</tt> instead. A type
     * representing the container's \c const data type.
     */
     ...
     * \a i must be between \c 0 and one less than the number of entries,
     * otherwise the results are undefined. Note that <em>no bounds checking
     * is performed</em>.
     */
     ...
     * expression <tt>((*\a fn)(t,\a d))</tt> is \c true, otherwise
    ...
    /**
     * Returns \c true if there exists an element \c t in self such that the
     * expression <tt>(*t == *\a a)</tt> is \c true, otherwise \c false.
     */
    ...
    /**
     * Inserts the item \a a in front of the item at position \a i in self.
     *
     * \exception RWBoundsErr
     * Thrown if this position is not between zero and the number of entries
     * in the collection.
     */
     ...
     * expression <tt>((*\a fn)(t,\a d))</tt> is \c true. Returns the number
     * of items replaced. Client data may be passed through parameter \a d.
     */
    ...
    /**
     * Replaces with \a newVal all elements \c t in self such that the
     * expression <tt>(*t == *\a oldVal)</tt> is \c true. Returns the number
     * of items replaced.
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Constructs a pointer-based doubly linked list by copying all elements
     * of \a lst.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrDlist::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTPtrDlist.
 *
 * RWTPtrDlistIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTPtrDlist. (An STL iterator
 * interface is provided through RWTPtrDlist::iterator.)
 *
 * The order of iteration over an RWTPtrDlist is dependent on the order of
 ...
 * being advanced with either pre-increment or \c operator().
 *
 * For both \c operator++ and \c operator(), iterating past the last element
 * returns a value equivalent to boolean \c false. Continued increments
 * return a value equivalent to \c false until reset() is called. For
 * \c operator--, decrementing past the first element returns a value
 * equivalent to \c false.
 *
 * \synopsis
 * #include<rw/tpdlist.h>
 * RWTPtrDlist<T,A> dl;
...
public:
     ...
     * undefined state and must be advanced before the first element is
     * accessible.
     */
     ...
     * reference an undefined value distinct from the reset value, and a
     * value equivalent to \c false will be returned. Otherwise, a value
     ...
     * has been reset or just created, this operator returns a value
     * equivalent to \c false, otherwise it returns a value equivalent to
     * \c true. If self references the first element, it will now be in the
     * reset state. If self has been iterated past the last value in the
     * list, it will now reference the last item in the list.
     */
    bool operator--();
     ...
     * last item in the container, #rwnil is returned.
     */
    T* operator()()
    {
     ...
     * If no such element exists, returns #rwnil equivalent to \c false.
     * Leaves self referencing the found item, or "past the end."
     */
     ...
     * Client data may be passed through parameter \a d. If no such element
     * exists, returns #rwnil. Leaves self referencing the found item, or
     * "past the end."
     */
     ...
     * \c true. If no such element exists, returns #rwnil.
     */
     ...
     * \c true. Client data may be passed through parameter \a d. If no such
     * element exists, returns #rwnil.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrDlist::const_iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTPtrDlist.
 *
 * RWTPtrDlistConstIterator provides a \c const iterator interface to the
 * C++ Standard Library-based collection represented by RWTPtrDlist. This
...
public:
    /**
     * Constructs a \c const iterator for the list \a l. The iterator begins
    ...
    /**
     * Constructs a \c const iterator from the iterator \a itr. The iterator
    ...
    /**
     * Advances self to the next element. If the iterator has been reset or
     * just created, self references the first element. If, before
     * iteration, self referenced the last value in the list, self now
     * references an undefined value distinct from the reset value, and a
     * value equivalent to \c false is returned. Otherwise, a value
     * equivalent to \c true is returned.
     */
    bool operator++();
     ...
     * has been reset or just created, this operator returns a value
     * equivalent to \c false, otherwise it returns a value equivalent to
     * \c true. If self references the first element, it is now in the reset
     * state. If self has been iterated past the last value in the list, it
     * now references the last item in the list.
     */
    bool operator--();
     ...
     * Returns the first element \c t encountered while iterating self
     * forward, such that the expression <tt>(*t == *\a a)</tt> is \c true.
     * If no such element exists, returns #rwnil. Leaves self referencing
     * the found item, or "past the end."
     */
