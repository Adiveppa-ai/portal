 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tools/ansilocale.h#1 $
 *
 **********************************************************************
 *
...
public:
    ...
    RWAnsiLocaleRef(const char* localeName);
    RWAnsiLocaleRef(const std::locale& loc);
 ...
 *
 * \brief
 * Implements the RWLocale interface, and encapsulates the C++ Standard
 * Library locale.
 *
 * RWAnsiLocale implements the RWLocale interface. It encapsulates the C++
 * Standard Library locale, \b std::locale and adds functionality for date
 * and time conversions. The C++ Standard Library \b std::locale provides
 * localized representations of dates, numbers, and currencies. RWAnsiLocale
 * offers a simple interface to \b std::locale so you can avoid working
 * directly with \b std::locale facets. Additionally, RWAnsiLocale provides
 * flexible conversions of strings to dates and times. While the C++
 * Standard Library \b std::locale accepts dates and times only in formats
 * defined by the C++ Standard Library function \b std::strftime(),
 * RWAnsiLocale accepts multiple date and time formats.
 *
 * There are three ways to use an RWAnsiLocale object:
 *
 * - By passing the object to functions that expect one, such as
 * RWDateTime::asString().
 * - By using the object to specify a global locale using the static member
 * function RWLocale::global(). This locale is then used as the default
 * argument to functions that use a locale.
 * - By imbuing a stream with the object, so that when an RWDateTime is
 * written to a stream using operator<<(), the appropriate formatting is
 * used automatically.
 *
 * For a description of the static members of the RWLocale interface that
 * operate on objects of type RWAnsiLocale, see the RWLocale class
 * description.
 *
 * \note
 * For both RWAnsiLocale and \b std::locale, the default global locale is
 * the locale defined in the C Standard Library file \c <locale.h>.
 *
 * \synopsis
 * #include <rw/locale.h>          // for RWLocale
 ...
 * Program output:
 *
 * Actual output will depend on the time the example is run, and on the
 * platform.
 *
 * \code
...
public:
    ...
    /**
     * Constructs an RWAnsiLocale object with locale information extracted
     * from the current C++ Standard Library global locale.
     */
    RWAnsiLocale();

    /**
     * Constructs an RWAnsiLocale object using the locale name
     * \a localeName.
     *
     * Valid values for \a localeName are \c NULL, \c "", \c "C", and any
     * value that is accepted by \b std::locale.
     *
     * If the value for \a localeName is \c NULL, the required locale
     * information is extracted from the current C++ Standard Library global
     * locale. Otherwise, \a localeName is used to construct a C++ Standard
     * Library \b std::locale object to be queried for the necessary locale
     * information.
     *
     * \exception std::runtime_error
     * If the locale name \a localeName is not recognized by \b std::locale.
     */
     ...
     * is the number of digits to place after the decimal separator. If
     ...
     * is the number of digits to place after the decimal separator. If
     * showpoint is \c true, the decimal separator appears regardless of the
     * precision.
     */
    ...
    /**
     * Converts components of the \a ptm object to a string, according to
     * the \a format character. The meanings assigned to the \a format
     * character are identical to those used in the C++ Standard Library
     * function \b std::strftime(). The members of \a ptm are assumed to be
     * set consistently.
     *
     * See
     * http://en.cppreference.com/w/cpp/chrono/c/strftime for a summary
     * of
     * \b std::strftime() formatting characters.
     */
    virtual RWCString asString(const struct tm* ptm, char format,
                               const RWZone& zone = RWZone::local()) const;

    /**
     * Converts components of the <tt>tm struct</tt> object \a ptm to a
     * string, according to the format string. Each format character in the
     * \a format string must be preceded by \c %. Any characters not
     * preceded by \c % are treated as ordinary characters which are
     * returned unchanged. You may represent the special character \c % with
     * \c %%. The meanings assigned to the \a format character are identical
     * to those used in the C++ Standard Library function
     * \b std::strftime(). The members of \a ptm are assumed to be set
     * consistently.
     *
     * See
     * http://en.cppreference.com/w/cpp/chrono/c/strftime for a summary
    ...
    /**
     * Returns a string containing the \a value parameter formatted
     * according to monetary conventions for the locale. The \a value
     * parameter is assumed to contain an integer representing the number of
     * units of currency (for example,
     * <tt>moneyAsString(1000., RWAnsiLocale::cs_local)</tt> in a US locale
     * would yield \c "$10.00"). The \c CurrSymbol parameter determines
     * which currency symbol should be applied if any -- the local (for
     * example, \c "$") or international (for example, \c "USD"), or none.
     */
     ...
     * separators are optional; if present they are checked. Returns \c true
     * for a valid monetary value, \c false for an error. If it returns
     * \c false, the \c double* parameter value is untouched; otherwise it
     * is set to the integral number of monetary units entered (for example,
     * cents in a U.S. locale).
     */
    virtual bool stringToMoney(const RWCString&, double*,
                               RWAnsiLocale::CurrSymbol = LOCAL) const;

    /**
     * Interprets \a date as a date, and extracts the month, day, and year
     * components to the \c tm parameter. Returns \c true for a valid date,
     * \c false otherwise. If it returns \c false, the \c struct \c tm
     * object is untouched; otherwise it sets the \c tm_mday, \c tm_mon, and
     * \c tm_year members. The expected format for the date string is the
     * one specified by the \b std::strftime() character \c x in the current
     * locale.
     *
    ...
    /**
     * Interprets \a time as a time, with hours, minutes, and optional
     * seconds. If the hour is in the range [1...12], the local equivalent
     * of "AM" or "PM" is allowed. Returns \c true for a valid time string,
     * \c false for an error. If it returns \c false, the \c tm object is
     * untouched; otherwise it sets the \c tm_hour, \c tm_min, and \c tm_sec
     * members. The expected format for the date string is the one specified
     * by \b std::strftime() character \c X (capital X) in the current
     * locale.
     *
    ...
    /**
     * Interprets \a value as a floating point number, by reading to the
     * first non-numeric or non-punctuation character, then converting.
     * Spaces are allowed before and after the (optional) sign, and at the
     * end. Digit group separators are allowed in the integer portion.
     * Returns \c true for a valid number, \c false for an error. If it
     * returns \c false, the \c double* parameter is untouched. All valid
    ...
    /**
     * Interprets \a value as an integer, by reading to the first
     * non-numeric or non-punctuation character, then converting. Spaces are
     * allowed before and after the (optional) sign, and at the end. Digit
     * group separators are allowed. Returns \c true for a valid integer,
     * \c false for an error. If it returns \c false, the \c long* parameter
     * is untouched. All valid numeric strings are accepted; all others are
    ...
    /**
     * Interprets \a month as a full or abbreviated month name, returning
     * values 1 through 12 to represent (respectively) January through
     * December, or 0 for an error. Ignores leading white space.
     */
    virtual int monthIndex(const RWCString& month) const;

    /**
     * Interprets \a day as a full or abbreviated weekday name, returning
     * values 1 through 7 to represent (respectively) Monday through Sunday,
     * or 0 for an error.
     */
    ...
    /**
     * Returns a string identifying the number of digits to be included in a
     * numerical group. A group is simply the digits between adjacent
     * thousand's separators.
     *
     * Each character of the string is an integer that specifies the number
     * of digits in a group, starting with the right most group.
     *
     * - If the value is equal to \c 0, the previous element is used
     * repeatedly.
    ...
    /**
     * Returns a #money_pattern object specifying the location of the
     * various syntactic elements in a monetary representation. The
     * enumeration values #symbol, #sign, and #value appear exactly once in
     * this pattern, with the remaining location taken by either #none or
     * #space. #none never occupies the first position in the #money_pattern
     * and #space never occupies the first or last position. Beyond these
     * restrictions, elements may appear in any order.
     */
    ...
    /**
     * Returns a #money_pattern object specifying the location of the
     * various syntactic elements in an international monetary
     * representation. The restrictions listed for pos_format() and
     * neg_format() apply.
     */
