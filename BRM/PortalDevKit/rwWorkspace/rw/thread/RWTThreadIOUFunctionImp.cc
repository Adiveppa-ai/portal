/**************************************************************************
 *
 * $Id: //spro/rel2016.1/rw/thread/RWTThreadIOUFunctionImp.cc#1 $
 *
...
/*****************************************************************************
  ...
  RWTThreadIOUFunctionImp - RWTFunctor<R()>-based thread runnable class.
...
RWTThreadIOUFunctionImp<Return>::RWTThreadIOUFunctionImp(const RWTIOUEscrow<Return>& escrow)
    :
    escrow_(escrow)
{
}
...
template <class Return>
void
RWTThreadIOUFunctionImp<Return>::_dispatchExec(void)
{
    ...
    RW_ASSERT(functor_.isValid()); // USAGE ERROR - Functor handle is empty!
    RW_ASSERT(escrow_.isValid()); // USAGE ERROR - Escrow handle is empty!
...
template <class Return>
RWTFunctor<Return()>
RWTThreadIOUFunctionImp<Return>::getFunctor(void) const
{
...
template <class Return>
void
RWTThreadIOUFunctionImp<Return>::run(void)
{
...
template <class Return>
RWTIOUResult<Return>
RWTThreadIOUFunctionImp<Return>::result(void) const
{
    return escrow_;
}
...
template <class Return>
void
RWTThreadIOUFunctionImp<Return>::setFunctor(const RWTFunctor<Return()>& functor)
{
    // Lock the thread object
    LockGuard lock(monitor());
    functor_ = functor;
...
template <class Return>
void
RWTThreadIOUFunctionImp<Return>::setIOUEscrow(const RWTIOUEscrow<Return>& escrow)
{
    // Lock the thread object
    LockGuard lock(monitor());
    escrow_ = escrow;
