 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvset.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Maintains a collection of values ordered according to a comparison
 * object.
 *
 * This class maintains a collection of values, which are ordered according
 * to a comparison object of type \c C. Class \c T is the type of items in
 ...
 * The structure \c std::less<T> from the C++ Standard Library header file
 * \c \<functional\> is an example.
 *
 * RWTValSet will not accept an item that compares equal to an item already
 * in the collection. (RWTValMultiSet may contain multiple items that
 * compare equal to each other.) Equality is based on the comparison object
 * and not on \c operator==. Given a comparison object \c comp, items \c a
 ...
 * \sa
 * \parblock
 * Class RWTValMultiSet offers the same interface to a collection that
 * accepts multiple items that compare equal to each other. RWTValMap
...
public:
    ...
    /**
     * If there exists an element \c t in self that compares equal to \a a,
     * assigns \c t to \a k and returns \c true. Otherwise, returns \c false
     * and leaves the value of \a k unchanged.
     */
     ...
     * and returns \c true. Returns \c false if there is no such element.
     */
     ...
     * insertion is successful, otherwise \c false. Insertion will fail if
     * the collection already holds an element with the equivalent key.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValSet::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTValSet.
 *
 * RWTValSetIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTValSet. (An STL iterator
 * interface is provided through RWTValSet::iterator.)
 *
 * The order of iteration over an RWTValSet is dependent on the supplied
...
public:
     ...
     * references an undefined value and \c false will be returned.
    ...
    T key() const
    {
        return *(const_cast<RWTValSetIterator*>(this)->iter_);
    }
    ...
    /**
     * Resets the iterator so that after being advanced, it references the
     * first element of the collection \a s.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValSet::const_iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValSet.
 *
 * RWTValSetConstIterator provides a \c const iterator for class RWTValSet.
 * A \c const iterator prohibits changes to the collection over which it is
 * iterating.
 *
 * The order of iteration over an RWTValSet is dependent on the supplied
 * comparator object parameter \c C as applied to the values stored in the
...
public:
     ...
     * just created, self now references the first element. If, before
     * iteration, self referenced the last association in the set, self now
     * references an undefined value and \c false is returned. Otherwise,
     * returns \c true.
     */
    bool operator++()    // Advance and test
    {
