/**************************************************************************
 *
 * $Id: //spro/rel2016.1/rw/functor/RWTFunctor2.cc#1 $
 *
...
RWTFunctor2<S1, S2>::RWTFunctor2(RWStaticCtor)
    : RWTFunctor<void(S1, S2)>(RW_STATIC_CTOR)
{
}

template <class S1, class S2>
RWTFunctor2<S1, S2>::RWTFunctor2(RWTFunctor2Imp<S1, S2>* functorImp)
    : RWTFunctor<void(S1, S2)>((RWTFunctorImpBase2<void, S1, S2>*)functorImp)
{
}

template <class S1, class S2>
RWTFunctor2<S1, S2>::RWTFunctor2(const RWTFunctor2<S1, S2>& second)
    : RWTFunctor<void(S1, S2)>(second)
{
}
...
{
    if (&second != this) {
        RWTFunctor<void(S1, S2)>::operator=(second);
    }
    return *this;
}
