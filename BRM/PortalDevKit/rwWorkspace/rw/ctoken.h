 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/ctoken.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Breaks a string into separate tokens, delimited by an arbitrary
 * whitespace. Can be used as an alternative to the C++ Standard Library
 * function \b std::strtok().
 *
 * Class RWCTokenizer is designed to break a string into separate tokens,
 * delimited by an arbitrary whitespace. Think of it as an iterator for
 * strings and as an alternative to the C++ Standard Library function
 * \b std::strtok() that has the unfortunate side effect of changing the
 * tokenized string.
 *
 * \synopsis
 * #include <rw/ctoken.h>
...
public:
     ...
     * are delimited by any character in \a s, or any embedded null.
     */
    RWCSubString          operator()(const char* s);

    /**
     * Advances to the next token and returns it as a substring. The tokens
     * are delimited by any of the first \a n characters in \a s. Buffer
     * \a s may contain nulls, and must contain at least \a n characters.
     * Tokens will not be delimited by nulls unless \a s contains nulls.
     */
    ...
    /**
     * Advances to the next token and returns it as a substring. The tokens
     * are delimited by any of the four characters in <tt>" \t\n\0"</tt>
     ...
     * <tt>tok(RWCString("ab"))</tt> treats either \c a or \c b as a
     * delimiter character. On the other hand,
     * <tt>tok(RWTRegex<char>("ab"))</tt> treats the two-character pattern,
     * \c ab, as a single delimiter.
     *
     * This method consumes consecutive occurrences of delimiters and skips
     * over any empty fields present in the string. To obtain empty fields
     * as well as non-empty fields, use the nextToken(RWTRegex<char>&)
     * method.
     */
     ...
     * occurrence of a delimiter to span multiple characters.
     *
     * For example, <tt>nextToken(RWCString("ab"))</tt> treats either \c a
