 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvdlist.h#1 $
 *
 **********************************************************************
 *
 ...
 * about using custom allocators, please see the <em>Essential Tools Module
 * User's Guide</em>.
 *
 * \synopsis
 ...
 * \sa
 * \parblock
 * Classes RWTValDeque, RWTValSlist, and RWTValOrderedVector also provide a
...
#  endif // !RW_NO_RVALUE_REFERENCES
     ...
     * \a k unchanged. Client data may be passed through parameter \a d.
     */
     ...
     *
     * \exception RWBoundsErr
     * Thrown if index \a i is not between \c 0 and the number of entries in
     * the collection.
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Returns a reference to the minimum element in the collection. Type
     * \c T must have well-defined less-than semantics
    ...
    /**
     * Returns a reference to the maximum element in the collection. Type
     * \c T must have well-defined less-than semantics
     * (<tt>T::operator<(const T&)</tt> or equivalent).
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES
     ...
     * <tt>((*\a fn)(t,\a d))</tt> is \c true. Returns the number of items
     * replaced. Client data may be passed through parameter \a d.
     */
    ...
    /**
     * Replaces all elements \c t in self such that the expression
     * <tt>(t == \a oldVal)</tt> is \c true with \a newVal. Returns the
    ...
    /**
     * Calls the destructor on all elements of self and replaces them by
     * copying all elements of \a rhs.
     */
    RWTValDlist<T, A>&
...
#  endif // !RW_NO_RVALUE_REFERENCES
    ...
    /**
     * Constructs a doubly-linked list by copying all elements of \a lst.
     */
 ...
 * turn, compare equal to each other.
 */
template <class T , class A>
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValDlist::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTValDlist.
 *
 * RWTValDlistIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTValDlist. (An STL iterator
 * interface is provided through RWTValDlist::iterator.)
 *
 * The order of iteration over an RWTValDlist is dependent on the order of
 * insertion of the values into the container.
 *
 ...
 * value equal to \c false until reset() is called.
 *
 * \synopsis
 * #include<rw/tvdlist.h>
 * RWTValDlist<T,A> dl;
...
public:
    /**
     * Creates an iterator for the dlist \a s. The iterator begins in an
     ...
     * reference an undefined value distinct from the reset value and
     ...
     * it will now be in the reset state. If self has been iterated past the
     * last value in the list, it will now reference the last item in the
     * list.
     */
    bool operator--();
    ...
    /**
     * Advances self to the next element and returns its value. If the
     * iterator has advanced past the last item in the container, returns
     * #rwnil.
     */
    bool operator()()
    {
    ...
    T& key() const
    {
        return *(const_cast<RWTValDlistIterator*>(this)->iter_);
    }

    /**
     * Removes the value referenced by self from the collection. Returns
     * \c true if the removal is successful, otherwise \c false.
     */
     ...
     * forward, such that the expression <tt>(t == \a a)</tt> is \c true.
     * Returns \c true if an element was found and removed, otherwise
     * returns \c false.
     */
     ...
     * Removes the first element \c t, encountered by iterating self
     * forward, such that the expression <tt>((*\a fn)(t,\a d))</tt> is
     * \c true. Returns \c true if an element was found and removed,
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValDlist::const_iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValDlist.
 *
 * RWTValDlistConstIterator provides a \c const iterator for Standard
 * Library-based RWTValDlist collections. The \c const iterator prohibits
 ...
 * return a value equal to \c false until reset() is called.
 *
 * \synopsis
 * #include <rw/tvdlist.h>
...
public:
     ...
     * references an undefined value distinct from the reset value, and this
     * operator returns \c false. Otherwise, returns \c true.
     */
    bool operator++();

    /**
     * Moves self back to the immediately previous element. If the iterator
     * has been reset or just created, returns \c false, otherwise it
     * returns \c true. If self now references the first element, it is now
     * in the reset state. If self has been iterated past the last value in
     * the list, it now references the last item in the list.
     */
    bool operator--();
