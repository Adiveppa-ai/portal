...
public:
    ...
    /**
     * Equality operator. Returns \c true if the RWMathVecIteratorBase
    ...
    /**
     * Inequality operator. Returns \c false if the RWMathVecIteratorBase
     * objects point to the same element.
     */
    bool operator!=(const RWMathVecIteratorBase<T>& x) const
    {
     ...
     * two RWMathVecIteratorBase objects, not necessarily the relative
     ...
     * between two RWMathVecIteratorBase objects, not necessarily the
 ...
 *
 * Class RWMathVecIterator is the random access iterator for the RWMathVec
 * collection class. The RWMathVecIterator class behaves like a pointer to
 * type \c T and is used to increment through elements of an RWMathVec.
 *
 * \note
...
public:
    ...
    /**
     * Constructs an RWMathVecIterator that is in an uninitialized state.
     * The iterator must be initialized before it is dereferenced.
     */
    RWMathVecIterator()
    {
        ;
    ...
    /**
     * Constructs an RWMathVecIterator from another RWMathVecIterator.
     * Initialized iterators can be obtained via the RWMathVec::begin() and
     * RWMathVec::end() member functions.
     */
    ...
    // Prefix increment and decrement operators
    /**
     * Prefix or postfix increment or decrement operator. The
     * RWMathVecIterator points to the next or previous element in the
     * RWMathVec. No error condition is set if the RWMathVecIterator goes
     * past the bounds of the vector. Compare to RWMathVec::begin() and
     * RWMathVec::end() to check that the iterator position is valid.
     */
     ...
     * moves the iterator that many elements before the iterator \a i.
     */
    ...
    // Assignment operators
    /**
     * Moves the iterator forward or backward \a d elements in the
     * RWMathVec. No error condition is set if the iterator goes past the
     * bounds of the vector. Incrementing by \a d and then decrementing by
     * \a d returns the iterator to its original position, even if doing so
     * takes it past the bounds of the vector.
     */
     ...
     * is positive, or \a n elements before self if \a n is negative.
     */
 ...
 *
 * \brief
 * The \c const random access iterator for the RWMathVec collection class.
 *
 * Class RWMathVecConstIterator is the \c const random access iterator for
 * the RWMathVec collection class. The RWMathVecConstIterator class behaves
 * like a pointer to type \c T and is used to increment through elements of
 * an RWMathVec. The RWMathVecConstIterator differs from RWMathVecIterator
 * in that the value in a dereferenced RWMathVecConstIterator may not be
 * changed.
 *
 * \note
...
public:
    ...
    /**
     * Constructs a RWMathVecConstIterator that is in an uninitialized
     * state. The RWMathVecConstIterator must be initialized before it is
     * dereferenced.
     */
    RWMathVecConstIterator()
    {
    ...
    /**
     * Constructs a RWMathVecConstIterator from another
     * RWMathVecConstIterator. Initialized RWMathVecConstIterator can be
     * obtained via the RWMathVec::begin() and RWMathVec::end().
     */
    ...
    /**
     * Constructs a RWMathVecConstIterator from another RWMathVecIterator.
     * Initialized RWMathVecIterator can be obtained via the
     * RWMathVec::begin() and RWMathVec::end().
     */
    ...
    /**
     * Prefix or postfix increment or decrement operator. The
     * RWMathVecConstIterator points to the next or previous element in the
     * RWMathVec. No error condition is set if the RWMathVecConstIterator
     * goes past the bounds of the vector. Compare to RWMathVec::begin() and
     * RWMathVec::end() to check that the RWMathVecConstIterator position is
     * valid.
     */
    ...
    // Binary operators
    /**
     * If the return value is positive, sets the RWMathVecConstIterator that
     * many elements past the RWMathVecConstIterator \a i. If the return
     * value is negative, sets the RWMathVecConstIterator that many elements
     * before the RWMathVecConstIterator \a i.
     */
    difference_type operator-(const RWMathVecIteratorBase<T>& i) const
    {
        return this->operator_minus(i);
    ...
    // Assignment operators
    /**
     * Moves the RWMathVecConstIterator forward or backward \a d elements in
     * the RWMathVec. No error condition is set if the
     * RWMathVecConstIterator goes past the bounds of the vector.
     * Incrementing by \a d and then decrementing by \a d returns the
     * RWMathVecConstIterator to its original position, even if doing so
     * takes it past the bounds of the vector.
     */
    ...
    // Binary operators
    /**
     * Returns a RWMathVecConstIterator that is \a d elements before self.
     */
    ...
    /**
     * Returns a RWMathVecConstIterator that is \a d elements past self.
     */
    RWMathVecConstIterator<T> operator+(difference_type d) const
    {
        RWMathVecConstIterator<T> tmp = *this;
...
public:
    ...
    /**
     * Constructs a 0-length vector, useful for declaring arrays of vectors.
     * Like any other vector, this vector can subsequently be reshaped or
     * resized (see member functions reshape() and resize()).
     */
    RWMathVec() : RWVecView()
    {
        ;
    ...
    /**
     * Copy constructor. The new vector and the old vector both view the
     * same data.
     */
    ...
    /**
     * Constructs a vector with \a n elements. The #RWUninitialized type is
    ...
    /**
     * Constructs a vector with \a n elements. The first has value
     * \a initval; each succeeding element is incremented by \a incr.
     */
     ...
     * the global operator operator>>() described in this entry.
     */
    RWMathVec(const char* s);

    /**
     * Constructs a new vector from an RWMathVecPick. A copy of \a p's data
     * is made.
     */
    RWMathVec(const RWMathVecPick<T>& p);

    /**
     * Constructs a vector with \a n elements, using the data in the vector
     * \a dat as initial data. A copy of \a dat is made. The vector \a dat
     * must have at least \a n elements.
     */
     ...
     * \a re and \a im, with the real part of the \c vec equal to \a re and
     ...
     * in the vector. A function of type RWMathVec<T>::mathFunType takes and
     * returns a \c T; a function of type RWMathVec<T>::mathFunType2 takes a
     * \c T and returns an RWMathVec<T>::norm_type.
     *
     * \sa
    ...
    /**
     * Returns the number of bytes required to store the vector to an RWFile
     * using member function \ref saveOn(RWFile&) const "saveOn(RWFile&)".
     */
    ...
    /**
     * Returns a pointer to the start of a vector's data. Should be used
     * with care, as this accesses the vector's data directly. You are
     * responsible for recognizing the length and the stride of the vector.
     */
    T*                     data()
    {
        return (T*)begin_;
     ...
     * reference to that object and that its data are in contiguous storage.
     */
    ...
    /**
     * Returns a vector \c pick. The results can be used as an lvalue. You
     * can think of the picked elements as representing a vector in the
     * order specified by the vector of indices \a v. Before using this
     * function, you must include the header file \c rw/math/mthvecpk.h.
     */
    ...
    /**
     * Returns an iterator that points to one element past the first element
     * in the vector.
     */
    ...
    /**
     * Makes self a view of \a v's data. The view currently associated with
     * the vector is lost.
     */
     ...
     * the contents of the vector are undefined; that is, they can be and
    ...
    /**
     * Stores self to a virtual stream, or in a binary format to an RWFile.
     * To use these functions with a user-defined type T, the corresponding
    ...
    /**
     * Returns a vector which views a slice of the data. The slice begins at
     * element \a start and extends for \a n elements. The increment between
     * successive elements in the slice is \a stride.
     */
    RWMathVec<T>  slice(int start, size_t n, int stride = 1) const;

    /**
     * Returns the distance between successive elements of the vector. The
     * stride can be computed using <tt>&v[i+1]-&v[i]</tt>.
     */
    int                     stride() const
    {
     ...
     * subscripting operators return a new view of the same data as the
     * vector being subscripted. An object of type #RWRange or #RWToEnd, the
     * global object #RWAll, or a character string may be substituted for an
     * RWSlice.
     */
     ...
     * dimensions. The expression <tt>this = v</tt> implies
     * \f$this_{i} = v_{i}\f$
     */
    ...
    /**
     * Assignment operator for scalars. The expression <tt>this = v</tt>
     * implies \f$this_{i} = v\f$
     */
    RWMathVec<T>&  operator=(const T& v);

    /**
     * Assignment by addition operator for scalars. The expression
     * <tt>this += v</tt> implies \f$this_{i} = this_{i} + v\f$
     */
    RWMathVec<T>&  operator+=(const T& v);

    /**
     * Assignment by subtraction operator for scalars. The expression
     * <tt>this -= v</tt> implies \f$this_{i} = this_{i} - v\f$
     */
    RWMathVec<T>&  operator-=(const T& v);

    /**
     * Assignment by multiplication operator for scalars. The expression
     * <tt>this *= v</tt> implies \f$this_{i} = this_{i} * v\f$
     */
    RWMathVec<T>&  operator*=(const T& v);

    /**
     * Assignment by division operator for scalars. The expression
     * <tt>this /= v</tt> implies \f$this_{i} = this_{i} / v\f$
     */
    RWMathVec<T>&  operator/=(const T& v);

    /**
     * Assignment by addition operator for vectors. The vectors must have
     * the same dimensions. The expression <tt>this += v</tt> implies
     * \f$this_{i} = this_{i} + v_{i}\f$
     */
    RWMathVec<T>&  operator+=(const RWMathVec<T>& v);

    /**
     * Assignment by subtraction operator for vectors. The vectors must have
     * the same dimensions. The expression <tt>this -= v</tt> implies
     * \f$this_{i} = this_{i} - v_{i}\f$
     */
    RWMathVec<T>&  operator-=(const RWMathVec<T>& v);
    ...
    /**
     * Assignment by multiplication operator for vectors. The vectors must
     * have the same dimensions. The expression <tt>this *= v</tt> implies
     * \f$this_{i} = this_{i} * v_{i}\f$
     */
    RWMathVec<T>&  operator*=(const RWMathVec<T>& v);

    /**
     * Assignment by division operator for vectors. The vectors must have
     * the same dimensions. The expression <tt>this /= v;</tt> implies
     * \f$this_{i} = this_{i} / v_{i}\f$
     */
    RWMathVec<T>&  operator/=(const RWMathVec<T>& v);
    ...
    /**
     * Increments each element of self. The functions taking an integer
     * parameter are invoked if the operator is used as a postfix operator.
     */
    RWMathVec<T>&  operator++(); // Prefix operator

    /**
     * Returns \c true if self and the argument are equivalent. To be
     * equivalent, they must have the same number of entries and each
     * element in self must equal the corresponding element in the argument.
     */
    ...
    /**
     * Decrements each element of self. The functions taking an integer
     * parameter are invoked if the operator is used as a postfix operator.
     */
    RWMathVec<T>&  operator --(); // Prefix operator
...
#  pragma warning (disable:597)
    ...
    /**
     * Implicit conversion operator to RWMathVec. The \c promote_type is
     * rw_numeric_traits::promote_type.
     *
     * \sa
 ...
 */
template <>
RW_MATH_GLOBAL RWMathVec<DComplex>::RWMathVec(const RWMathVec<double>& re, const RWMathVec<double>& im);
 ...
 *
 * Class RWConvertMathVec converts between RWMathVec types with different
 * datatypes.
 *
 * \synopsis
...
struct RWConvertMathVec {
    /**
     * Constructs a new vector of type RWMathVec that can be used anywhere
     * as a type RWMathVec. Note that a temporary array is created every
     * time this constructor is invoked, but the conversion operator below
     * can be used repeatedly without creating additional temporaries.
     */
    ...
    /**
     * Automatic conversion operator that enables class RWConvertMathVec to
     * be used as an RWMathVec. While the actual conversion takes place in
...
/**
 * \relates RWMathVec
 * Unary minus operator, applied element-by-element. For example, for
 * vectors \c u, and \a v, the expression <tt>u = -v</tt> implies
 * \f$u_{i} = -v_{i}\f$.
 */
...
/**
 * \relates RWMathVec
 * Unary plus operator, applied element-by-element. For example, for vectors
 * \c u, and \a v, the expression <tt>u = +v</tt> implies
 * \f$u_{i} = +v_{i}\f$.
 */
...
/**
 * \relates RWMathVec
 * Addition operator, applied element-by-element. For example, for vectors
 * \a u, \a v, and \c w, the expression <tt>w = u + v</tt> implies
 * \f$w_{i} = u_{i} + v_{i}\f$.
 */
template< class T > RWMathVec<T>
...
/**
 * \relates RWMathVec
 * Subtraction operator, applied element-by-element. For example, for
 * vectors \a u, \a v, and \c w, the expression <tt>w = u - v</tt> implies
 * \f$w_{i} = u_{i} - v_{i}\f$.
 */
template< class T > RWMathVec<T>
operator-(const RWMathVec<T>& u, const RWMathVec<T>& v);
...
/**
 * \relates RWMathVec
 * Multiplication operator, applied element-by-element. For example, for
 * vectors \a u, \a v, and \c w, the expression <tt>w = u * v</tt> implies
 * \f$w_{i} = u_{i} * v_{i}\f$. This implies an element-by-element multiply,
 * \e not the inner product. If you want the inner product, use global
...
/**
 * \relates RWMathVec
 * Division operator, applied element-by-element. For example, for vectors
 * \a u, \a v, and \c w, the expression <tt>w = u / v</tt> implies
 * \f$w_{i} = u_{i} / v_{i}\f$.
 */
template< class T >  RWMathVec<T>
 ...
 * \relates RWMathVec
 * Addition operator, applied element-by-element. For example, for vectors
 * \a u, \c w, and scaler \a s, the expression <tt>w = u + s</tt> implies
 * \f$w_{i} = u_{i} + s\f$.
 */
template< class T > RWMathVec<T>
...
/**
 * \relates RWMathVec
 * Subraction operator, applied element-by-element. For example, for scaler
 * \c s, and vectors \a v, and \c w, the expression <tt>w = s - v</tt>
 * implies \f$w_{i} = s - v_{i}\f$.
 */
template< class T > RWMathVec<T>
 ...
 * \relates RWMathVec
 * Multiplication operator, applied element-by-element. For example, for
 * vectors \a u, \c w, and scaler \a s, the expression <tt>w = u * s</tt>
 * implies \f$w_{i} = u_{i} * s\f$.
 */
template< class T > RWMathVec<T>
operator*(const RWMathVec<T>& u, const T& s);
...
/**
 * \relates RWMathVec
 * Division operator, applied element-by-element. For example, for scaler
 * \a s, and vectors \a v, and \c w, the expression <tt>w = s / v</tt>
 * implies \f$w_{i} = s / v_{i}\f$.
 */
template< class T > RWMathVec<T>
operator/(const T& s, const RWMathVec<T>& v);
 ...
 * numbers are separated by spaces. If \c T is a user-defined type, the
 * following operator must be defined for RWvostream and/or RWFile:
 *
 ...
 * separated by white space. If the sequence of numbers begins with a left
 * bracket \c [, the operator reads to a matching right bracket \c ]. If no
 * left bracket is present, it reads to end of file. The vector \a v is
 * initialized to the correct length. If \c T is a user-defined type, the
 * following operator must be defined for RWvistream and/or RWFile:
 *
 * <tt>operator>>(ostream&s,constT&t);</tt>
 */
template< class T >
 ...
 * \relates RWMathVec
 * Multiplication operator, applied element-by-element. For example, for
 * scaler \a s, and vectors \a v, and \c w, the expression
 * <tt>w = s * v</tt> implies \f$w_{i} = s * v_{i}\f$.
 */
template< class T >   RWMathVec<T>
operator*(const T& s, const RWMathVec<T>& v);
...
/**
 * \relates RWMathVec
 * Addition operator, applied element-by-element. For example, for scaler
 * \a s, and vectors \a v, and \c w, the expression <tt>w = s + v</tt>
 * implies \f$w_{i} = s + v_{i}\f$.
 */
template< class T >   RWMathVec<T>
 ...
 * if abs() is invoked for class \ref RWMathVec "RWMathVec<DComplex>", a
 * vector of class \ref RWMathVec "RWMathVec<double>" is returned.
 */
RWMathVec<signed char>   RW_MATH_SYMBOLIC abs(const RWMathVec<signed char>&);
 ...
 * \relates RWMathVec
 *
 * Converts an \ref RWMathVec "RWMathVec<double>" instance into a
 * corresponding \ref RWMathVec "RWMathVec<float>" instance. Note that this
 * is a narrowing operation; high order bits are removed.
 *
 * \sa
 * RWConvertMathVec
 */
 ...
 *
 * Converts an \ref RWMathVec "RWMathVec<double>" instance into a
 * corresponding \ref RWMathVec "RWMathVec<int>" instance. Note that
 * truncation occurs.
 *
 * \sa
 * RWConvertMathVec
 ...
 * \relates RWMathVec
 *
 * Converts an \ref RWMathVec "RWMathVec<float>" instance into a
 * corresponding \ref RWMathVec "RWMathVec<int>" instance. Note that
 * truncation occurs.
 *
 * \sa
 ...
 * \relates RWMathVec
 *
 * Converts an \ref RWMathVec "RWMathVec<int>" instance into a corresponding
 * \ref RWMathVec "RWMathVec<signed char>" instance. Note that this is a
 * narrowing operation; high order bits are removed.
 *
 * \sa
 * RWConvertMathVec
 */
RWMathVec<signed char> RW_MATH_SYMBOLIC toChar(const RWMathVec<int>& v);
 ...
 * \relates RWMathVec
 *
 * Takes a vector \a x and returns a vector \e y of values corresponding to
 * the cumulative sum of \a x:
 *
 * \f[
 * y_i = \sum_{j=0}^{i} x_j
 * \f]
 ...
 * of \a x. An element <em>y(i)</em> where \e i is not \c 0 is set equal to
 * \f$x(i) - x(i-1)\f$.
 *
 * For example, if <em>x = [2, 4, 5, 8]</em> then <em>y = [2, 2, 1, 3].</em>
 */
template< class T >
 ...
 * \f]
 *
 * For example, if <em>x = [ 0, 1, 2]</em> and <em>y = [ 1, 3, 2]</em>, then
 * <em>z = 0 + 3 + 4 = 7</em>. For vectors of type complex, see also
 * function conjDot().
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Return the \e index of the maximum element of the vector. If instead you
 * want the maximum \e value use function maxValue().
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Return the \e index of the minimum element of the vector. If instead you
 * want the minimum \e value use function minValue().
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Takes a vector <em>x</em><sub>i</sub> with <em>n</em> elements as an
 * argument and returns the product of the elements of the vector. For
 * example, if <em>x = [1, 4, 3]</em>, <em></em>then <em>y</em> = 12.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Returns the outer product of two vectors. The outer product of the
 * vectors \f$x = (x_{1}, x_{2},\ldots , x_{n})\f$ and
 * \f$y = (y_{1}, y_{2},\ldots , y_{m})\f$ is the matrix <em>A</em> whose
 * <em>ij</em>th entry is
 * <em>x</em><em><sub>i</sub></em><em>y</em><em><sub>j</sub></em>: that is,
 * the <em>j</em>th column of <em>A</em> is
 * <em>y</em><em><sub>j</sub></em><em>x</em>.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Takes a vector <em>x</em> as an argument and returns a vector <em>y</em>
 * whose elements are in reversed order. For example, if <em>x = [2, 5, 1,
 * 6]</em>, then <em>y = [6, 1, 5, 2]</em>. The reversed vector <em>y</em>
 * is actually a reversed <em>slice</em> of the original vector. Hence, the
 ...
 * \relates RWMathVec
 *
 * Returns the sum of the elements of the vector. For example, if <em>x =
 * [1, 4, 3, 4]</em>, then <em>y = 12</em>.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Takes \a x as an argument and returns hyperbolic cosines \e y such that
 ...
 * \relates RWMathVec
 *
 * Takes a RWMathVec an argument and returns <em>\<x\>,</em> the mean value,
 * where:
 *
 * \f[
 ...
 * \f]
 *
 * For example, if <em>x = [1, 4, 3, 4]</em>, then <em>\<x\> = 3</em>.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * The function sin() takes <em>x</em> as an argument and returns <em>y</em>
 * such that \f$y_{i} = sin(x_{i})\f$. The <em>x</em><sub>i</sub> are in
 * radians. For complex classes, the complex sine is returned.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * The function sinh() takes <em>x</em> as an argument and returns
 * <em>y</em> such that \f$y_{i} = sinh(x_{i})\f$. For complex classes, the
 * complex hyperbolic sine is returned.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * The square root function. Takes <em>x</em> as an argument and returns
 * <em>y</em> such that \f$y_{i} = (x_{i})^{1/2}\f$. For complex classes,
 * the principal value of the complex square root is returned.
 */
template< class T >
 ...
 * \f]
 *
 * where <\a x> is the mean of the vector and \e N is the length of the
 * vector. Note that this is the biased variance.
 */
 ...
 * \relates RWMathVec
 *
 * Returns the arguments <em>a</em><sub>i</sub> (in radians) of \e x in the
 * range \f$-\pi < a_{i} \leq \pi\f$, where
 * \f$a_{i} = tan_{-1}[Im(x_{i})/Re(x_{i})]\f$. Multiple valued functions of
 * complex arguments, such as sqrt() and log(), are therefore constrained to
 * their principle value.
 */
 ...
 * <em>x</em><sup>*</sup>. For example, if <em>x</em><sub>i</sub> = (2.3,
 * 1.4), then <em>x</em><sub>i</sub><sup>*</sup> = (2.3, -1.4).
 */
 ...
 * \relates RWMathVec
 *
 * Takes complex <em>x</em> as an argument and returns <em>y</em> containing
 * the imaginary parts \f$y_{i} = Im(x_{i})\f$. With most versions of
 * complex, the results can be used as an l-value:
 *
 * \code
 ...
 * \relates RWMathVec
 *
 * Given a complex sequence \f$C(n), n=0, \ldots , 2N\f$ points long, it is
 * a complex conjugate-even sequence if:
 *
 * \f[
 ...
 * parts of \f$C(0)\f$ and \f$C(N)\f$ must be 0. Given the lower half of the
 * sequence, that is, \f$C(n), n=0, \ldots , N\f$, this function expands it
 ...
 * \relates RWMathVec
 *
 * Given a complex sequence \f$C(n), n=0, \ldots, 2N\f$ points long, it is a
 * complex conjugate-odd sequence if:
 *
 * \f[
 * C(n) = - conj[C(-n)] = - conj[C(2N - n)]
 * \f].
 ...
 * \f$C(0)\f$ and \f$C(N)\f$ must be 0. Given the lower half of the
 * sequence, that is, \f$C(n), n=0, \ldots , N\f$, this function expands it
 * out to the entire sequence \f$C(n), n=0, ..., 2N\f$.
 */
 ...
 * \relates RWMathVec
 *
 * Takes a complex argument <em>x</em> and returns real <em>y</em>
 * containing the real part \f$y_{i}=Re(x_{i})\f$. With most versions of
 * complex, the results can be used as an l-value:
 *
 * \code
 * RWMathVec<DComplex>  v(5,  0);        //  (0,  0),  (0,  0),  ...
 ...
 *
 * \brief
 * Returns a complex vector <em>v</em> containing the complex
 * <em>N</em><sup>th</sup> roots of unity.
 *
 * Returns a complex vector <em>v</em> containing the complex
 * <em>N</em><sup>th</sup> roots of unity, that is:
 *
 * \f[
 * \begin{array}{l r}
 ...
 * \ref RWMathVec "RWMathVec<DComplex>" with \a nterms elements.
 */
 ...
 * \f]
 *
 * If rootsOfOne() is invoked with argument \a N, the \a N roots are
 * returned in a vector of type \ref RWMathVec "RWMathVec<DComplex>" with
 * \a N elements.
 */
 ...
 * \relates RWMathVec
 *
 * Returns the spectral variance of a Fourier transformed series. If \e v is
 * a complex series containing the complex spectral estimates, the spectral
 * variance \e S is given by:
 *
 * \f[
 ...
 * \relates RWMathVec
 *
 * Returns arc cosines \e y such that \f$y_{i} = cos^{-1} (x_{i})\f$. The
 * <em>y</em><sub>i</sub>(in radians) are in the range
 * \f$0 < y_{i} \leq \pi\f$, for elements <em>x</em><sub>i</sub> with
 * absolute values \f$|x_{i}| \leq 1\f$.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Takes \a x as an argument and returns arc sines \e y such that
 * \f$y_{i} = sin^{-1} (x_{i})\f$. The <em>y</em><sub>i</sub> (in radians)
 * are in the range \f$-\pi/2 < y_{i} \leq \pi/2\f$, for elements
 ...
 * \relates RWMathVec
 *
 * Takes \a x and returns \e y of arc tangents (in radians), such that
 * \f$y_{i} = tan^{-1}x_{i}\f$, where \f$-\pi/2 < y_{i} \leq \pi/2\f$.
 *
 * At least one of the arguments <em>x</em><sub>i</sub> or
 * <em>y</em><sub>i</sub> must be nonzero.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Takes two arguments and returns quadrant correct arc tangents \e z (in
 * radians), such that \f$z_{i} = tan^{-1} ( x_{i}/y_{i})\f$. For each
 * element \e i, the expression \f$atan2(x_{i}, y_{i})\f$ is mathematically
 * equivalent to:
 *
 * \f[
 ...
 *
 * Takes \a x as an argument and returns \e y such that
 * <em>y</em><sub>i</sub> corresponds to the next integer greater than or
 * equal to <em>x</em><sub>i</sub>. For example, if <em>x = [ -1.3, 4.3,
 * 7.9]</em> then <em>y = [ -1, 5, 8].</em>
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Given a real sequence \f$V(n), n=0, \ldots , 2N\f$ points long, it is an
 * even sequence if:
 *
 * \f[
 * V(n) = V(-n) = V(2N - n)
 * \f]
 *
 * Given the lower half of an even sequence, that is,
 * \f$V(n), n=0, \ldots , N\f$, expandEven() expands it out to the entire
 * sequence \f$V(n), n=0, \ldots, 2N\f$.
 */
template< class T >
 ...
 * \f]
 *
 * Note that for a sequence to be odd, \f$V(0) = V(N) = 0\f$. Given the
 * lower half of an odd sequence, that is, \f$V(n), n=1, \ldots , N-1\f$,
 * expandOdd() expands it out to the entire sequence
 * \f$V(n), n=0, \ldots , 2N\f$.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Takes <em>x</em> as an argument and returns <em>y</em> such that
 * <em>y</em><sub>i</sub> corresponds to the largest integer less than or
 * equal to <em>x</em><sub>i</sub>. For example, if <em>x = [ 1.3, 4.4,
 * -3.2]</em>, then <em>y = [ 1, 4, -4].</em>
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Takes \a x as an argument and returns natural logarithms <em>y</em> such
 * that \f$y_{i}=log(x_{i})\f$. The arguments \a x <sub>i</sub> must not be 0.
 ...
 * \relates RWMathVec
 *
 * Takes \a x as an argument and returns base 10 logarithms <em>y</em> such
 * that \f$y_{i} = log_{10}(x_{i})\f$. The arguments \a x <sub>i</sub> must
 * not be 0.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * The function tan() takes argument <em>x</em> and returns <em>y</em> such
 * that \f$y_{i} = tan(x_{i})\f$.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * The function tanh() takes argument <em>x</em> and returns <em>y</em> such
 * that \f$y_{i} = tanh(x_{i})\f$.
 */
template< class T >
 ...
 * \relates RWMathVec
 *
 * Takes complex \a x as an argument and returns real \e y containing the
 * norm:
 *
 * \f[
 ...
 * \relates RWMathVec
 *
 * The function l2Norm() is the square root of the sum of the squares of its
 * entries:
 *
 * \f[
 ...
 * squares of its entries. For a vector, the formula is:
 *
 * \f[
