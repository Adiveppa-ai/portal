#ifndef __RWDB_CHECKCONSTRAINT_H__
#define __RWDB_CHECKCONSTRAINT_H__

/**************************************************************************
 *
 * $Id: //spro/rel2016.1/rw/db/checkconstr.h#1 $
 *
 ...
 *
 * \brief
 * Represents check constraints, used when creating a database table or
 * fetching a database table schema.
 *
 * RWDBCheckConstraint represents a table's check constraint, used when
 * building up an RWDBSchema in preparation for an
 * RWDBDatabase::createTable() call. The check condition is the definition
 * of the constraint. When creating a table, the instances of RWDBColumn, if
 * any, are ignored.
 *
 * RWDBCheckConstraint instances are also used as elements of lists that are
 * returned from RWDBTable::checkConstraints(). If the table columns in the
 * definition of the constraint can be obtained from the database, they will
 * be available via the methods operator[]() and column() when the check
 * constraints are fetched via RWDBTable::checkConstraints().
 *
 * RWDBCheckConstraint is designed around the Interface/Implementation
 * paradigm. An RWDBCheckConstraint instance is an interface to a
 ...
 */
class RWDB_GLOBAL RWDBCheckConstraint : public RWCollectable
{
public:
    ...
    /**
     * Constructs a check constraint with check condition \a checkCond and
     * name \a constraintName. If \a constraintName is the empty string, no
     * name will be used when creating the table.
     */
     ...
     *
     * \note
     * This does not change the name of the check constraint on a table that
     * already exists in the database.
     */
    RWDBCheckConstraint&  constraintName(const RWCString& newName);
    ...
    /**
     * Sets the check condition to \a cond. Returns a reference to self. To
     * unset a check condition, pass an empty RWDBExpr created with the
     * default constructor.
     *
     * \note
     * This does not change the check condition of the check constraint on a
     * table that already exists in the database.
     */
    RWDBCheckConstraint&  checkCondition(const RWDBExpr& cond);

    /**
     * Returns the check condition. If there is no check condition, returns
     * an empty RWDBExpr.
     */
     ...
     */
    RWDBCheckConstraint&  operator=(const RWDBCheckConstraint& constr);
     ...
     * RWDBCheckConstraint is treated as out of range. Note that the
     * returned RWDBColumn is not a deep copy; modifications to RWDBColumn
     * change the containing RWDBCheckConstraint.
     */
    ...
    /**
     * Returns \c true if self's check condition is set, otherwise returns
     * \c false.
     */
    bool                  isValid() const;
    ...
    /**
     * If \a c is an RWDBCheckConstraint according to the method isA(),
     * compares the result of its asString() method to the result of this
     * RWDBCheckConstraint object's asString() method using the default
     * phrasebook. Returns \c true if they are equal, otherwise returns
     * \c false. If \a c is not an RWDBCheckConstraint, returns\c false.
     *
    ...
    operator>>(RWvistream& s, RWDBCheckConstraint*& pCl);

    friend RWDB_SYMBOLIC RWFile&
    operator>>(RWFile& f, RWDBCheckConstraint*& pCl);

    RW_INLINE_FRIEND
    RWvostream& operator<<(RWvostream& v, const RWDBCheckConstraint& Cl)
    {
    ...
    RW_INLINE_FRIEND
    RWFile& operator<<(RWFile& f, const RWDBCheckConstraint& Cl)
    {
    ...
    RW_INLINE_FRIEND
    RWvistream& operator>>(RWvistream& v, RWDBCheckConstraint& Cl)
    {
    ...
    RW_INLINE_FRIEND
    RWFile& operator>>(RWFile& f, RWDBCheckConstraint& Cl)
    {
...
private:
    ...
    friend struct RWInit(RWDBCheckConstraint);
    friend RWCollectable* rwCreateFN(RWDBCheckConstraint)();

    /**
     * \internal
     */
    RWDBCheckConstraint(RWDBCheckConstraintImp* imp);

    RWDBCountedRef<RWDBCheckConstraintImp> impl_;
};

#define RWDBCheckConstraintListDEFAULT_CAPACITY 10
 ...
 *
 * RWDBCheckConstraintList is an ordered collection of RWDBCheckConstraint
 * instances. RWDBCheckConstraintList is used by the DB Interface Module as
 * an input/output parameter in the function RWDBTable::checkConstraints().
 * When the list is populated with instances of RWDBCheckConstraint, the
 * instances are owned by the containing list. The destructor deallocates
 * the contents of the RWDBCheckConstraintList by calling clearAndDestroy().
 *
 * \synopsis
 * #include <rw/db/checkconstr.h>
 * RWDBCheckConstraintList aList;
 *
...
public:
     ...
     * Constructor.
     */
    RWDBCheckConstraintList() : checkConstraintList_(RWDBCheckConstraintListDEFAULT_CAPACITY) {}

    /**
     * Copy constructor. Makes a copy of \a aList.
     */
    RWDBCheckConstraintList(const RWDBCheckConstraintList& aList)
        : checkConstraintList_(RWDBCheckConstraintListDEFAULT_CAPACITY)
    {
        RWDBCheckConstraintList tmpList;
        tmpList.checkConstraintList_.resize(aList.entries());
        RWTScopedPointer<RWDBCheckConstraint> tmp(0);
        for (size_t i = 0; i < aList.entries(); ++i) {
            tmp.reset(new RWDBCheckConstraint(aList[i]));
            tmpList.append(tmp.get());
            tmp.release();
        }
        tmpList.checkConstraintList_.swap(checkConstraintList_);
    }
    ...
    /**
     * Returns a reference to the RWDBCheckConstraint at a position \a index
     * in self. Throws RWBoundsErr if the \a index is out of range.
     */
    const RWDBCheckConstraint& operator[](size_t index) const
    {
        return *static_cast<const RWDBCheckConstraint*>(checkConstraintList_[index]);
    }
     ...
     * in self. Throws RWBoundsErr if the \a index is out of range.
     */
    RWDBCheckConstraint& operator[](size_t index)
    {
        return *static_cast<RWDBCheckConstraint*>(checkConstraintList_[index]);
    }

    /**
     * Returns a reference to the RWDBCheckConstraint at a position \a index
     * in self. Bounds checking is enabled by defining the preprocessor
     * directive \c RWBOUNDS_CHECK before including the header file
     * \c rw\\db\\checkconstr.h. In this case, throws RWBoundsErr if the
    ...
    size_t entries() const
    {
        return checkConstraintList_.entries();
    }

    /**
     * Adds \a cc to the end of the collection and returns it. If
     * successful, self assumes ownership of \a cc. Returns #rwnil if the
     * insertion was unsuccessful.
     */
    RWDBCheckConstraint* append(RWDBCheckConstraint* cc)
    {
        RW_ASSERT(0 != cc);
        return static_cast<RWDBCheckConstraint*>(checkConstraintList_.append(cc));
    }
    ...
    {
        checkConstraintList_.clearAndDestroy();
    }
     ...
     */
    RWDBCheckConstraint* removeAt(size_t index)
    {
        return static_cast<RWDBCheckConstraint*>(checkConstraintList_.removeAt(index));
    }

    RWDBCheckConstraint* insert(RWDBCheckConstraint* cc)
    {
        RW_ASSERT(0 != cc);
        return static_cast<RWDBCheckConstraint*>(checkConstraintList_.insert(cc));
    }
    ...
    RWspace binaryStoreSize() const
    {
        return checkConstraintList_.recursiveStoreSize();
    }
    ...
    {
        RWDBCheckConstraintList ret;
        RWTScopedPointer<RWDBCheckConstraint> tmp(0);
        for (size_t i = 0; i < checkConstraintList_.entries(); ++i) {
            tmp.reset(new RWDBCheckConstraint((*this)[i].clone()));
            ret.append(tmp.get());
            tmp.release();
        }
        return ret;
    ...
    /**
     * Assignment operator. Destroys the contents of self and makes a copy
     * of \a aList.
     */
    RWDBCheckConstraintList& operator=(const RWDBCheckConstraintList& aList)
    {
        if (this != &aList) {
            RWDBCheckConstraintList tmp(aList);
            tmp.checkConstraintList_.swap(checkConstraintList_);
        }
        return *this;
    }
...
private:
    ...
    RWOrdered checkConstraintList_;
};
 ...
 * Saves \a obj to \a f.
 */
inline RWFile& operator<<(RWFile& f, const RWDBCheckConstraintList& obj)
{
    obj.saveGuts(f);
    return f;
}
 ...
 */
inline RWFile& operator>>(RWFile& f, RWDBCheckConstraintList& obj)
{
    obj.restoreGuts(f);
    return f;
}
 ...
 * Saves \a obj to \a s.
 */
inline RWvostream& operator<<(RWvostream& s, const RWDBCheckConstraintList& obj)
{
    obj.saveGuts(s);
    return s;
}
 ...
 */
inline RWvistream& operator>>(RWvistream& s, RWDBCheckConstraintList& obj)
{
    obj.restoreGuts(s);
    return s;
}
