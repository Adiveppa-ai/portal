 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tpsrtdli.h#1 $
 *
 **********************************************************************
 *
...
/**
 * \ingroup tools_stl_based_collection_group
 *
 * \brief
 * A sorted pointer-based collection of values implemented as a
 * doubly-linked list.
 *
 ...
 * comparison object of type \c C. Class \c T is the type pointed to by the
 * items in the collection. Class \c A is an allocator of objects of type
 * \c T. For more information about using custom allocators, please see the
 * <em>Essential Tools Module User's Guide</em>.
 *
 * \c C must induce a total ordering on elements of type \c T via a public
 * member <tt>bool operator()(const T& x, const T& y)</tt> that returns
 * \c true if \c x should precede \c y within the collection. The structure
 * \b std::less<T> from the C++ Standard Library header file
 * \c \<functional\> is an example. Note that items in the collection
 * will be dereferenced before being compared.
 *
 * The \c value type must have \c operator== and \c operator< defined. This
 * requirement is imposed by the C++ Standard Library.
 *
 * \synopsis
 ...
 * \sa
 * \parblock
 * Class RWTPtrSortedVector is an alternative always-sorted pointer-based
 * collection. RWTPtrDlist is an unsorted pointer-based doubly-linked list.
 *
...
public:
    ...
    /**
     * A type that provides a bidirectional iterator over the elements in
     * the container.
     */
    ...
    /**
     * A type that provides a bidirectional, reverse-order iterator over the
     * elements in the container.
     */
     ...
     *
     * \deprecated
     * As of SourcePro 12.5, use <tt>const T*</tt> instead.
     */
    ...
    /**
     * Removes all items from the collection and uses
     * <tt>operator delete</tt> to destroy the objects pointed to by those
     * items. Do not use this method if multiple pointers to the same object
     * are stored.
     */
     ...
     * expression <tt>((*\a fn)(t,\a d))</tt> is \c true, otherwise returns
    ...
    /**
     * Returns \c true if there exists an element \c t in self such that the
     * expression <tt>(*t == *\a a)</tt> is \c true, otherwise returns
     * \c false.
     */
    ...
    /**
     * If there exists an element \c t in self such that the expression
     * <tt>(*t == *\a a)</tt> is \c true, returns \c t. Otherwise, returns
     * #rwnil.
     */
     ...
     *
     * Calling this function on an empty collection results in undefined
     * behavior.
     */
    reference first()
    {
    ...
    /**
     * Adds the items from \a a to self in an order preserving way. Returns
     ...
     * expression <tt>((*\a fn)(t,\a d))</tt> is \c true. Returns #rwnil if
     * there is no such element. Client data may be passed through parameter
     * \a d.
     */
    ...
    /**
     * Removes and returns the first element \c t in self such that the
     * expression <tt>(*t == *\a a)</tt> is \c true. Returns #rwnil if there
     * is no such element.
     */
     ...
     * <tt>((*\a fn)(t,\a d))</tt> is \c true. Returns the number of items
     ...
     * Thrown if this position is not between zero and one less than the
     ...
     *
     * \exception RWBoundsErr
     * Thrown if index \a i is not between \c 0 and one less than the number
     * of entries in self.
     */
     ...
     * such item exists. Client data may be passed through parameter \a d.
     */
    ...
    /**
     * Clears all elements of self and replaces them by copying all elements
     * of \a rhs.
     */
...
#    endif
    ...
    /**
     * Move assignment. Self sorts and takes ownership of the data owned by
     * \a rhs.
     *
     * \conditional
     * This method is only available on platforms with rvalue reference
...
#    endif
    ...
    /**
     * Constructs a doubly-linked list by iterating over all elements in
     * \a lst and performing an order preserving insertion on self for each.
     */
...
#    endif
    ...
    /**
     * Move constructor. The constructed list sorts and takes ownership of
...
#    endif
    ...
    /**
     * Constructs a vector by copying elements from the range [\a first,
     * \a last).
     *
     * \c InputIterator is an input iterator type that points to elements
     * that are convertible to #value_type objects.
 ...
 * iterating through both collections produces individual elements that, in
 * turn, compare equal to each other. Elements are dereferenced before being
 * compared.
 */
template <class T, class C, class A>
 ...
 * \relates RWTPtrSortedDlist
 *
 * Saves the collection \a coll onto the output stream \a strm, or a
 * reference to it if it has already been saved.
 */
template <class T, class C , class A>
 ...
 * \relates RWTPtrSortedDlist
 *
 * Saves the collection \a coll onto the RWFile \a strm, or a reference to
 ...
 * \relates RWTPtrSortedDlist
 *
 * Restores the contents of the collection \a coll from the input stream
 * \a strm.
 */
template <class T, class C , class A>
 ...
 * \relates RWTPtrSortedDlist
 *
 * Looks at the next object on the input stream \a strm and either creates a
 * new collection off the heap and sets \a p to point to it, or sets \a p to
 * point to a previously read instance. If a collection is created off the
 * heap, then the application is responsible for deleting it.
 */
template <class T, class C , class A>
 ...
 * \relates RWTPtrSortedDlist
 *
 * Looks at the next object on the RWFile \a strm and either creates a new
 * collection off the heap and sets \a p to point to it, or sets \a p to
 * point to a previously read instance. If a collection is created off the
 * heap, then the application is responsible for deleting it.
 */
template <class T, class C , class A>
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrSortedDlist::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTPtrSortedDlist.
 *
 * RWTPtrSortedDlistIterator provides an iterator interface to the C++
 * Standard Library-based collection represented by RWTPtrSortedDlist. (An
 * STL iterator interface is provided through RWTPtrSortedDlist::iterator.)
 *
 * The order of iteration over an RWTPtrSortedDlist is dependent on the
 * comparator object parameter \c C as applied to the values stored in the
 * container.
 *
 * The current item referenced by this iterator is undefined after
 * construction or after a call to reset(). The iterator becomes valid after
 * being advanced with either a pre-increment or operator()().
 *
 * For both operator++() and operator()(), iterating past the last element
 * will return a value equivalent to boolean \c false. Continued increments
 * will return a value equivalent to \c false until reset() is called.
 *
 * The \c value type must have \c operator== and \c operator< defined. This
 * requirement is imposed by the C++ Standard Library.
 *
...
public:
    /**
     * Creates an iterator for the list \a l. The iterator begins in an
     ...
     * iteration, self referenced the last association in the list, self
     * will now point to an undefined value, and a value equivalent to
     * \c false will be returned. Otherwise, a value equivalent to \c true
     * is returned.
     */
    bool operator++();

    /**
     * Moves self back to the immediately previous element. If the iterator
     * has been reset or just created, this operator will return \c false,
     * otherwise it will return \c true. If self references the first
     * element, it will now be in the reset state. If self has been iterated
     * past the last value in the list, it will now reference the last item
     * in the list.
     */
    bool operator--();
     ...
     * iterator and returns its value. If the iterator has advanced past the
     * last item in the container, #rwnil will be returned.
     */
    T* operator()()
    {
    ...
    /**
     * Returns the first element \c t encountered by iterating self forward,
     * such that the expression <tt>(*t == *\a a)</tt> is \c true. Otherwise
     * returns #rwnil. Leaves self referencing found item or "off the end."
     */
    T* findNext(const T* a);

    /**
     * Returns the first element \c t encountered by iterating self forward,
     * such that the expression <tt>((*\a fn)(t,\a d))</tt> is \c true.
     * Otherwise returns #rwnil. Leaves self referencing found item or "off
     * the end." Client data may be passed through parameter \a d.
     */
     ...
     * collection. Undefined if self is not referencing a value within the
     * list. Leaves self referencing prior item or in reset state.
     */
    T* remove();

    /**
     * Returns and removes the first element \c t, encountered by iterating
     * self forward, such that the expression <tt>(*t == *\a a)</tt> is
     * \c true. Otherwise returns #rwnil. Leaves self referencing prior item
     * or in reset state.
     */
    ...
    /**
     * Resets the iterator so that after being advanced it will point to the
     * first element of the collection \a l.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTPtrSortedDlist::const_iterator to iterate
 * over the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTPtrSortedDlist.
 *
 * RWTPtrSortedDlistConstIterator provides a \c const iterator interface to
 * C++ Standard Library-based RWTPtrSortedDlist collections. A \c const
 ...
 *
 * For both operator++() and operator()(), iterating past the last element
 * returns a value equivalent to boolean \c false. Continued increments
 * return a value equivalent to \c false until reset() is called.
 *
 * The \c value type must have \c operator== and \c operator< defined. This
 * requirement is imposed by the C++ Standard Library.
 *
...
public:
    /**
     * Creates a \c const iterator for the list \a l. The iterator begins in
     * an undefined state and must be advanced before the first element is
     * accessible.
     */
    ...
    /**
     * Advances self to the next element. If the iterator has been reset or
     * just created, self now references the first element. If, before
     * iteration, self referenced the last association in the list, self now
     * points to an undefined value and a value equivalent to \c false is
     * returned. Otherwise, a value equivalent to \c true is returned.
     */
    bool operator++();

    /**
     * Moves self back to the immediately previous element. If the iterator
     * has been reset or just created, this operator returns \c false,
     * otherwise it returns \c true. If self references the first element,
     * it is now in the reset state. If self has been iterated past the last
     * value in the list, it now references the last item in the list.
     */
    bool operator--();
    ...
    /**
     * Advances self to the next element, dereferences the resulting
     * iterator, and returns its value. If the iterator has advanced past
     * the last item in the container, #rwnil is returned.
     */
    const T* operator()()
    {
        return ++(*this) ? key() : rwnil;
    ...
    /**
     * Resets the iterator so that after being advanced, it points to the
     * first element of the collection.
     */
    void reset()
    {
