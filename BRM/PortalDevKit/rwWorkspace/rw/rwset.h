 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/rwset.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * A group of unordered elements, not accessible by an external key, where
 * duplicates are not allowed.
 *
 ...
 * RWCollectable::hash() and RWCollectable::isEqual(). Use the function
 * \ref RWCollectable::hash() "hash()" to find objects with the same hash
 * value, and then use \ref RWCollectable::isEqual() "isEqual()" to confirm
 * the match.
 *
 * An item \c c is considered to be "already in the collection" if there is
 * a member of the collection with the same hash value as \c c for which
 * \ref RWCollectable::isEqual() "isEqual(c)" returns \c true. In this case,
 * method \ref insert() "insert(c)" will not add it, thus insuring that
...
public:
    ...
    /**
     * Returns \c __RWSET, i.e. a <em>class identifier</em>, that identifies
     * this object's class.
     */
...
public:
    ...
    /**
     * Constructs an empty set with RWCollection::DEFAULT_CAPACITY hashing
     * buckets.
     */
    RWSet();
    ...
    /**
     * Copy constructor. Makes a shallow copy of the collection \a h.
     */
    RWSet(const RWSet& h) :  RWHashTable(h) { }

#if !defined(RW_NO_RVALUE_REFERENCES)
    /**
     * Move constructor. The constructed RWSet takes ownership of the data
     * owned by \a h.
     *
     * \conditional
    ...
    /**
     * Returns \c true if self is a subset of \a h, that is, every element
     * of self has a counterpart in \a h which isEqual().
     */
    ...
    /**
     * Returns \c true if self is a proper subset of \a h, that is, every
     * element of self has a counterpart in \a h which isEqual(), but where
     ...
     * effect desired). This function may be slightly more efficient than
     * operator*=().
     */
    ...
    /**
     * Adds \a c to the collection and returns it. If an item is already in
     * the collection which isEqual() to \a c, then the existing item is
    ...
    /**
     * Returns the count of entries that compare isEqual() to the item
     * pointed to by \a target. Because duplicates are not allowed for this
     * collection, only 0 or 1 can be returned.
     *
     * \note
     * If self is a collection of key-value pairs (e.g. RWHashDictionary),
     * this method compares \a target with the keys in the collection.
     */
 ...
 *
 * \brief
 * Iterator for class RWSet, allowing sequential access to all the elements
 * of RWSet.
 *
...
public:
    ...
    /**
     * Constructs an iterator for an RWSet. After construction, the position
 ...
 *
 * \brief
 * Const iterator for class RWSet, to allow sequential access to all
 * elements of RWSet.
 *
 * One of two iterators for class RWSet. This \c const iterator allows
 * sequential access to all the elements of RWSet and prohibits any changes
 * to the contents of the data structure.
 *
 * \note
 * An RWSet is unordered; elements are not accessed in any particular order.
 *
