 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/btrdict.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Represents a dictionary for the storage and retrieval of key-value pairs,
 * and implemented as a B-tree.
 *
 * Dictionary class implemented as a B-tree, for the storage and retrieval
 * of key-value pairs. Both the keys and values must inherit abstract base
 * class RWCollectable. The elements are ordered internally according to the
 * value returned by virtual function RWCollectable::compareTo() of the
 * keys. Duplicate keys are not allowed.
 *
 * The B-tree is \e balanced. That is, nodes are never allowed to have less
 * than a certain number of items (called the \e order). The default order
 * is 50, but may be changed by resetting the value of the static constant
 * \c order in the header file \c <btree.h> and recompiling. Larger values
 * result in shallower trees, but less efficient use of memory.
 *
 * \synopsis
...
public:
    ...
    /**
     * Move constructor. The constructed RWBTreeDictionary takes ownership
     * of the data owned by \a btr.
     *
     * \conditional
    ...
    /**
     * Returns \c true if self and \a btr are equivalent. That is, they must
     * have the same number of items and for every item in self, there must
     * be an item in \a btr that compares equal.
     */
    ...
    /**
     * Removes all key-value pairs from the collection, and deletes \e both
     * the key and the value.
     */
    virtual void clearAndDestroy();

    /**
     * Returns the key in the collection that compares equal to the object
     * pointed to by \a target, or #rwnil if no key is found.
     */
    virtual RWCollectable* find(const RWCollectable* target) const;

    /**
     * Returns the key in the collection that compares equal to the object
     * pointed to by \a target, or #rwnil if no key was found. The value is
     * put in \a v. You are responsible for defining \a v before calling
     * this function.
     */
    RWCollectable* findKeyAndValue(const RWCollectable* target, RWCollectable*& v) const;

    /**
     * Returns the \e value associated with the key that compares equal to
     * the object pointed to by \a target, or #rwnil if no key was found.
     */
     ...
     * Returns the \e value associated with the key that compares equal to
     * the object pointed to by \a target, or #rwnil if no key was found.
     * Replaces the value with \a newValue (if a key was found).
     */
    ...
    /**
     * Removes the key-value pair for which the key compares equal to the
     * object pointed to by \a target. Returns the key, or #rwnil if no
     * match was found.
     */
    virtual RWCollectable* remove(const RWCollectable* target);

    /**
     * Removes \e and deletes the key-value pair for which the key compares
     * equal to the object pointed to by \a target. Note that both the key
     * and the value are deleted. Does nothing if the key is not found.
     */
    virtual void removeAndDestroy(const RWCollectable* target);

    /**
     * Removes the key-value pair for which the key compares equal to the
     * object pointed to by target. Returns the key, or #rwnil if no match
     * was found. The value is put in \a v. You are responsible for defining
    ...
    /**
     * Evaluates the function pointed to by \a testfunc for the key of each
     * item in the RWBTreeDictionary. It inserts keys and values for which
     * the function returns \c true into a new RWBTreeDictionary allocated
     * off the heap and returns a pointer to this new collection. Because
     * the new dictionary is allocated <em>off the heap</em>, you are
     * responsible for deleting it when done. This is not a virtual
     * function.
     */
     ...
     * the value of each item in the RWBTreeDictionary. It inserts keys and
     * values for which the function returns \c true into a new
     * RWBTreeDictionary allocated off the heap and returns a pointer to
     * this new collection. Because the new dictionary is allocated <em>off
     * the heap</em>, you are responsible for deleting it when done. This is
