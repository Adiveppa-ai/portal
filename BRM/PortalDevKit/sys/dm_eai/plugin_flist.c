...
/*******************************************************************
 *
 * @(#)plugin_flist.c 1.2 99/11/12
 *
 * Copyright (c) 2000, 2022, Oracle and/or its affiliates.All rights reserved. 
 * or its licensors and may be used,reproduced, stored or transmitted only in accordance with a valid Oracle license
...
PublishEvent(void *context, void *payload, char *srvc_poidp)
{
	...
	pin_snprintf(msg, sizeof(msg), "Publishevent  : pid = %d, service poid = %s", pid,
		srvc_poidp);
	PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_DEBUG, msg);
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "flist input to dm_flist",
		(pin_flist_t *)payload);
	return PIN_RESULT_PASS;
}
