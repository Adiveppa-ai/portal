 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/bench.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Abstract base class to facilitate automatic benchmarking of some piece of
 * code.
 *
 ...
 * function should perform \c N operations of the type that you are trying
 * to benchmark. RWBench will call doLoop() over and over again until a
 * preset amount of time has elapsed. It will then sum the total number of
 * operations performed.
 *
 * To run, construct an instance of your derived class and then call go().
 * Then call report() to get a standard summary. For many compilers, this
 * summary will automatically include the compiler type and memory model.
 * You can call ops(), outerLoops(), etc. for more detail.
 *
 * If you wish to correct for overhead, then provide an idleLoop() function
 * which should do all non-benchmark-related calculations.
 *
 * \synopsis
 * #include <rw/bench.h>
 * (Abstract base class)
...
public:
    ...
    /**
     * The parameter \a duration is the nominal amount of time that the
     * benchmark should take in seconds. The virtual function
     * doLoop(unsigned long) is called over and over again until at least
     * this amount of time has elapsed. The parameter \a ILO is the number
     * of inner loop operations that should be performed. This parameter is
     * passed in as parameter \c N to \ref doLoop() "doLoop(N)". Parameter
     * \a machine is an optional null terminated string that should describe
     * the test environment (perhaps the hardware the benchmark is being run
     * on).
     */
    ...
    /**
     * This is a non-virtual function that provides the same service as
     * \ref parse(int, char*[]) "parse(int argc, char * argv[])", but is
     * designed for Windows users. It extracts tokens from the
     * null-terminated command argument provided by Windows, then calls the
     * virtual \ref parse(int, char*[]) "parse()" for ANSI C command
     * arguments.
     */
     ...
     * the specializing class. This function is repeatedly called until a
     * time duration has elapsed. It should perform the operation to be
     * benchmarked \a N times. See the example.
     */
    virtual void   doLoop(unsigned long n) = 0;

    /**
     * This function can help to correct the benchmark for overhead. The
     * default definition merely executes a "for()" loop \a n times. See the
     * example.
     */
    ...
    /**
     * This function prints information to the stream about the compiler and
    ...
    /**
     * This function accesses the name of the machine which is passed into
     * the benchmark object through parse().
     */
     ...
     * (the product of the number of times outerLoops() was called times the
