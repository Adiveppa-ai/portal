...
/**********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tools/cmpstream.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Compresses data and serializes it to an associated \b std::streambuf.
 *
 * Class RWCompressedOStream compresses data and serializes it to an
 * associated \b std::streambuf. The results can be restored by using its
 * counterpart RWCompressedIStream.
 *
 * \sa
 * RWCompressedStreamBuffer
 *
 * \note
 * RWCompressedOStream can be interrogated as to the stream state using
 * member functions \b std::ios::good(), \b std::ios::bad(),
 * \b std::ios::eof(), etc.
 *
 * \synopsis
...
public:
    ...
    /**
     * Constructor initializes and opens the underlying stream buffer with
     * the provided \a stream_buffer, \a mode, \a block_size, and
     * \a compressor. If the open fails, \b failbit is set on this stream.
     *
     * \note
     * The \a mode is always combined with the appropriate flags to allow
     * the underlying stream buffer to be used for output.
     *
     * \sa
    ...
    /**
     * Constructor initializes and opens the underlying stream buffer with
     * the provided \a mode, \a block_size, \a compressor, and the stream
     * buffer associated with \a stream. If the open fails, \b failbit is
     * set on this stream.
     *
     * \note
     * The \a mode is always combined with the appropriate flags to allow
    ...
    /**
     * Opens the underlying stream buffer with the provided
     * \a stream_buffer, \a mode, and \a block_size. If the open fails,
     * \b failbit is set on this stream.
     *
     * \note
     * The \a mode is always combined with the appropriate flags to allow
     * the underlying stream buffer to be used for output.
    ...
    /**
     * Closes the underlying stream buffer. If the close fails, \b failbit
     * is set on this stream. After this method has been called, this stream
     * cannot be used for output until a successful call to open() has been
     * made.
     */
    void
    close()
    ...
    /**
     * Returns \c true if the underlying stream buffer is open and is ready
     * for use, otherwise returns \c false.
     */
    bool is_open() const
    {
 ...
 *
 * \brief
 * Deserializes and decompresses data from an associated \b std::streambuf.
 *
 * Class RWCompressedIStream deserializes and decompresses data from an
 * associated \b std::streambuf. The data on that streambuf must have been
 * serialized with its counterpart RWCompressedOStream.
 *
 * \sa
 * RWCompressedStreamBuffer
 *
 * \note
 * RWCompressedIStream can be interrogated as to the stream state using
 * member functions \b std::ios::good(), \b std::ios::bad(),
 * \b std::ios::eof(), etc.
 *
 * \synopsis
...
public:
     ...
     * \note
     * The \a mode is always combined with the appropriate flags to allow
     * the underlying stream buffer to be used for input.
     *
     * \sa
     * RWCompressedStreamBuffer::open()
     */
     ...
     * Closes the underlying stream buffer. If the close fails, \b failbit
     * is set on this stream. After this method has been called, this stream
     * cannot be used for input until a successful call to open() has been
     * made.
     */
    void
    close()
 ...
 *
 * \brief
 * Compresses and serializes data to or deserializes and decompresses data
 * from an associated \b std::streambuf.
 *
 * Class RWCompressedIOStream compresses and serializes data to or
 * deserializes and decompresses data from an associated \b std::streambuf.
 * Data read from the associated streambuf must have been serialized with
 * RWCompressedOStream or RWCompressedIOStream, and any data written to the
 * associated streambuf can be deserialized with RWCompressedIStream or
 ...
 *
 * \note
 * RWCompressedIOStream can be interrogated as to the stream state using
 * member functions \b std::ios::good(), \b std::ios::bad(),
 * \b std::ios::eof(), etc.
 *
 * \synopsis
...
public:
     ...
     * \note
     * The \a mode is always combined with the appropriate flags to allow
     * the underlying stream buffer to be used for input and output.
     *
     * \sa
     * RWCompressedStreamBuffer::open()
     */
     ...
     * Closes the underlying stream buffer. If the close fails, \b failbit
     * is set on this stream. After this method has been called, this stream
     * cannot be used for input or output until a successful call to open()
     * has been made.
     */
    void
    close()
