 ...
 *
 * $Id: //spro/rel2016.1/rw/tools/pointer.h#1 $
 *
 ***************************************************************************/
...
/**
 * \internal
 * Provides a storage implementation for the pointer and deleter objects.
 */
...
/**
 * \internal
 * Specialization of RWTScopedPointerStorage for situations where the
 * deleter type is empty. Leverages the empty base class optimization.
 */
template <typename T, typename D>
 ...
 *
 * Specialization of rw_swap for RWTScopedPointerStorage. This is equivalent
 * to:
 *
 * \code
 * lhs.swap(rhs);
 ...
 * argument. When the RWTScopedPointer is destroyed, the call operator will
 * be invoked on an instance of \c D, passing the underlying pointer as an
 * argument. Unless a custom deletion object is specified, the \c delete
...
public:
    ...
    /**
     * If the associated pointer is not null and is not equal to \a ptr,
     * calls the associated deleter object, passing in the associated
     * pointer. Associates self with the new pointer \a ptr.
     *
     * \note
 ...
 *
 * \brief
 * Single owner smart pointer for array types.
 *
 * RWTScopedPointer maintains sole ownership of a pointer to an object and
 * manages destruction of that object when the RWTScopedPointer goes out of
 * scope, or the RWTScopedPointer is assigned a pointer to a different
...
public:
    ...
    /**
     * Destructor. If the associated pointer is not null, calls the
     * associated deleter object, passing in the associated pointer.
     */
    ~RWTScopedPointer()
    {
        if (get()) {
 ...
 * \relates RWTScopedPointer
 *
 * Equivalent to calling \c operator== on the associated pointers of \a lhs
 * and \a rhs.
 */
