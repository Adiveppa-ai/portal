 ...
 ***************************************************************************
 *
 * $Id: //spro/rel2016.1/rw/randmrg32k3a.h#1 $
 *
 ...
 * generators derived from abstract base class RWTRand. RWRandGenMRG32K3A is
 * a combined multiple recursive generator with two components of order 3.
 *
 * \f[
 * \begin{align}
 * x_{n} &= a_{11}x_{n-1} + a_{12}x_{n-2} + a_{13}x_{n-3}(\bmod m_{1})\\
 * y_{n} &= a_{21}y_{n-1} + a_{22}y_{n-2} + a_{23}y_{n-3}(\bmod m_{2})\\
 * z_{n} &= x_{n} - y_{n}(\bmod m_{1})\\
 * u_{n} &= z_{n}/m_{1}\\
 * a_{11} &= 0, a_{12} = 1403580, a_{13} = -810728, m_{1} = 2^{32}- 209
 * \end{align}
 * \f]
 *
 * Unlike class RWRandGenerator, each instance of RWRandGenMRG32K3A contains
 ...
 * RWRandGenMRG32K3A consists of a seed and a series of six integers.
 *
 * \synopsis
...
public:
    ...
    /**
     * Returns \c true for two distinct instances of RWRandGenMRG32K3A if
