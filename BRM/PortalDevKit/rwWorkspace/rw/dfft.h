 ...
 *
 * \brief
 * Performs double precision complex fast Fourier transforms (FFTs) of a
 * real vector.
 *
 * Class DoubleFFTServer performs double precision complex fast Fourier
 * transforms (FFTs) of a real vector.
 *
 * At any given moment, the server is configured to transform a real vector
 * of a specified length. This length can be set one of three ways:
 *
 * - At construction time
 * - With the function setOrder()
 * - By the server from a supplied vector length.
 *
 * If you use the last method, the server automatically reconfigures to the
 * new length if the vector length changes. This is an expensive
 * calculation. Hence, it is most efficient to call this method repeatedly
 * for series of the same length. Different FFT servers may be constructed
 * to handle different lengths.
 *
 * The real vector must have an even number of points.
 *
 * Let <tt>V(j), j=0, 1, ..., 2N-1</tt> be a \c 2N point long real sequence.
 * Its transform is a complex conjugate-even sequence, that is:
 *
 * <tt>C(n) = conj[C(-n)]</tt>
 *
 * or:
 ...
 * conjugate-even sequence, that is, <tt>C(n), n=0, ..., N</tt>. The upper
 * half can be recovered from the relationship
 * <tt>C(n) = conj[C(2N-n)]</tt>. Note that the returned complex
 * conjugate-even sequence <tt>C(n)</tt> has <tt>N+1</tt> complex points, or
 * a total of <tt>2N+2</tt> numbers. The two extra numbers are the imaginary
 ...
 * \f]
 *
 * where \c V is real and \c C is complex. Note that only points
 * <tt>C(n), n=0, ..., N</tt> should be given. Also note that the sum is not
 * normalized; a call to fourier() followed by a call to ifourier() results
 * in the original series multiplied by \c 2N.
 *
 * \synopsis
 * #include <rw/dfft.h>
...
public:
    ...
    /**
     * Constructs a double precision FFT server designed to transform a
     * vector of length \a oforder.
     */
    ...
    /**
     * Returns the order of an FFT server, that is, the vector length it is
     * currently configured to transform.
     */
    size_t                order() const
    {
        return serverLength;
    ...
    /**
     * Returns the inverse FFT of a complex conjugate-even sequence of
     * <tt>N+1</tt> complex numbers as a real sequence of \c 2N points. The
     * expression
     *
     * \code
     * DoubleFFTServer  f;
     ...
     * \endcode
     *
     * prints the inverse Fourier transform of \a v as an
     * \ref RWMathVec "RWMathVec<double>" vector.
     */
