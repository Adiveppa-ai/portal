...
/**********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/stdex/hashtable.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Iterator over items contained in an rw_hashtable.
 *
...
public:
     ...
     * \internal
     *
     * Constructs an invalid rw_hashtable_iterator. The behavior of any
     ...
     * iterator \a inner.
     *
     * If \a inner is not an iterator in the container \a container and in
     * the bucket \a bucket, the behavior is undefined.
     */
 ...
 *
 * \brief
 * Defines a behavioral policy for containers that do not allow duplicate
 ...
 *
 * \brief
 * Defines a behavioral policy for containers that allow duplicate values to
 * be inserted.
 *
...
struct rw_allow_duplicates {
     ...
     * \internal
     *
     * Inserts a copy of \a val into the container \a container. An iterator
     * pointing to the newly inserted object is returned, along with the
     * status \c true.
     */
    template <typename C, typename V, typename EQ>
    static std::pair<typename C::iterator, bool>
     ...
     * the \a key_equal equality function are removed from \a container. The
     * number of items that were removed is returned.
     */
    template <typename C, typename K, typename EQ>
    static typename C::size_type
    erase(C& container, const K& key, EQ key_equal);
 ...
 *
 * \brief
 * Maintains a hash-based collection of type T
 *
 ...
 * <em>associative container</em> specification, rw_hashtable provides for
...
#  else
     ...
     *
     * \note
     * If the value specified for \a cap is zero, the default number of
     * buckets is used.
     */
    ...
    rw_hashtable(InputIterator first, InputIterator last,
                 ...
                 const hasher& h = hasher(),
                 const key_equal& eq = key_equal())
        : buckets_(cap ? cap : RW_DEFAULT_CAPACITY), hash_(h), eq_(eq), size_(0)
    {
     ...
     * in the container is returned. Otherwise, the value \a val is inserted
     * into the container and an iterator to the new item is returned.
     */
...
rw_hashtable_iterator<C, I>::rw_hashtable_iterator(C& container)
    : container_(&container), bucket_(0)
      // SP-21042 work around a compiler bug for single threaded builds on HP
#if defined(__HP_aCC) && (__HP_aCC <= 62802) && !defined(RW_MULTI_THREAD) && !defined(RW_DEBUG)
    , inner_iter_(container[0].begin())
#endif
{
...
rw_hashtable<T, Hash, EQ, A, DP, KP>::rw_hashtable(
    ...
    : buckets_(other.buckets_), hash_(other.hash_), eq_(other.eq_), size_(other.size_)
{
}
...
#  if !defined(RW_NO_STDVECTOR_RVALUE_SUPPORT)
    ...
    , hash_(rw_move(other.hash_))
    , eq_(rw_move(other.eq_))
    , size_(rw_move(other.size_))
{
}
#  else
{
...
rw_hashtable<T, Hash, EQ, A, DP, KP>::resize(size_type cap)
{
    ...
    for (typename bucket_list_type::iterator outer = buckets_.begin();
        ...
        for (typename bucket_type::iterator inner = outer->begin(); inner != outer->end(); /* see below */) {
            const size_type loc =
                hash_function()(key_policy::key_from_value(*inner)) % other_buckets.size();
            bucket_type& bucket = other_buckets[loc];
            // splice shifts the inner iterator to the next node
...
{
    const size_type erased = duplicates_policy::erase(*this, key,
                             key_policy::value_to_key_eq(key_eq()));
    size_ -= erased;
    return erased;
}
...
{
    bucket_type& bucket = buckets_[iter.bucket()];
    const typename bucket_type::iterator inner_iter =
        bucket.erase(iter.inner());
    size_ -= 1;
...
rw_hashtable<T, Hash, EQ, A, DP, KP>::clear()
{
    for (typename bucket_list_type::iterator iter = buckets_.begin();
        ...
        const typename bucket_type::size_type entries = iter->size();
        iter->clear();
        size_ -= entries;
    }
}
