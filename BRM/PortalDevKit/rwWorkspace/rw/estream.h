 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/estream.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Specializes the base class RWbostream to store values in a portable
 * binary format
 *
 ...
 * counterpart, RWeistream.
 *
 * See the entry for RWeistream for a general description of the endian
 * stream classes.
 *
 * \synopsis
...
public:
    ...
    /**
     * Specifies the format to use to represent numeric values in the
     * stream.
     */
    enum Endian {
        /**
     ...
     * the \b std::ostream \a str. Values placed into the stream have an
     * endian format given by \a fmt. For DOS, the \a str must have been
     * opened in binary mode.
     *
     * \exception RWStreamErr
     * Thrown if the underlying \b std::streambuf is not empty.
     */
    RWeostream(std::ostream& str, Endian fmt = HostEndian);

    /**
     * Constructs an RWeostream from the \b std::streambuf \a s. Values
     * placed into the stream have an endian format given by \a fmt. For
     * DOS, the \b std::streambuf \a s must have been created in binary
     * mode.
     *
     * \exception RWStreamErr
     * Thrown if the \b std::streambuf \a sb is not empty.
     */
     ...
     * This function is not supported with endian streams.
     *
     * \note
 ...
 *
 * \brief
 * Specializes the base class RWbistream to restore values previously stored
 * by RWeostream
 *
 * Class RWeistream specializes the base class RWbistream to restore values
 * previously stored by RWeostream. Please note, RWeostream must be opened
 * before RWeistream, or the application does not function properly.
 *
 * The endian streams, RWeistream and RWeostream, offer an efficient
 * compromise between the portable streams (RWpistream, RWpostream) and the
 * binary streams (RWbistream, RWbostream). By compensating for differences
 * in big-endian vs. little-endian formats, as well as sizes of the various
 * integral types, the endian streams offer portability without incurring
 * the stream-size overhead of translating values into a series of printable
 * characters. For example, data stored in little-endian format by an
 * RWeostream object in a DOS program can be retrieved by an RWeistream
 * object on any of several machines, regardless of its native endian format
 * or the sizes of its integral types. Endian streams work properly when
 * shared among a group of platforms that:
 *
 * - Share a common size and representation (apart from endian format) for
 * types \c float and \c double.
 * - Use two's complement format for negative integral values.
 *
 * As with the portable streams, care must be taken when storing or
 * retrieving variables of type \c char. Endian stream methods treat \c char
 * types as numbers except where the method description explicitly states
 * that the \c char is being treated, instead, as a character. See the entry
 * for RWpostream for an example of this distinction.
 *
 * Data stored in an integral type on one platform may be too large to fit
 * into that type on a receiving platform. If so, the RWeistream class'
 * failbit is set.
 *
 * Endian streams can be interrogated as to their stream state using member
...
public:
     ...
     * the \b std::istream \a str. For DOS, \a str must have been created in
     * binary mode.
     *
     * \exception RWStreamErr
     * Thrown if the data in \a str does not describe a valid endian stream.
     */
     ...
     *
     * \exception RWStreamErr
     * Thrown if the data in \a sb does not describe a valid endian stream.
     */
...
public:
    ...
    /**
     * Returns the endian format (RWeostream::BigEndian or
