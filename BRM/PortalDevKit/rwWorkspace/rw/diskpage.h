 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/diskpage.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Deprecated. Specialized type of buffered page heap that swaps its pages
 * to disk as necessary.
 *
 * \deprecated
 * As of SourcePro 11.1.
 *
 * RWDiskPageHeap is deprecated and is no longer supported. It may be
 * removed from future releases.
 *
 * Class RWDiskPageHeap is a specializing type of buffered page heap. It
 ...
 * together. The list is then walked, confirming that it contains 100 nodes.
 * Each node is a single page. The "pointer" to the next node is actually
 * the handle for the next page.
 * \code
 * #include <iostream>
 * #include <iomanip>
 * #include <rw/diskpage.h>
...
public:
    ...
    /**
     * Constructs a new disk-based page heap using a temporary file. The number
     * of buffers, each the size of the page size, is \c 10. No more than this
     * many pages can be locked at any one time. The size of each page is
     * \c 512. To see whether a valid RWDiskPageHeap has been constructed, call
     * member function isValid().
     *
     * Windows users may be required to call \c AnsiToOEM on a Windows
     * character constant for the filename in this constructor, because this
     * constructor uses a C function call that may resolve to a DOS system
     * call. In that case, DOS might not be able to recognize Windows
     * character sets.
     *
     * \note
     * Determining the temporary filename may result in a system call that
     * requires administrator access.
     */
    RWDiskPageHeap();

    /**
     * Constructs a new disk-based page heap. The heap uses a file with
     * filename \a filename, otherwise it negotiates with the operating
     * system for a temporary filename. The number of buffers, each the size
     * of the page size, is \a nbufs. No more than this many pages can be
     * locked at any one time. The size of each page is given by \a pgsize.
     * To see whether a valid RWDiskPageHeap has been constructed, call
     * member function isValid().
     *
     * Windows users may be required to call \c AnsiToOEM on a Windows
     * character constant for the filename in this constructor, because this
     ...
     *
     * \note
     * Passing a null pointer for the parameter \a filename may result in a
     * system call that requires administrator access.
     */
     ...
     * operating system. All pages should have been deallocated before the
     ...
     * If there is no more space (for example, the disk is full), the
     * function returns 1.
     */
    virtual size_t        allocate();

    /**
     * Deallocates the page associated with handle \a h. It is not an error
