 ...
 *
 * $Id: //spro/rel2016.1/rw/tools/traits/RWTResultOf.h#1 $
 *
 ***************************************************************************/
 ...
 *
 * \brief
 * Determines the return type of \c T
 *
 ...
 * return type of \c F.
 * - If \c F is a member function pointer, \c type is the return type of
 * \c F.
 * - If the compiler supports the \c decltype keyword, \c type is the result
 * of <tt>decltype(f(a1, a2, ..., aN))</tt>.
 * - If the compiler supports the \c std::result_of type trait, \c type is
 * an alias for \c std::result_of<T>::type.
 * - If the compiler supports the \c std::tr1::result_of type trait, \c type
 * is an alias for \c std::tr1::result_of<T>::type.
 * - If \c F is a class type with a member type \c F::result_type, \c type
 * is an alias for \c F::result_type;
 * - If \c F is a class type without a member type \c F::result_type, or if
 * \c F::result_type is not a type:
 *     - If <tt>N = 0</tt> (no arguments) \c type is \c void.
 *     - If <tt>N > 0</tt> \c type is
 *     <tt>F::template result<F(A1, A2, ..., AN)>::%type</tt>
