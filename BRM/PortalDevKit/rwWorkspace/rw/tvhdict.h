 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvhdict.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Maintains a collection of keys, each with an associated item stored
 * according to a hash object.
 *
 * This class maintains a collection of keys, each with an associated item
 * of type \c T. These pairs are stored according to a hash object of type
 * \c H. Class \c A is an allocator of objects of class \c T. For more
 * information about using custom allocators, please see the <em>Essential
 ...
 *
 * RWTValHashMap will not accept a key that is equivalent to any key already
 * in the collection. (RWTValHashMultiMap may contain multiple keys that are
 * equivalent to each other.) Equality is based on an equality object and
 * not on the equality operator. If your type has an equality operator, then
 * you may want to use the templatized \c std::equal_to function object
 * provided by the C++ Standard Library; otherwise, you must define your own
 * equality object.
 *
 ...
 * \sa
 * \parblock
 * Class RWTValHashMultiMap offers the same interface to a collection that
 * accepts multiple keys that compare equal to each other.
 *
 * Class \ref rw_hashmap "rw_hashmap<K,T,H,EQ,A>" is the C++ Standard
...
public:
     ...
     *
     * \deprecated
     * As of SourcePro 12.5, use const #key_type& instead.
     */
     ...
     *
     * \deprecated
     * As of SourcePro 12.5, use const #mapped_type& instead.
     */
    ...
    /**
     * Returns a reference to the underlying collection that serves as the
     * implementation for self. This reference may be used freely, providing
     * access to the C++ Standard Library interface as well as
     * interoperability with other software components that make use of the
     * C++ Standard Library-compliant collections.
     */
    container_type& std()
    {
     ...
     * its associated item will have its destructor called.
     */
    void clear()
    {
        std().clear();
     ...
     *
     * \note
     * This method requires the type #mapped_type to have a default
     * constructor.
     */
    ...
    /**
     * Returns a reference to the minimum key in the collection. Type
     * #key_type must have well-defined less-than semantics
    ...
    /**
     * Returns a reference to the maximum key in the collection. Type
     * #key_type must have well-defined less-than semantics
     * (<tt>K::operator<(const K&)</tt> or equivalent).
     */
...
#  endif // !RW_NO_RVALUE_REFERENCES && !RW_BROKEN_RVALUE_OVERLOAD_RESOLUTION
     ...
     * the key is not in the dictionary, then it is added with an associated
     * item provided by the default constructor for type #mapped_type.
     *
     * \note
     * This method requires the type #mapped_type to have a default
     * constructor.
    ...
    /**
     * Changes the capacity of self by creating a new hashed map with a
     * capacity of \a sz. This method copies every element of self into the
     * new container and finally swaps the internal representation of the
     * new container with the internal representation of self.
     */
    void
    resize(size_type sz)
    {
     ...
     * the \c H and \c EQ objects. This method does not copy or destroy any
...
#  endif
     ...
     * \a last). The underlying representation has \a sz buckets, uses \a h
     * for its hashing function, and uses \a eq to determine equality
     * between elements.
     *
     * \c InputIterator is an input iterator type that points to elements
     * that are convertible to #value_type objects.
    ...
    /**
     * Creates an empty hashed map which uses the hash object \a h and has
     * an initial capacity of \a sz.
     */
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValHashMap::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTValHashMap.
 *
 * RWTValHashMapIterator provides an iterator interface to the C++ Standard
 * Library-based collection represented by RWTValHashMap. (An STL iterator
 * interface is provided through RWTValHashMap::iterator.)
 *
 * Iteration over an RWTValHashMap is pseudorandom and dependent on the
 * capacity of the underlying hash table and the hash function being used.
 * The only usable relationship between consecutive elements is that
 ...
 * being advanced with either a pre-increment or an operator()().
 *
...
public:
     ...
     * will now reference an undefined value and \c false is returned.
    ...
    /**
     * Returns the key portion of the association currently pointed to by
     * self.
     */
    K key() const
    {
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValHashMap::const_iterator to iterate over
 * the elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValHashMap.
 *
 * RWTValHashMapConstIterator provides a \c const iterator for
 * RWTValHashMap. The \c const iterator prohibits changes to the underlying
 * collection.
 *
 * Iteration over an RWTValHashMap is pseudorandom and dependent on the
 * capacity of the underlying hash table and the hash function being used.
...
public:
     ...
     * iteration, self referenced the last association in the multimap, self
     * now references an undefined value and \c false is returned.
