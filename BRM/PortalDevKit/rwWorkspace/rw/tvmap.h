 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tvmap.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Maintains a collection of keys, each with an associated item and ordered
 * according to a comparison object.
 *
 * This class maintains a collection of keys, each with an associated item
 * of type \c T. Class \c A is an allocator of objects of class \c T. For
 * more information about using custom allocators, please see the
 * <em>Essential Tools Module User's Guide</em>.
 *
 * Order is determined by the key according to a comparison object of type
 * \c C. \c C must induce a total ordering on elements of type \c K via a
 * public member:
 *
 * \code
 ...
 * \c std::less<T> from the C++ Standard Library header file
 * \c \<functional\> is an example of such a comparison object.
 *
 ...
 *
 * The value type must have \c operator==() and \c operator<() defined. This
 ...
 * \sa
 * \parblock
 * Class RWTValMultiMap offers the same interface to a collection that
 * accepts multiple keys that compare equal to each other. RWTValSet
 * maintains a collection of keys without the associated values.
 *
 * Class <b>std::map<K,T,C,A></b> is the C++ Standard Library collection
...
public:
     ...
     *
     * \deprecated
     * As of SourcePro 12.5, use #value_type instead.
     */
     ...
     * that serves as the implementation for self. This reference may be
     * used freely, providing access to the C++ Standard Library interface
    ...
    /**
     * Clears the collection by removing all items from self. Each key and
     * its associated item has its destructor called.
     */
    void clear()
    {
        std().clear();
     ...
     * <tt>((*\a fn)(a,\a d))</tt> is \c true, assigns \c a to \a r and
     * returns \c true. Otherwise, returns \c false and leaves the value of
     * \a r unchanged. Client data may be passed through parameter \a d.
     */
     ...
     * assigns \c j to \a r and returns \c true. Otherwise, returns \c false
     * and leaves the value of \a r unchanged.
     */
    ...
    /**
     * If there exists a key \c j in self that compares equal to \a key,
     * assigns the item associated with \c j to \a r and returns \c true.
     * Otherwise, returns \c false and leaves the value of \a r unchanged.
     */
    ...
    /**
     * If there exists a key \c j in self that compares equal to \a key,
     * assigns \c j to \a kr, assigns the item associated with \c j to
     * \a tr, and returns \c true. Otherwise, returns \c false and leaves
     * the values of \a kr and \a tr unchanged.
     */
    ...
    /**
     * Returns a reference to the minimum key in the collection. Order is
     * determined according to the comparison object \c C.
     */
    ...
    /**
     * Returns a reference to the maximum key in the collection. Order is
     * determined according to the comparison object \c C.
     */
    ...
    /**
     * Removes the first association with key \c j in self such that \c j
     * compares equal to \a key and returns \c true. Returns \c false if
     ...
     * The function returns \c true unless the collection already holds an
     * association with the equivalent key.
     */
    ...
    /**
     * Looks up \a key and returns a reference to its associated item. If
     * the key is not in the collection, it is added with an associated item
     * provided by the default constructor for type #mapped_type.
     */
    ...
    /**
     * Swaps the data held by self with the data held by \a other. This
     * method does not copy or destroy any of the items swapped; rather, it
...
#if !defined(RW_NO_RVALUE_REFERENCES)
    /**
     * Move constructor. The constructed map takes ownership of the data
     * owned by \a rws.
     *
     * \conditional
     * This method is only available on platforms with rvalue reference
 ...
 * otherwise \c false. Assumes that type #mapped_type has well-defined
 * less-than semantics.
 */
template <class K, class T, class C, class A>
bool operator<(const RWTValMap<K, T, C, A>& lhs, const RWTValMap<K, T, C, A>& rhs)
 ...
 * \relates RWTValMap
 *
 * Saves the collection \a coll to the RWFile \a file, or a reference to it
 ...
 * \relates RWTValMap
 *
 * Looks at the next object on the stream represented by RWFile \a file and
 * either creates a new collection off the heap and sets \a p to point to
 * it, or sets \a p to point to a previously read instance. If a collection
 * is created off the heap, the application is responsible for deleting it.
 */
template <class K, class T, class C, class A>
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValMap::iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides an iterator for RWTValMap.
 *
 * RWTValMapIterator provides an iterator interface to the C++ Standard
 * Library collection represented by RWTValMap. (An STL iterator interface
 * is provided through RWTValMap::iterator.)
 *
 * The order of iteration over an RWTValMap is dependent on the supplied
 * comparator object as applied to the key values of the stored
 * associations.
 *
 ...
 * returns a value equivalent to \c false. Continued increments return a
 * value equivalent to \c false until reset() is called.
 *
 * The value type must have \c operator==() and \c operator<() defined. This
 * requirement is imposed by the C++ Standard Library.
 *
...
public:
    /**
     * Creates an iterator for the map \a h. The iterator begins in an
     * undefined state and must be advanced before the first association is
     * accessible.
     */
     ...
     * iteration, self pointed to the last association in the map, self will
     * reference an undefined value and will return \c false. Otherwise, it
     * returns \c true.
     */
    ...
    /**
     * Advances self to the next element. If the iterator has advanced past
     * the last element in the collection, returns \c false, otherwise
     * returns \c true.
     */
    bool operator()()
    {
    ...
    K key() const
    {
        return (*(const_cast<RWTValMapIterator*>(this)->iter_)).first;
    }
    ...
    T value() const
    {
        return (*(const_cast<RWTValMapIterator*>(this)->iter_)).second;
    }
 ...
 *
 * \deprecated
 * As of SourcePro 12.5, use RWTValMap::const_iterator to iterate over the
 * elements in this collection.
 *
 * \brief
 * Deprecated. Provides a \c const iterator for RWTValMap.
 *
 * RWTValMapConstIterator provides a \c const iterator for RWTValMap. A
 * \c const iterator prohibits changes to the underlying collection.
 *
 * The order of iteration over an RWTValMap is dependent on the supplied
 * comparator object as applied to the key values of the stored
 * associations.
...
public:
     ...
     * iteration, self pointed to the last association in the map, self now
     * references an undefined value and returns \c false, otherwise returns
     * \c true.
     */
    bool operator++()    // Advance and test
    {
