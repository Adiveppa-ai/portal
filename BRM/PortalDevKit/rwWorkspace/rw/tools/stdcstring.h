 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tools/stdcstring.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \deprecated
 * As of SourcePro 4, use \ref RWTRegex "RWTRegex<char>" instead.
 *
 * \brief
 * Deprecated. This class is a typedef for
 * \ref RWTRegularExpression "RWTRegularExpression<char>".
 */
 ...
 *
 * \brief
 * Alternate implementation of RWCSubString when \c RW_COPY_ON_WRITE_STRING
 * is not defined.
 *
 * RWStandardCSubString should be directly used only with instances of class
 * RWStandardCString. RWCSubString should be used with instances of class
 * RWCString.
 *
 * \sa
 * RWCSubString for additional documentation.
 */
 ...
 *
 * \brief
 * Alternate implementation of RWCConstSubString when
 * \c RW_COPY_ON_WRITE_STRING is not defined.
 *
 * RWStandardCConstSubString should be directly used only with instances of
 * class RWStandardCString. RWCConstSubString should be used with instances
 * of class RWCString.
 *
 * \sa
 * RWCConstSubString for additional documentation.
 */
 ...
 *
 * \brief
 * Alternate implementation of RWCString when \c RW_COPY_ON_WRITE_STRING is
 * not defined.
 *
 * Unless the specific performance characteristics of RWStandardCString are
 * required, applications should use RWCString instead. See the
 * <em>Essential Tools Module User's Guide</em> for additional information.
 *
 * \sa
 * RWCString for additional documentation.
 */
 ...
 *
 * \brief
 * Allows some subsection of an RWCString to be addressed by defining a
 ...
 * example the 7<sup>th</sup> through the 11<sup>th</sup> elements,
 * inclusive, would have a starting position of 7 and an extent of 5. The
 * specification of a starting position and extent can also be done on your
 * behalf by such functions as RWCString::strip() or the overloaded function
 * call operator taking a regular expression as an argument. There are no
 * public constructors -- RWCSubString%s are constructed by various
 * functions of the RWCString class and then destroyed immediately.
 *
...
public:
     ...
     * the statement has no effect. Returns a reference to self.
     */
     ...
     * \c RWBOUNDS_CHECK before including \c <rw/cstring.h>.
     *
     ...
     *
     * \note
     * Unlike RWCString::data(), the <tt>const char*</tt> returned by this
     * function is not null-terminated at its extent.
     */
    const char* startData() const;

    /**
     * Changes all upper-case letters in self to lower-case. Uses the C++
     * Standard Library function \b std::tolower().
     */
    void toLower();

    /**
     * Changes all lower-case letters in self to upper-case. Uses the C++
     * Standard Library function \b std::toupper().
     */
    void toUpper();
 ...
 * <em>starting position</em> and an \e extent
 *
 * The class RWCConstSubString allows some subsection of an RWCString to be
 * addressed by defining a <em>starting position</em> and an \e extent. For
 * example, the 7<sup>th</sup> through the 11<sup>th</sup> elements,
 * inclusive, would have a starting position of 7 and an extent of 5. The
 * specification of a starting position and extent is also done on your
 * behalf by such functions as RWCString::strip(). There are no public
 * constructors other than one that creates an RWCConstSubString from an
 * existing RWCSubString. In general, RWCConstSubString%s are constructed by
 * various functions of the RWCString class and then destroyed immediately.
 *
 * A <em>zero length</em> substring is one with a defined starting position
 * and an extent of zero. It can be thought of as starting just before the
 * indicated character, but not including it. It can be used as an lvalue. A
...
public:
     ...
     *
     * \note
     * The resulting substring instance should not be used if the RWCString
    ...
    /**
     * \copydoc RWCSubString::operator[](char)
     */
    char operator[](char i) const;
     ...
     *
     * \note
     * Unlike RWCString::data(), the returned <tt>const char*</tt> returned
     * by this function is not null-terminated at its extent.
     */
    const char* startData() const;
 ...
 * the string. Use function RWCString::length() to get the number of bytes
 * in a string, and function RWCString::mbLength() to get the number of
 ...
 * use a SBCS, then RWCString::length() is preferred.
 ...
 * and program defensively. In any case, class RWCString can handle embedded
 * nulls.
 *
 * Parameters of type <tt>const char*</tt> must not be passed a value of
 * zero. This is detected in the debug version of the library.
 *
 * A separate class RWCSubString supports substring extraction and
 * modification operations. RWCConstSubString supports substring extractions
 * on \c const RWCString instances.
 *
 * \synopsis
 * #include <rw/cstring.h>
 * RWCString a;
...
public:
     ...
     * on the enumeration RWCString::scopeType.
     *
     * Specifies whether regular expression functions replace the first
     * \ref RWCString::one "one" substring matched by the regular expression
     * or replace \ref RWCString::all "all" substrings matched by the
     * regular expression.
     */
    ...
    /**
     * Specifies whether characters are stripped from the beginning of the
     * string, the end, or both.
     */
    enum stripType {
...
public:
    ...
    /**
     * Converts from a substring. The created string copies the substring
    ...
    /**
     * Converts from a const substring. The created string copies the
     ...
     * string copies the data pointed to by \a str. Exactly \a len bytes are
     * copied, <em>including any embedded nulls</em>. Hence, the buffer
     * pointed to by \a str must be at least \a len bytes long.
     */
     ...
     * between 0 and the length of the string less one. Bounds checking is
     * performed if the preprocessor macro \c RWBOUNDS_CHECK has been
     * defined before including \c <rw/cstring.h>.
     *
     * \exception RWBoundsErr
     * if RWBOUNDS_CHECK is defined and the index is out of range.
     */
    ...
    /**
     * Returns the \a i <sup>th</sup> byte in self. The index \a i must be
     * between 0 and the length of the string less one. \a i is converted to
     * a \c size_t and bounds checking is performed.
     *
     * \exception RWBoundsErr
     * if the index is out of range.
     */
     ...
     *
     * \exception RWBoundsErr
     * if the library was built using the \c RW_DEBUG flag, and \a start or
    ...
    /**
     * Appends a copy of the null-terminated character string pointed to by
    ...
    /**
     * Appends a copy of the character string \a str to self. Exactly \a len
     * bytes are copied, <em>including any embedded nulls</em>. Hence, the
     * buffer pointed to by \a str must be at least \a len bytes long.
     * Returns a reference to self.
     */
     ...
     * at least \a len bytes long. Returns a reference to self.
     */
     ...
     * characters to consider, \a nChars, this method calculates the number
     * of bytes required to store the sequence. If \a nChars is #RW_NPOS,
     * then the method counts characters up to, but not including, the first
     * \c NULL character in the sequence. The method returns the number of
     * bytes required to represent the string. If an error occurs during the
     * operation, #RW_NPOS is returned.
     */
    static size_t byteCount(const char* str, size_t nChars = RW_NPOS);

    /**
     * Returns the current capacity of self. This is the number of bytes the
     ...
     * capacity is never reduced below the current number of bytes. Returns
     * the actual capacity.
     */
     ...
     * \b std::strcoll() on self and the argument \a str. This supports
     ...
     *
     * \deprecated
     * As of SourcePro 12, use operator=(const RWStandardCString&) and
    ...
    /**
     * Provides access to the RWCString instances's data as a
     * null-terminated string. This datum is owned by the RWCString and may
     * not be deleted or changed. If the RWCString object itself changes or
     * goes out of scope, the pointer value previously returned becomes
     * invalid. While the string is null-terminated, its \e length is still
     * given by the member function length(). That is, it may contain
     * embedded nulls.
     */
    const char* data() const;
    ...
    /**
     * \deprecated
     * As of SourcePro 12, use RWCString::firstOf(char, size_t) const
     * instead.
     */
    ...
    /**
     * \deprecated
     * As of SourcePro 12, use RWCString::firstOf(const char*, size_t) const
     * instead.
     */
    RW_DEPRECATE_FUNC("Use RWCStandardCString::firstOf(...) instead")
    ...
    /**
     * \deprecated
     * As of SourcePro 12, use
     * RWCString::firstOf(const char*, size_t, size_t) const instead.
     */
    RW_DEPRECATE_FUNC("Use RWCStandardCString::firstOf(...) instead")
    ...
    /**
     * Returns the index of the first occurrence of a character in self that
     * is not \a c, starting at position \a pos. Returns #RW_NPOS if there
     * is no match.
     */
    ...
    /**
     * Returns the index of the first occurrence in self, starting at
     * position \a pos, of any character not in \a str. Returns #RW_NPOS if
     * there is no match.
     */
     ...
     * self, starting at position \a pos. Returns #RW_NPOS if there is no
     * match.
     */
    ...
    /**
     * Returns the index of the first occurrence in self, starting at
     * position \a pos, of any character in \a str. Returns #RW_NPOS if
     * there is no match.
     */
    ...
    /**
     * Pattern matching. Starting with index \a start, searches for the
     * first occurrence of the first \a patlen bytes from \a pat in self and
     ...
     * If \a len is greater than the length of \a str, the entire string is
     * inserted. Returns a reference to self.
     */
    ...
    /**
     * \deprecated
     * As of SourcePro 12, use RWCString::lastOf(char, size_t) const
     * instead.
     */
    ...
    /**
     * \deprecated
     * As of SourcePro 12, use RWCString::lastOf(const char*, size_t) const
     * instead.
     */
    RW_DEPRECATE_FUNC("Use RWStandardCString::lastOf(...) instead")
     ...
     * \deprecated
     * As of SourcePro 12, use
     * RWCString::lastOf(const char*, size_t, size_t) const instead.
     */
    RW_DEPRECATE_FUNC("Use RWStandardCString::lastOf(...) instead")
    ...
    /**
     * Returns the index of the last occurrence of a character in self that
     * is not \a c, ending at position \a pos. Returns #RW_NPOS if there is
     * no match.
     */
     ...
     * \a pos, of any character not in the null-terminated string \a str.
     * Returns #RW_NPOS if there is no match.
     *
     * \note
     * This function is incompatible with \a str strings with embedded
     * nulls.
    ...
    /**
     * Returns the index of the last occurrence in self, ending at position
     * \a pos, of any character not in the last \a len bytes of \a str.
     * Returns #RW_NPOS if there is no match.
     */
     ...
     * self, ending at position \a pos. Returns #RW_NPOS if there is no
     * match.
     */
    ...
    /**
     * Returns the index of the last occurrence in self, ending at position
     * \a pos, of any character in the null-terminated string \a str.
     * Returns #RW_NPOS if there is no match.
     *
     * \note
     * This function is incompatible with \a str strings with embedded
    ...
    /**
     * Returns the index of the last occurrence in self, ending at position
     * \a pos, of any character in the last \a len bytes of \a str. Returns
    ...
    /**
     * Returns the number of multibyte characters in self, according to the
     * C++ Standard Library function \b std::mblen(). Returns #RW_NPOS if a
     * bad character is encountered. If self does not contain any multibyte
     * characters, then mbLength() == length().
     */
    size_t mbLength() const;

    /**
     * Given a multibyte character sequence, \a str, and a number of bytes
     * to consider, \a nBytes, this method calculates the number of
     * multibyte characters in the sequence. If \a nBytes is #RW_NPOS, then
     * the method counts the characters up to, but not including, the first
     * occurrence of a \c NULL character. The method returns the length, in
     * characters, of the input multibyte sequence if successful. If an
     * error occurs in the conversion, then #RW_NPOS is returned.
     */
     ...
     * \a len bytes are copied, <em>including any embedded nulls</em>.
     * Hence, the buffer pointed to by \a str must be at least \a len bytes
    ...
    /**
     * Reads bytes from the input stream \a strm, replacing the previous
     * contents of self, until a newline (or \c EOF) is encountered. The
     * newline is removed from the input stream but is not stored. Null
     * characters are treated the same as other characters. If the
     * \a skipWhite argument is \c true, then leading white space is skipped
     * (using the iostream library manipulator \c ws).
     */
     ...
     * is encountered. The delimiter is removed from the input stream but is
     * not stored.
     */
    std::istream& readToDelim(std::istream& strm, char delim = '\n');

    /**
     * White space is skipped before reading from \a strm. Bytes are then
     * read from the input stream \a strm, replacing previous contents of
     * self, until trailing white space or \c EOF is encountered. The white
     * space is left on the input stream. Null characters are treated the
     * same as other characters. White space is identified by the C++
     * Standard Library function \b std::isspace().
     *
     * \note
     ...
     * starting at index \a pos, which must be no greater than length().
     * Returns a reference to self.
     */
    ...
    /**
     * Replaces the substring beginning at \a pos and of length \a len with
     * the first \a strLen bytes of string \a str. If the sum of \a pos and
     * \a len is greater than the length of self, the substring replaced is
     * from \a pos to end of string. If \a strLen is greater than the length
     * of \a str, all of \a str is used.
     *
     * Returns a reference to self.
     *
     * \exception RWBoundsErr
    ...
    /**
     * Replaces the substring beginning at \a pos and of length \a len with
     * \a strLen bytes from \a str. If the sum of \a pos and \a len is
     * greater than the length of self, the substring replaced is from
     * \a pos to end of string.
     *
     * Returns a reference to self.
    ...
    /**
     * Changes the length of self to \a len bytes, adding blanks (i.e.,
     * <tt>' '</tt>) or truncating as necessary.
     */
    void resize(size_t len);

    /**
     * \internal
     * Restores an instance of RWCString from \a fil into which an instance
    ...
    /**
     * \internal
     * Restores an instance of RWCString from \a vis into which an instance
    ...
    /**
     * \internal
     * Saves an instance of RWCString to \a fil. The instance can be
    ...
    /**
     * \internal
     * Saves an instance of RWCString to \a vos. The instance can be
     ...
     * C Standard Library facilities declared in \c <ctype.h>.
     *
     * \note
...
#if defined(DOXYGEN)
    // A bug in Doxygen causes protected inheritance to not be documented
    // if protected functions are not documented. Workaround it by making
...
/**
 * \relates RWCString
 * Returns the result of applying \b std::strxfrm() to \a str to allow
 * quicker collation than RWCString::collate().
 *
 * \note
...
/**
 * \relates RWCString
 * Restores a string into \a str from an RWFile, replacing the previous
 * contents of \a str. If the file produces an error on extraction, the
 * operator returns the original string contents unmodified. Be sure to
 * check the file to determine if an extraction error occurred.
 */
...
/**
 * \relates RWCString
 * Restores a string into \a str from a virtual stream, replacing the
 * previous contents of \a str. If the virtual stream produces an error on
 * extraction, the operator returns the original string contents unmodified.
 * Be sure to check the virtual stream to determine if an extraction error
 * occurred.
 */
...
/**
 * \relates RWCString
 * Outputs an RWCString on \b std::ostream \a os.
 */
 ...
 * Outputs an RWCSubString on \b std::ostream \a os.
 */
RW_TOOLS_SYMBOLIC std::ostream&
 ...
 * Outputs an RWCConstSubString on \b std::ostream \a os.
 */
RW_TOOLS_SYMBOLIC std::ostream&
 ...
 *
 * \relates RWCString
 * Outputs an RWCString on \b std::wostream \a os.
 */
 ...
 *
 * \relates RWCSubString
 * Outputs an RWCSubString on \b std::wostream \a os.
 */
RW_TOOLS_SYMBOLIC std::wostream&
 ...
 *
 * \relates RWCConstSubString
 * Outputs an RWCConstSubString on \b std::wostream \a os.
 */
RW_TOOLS_SYMBOLIC std::wostream&
 ...
 * returns \c false. Use member RWCString::collate() or
 ...
 * \relates RWCString
 *
 * Returns \c true if \a lhs has a length of one and the first character
 * \a lhs is identical to the char \a rhs.
 */
bool
 ...
 * \relates RWCString
 *
 * Returns \c true if \a rhs has a length of one and the first character
 * \a rhs is identical to the char \a lhs.
 */
bool
...
/**
 * \relates RWCString
 * Returns \c true if \a lhs is empty or the first character in \a lhs is
 * less than \a rhs. Otherwise returns false.
 */
bool
operator< (const RWStandardCString& lhs, char rhs);
...
/**
 * \relates RWCString
 * Returns true if the length of \a rhs is at least one and the first
 * character of \a rhs is greater than \a lhs.
 */
bool
operator< (char lhs, const RWStandardCString& rhs);
 ...
 *
 * \deprecated
 * As of SourcePro 12, use RWStandardCString::hash(const RWStandardCString&)
 * instead.
 */
