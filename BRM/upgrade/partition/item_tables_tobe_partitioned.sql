--
-- @(#)$Id: item_tables_tobe_partitioned.sql /cgbubrm_7.5.0.portalbase/1 2015/11/27 06:09:24 nishahan Exp $
--
...
--
--
-- sql script to find the list of item tables to be partitioned.
--
...
DECLARE
	...
	AS table_name
        ...
        AND (do.name = '/item' OR do.name LIKE '/item/%')
      	...
      	EXCEPTION
      		WHEN no_data_found THEN
            		INSERT INTO tables_tobe_partitioned_t (table_name, base_name, object_name, partition_exchanged)
                  	VALUES (UPPER(partition_tables_rec.table_name), 'ITEM_T', '/item', 0);
...
EXCEPTION
	WHEN OTHERS THEN
               	pin_upg_common.log_info (C_STEP_NAME, NULL, NULL, 'E', 
			'Error finding the item table names to be partitioned', sqlerrm);
END;
/
