   ...
   for $i ( 0 .. $#myValidationFileContentLines) {
       for $j ( 0 .. $#{$myValidationFileContentLines[$i]} ) {
           ...
           for $i ( 0 .. $#mySTATUSList)
           {
			...
			if($myStatusList[0] eq "FAIL.MAJOR") {
				$myFailMajorList .= "\n".$myValidationLine; 
			} elsif($myStatusList[0] eq "FAIL.CRITICAL") { 
				$myFailCriticalList .="\n".$myValidationLine;
			} elsif($myStatusList[0] eq "FAIL.MINOR") { 
				$myFailMinorList .="\n".$myValidationLine;
			} elsif($myStatusList[0] eq "FAIL.WARNING") { 
				$myFailWarningList .= "\n".$myValidationLine;
			} elsif($myStatusList[0] eq "PASS.NORMAL") { 
				$myPassNormalList .= "\n".$myValidationLine;
			} elsif($myStatusList[0] eq "PASS.WARNING") { 
				$myPassWarningList .= "\n".$myValidationLine;
			}
               }
            }
...
sub alertResultsOnActionList()
{
   ...
   for $i ( 0 .. $#mySTATUSList)
   {
		...
		if($myStatusList[0] eq "PASS.NORMAL") {
			if ( $myPassNormalList ne "") {
