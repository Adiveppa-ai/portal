 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/btree.h#1 $
 *
 **********************************************************************
 *
 ...
 * and for which duplicates are not allowed.
 *
 * Class RWBTree represents a group of ordered elements, not accessible by
 * an external key. Duplicates are not allowed. An object stored by class
 * RWBTree must inherit abstract base class RWCollectable. The elements are
 * ordered internally according to the value returned by virtual function
 * RWCollectable::compareTo().
 *
 * This class has certain advantages over class RWBinaryTree. First, the
 * B-tree is automatically \e balanced. (With class RWBinaryTree, you must
 * call member function RWBinaryTree::balance() explicitly to balance the
 * tree.) Nodes are never allowed to have less than a certain number of
 * items (called the \e order). The default order is 50, but may be changed
 * by resetting the value of the static constant \c order in the header file
 * \c <rw/btree.h> and recompiling. Larger values result in shallower trees,
 * but less efficient use of memory.
 *
 * Because many keys are held in a single node, class RWBTree also tends to
...
public:
    ...
    /**
     * Move constructor. The constructed RWBTree takes ownership of the data
     * owned by \a btr.
     *
     * \conditional
     ...
     * traversed while descending from the root node to an external (leaf)
     * node.
     */
     ...
     * anything to the items that could change the ordering of the
     * collection.
     */
    ...
    /**
     * Inserts the item \a c into the collection and returns it. The item
     * \a c is inserted according to the value returned by compareTo(). If
     * an item is already in the collection that isEqual() to \a c, then the
     * old item is returned and the new item is not inserted. Returns #rwnil
     * if the insertion was unsuccessful.
     *
     * \note
     * If self is a collection of key-value pairs (e.g. RWBTreeDictionary),
    ...
    /**
     * Returns the number of items that compare equal to \a target. Since
     * duplicates are not allowed, this function can only return 0 or 1.
     */
    virtual size_t occurrencesOf(const RWCollectable* target) const;

    /**
     * Removes and returns the first item that compares equal to the object
     * pointed to by \a target. Returns #rwnil if no item was found.
     */
