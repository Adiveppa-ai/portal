 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/rwdate.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * Class RWDate represents a date, stored as a Julian day number. The member
 * function isValid() can be used to determine whether an RWDate is a valid
 * date. For example, isValid() would return \c false for the date 29
 * February 1991 because 1991 is not a leap year. See the chapter on date
 * and time classes in the <em>Essential Tools Module User's Guide</em>.
 *
 * \note
 * RWDate instances can be converted to and from the C Standard Library type
 * <tt>struct tm</tt> defined in \c <time.h>.
 *
 * We recommend creating programs that use four-digit year specifiers. For
 * more information, see the section on RWDate in the <em>Essential Tools
 * Module User's Guide</em>.
 *
 * Note that because the default constructor for this class creates an
 * instance holding the current date, constructing a large array of RWDate
 * may be slow.
 *
 * \code
 ...
 * \endcode
 *
 * When using the Essential Tools Module template collections, consider the
 * following:
 *
 * \code
 ...
 * above declaration results in only one call to the default constructor,
 * followed by 5000 invocations of the copy constructor. In the case of
 * RWDate, the copy constructor amounts to an assignment of one \c long to
...
public:
    ...
    /**
     * Constructs a date from the Julian Day number \a jd. Note that it is
     * possible to construct a valid RWDate that represents a day previous
     * to the beginning of the Gregorian calendar for some locality. Rogue
     * Wave doesn't know the specifics for your locality, so will not
     ...
     * The member function isValid() can be used to test whether the results
     * are a valid date.
     */
     ...
     * year. The locale argument is used to convert the month name. Days
     * should be 1-31, months may be specified as (for example): "January",
     * "JAN", or "Jan", and the year may be specified as (for example) 1990,
     * or 90. The member function isValid() can be used to test whether the
    ...
    /**
     * Constructs an RWDate with the given day of the month, month of the
     * year, and year. Days should be 1-31, months should be 1-12, and the
     * year may be specified as (for example) 1990, or 90. The member
     * function isValid() can be used to test whether the results are a
     * valid date.
     */
    RWDate(unsigned day, unsigned month, unsigned year);

    /**
     * Constructs a date whereby the locale argument reads and converts a
     * full line to a date. Because RWLocale cannot rigorously check date
     * input, the member function isValid() must be used to test whether the
     * results are a valid date.
     */
     ...
     * rigorously check date input, the member function isValid() must be
     * used to test whether the results are a valid date.
     */
     ...
     * argument members \c tm_year, \c tm_mon, and \c tm_mday. Note that the
     * numbering of months and years used in <tt>struct tm</tt> differs from
     * that used for RWDate operations. <tt>struct tm</tt> is declared in
     * the C Standard Library include file \c \<time.h\>.
     */
    RWDate(const struct tm* tmbuf);  // from members: tm_year, tm_mon, tm_mday

    /**
     * Constructs an RWDateTime using the RWTimeTuple \a tt. Only the year,
     * month and day of month values are referenced. All other members of
     * \a tt are ignored.
     *
     * If the \a tt represents a date prior to -4713-11-25, an invalid
     * RWDate instance is constructed.
     *
     * In 32-bit builds, if the \a tt represents a date after
     * 11754509-07-22, and invalid RWDate instance is constructed.
     */
    explicit RWDate(const RWTimeTuple& tt);

    /**
     * Constructs an RWDateTime using the RWTimeTupleOffset \a tt. Only the
     * year, month and day of month values are referenced. All other members
     * of \a tt are ignored.
     *
     * If the \a tt represents a date prior to -4713-11-25, an invalid
     * RWDate instance is constructed.
     ...
     * 11754509-07-22, and invalid RWDate instance is constructed.
     */
    explicit RWDate(const RWTimeTupleOffset& tt);
     ...
     * argument. Formats are as defined in the C++ Standard Library function
     * \b std::strftime(), explained in the description for RWLocale.
     */
     ...
     * with the time members set to 0 and \c tm_isdst set to -1. Note that
     * the encoding for months and days of the week used in
     * <tt>struct tm</tt> differs from that used elsewhere in RWDate. If the
     * date is invalid, all fields are set to -1.
     */
    void                  extract(struct tm* tmbuf) const;

    /**
     * Returns the day of the year (1-366) corresponding to the first day of
     * the month \a month (1-12) of the year for this RWDate.
     */
    unsigned              firstDayOfMonth(unsigned month) const;

    /**
     * Returns the day of the year (1-366) corresponding to the first day of
     * the month and year for this RWDate.
     */
    ...
    /**
     * Returns \c true if the year of this RWDate is a leap year, otherwise
     * \c false.
     */
    bool             leap() const
    {
    ...
    /**
     * Returns the date of the previous \a dayName (for example, the date of
     ...
     * the optional RWLocale argument. The function defaults to using the
    ...
    /**
     * Prefix increment operator. Adds one day to self, returning the
     * result.
     */
    RWDate&               operator++()
    {
    ...
    /**
     * Prefix decrement operator. Subtracts one day from self, returning the
     * result.
     */
    RWDate&               operator--()
    {
    ...
    /**
     * Postfix increment operator. Adds one day to self, returning the
     * initial value.
     */
    RWDate                operator++(int)
    {
    ...
    /**
     * Reads \a t from \b std::istream \a s. One full line is read, and the
     * string contained is converted according to the locale imbued in the
     * stream (see class RWLocale), or by RWLocale::global() if none. The
     * function RWDate::isValid() must be used to test whether the results
     * are a valid date.
     */
    ...
    /**
     * If \a d1 > \a d2, returns the number of days between \a d1 and \a d2.
    ...
    /**
     * Returns \c true if the date \a d1 is before \a d2, otherwise
     * \c false.
     */
    ...
    /**
     * Returns \c true if the date \a d1 is the same as \a d2, otherwise
     * \c false.
     */
    friend bool        operator==(const RWDate& d1, const RWDate& d2)
    {
    ...
    /**
     * Returns \c true if the date \a d1 is before or the same as \a d2,
     * otherwise \c false.
     */
    friend bool        operator<=(const RWDate& d1, const RWDate& d2)
    {
    ...
    /**
     * Returns \c true if the date \a d1 is after or the same as \a d2,
     * otherwise \c false.
     */
    friend bool        operator>=(const RWDate& d1, const RWDate& d2)
    {
    ...
    /**
     * Returns \c true if the date \a d1 is not the same as \a d2, otherwise
     * \c false.
     */
    friend bool        operator!=(const RWDate& d1, const RWDate& d2)
    {
     ...
     * \a dayName ( \e Monday = 1, ..., \e Sunday = 7). Names are
     * interpreted by the optional RWLocale argument. Returns 0 if no match
     * is found.
     */
    ...
    /**
     * Returns the Julian day corresponding to the given month (1-12), day
    ...
    /**
     * Returns the name of the day of the week \a day ( \e Monday = 1, ...,
     * \e Sunday = 7), formatted for the given locale. The function defaults
     * to the RWLocale global locale.
     */
 ...
 * \relates RWDate
 *
 * Restores the date into \a date from the virtual stream \a str, replacing
 * the previous contents of \a date.
 */
 ...
 * \relates RWDate
 *
 * Restores the date into \a date from the RWFile \a file, replacing the
 * previous contents of \a date.
 */
