 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/tpvector.h#1 $
 *
 **********************************************************************
 *
 ...
 * objects of type \c T. It is most useful when you know precisely how many
 * pointers must be held in the collection. If the intention is to insert an
 * unknown number of objects into a collection, then class
 * RWTPtrOrderedVector may be a better choice. Class \c T can be of any
 * type.
 *
 * \synopsis
...
public:
     ...
     * elements are undefined. Hence, they can (and probably will) be
     * garbage.
     */
    ...
    /**
     * Constructs a vector of length \a n, with each element pointing to the
     * item \a ival.
     */
     ...
     * The two vectors will then have the same length, and pointers held by
     * the two vectors will point to the same items.
     */
    ...
    /**
     * Sets self to a shallow copy of \a rhs. The two vectors will then have
     * the same length, and pointers held by the two vectors will point to
     * the same items.
     */
    ...
    /**
     * Returns the \a i <sup>th</sup> value in the vector. The index \a i
     * must be between zero and the length of the vector, less one. No
     ...
     *
     * \exception RWBoundsErr
     * Thrown if the index \a i is not between zero and the length of the
     * vector, less one.
     */
    reference operator[](size_type i)
    {
