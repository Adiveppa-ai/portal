 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/iterator.h#1 $
 *
 **********************************************************************
 *
 ...
 * for positioning and resetting the iterator. They are all <em>pure
 * virtual</em> functions, meaning that deriving classes must supply a
 * definition. The descriptions below are intended to be generic -- all
 * inheriting iterators generally follow the described pattern.
 *
 * \synopsis
...
public:
     ...
     * was found. For most collections, an item "matches" the target if
     * either RWCollectable::isEqual() or RWCollectable::compareTo()
     ...
     * collection" (i.e., RWIdentitySet and RWIdentityDictionary), it looks
     * for an item with the same address (i.e., "is identical to").
     */
    ...
    /**
     * Advances the iterator and returns the next item, or #rwnil if the end
 ...
 *
 * \brief
 * Contains virtual functions for positioning and resetting the \c const
 * iterator
 *
 * Class RWConstIterator is an abstract base class for \c const iterators
 * used by the Smalltalk-like collection classes. The \c const iterators do
 * not change any information in the data structure upon which they iterate.
 *
 * The class contains virtual functions for positioning and resetting the
 * iterator. They are all <em>pure virtual</em> functions, meaning that
 * deriving classes must supply a definition. The descriptions below are
 * intended to be generic -- all inheriting iterators generally follow the
 * described pattern.
 *
 * \synopsis
 * #include <rw/iterator.h>
