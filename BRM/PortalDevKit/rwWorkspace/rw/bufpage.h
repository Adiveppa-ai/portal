 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/bufpage.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Deprecated. Abstract base class representing a virtual page heap accessed
 * through a buffer.
 *
 * \deprecated
 * As of SourcePro 1.
 *
 * RWBufferedPageHeap is deprecated and is no longer supported. It may be
 ...
 * virtual functions swapIn(size_t,void*) and swapOut(size_t,void*).
 *
 * The specializing class should also supply appropriate definitions for the
 * public functions allocate() and deallocate(size_t).
 *
...
protected:
     ...
     * definition for this pure virtual function. The Function swapIn()
     * should copy the page with handle \a h into the buffer pointed to by
     * \a buf.
     */
     ...
     * should copy the page with handle \a h from the buffer pointed to by
     * \a buf to the swapping medium.
     */
...
public:
    ...
    /**
     * Constructs a buffered page heap with page size \a pgsize. The number
     * of buffers (each of size \a pgsize) that are allocated on the heap is
     * specified by \a nbufs. If there is insufficient memory to satisfy the
     * request, the state of the resultant object as returned by member
     * function isValid() is \c false, otherwise \c true.
     */
    ...
    /**
     * Returns \c true if self is in a valid state. A possible reason why
     * the object might not be valid is insufficient memory to allocate the
     * internal buffers.
     */
    bool             isValid()
    {
     ...
     * return zero if it cannot satisfy the request. The size of the page is
     ...
     * definition does is release any buffers associated with the handle
     * \a h. Just as the actual page allocation is done by the specializing
     * class through virtual function allocate(), so must the actual
     * deallocation be done by overriding deallocate().
     */
