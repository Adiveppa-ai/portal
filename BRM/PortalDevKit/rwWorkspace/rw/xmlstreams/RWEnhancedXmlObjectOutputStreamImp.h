#ifndef rw_xmlstreams_RWEnhancedXmlObjectOutputStreamImp_h_
#define rw_xmlstreams_RWEnhancedXmlObjectOutputStreamImp_h_

/*********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/xmlstreams/RWEnhancedXmlObjectOutputStreamImp.h#1 $
 *
...
#include <rw/xmlstreams/RWTXmlStreamPrimitive.h>
...
//--The class 'RWEnhancedXmlObjectOutputStreamImp' writes objects as XML
 ...
 *
 * The class RWEnhancedXmlObjectOutputStreamImp writes objects as XML into
 * the supplied RWDataOutputStream. Implements the body idiom from the
 * handle/body pattern.
 *
 * \example
 * \code
 ...
 * \endexample
 */
class RW_XMLSTREAMS_GLOBAL RWEnhancedXmlObjectOutputStreamImp
    : public RWObjectOutputStreamImp,
...
protected:
    ...
    /**
     * Constructs an RWEnhancedXmlObjectOutputStreamImp instance that takes
     * an ostream to use as the destination.
     *
     * The value \a rootElementType indicates the root element of the
     * generated document. This value should be either #objectTag,
     * #nestedObjectTag, #sequenceTag, #primitiveTag, or #mapTag, depending
     * on what sort of object is streamed into the document at the top
     * level.
     *
    ...
    RWEnhancedXmlObjectOutputStreamImp(std::ostream& ostr, const RWCString& rootElementType, bool escape)
        ...
        curElem_(&rootElem_)
    {
        rootElem_.addAttribute(RWXmlAttribute(RWXmlName(xsdNamespace, RWXmlNamespace::getXmlns()), xsdNamespaceValue));
        rootElem_.addAttribute(RWXmlAttribute(RWXmlName(xsiNamespace, RWXmlNamespace::getXmlns()), xsiNamespaceValue));
        rootElem_.addAttribute(RWXmlAttribute(RWXmlName(rwNamespace, RWXmlNamespace::getXmlns()), rwNamespaceValue));
    }
    ...
    //--into the document at the top level.
    /**
     * \copydoc RWEnhancedXmlObjectOutputStreamImp(std::ostream&, const RWCString&, bool)
     *
    ...
    /**
     * Constructs an RWEnhancedXmlObjectOutputStreamImp instance that takes
     * an RWDataOutputStream to use as the destination.
     *
     * The value \a rootElementType indicates the root element of the
     * generated document. This value should be either #objectTag,
     * #nestedObjectTag, #sequenceTag, #primitiveTag, or #mapTag, depending
    ...
    /**
     * Destructor. Delete the \c storeTable even if an exception is thrown.
     */
    ~RWEnhancedXmlObjectOutputStreamImp()
    {
        if (getContextNestingLevel() > 0) {
            RWEnhancedXmlObjectOutputStreamImp::closeOuterContext(false);
        }

    }

public:
    //--Constructs an 'RWEnhancedXmlObjectOutputStreamImp' instance that uses 'sink'
    ...
    /**
     * Constructs an RWEnhancedXmlObjectOutputStreamImp instance that passes
     * the serialized object data to the output stream \a sink, and returns
     * a handle to it. Use this method when the data output stream is a
     * standard library ostream. Throws no exceptions.
     *
     * The parameter \a sink is the ostream that is used as the sink of
     * bytes.
     *
     ...
     *
     * The parameter \a escape sets whether to escape XML reserved
     * characters in the data being serialized.
     */
    ...
    //--Parameter 'formatter' will be returned with a reference to the formatting object
    /**
     * \copydoc RWEnhancedXmlObjectOutputStreamImp::make(std::ostream&, const RWCString&, bool)
     *
     ...
     * RWEnhancedXmlObjectOutputStreamImp::make(outfile,formatter);
     * \endcode
     *
    ...
    /**
     * Constructs an RWEnhancedXmlObjectOutputStreamImp instance that passes
     * the serialized object data to the RWDataOutputStream \a sink, and
     * returns a handle to it. Throws no exceptions.
     *
     * The parameter \a sink is the ostream that is used as the sink of
     * bytes.
     *
    ...
    /**
     * Puts a value into the stream, where the type of the value is defined
     * by the first parameter.
     */
    virtual void          putString(const RWCString& value);

    /**
     * \copydoc RWEnhancedXmlObjectOutputStreamImp::putString(const RWCString&)
     */
...
protected:
    ...
    RWCString getPrimitiveName(const char* typeName);
...
#endif // rw_xmlstreams_RWEnhancedXmlObjectOutputStreamImp_h_
