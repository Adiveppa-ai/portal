 ...
 **********************************************************************
 *
 * $Id: //spro/rel2016.1/rw/filemgr.h#1 $
 *
 **********************************************************************
 *
 ...
 *
 * \brief
 * Allocates and deallocates storage in a disk file, much like a "freestore"
 * manager
 *
 ...
 * list of free space within the file.
 *
 * \note
 ...
 *
 * If a file is managed by an RWFileManager, then reading or writing to
 * unallocated space in the file has undefined results. In particular,
 * overwriting the end of allocated space is a common problem which usually
 * results in corrupted data. One way to encounter this problem is to use
 * \ref RWCollection::binaryStoreSize() "binaryStoreSize()" to discover the
 * amount of space needed to store an RWCollection. For most purposes, the
 * storage size of an RWCollection is found using the RWCollectable method
 * \ref RWCollection::recursiveStoreSize() "recursiveStoreSize()".
 *
 * \synopsis
...
public:
     ...
     * using mode \a mode. The mode is as given by the C Standard Library
     ...
     * <tt>"wb+"</tt>). If the file exists and is not empty, then the
     * constructor assumes it contains an existing file manager; other
     * contents cause the function to throw an exception of type
     * RWExternalErr. If no file exists or if an existing file is empty,
     * then the constructor attempts to create the file (if necessary) and
     * initialize it with a new file manager. The resultant object should be
     * checked for validity using function isValid(). RWFileErr is a
     * possible exception that could be thrown.
     */
    ...
    /**
     * Allocates \a s bytes of storage in the file. Returns the offset to
     * the start of the storage location. The very first allocation for the
     * file is considered "special" and can be returned at any later time by
     * the function start(). RWFileErr is a possible exception that could be
     * thrown.
     */
    RWoffset              allocate(RWspace s);

    /**
     * Deallocates (frees) the storage space starting at offset \a t. This
